...
| **Dự án**                         | Gom các phiên theo (các) thư mục mà chúng chạy từ đó thành một dự án do người dùng đặt tên, thay vì các khung nhìn Agent/Phiên dạng phẳng. Các dự án được liệt kê dưới dạng một bảng gọn — mỗi dự án một hàng (tên, số phiên, số đang hoạt động, hoạt động gần nhất, cùng các thao tác nhanh ghim/xem kế hoạch/báo cáo/xóa), hàng có thể kéo để sắp xếp lại — thay vì mỗi dự án một thẻ đầy đủ, nhờ đó một máy có nhiều dự án không biến trang thành một danh sách cuộn dọc dài dằng dặc. Một dự án sở hữu một hoặc nhiều thư mục làm việc (một thư mục chỉ thuộc về nhiều nhất một dự án); tư cách thành viên được suy ra bằng cách đối chiếu `cwd` của mỗi phiên với các thư mục đã ánh xạ của dự án — không có thay đổi schema nào trên chính `sessions`. Mỗi hàng có thể mở rộng qua biểu tượng mũi tên (mặc định thu gọn) để hiện các ánh xạ thư mục, checklist AGENT-PLAN.md nếu có, và các phiên của nó hiển thị dưới dạng một hàng có thể cuộn ngang gồm các `SessionCard` (mỗi thư mục trong bucket **Chưa phân loại** cũng dùng cùng kiểu hàng thẻ phiên mặc định thu gọn này). Tạo một dự án (có thể gắn ngay một thư mục hiện có), đổi tên nó inline, **ghim** nó (qua biểu tượng ghim của hàng) để nổi lên trên mọi dự án chưa ghim bất kể thứ tự kéo thủ công — các hàng đã ghim và chưa ghim đều giữ nguyên thứ tự tương đối trong nhóm của mình, nhưng nhóm đã ghim luôn hiển thị trước, và trạng thái ghim được chia sẻ phía server (không như thứ tự kéo thủ công vốn chỉ lưu theo từng trình duyệt) — thêm/xóa ánh xạ thư mục về sau, hoặc xóa dự án (các ánh xạ thư mục biến mất theo kiểu cascade; các phiên bên dưới không bị ảnh hưởng và chỉ đơn giản quay về bucket **Chưa phân loại**, nơi cũng cung cấp thao tác một-chạm "Tạo dự án mới từ thư mục này"). Hậu thuẫn bởi `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id` (đổi tên và/hoặc ghim), và `POST/DELETE /api/projects/:id/paths[/:pathId]` |
| **Lịch** (bảng liên dự án) | Trang độc lập (`/focus-calendar`, nhãn thanh bên **Lịch**, tiêu đề trang cũng chỉ là "Lịch," đặt ngay sau Dự án) hiển thị cùng một lịch xem-theo-ngày dạng làn bơi (swimlane day-view calendar) như tab Lịch trong báo cáo thời gian tiêu điểm theo từng dự án, nhưng trên **tất cả** các dự án đang được giám sát cùng lúc, chiếm trọn chiều rộng khả dụng của trang (chỉ 25px đệm trái/phải của riêng nó, phá vỡ phần đệm thông thường của khung ứng dụng) thay vì một cột căn giữa, có giới hạn chiều rộng. Ba bộ lọc độc lập kết hợp theo kiểu giao nhau thay vì một chuỗi phụ thuộc: một bộ lọc **dự án** (chip — mỗi dự án có hoạt động trong khoảng thời gian hiện tại một chip, cộng với "Tất cả dự án" và một chip màu hổ phách **"Chưa phân loại"** cố định, luôn hiển thị, giới hạn phạm vi cho các phiên có thư mục chưa được ánh xạ vào dự án nào — "Chưa phân loại" và một lựa chọn dự án thật sự loại trừ lẫn nhau, được ràng buộc cả ở phía client lẫn phía server), một bộ lọc **phiên** (một dropdown toàn cục duy nhất liệt kê mọi phiên trên tất cả các dự án — không giới hạn theo dự án đang được chọn, nên việc chọn một phiên có thư mục không thuộc dự án đang chọn là một tổ hợp hợp lệ, chỉ đơn giản cho ra kết quả rỗng), và một bộ lọc **khoảng thời gian** (điều hướng Trước/Hôm nay/Tiếp theo theo từng ngày, mặc định là "hôm nay," một nhãn thứ/ngày có thể bấm để mở bộ chọn ngày gốc (native date picker) cho phép nhảy thẳng tới bất kỳ ngày nào mà không cần bấm mũi tên từng bước, cộng với chế độ khoảng ngày tùy chỉnh) — thay đổi một trong số đó không bao giờ đặt lại các bộ lọc còn lại. Được cung cấp bởi một endpoint tổng hợp mới, `GET /api/focus-report`, luôn yêu cầu một khoảng `from`/`to` tường minh (không có giá trị mặc định ẩn phía server) và tái sử dụng nguyên vẹn các hàm dựng báo cáo của `server/lib/focus-report.js`. Modal báo cáo theo từng dự án hiện có (mở từ thẻ của một dự án) không bị ảnh hưởng — phần hiển thị ô thống kê/nút chuyển List-Lịch/thân danh sách của nó được dùng chung với trang này qua `FocusReportBody.tsx` thay vì bị nhân bản. Vì một báo cáo giờ đây có thể trải rộng trên nhiều dự án, ô thống kê Concurrency ở đây được đổi nhãn thành **"Phiên tác tử đồng thời"** (cùng con số `concurrency_ratio`, được đọc đúng là mức chồng lấn liên dự án thay vì tình trạng đa nhiệm của riêng một dự án) |
| **Tiêu điểm** (báo cáo hoạt động) | Trang độc lập (`/focus`, nhãn thanh bên **Tiêu điểm**, đặt ngay sau Lịch) trả lời câu hỏi "chúng ta đã thực sự làm gì" dưới dạng một danh sách đơn giản, dễ đọc cho các bên liên quan thay vì một lưới dạng làn bơi (swimlane grid). Dùng chung ba bộ lọc độc lập như bảng Lịch — chip dự án (gồm "Tất cả dự án" và chip màu hổ phách **"Chưa phân loại"**), một dropdown phiên toàn cục, và bộ chọn khoảng thời gian theo ngày/khoảng tùy chỉnh — chia sẻ đúng cùng component `ProjectScopeFilters` và endpoint `GET /api/focus-report`, nên không cần thêm công việc phía backend. Cũng có cùng tính năng **thu phóng cửa sổ giờ** trong ngày như bảng Lịch — các mức thời lượng đặt sẵn (`4h`/`8h`/`12h`/`24h`), bộ điều khiển bước nhảy/nhập giờ bắt đầu, nút chuyển "Trực tiếp" (Live), và các mốc bắt đầu nhanh (`HourWindowZoomBar`/`useHourWindowZoom`, được tách ra từ `FocusCalendarView` để cả hai trang dùng chung một control duy nhất) — mặc định ở đây là toàn bộ khoảng thời gian không thu phóng thay vì mặc định 4 giờ của bảng Lịch, vì trang này trước nay luôn hiển thị trọn khoảng đã chọn và việc thu phóng chỉ là một tùy chọn thu hẹp thêm. Bên dưới cùng các ô thống kê quen thuộc (Tổng thời gian agent, Concurrency, % trên mục đã khai báo, % ngoài kế hoạch, đã loại trừ thời gian rảnh) là **thẻ hoạt động**: mỗi dòng là một việc khác biệt đã xảy ra trong khoảng thời gian đã chọn (thu hẹp thêm theo cửa sổ thu phóng khi đang bật, để các ô thống kê và danh sách không bao giờ lệch nhau) — một mục kế hoạch, một rẽ hướng/bug/feature (được gộp theo tiêu đề trên mọi phiên đã rơi vào đó), hoặc thời gian chưa phân loại — mỗi dòng hiển thị một chip loại (dùng lại từ vựng `FOCUS_KIND_CONFIG`/`FOCUS_KIND_ICONS` sẵn có của ứng dụng), một con số thời gian thực/hoạt động, một khoảng **giờ bắt đầu/kết thúc** dễ đọc (ví dụ "8:49 AM – 9:12 AM", qua hàm `formatTimeRange()` trong `client/src/lib/format.ts` — từ thời điểm bắt đầu của đoạn đóng góp sớm nhất đến thời điểm kết thúc của đoạn muộn nhất đối với một dòng đã gộp, không nhất thiết liên tục), theo sau là một dấu gạch ngang dài và **thời lượng** trôi qua giữa hai mốc đó ở dạng ngày/giờ/phút (ví dụ "8:49 AM – 9:12 AM — 23m", qua hàm `formatDurationLong()`), và, đối với một mục do bộ phân loại **suy luận (inferred)**, một câu `inferred_reason` (hàm `groupFocusActivity()` trong `client/src/lib/focusActivity.ts`, được khóa theo từng cwd để cùng một số thứ tự mục ở hai dự án khác nhau không bao giờ gộp làm một; các đoạn chưa phân loại **có** lý do suy luận vẫn giữ nguyên một dòng tường thuật cho mỗi phiên — hai phiên đã thực sự làm hai việc ngoài kế hoạch khác nhau sẽ hiển thị thành hai dòng, chứ không phải một dòng chỉ mang câu chuyện của phiên lớn hơn — trong khi chỉ các đoạn chưa phân loại không có lý do mới bị gộp vào một nhóm đuôi chung; khi nhiều hơn một phiên cùng rơi vào chung một khóa, nhãn/lý do hiển thị lấy từ phiên đóng góp tỷ trọng thời gian lớn nhất, và ghi chú "+N phiên khác" là một nút **bật/tắt** mở rộng ra tên riêng, khoảng giờ, tỷ lệ thời gian thực/hoạt động và lý do của từng phiên đóng góp). Phía trên thẻ hoạt động là một khối **Tóm tắt** do AI tổng hợp — các gạch đầu dòng bằng ngôn ngữ đời thường "chúng ta đã thực sự làm gì" cho khoảng thời gian đã chọn, **được nhóm theo dự án** ở phạm vi tất cả dự án (mỗi dự án có hoạt động là một bộ gạch đầu dòng riêng, dự án chiếm tỷ trọng thời gian thực lớn nhất đứng trước, cộng thêm một nhóm "Chưa phân loại" cho các thư mục chưa được ánh xạ; phạm vi một dự án duy nhất hiển thị nhóm duy nhất của nó mà không có tiêu đề nhóm) (`GET /api/focus-report/summary`, dùng lại hợp đồng `claude -p` headless và các tùy chỉnh của dịch vụ suy luận tiêu điểm; được cache phía server và gác bằng một digest dữ liệu, nên một ngày đã kết thúc chỉ sinh một lần, còn một ngày đang diễn ra chỉ sinh lại khi dữ liệu của nó thay đổi; một ghi chú nhỏ ở chân khối nêu tên model đã viết các gạch đầu dòng). Các cửa sổ nhiều ngày được tóm tắt **theo phân cấp** — mỗi ngày được tóm tắt trước (và được cache vĩnh viễn một khi dữ liệu của nó ngừng thay đổi), rồi một lần tổng hợp (rollup) duy nhất tổng hợp cả cửa sổ từ các gạch đầu dòng theo từng ngày, với ngân sách gạch đầu dòng tăng dần từ 4 (một hoặc hai ngày) lên 8 (nhiều tuần) — nhờ đó khi thu nhỏ ra ba tuần vẫn giữ được các sản phẩm cụ thể mà các bản tóm tắt hàng ngày nêu tên, thay vì trung bình hóa hàng chục phiên thành những chủ đề mơ hồ. Bị ẩn hoàn toàn khi không khả dụng (đường LLM tắt, cửa sổ trống, hoặc sinh thất bại), không bao giờ là một trạng thái lỗi. Ở phạm vi "tất cả dự án", mỗi dòng được gắn thêm tiền tố tên dự án của nó; ở phạm vi một dự án duy nhất thì tiền tố này được bỏ qua. Thu gọn các dòng vượt quá 5 đằng sau nút chuyển "hiện thêm"/"ẩn bớt". Một khai báo `ccam focus push/bug/feature` trực tiếp hiện chưa có lý do riêng biệt với nhãn của chính nó — chỉ các mục do bộ phân loại suy luận mới có — đây là một khoảng trống đã biết, chưa được khắc phục |
| **Quản lý dự án** (tổng hợp danh mục dự án) | Trang độc lập (`/project-manager`, nhãn thanh bên **Quản lý dự án**, đặt ngay sau Tiêu điểm) trả lời câu hỏi "mọi dự án tôi đang theo dõi hiện ra sao" chỉ trong một cái nhìn, thay vì phải kiểm tra từng dự án riêng lẻ. Một bảng tổng hợp (mỗi dự án một hàng: thanh tiến độ mục tiêu/cột mốc, huy hiệu nhịp độ, số quyết định đang mở, hoạt động gần nhất) nằm phía trên **Hàng đợi quyết định** (Decision Queue) — các mục đối chiếu tầng 6 đang chờ xử lý (`pace_alert`/`detour_volume`/`detour_disposition`/lỗi ghi ngược) được hiển thị dưới dạng thẻ có thể thao tác, mỗi thẻ được giải quyết ngay tại chỗ (`fold_in`/`new_item`/`deliberate`/`discard` đối với một rẽ hướng, được điền sẵn văn bản đề xuất từ chính lượt đối chiếu; resolve/dismiss/retry cho mọi trường hợp còn lại) — cùng một dải **Theo dõi nhịp độ** (Pace Watch, mọi mục bị trễ trên tất cả các dự án, mục trễ nặng nhất lên trước) và một nhật ký **Đã xử lý gần đây** (Recently Resolved). Một hàng chip phạm vi dự án lọc đồng thời bảng, hàng đợi và dải theo dõi về một dự án duy nhất. Trang này kết hợp ba endpoint sẵn có/mới (`GET /api/projects`, `GET /api/portfolio/summary`, `GET /api/decision-queue`) thay vì nhân bản logic của chúng, và là trang client đầu tiên được xây trên nền công việc quản lý danh mục dự án ở các tầng 4–6 (detour disposition, pace tracking, reconciliation) — vốn trước đó chỉ tồn tại phía server |
| **Bảng Kanban**                   | Ba chế độ với nút chuyển ở đầu trang (lưu trong `localStorage`): **Agent** — 4 cột (Đang làm / Đang chờ / Hoàn tất / Lỗi); **Phiên** — 5 cột (Hoạt động / Đang chờ / Hoàn tất / Lỗi / Bỏ dở); và **Dự án** — mỗi cột là một dự án (các phiên được nhóm theo thư mục mà dự án đó sở hữu, màu cột xoay vòng để khác với các cột trạng thái), kèm một cột **Chưa phân loại** cuối cùng cho các phiên từ thư mục chưa thuộc dự án nào. Khi đã tạo ít nhất một nhóm **màn hình** (mô phỏng một màn hình vật lý — tạo bằng nút **+ Thêm màn hình** ở đầu trang), các cột dự án được gán vào đó sẽ hiển thị bên trong một khung có viền, có thể kéo, dành riêng cho màn hình đó — chứa chúng thật sự, chứ không chỉ gắn nhãn — với các khung xếp cạnh nhau trong cùng một hàng cuộn ngang đó, cộng thêm một khung **Chưa phân nhóm** ở cuối thật sự chứa những dự án chưa được gán (giống khung của một màn hình, nhưng cố định — không kéo, không đổi tên, không xóa được). Kéo một cột dự án vào khung của một màn hình (hoặc vào cột khác đã ở trong khung đó) để chuyển nó vào đó, kéo vào khung Chưa phân nhóm (hoặc quá cuối hàng) để bỏ gán — cột sẽ nằm ngay bên trong khung đó, và kéo khung của một màn hình (bằng phần tiêu đề của nó) vào khung của màn hình khác để đổi chỗ cả khung sang trái/phải — mô phỏng cách các màn hình thực sự đặt trên bàn làm việc. Đổi tên khung ngay tại chỗ, thu gọn/mở rộng nó qua biểu tượng chevron ở đầu khung — thu gọn sẽ đưa cả khung ra khỏi hàng chính, chuyển hẳn sang một dải mỏng riêng ở phía trên, dùng chung cho mọi màn hình đã thu gọn, nhờ đó giải phóng thật sự khoảng không gian ngang mà nó vẫn sẽ chiếm chứ không chỉ thu hẹp tại chỗ; mở rộng lại sẽ đưa nó quay về đúng vị trí theo thứ tự trong hàng chính (trong suốt quá trình đó các cột của nó vẫn được mount — chỉ ẩn về mặt hiển thị — nên trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó) — hoặc xóa nó (các dự án của khung đó quay về Chưa phân nhóm). Biểu tượng nút thứ hai cạnh chevron mở một **menu bố cục** — một popover gồm các ô hình ảnh, mỗi ô là một tổ hợp orientation+wrap hoàn chỉnh (Cột 1–4 hoặc Tự động, Hàng 1–4 hoặc Tự động), mỗi ô hiển thị bản xem trước thu nhỏ đúng cách sắp xếp đó — nên chọn một bố cục chỉ cần một lần nhấp để mở menu và một lần nhấp vào ô muốn chọn, áp dụng cả hai giá trị cùng lúc rồi đóng popover ngay lập tức. Độc lập theo từng màn hình, lưu trong trường `orientation` của cùng `MonitorGroup` đó (**ngang**, mặc định, các cột dự án xếp cạnh nhau, hoặc **dọc**, xếp chồng từ trên xuống) và trường `wrap` (`*` mặc định, không giới hạn như hiện tại, hoặc `1`–`4` giới hạn số cột dự án nằm trên một hàng/cột trước khi xuống hàng/cột mới). Mỗi cột dự án (và cột Chưa phân loại độc lập) đều có cùng biểu tượng chevron đó, cho phép thu gọn riêng cột đó tại chỗ xuống thành một thanh tiêu đề mảnh, bất kể nó đang nằm trong khung nào. Cả hai đều là một cấu hình bố cục toàn cục duy nhất — được lưu trữ phía máy chủ (bảng `dashboard_layout`, qua `GET`/`PUT /api/monitors`) và dùng chung cho mọi máy tính kết nối tới dashboard này (ứng dụng không có tài khoản người dùng, nên chỉ có một bố cục chung cho tất cả), không còn là cách sắp xếp cá nhân lưu trong `localStorage` nữa — thay đổi thực hiện từ một máy sẽ được đẩy trực tiếp tới mọi máy khác đang kết nối qua thông điệp WebSocket `monitors_updated`, không cần tải lại trang. Cột **Đang chờ** ánh xạ trực tiếp trạng thái lưu trữ `waiting` của Agent — được đặt khi Claude Code đang ngồi ở dòng nhập (phiên mới, giữa các lượt, hoặc đang bị chặn bởi Notification xin quyền) và chuyển sang `working` ngay khi người dùng tiếp tục (UserPromptSubmit / PreToolUse). Mỗi tiêu đề cột có biểu tượng `?` với tooltip giải thích vòng đời. Mỗi cột trạng thái ở chế độ Agent/Phiên cũng có menu bố cục riêng — cùng biểu tượng lưới kết hợp cạnh huy hiệu số lượng, đúng vị trí menu của khung màn hình trong chế độ Dự án — mở cùng popover các ô hình ảnh để chuyển đổi các thẻ của riêng cột đó giữa kiểu xếp chồng dọc mặc định, một hàng cuộn ngang gồm các thẻ có chiều rộng cố định, và bất kỳ số lượng trước khi xuống dòng cố định nào. Khác với menu của màn hình ở trên, menu này chỉ lưu cục bộ theo trình duyệt (`localStorage`, khóa theo chế độ + trạng thái, ví dụ `agents-working`, ở cả hai khóa `kanban-status-column-orientation` và `kanban-status-column-wrap`) chứ không chia sẻ qua máy chủ, và mỗi cột trạng thái có thiết lập độc lập với các cột khác. Mỗi cột tìm nạp theo trạng thái từ máy chủ (không giới hạn thực tế mỗi cột), sau đó phân trang phía client với 10 thẻ mỗi cột kèm nút "Hiện thêm". Đăng ký WebSocket bám theo chế độ đang xem (`agent_*` so với `session_*`) nên cập nhật khác chế độ không gây tải lại. Huy hiệu Đang chờ hiển thị `awaiting_reason` của hàng — **Cần phản hồi** (`notification`), **Xong lượt** (`stop`), **Chờ prompt** (`session_start`), **Bị ngắt** (`interrupted`) — dưới dạng tooltip khi hover, chỉ giữ ở dạng tooltip trên các thẻ nhỏ gọn để tiêu đề giữ nguyên không gian; các bề mặt rộng hơn (bảng Phiên, đầu trang chi tiết phiên) hiển thị thêm bốn lý do này ngay trên dòng dưới dạng chip lồng bên trong, với các lý do khẩn cấp (lời nhắc xin quyền, bị ngắt) dùng sắc hổ phách nóng hơn. Ba lý do còn lại — **SubAgents** (`subagent`), **Shell** (`shell`), **Monitor** (`monitor`) — nghĩa là Claude vẫn đang chủ động làm việc thông qua một tiến trình con (một đội subagent nó đã tạo ra, một lệnh gọi Bash đồng bộ, hoặc một lệnh gọi công cụ Monitor) chứ không phải đang bị chặn chờ người dùng, nên chúng thay thế toàn bộ chữ trên huy hiệu, kể cả trên các thẻ nhỏ gọn, thay vì lồng bên trong "Đang chờ". Huy hiệu model trên mỗi thẻ chuyển sang màu đỏ tươi khi phiên đang chạy một model cao cấp/đắt tiền (họ Opus hoặc Fable). Thẻ phiên còn hiển thị thêm một dải token gọn gàng — **đầu vào / đầu ra / cache đọc / cache ghi** — kèm tổng **Effective (hiệu dụng)** canh phải, một khi phiên đã dùng token — cache đọc (toàn bộ phần đầu hội thoại được đọc lại mỗi lượt, tính phí ~10% giá đầu vào) và cache ghi (ngữ cảnh mới, tính phí cao hơn) được tách thành hai con số riêng, còn Effective là tổng quy đổi về đầu vào, được server tính theo đúng biểu giá thực của từng model, nên nó biến động cùng con số chi phí $ của thẻ thay vì thổi phồng các phiên nặng cache như một tổng token thô. Một thẻ thống kê gọn nhẹ **Phiên tác tử đồng thời** nằm ở đầu trang, bên trái nút chuyển chế độ xem, hiển thị tỷ lệ đồng thời trên toàn bộ dự án của hôm nay (effort time ÷ active hoặc open-session wall-clock time — cùng con số với ô Concurrency trong báo cáo Focus) kèm nút đảo để chọn tỷ lệ nào là chính; nó ẩn đi cho đến khi lần tải đầu tiên hoàn tất và biến mất trở lại nếu lần tải thất bại, đồng thời tự làm mới theo cùng các sự kiện WebSocket phiên/agent mà các thẻ khác của bảng phản ứng theo, chứ không dùng bộ đếm giờ polling |
...
| **Giám sát theo kế hoạch**        | Trả lời ngay trong một cái nhìn câu hỏi "phiên này đang làm gì, và nó phục vụ phần nào của kế hoạch dự án?". Mỗi repo được giám sát có thể giữ một tệp `AGENT-PLAN.md` do con người phê duyệt (các mục checkbox được đánh số) ở thư mục gốc; một lần poll nền (`DASHBOARD_PLAN_POLL_MS`, cộng thêm việc nhập cơ hội khi có `SessionStart`) sao chiếu nó ở chế độ chỉ-đọc vào dashboard, khóa theo cwd và gộp lên cấp Dự án. Các phiên khai báo vị trí của mình bằng cách chạy `ccam focus set/push/pop/done` trong tool Bash của chúng — **luồng hook** là kênh ghi (sự kiện `PostToolUse` vốn đã mang session id), nên tính năng hoạt động trong mọi dự án đã gắn hook mà không cần thiết lập gì thêm cho từng dự án. Trang Dự án hiển thị mỗi kế hoạch dưới dạng checklist có thể thu gọn với thanh tiến độ và chip phiên theo từng mục; mỗi thẻ phiên có một dòng breadcrumb tiêu điểm với một biểu tượng dùng chung thể hiện trạng thái hiện tại của nó (biểu tượng tâm ngắm cho mục đã biết, biểu tượng rẽ hướng/feature/bug trong các trường hợp khác) — ví dụ `Item 4: Migrate auth ▸ npm conflict (23m)` — được hiển thị ngay cả với một phiên đang ở giữa một đoạn rẽ hướng mà chưa khai báo mục gốc nào; Chi tiết phiên có thêm banner tiêu điểm cùng tab **Plan** (checklist, dòng thời gian tiêu điểm được diễn giải từ các sự kiện `Focus`, và micro-plan TodoWrite mới nhất của phiên qua `GET /api/sessions/:id/todos`). Một **kiểm tra trôi lệch (drift audit)** (`DASHBOARD_FOCUS_AUDIT_*`) định kỳ xác nhận rằng hoạt động gần đây của một phiên có tiêu điểm khớp với khai báo của nó — `claude -p` headless trên một model nhỏ với phương án dự phòng so trùng từ khóa — và đóng dấu huy hiệu hổ phách "có thể là detour chưa khai báo?"; nó không bao giờ ghi đè tiêu điểm, và các khai báo không bao giờ xóa huy hiệu. `ccam focus bug`/`feature "<title>" "<summary>" [--detail "<text>"]` push một detour có gắn nhãn loại; dòng tiêu điểm theo từng mục của khung nhìn Plan phản ánh trạng thái hiện tại thực sự của mỗi phiên (mục đã biết, rẽ hướng thường, feature, hoặc bug) với biểu tượng tương ứng, được xếp vào nhóm theo mục kế hoạch mà nó được khai báo dưới (hoặc dòng "Unknown item" khi chưa đặt mục nào), có thể bấm để xem chi tiết đối với các khai báo bug/feature. Một biểu tượng báo cáo trên mỗi thẻ/đầu trang dự án (trang Dự án và khung nhìn Dự án của Kanban) mở một **báo cáo thời gian tiêu điểm** giới hạn trong dự án đó — `GET /api/projects/:id/focus-report` diễn giải lại lịch sử sự kiện `Focus` của từng phiên thành các đoạn có dấu thời gian (`server/lib/focus-report.js`) và khấu trừ các khoảng không có hoạt động hook vượt quá một cửa sổ ân hạn có thể cấu hình (`DASHBOARD_FOCUS_IDLE_GRACE_SECONDS`) để thời gian chờ nhàn rỗi không làm phồng lên con số "thời gian trên mục"; popup hiển thị các ô thống kê hoạt động/nhàn rỗi, một dòng thời gian phân đoạn theo từng phiên, một bảng tổng hợp theo từng mục (các đoạn rẽ hướng được gộp vào mục mà nó làm gián đoạn), và một biểu đồ chia theo loại trên toàn dự án. Các phiên chưa bao giờ khai báo tiêu điểm không phải là một lỗ hổng câm lặng trong báo cáo: một bộ phân loại **suy luận tiêu điểm (focus inference)** chạy nền (`DASHBOARD_FOCUS_INFER_*`) tiêu hóa hoạt động của từng phiên im lặng đó (prompt, các tệp đã chạm tới, các lệnh đã chạy) và quy thời gian của nó cho một mục kế hoạch hoặc một đoạn rẽ hướng được suy luận — hiển thị kèm chip `≈ inferred` để phần quy kết phỏng đoán không bao giờ bị đọc nhầm thành lịch sử đã khai báo. Một dự án **hoàn toàn không có `AGENT-PLAN.md`** vẫn được phân loại, chỉ theo cách khác: không có danh sách mục nào để đối chiếu, bộ phân loại chuyển thẳng sang việc xin một câu tóm tắt đơn giản về việc hoạt động của phiên đã hoàn thành gì, được lưu theo cùng cách (`kind: "unclassified"` kèm một `reason` thực sự) — và ngay cả một phán quyết `"unclassified"` có độ tin cậy thấp TRONG một dự án đã có kế hoạch cũng giữ lại bất kỳ lý do nào mà bộ phân loại đã viết ra, thay vì để văn bản đó bị âm thầm loại bỏ. Một phiên chưa có khai báo lẫn phán quyết đáng tin cậy (thường gặp nhất là một phiên đang chạy và chưa đủ yên tĩnh để bộ phân loại xử lý) vẫn sẽ hiển thị — dưới dạng khối màu xám trung tính **"No focus"**, có mục chú giải riêng bên cạnh item/detour/feature/bug — thay vì vô hình cho đến khi được phân loại hoặc kết thúc. Báo cáo cũng tách **thời gian nỗ lực (effort time)** (cộng dồn qua mọi phiên — bị thổi phồng khi các phiên chạy đồng thời) khỏi **thời gian thực tế (wall-clock time)** (hợp của khoảng thời gian riêng của từng phiên), với một ô thống kê **Concurrency** biến khoảng chênh lệch đó thành một con số dễ hiểu ("1.08x") thay vì hai khoảng thời gian trông như không khớp với nhau. Nút chuyển List/Calendar trong báo cáo sẽ chuyển sang **lịch xem-theo-ngày dạng làn bơi (swimlane day-view calendar)** — các phiên được đặt trên một trục 24 giờ thực, với các phiên chồng lấn được tách thành các làn cạnh nhau để tính đồng thời được thể hiện qua hình học thay vì một con số thống kê |
...
| **Nguồn dữ liệu từ xa**            | Thu thập mức sử dụng Claude Code từ các máy khác qua SSH — dashboard chạy `rsync` thư mục `~/.claude/projects` của mỗi máy từ xa rồi nhập vào, gắn thẻ các phiên theo nguồn. Có bộ chọn phạm vi dữ liệu toàn cục (chỉ máy cục bộ / tất cả / nguồn cụ thể) để thu hẹp toàn bộ ứng dụng theo nguồn đã chọn. Gần như thời gian thực nhờ một tiến trình poll nền. Các phiên từ xa **không nhận hook cục bộ trực tiếp**, nên chúng bị loại khỏi mọi heuristic liveness/stale cục bộ (probe thu dọn `ps`/`lsof`, lượt quét transcript của watchdog, đợt dọn dẹp 1 giờ lúc khởi động và lượt quét phiên bỏ dở định kỳ — tất cả đều được kiểm soát bởi điều kiện `source = 'local'`); thay vào đó **trạng thái live của chúng được đối chiếu từ transcript đã sao chép (mirror) ở mỗi lần đồng bộ** — một transcript được sửa trong vòng `DASHBOARD_REMOTE_ACTIVE_WINDOW_MS` (mặc định 10 phút) nghĩa là phiên vẫn `active`, còn khi bản sao ngừng cập nhật lâu hơn ngưỡng này thì phiên được đối chiếu về `completed` |
| **Thiết kế đáp ứng**              | Bố cục thân thiện với thiết bị di động với lưới xếp chồng, bảng có thể cuộn và thanh bên có thể thu gọn                                                                                                                                                                                      |
| **Bản địa hóa giao diện người dùng**                | Chuyển đổi ngôn ngữ tích hợp với bản sao giao diện người dùng được dịch và nhãn trợ năng cho tiếng Anh (`en`), tiếng Trung (`zh`), tiếng Việt (`vi`) và tiếng Hàn (`ko`). Phạm vi đa ngôn ngữ trải dài hết toàn bộ tooltip ở trang Workflows: cách tính của các thẻ thống kê và các diễn giải theo bucket giá trị, popover "Cái gì / Cách đọc / Vì sao" cho mỗi biểu đồ, mọi tooltip hover trên đồ thị (orchestration, tool flow, pipeline, model delegation, concurrency), bảng chi tiết Workflow Patterns với mô tả và gợi ý, popover thông tin Settings → Định giá mô hình, khung CLAUDE_HOME và toàn bộ luồng Import History                                                                                                                                                                       |
...
| **Mức sử dụng**                    | Ảnh chụp nhanh giới hạn tốc độ / mức sử dụng gói cước cho tài khoản Claude hiện tại tại `/usage`. Một nút **"Ghi lại ngay"** khởi chạy `claude` trong một phiên tmux tách biệt (detached), tự điều khiển các bảng TUI `/status` và `/usage` của chính nó, chụp lại văn bản đã render, rồi phân tích theo kiểu best-effort thành thông tin tài khoản (email, tổ chức, phương thức đăng nhập, phiên bản CLI, model), chi phí/thời lượng/số dòng đã thay đổi của phiên, số lượng token (đầu vào, đầu ra, đọc cache, ghi cache), và hai tỷ lệ phần trăm giới hạn tốc độ quan trọng nhất — **cửa sổ phiên (5 giờ)** đang trượt liên tục và **cửa sổ hàng tuần** (kèm phân tích theo từng model) — mỗi cái hiển thị dưới dạng thanh mã màu (xanh lá/vàng/cam/đỏ) kèm thời điểm đặt lại. Bảng **Session reset times** (Thời điểm đặt lại phiên) vẽ biểu đồ thời điểm đặt lại cửa sổ phiên tiếp theo của mỗi tài khoản trong 24 giờ tới, tô nửa bên phải của vạch đánh dấu đặt lại theo màu riêng của cửa sổ hàng tuần một khi cửa sổ hàng tuần — chứ không phải phiên — mới là giới hạn thực sự, kèm theo nhãn "Capped by weekly (còn X%)" nêu rõ điều đó; bảng **Weekly reset countdown** (Đếm ngược đặt lại hàng tuần) làm điều tương tự cho việc đặt lại theo ngày lịch của cửa sổ hàng tuần. Thẻ **Color thresholds** (Ngưỡng màu sắc) cho phép bạn cấu hình chính xác điểm bắt đầu của từng dải màu, độc lập cho phạm vi phiên và hàng tuần cùng phạm vi **tốc độ tiêu thụ** tương ứng của chúng (xem bên dưới) — được lưu ở phía server (bảng đơn `color_thresholds`, qua `GET`/`PUT /api/color-thresholds`) và chia sẻ trên mọi máy tính đã kết nối, phát trực tiếp qua `color_thresholds_updated` khi có thay đổi. Mọi lần ghi lại đều được lưu trữ (bảng `usage_captures`) bất kể việc phân tích có thành công hay không: `status` là `ok`/`partial`/`error`, và văn bản pane thô đã chụp luôn được giữ lại làm phương án dự phòng, nên một thay đổi định dạng do phiên bản CLI mới gây ra chỉ hạ cấp xuống thành một khối văn bản thô dễ đọc chứ không làm mất dữ liệu. Danh sách lịch sử bên dưới lần ghi lại gần nhất hiển thị các lần chạy trước đó; chỉ một lần ghi lại có thể chạy tại một thời điểm (409 nếu đã có một lần đang chạy). Same-origin guard ngăn chặn việc kích hoạt drive-by từ trình duyệt, giống như Chạy Claude. Một bảng **Tài khoản** phía trên — có thể thu gọn qua biểu tượng chevron trên tiêu đề riêng của nó, mặc định thu gọn (không lưu lại) — theo dõi song song bất kỳ số lượng tài khoản Claude nào khác: mỗi tài khoản chỉ cần trỏ tới một `CLAUDE_CONFIG_DIR` đã đăng nhập sẵn qua `claude login`, và nút "Ghi lại ngay" theo từng tài khoản đọc trực tiếp thông tin xác thực OAuth của lượt đăng nhập đó từ bộ lưu trữ của CLI (Keychain trên macOS, hoặc tệp `.credentials.json` ở nơi khác) rồi lấy mức sử dụng trực tiếp từ API của Anthropic — không cần cookie phiên trình duyệt, mật khẩu, hay quy trình đăng nhập bổ sung. Mỗi hàng tài khoản luôn hiển thị thanh cửa sổ phiên và cửa sổ hàng tuần của riêng nó (kèm thời điểm đặt lại và thời gian kể từ lần ghi lại gần nhất) để thấy ngay tình trạng của mọi tài khoản; mở rộng một hàng sẽ tải lười lịch sử ghi lại của tài khoản đó (chỉ gồm thời điểm và tỷ lệ phần trăm — không có chi tiết chi phí/token/model, vì đường dẫn OAuth không có dữ liệu đó). Khi đăng nhập của một tài khoản bị thiếu hoặc hết hạn, huy hiệu **Needs login** của hàng đó có thể bấm được (macOS): nó mở một cửa sổ Terminal mới đã chạy sẵn `CLAUDE_CONFIG_DIR=<dir> claude`, nên việc xác thực lại chỉ là hoàn tất quy trình đăng nhập rồi đóng cửa sổ. Mọi tài khoản đang bật cũng tự động được ghi lại mỗi 5 phút (một scheduler chạy nền, có thể tắt ở phía server), nên tỷ lệ phần trăm luôn cập nhật mà không cần bấm thủ công. Thẻ **Activity** (Hoạt động) liệt kê thước đo mức sử dụng thực tế của từng tài khoản — đang hoạt động (đã dùng trong 15 phút gần nhất) hay đã bao lâu kể từ lần thực sự sử dụng gần nhất — được suy ra từ việc tỷ lệ phần trăm phiên/hàng tuần của chính tài khoản đó có thực sự tăng lên giữa các lần ghi lại hay không, chứ không dựa vào bất cứ điều gì gắn với `CLAUDE_CONFIG_DIR` của nó, vì công việc thực tế thường được thực hiện qua hồ sơ đã đăng nhập ở `~/.claude` mặc định chứ không phải qua thư mục config riêng của tài khoản này. Thẻ **Consumption rate** (Tốc độ tiêu thụ) dự đoán, theo từng tài khoản, thời điểm hạn mức phiên và hàng tuần sẽ dùng hết nếu tiếp tục với tốc độ hiện tại: một đường xu hướng %/giờ được khớp trên các lần ghi lại gần đây của tài khoản kể từ khi cửa sổ hiện tại bắt đầu, và — chỉ khi xu hướng đó thực sự đang tăng — được ngoại suy đến thời điểm sẽ chạm mốc 100%. Nếu thời điểm dự đoán hết hạn mức đến trước khi cửa sổ tự đặt lại, thẻ sẽ hiển thị bộ đếm ngược có màu đến thời điểm đó ("Runs out in 2h 15m"); nếu cửa sổ sẽ được đặt lại trước (hoặc mức sử dụng đang đi ngang/giảm), thẻ sẽ hiển thị "On track — resets before limit" thay vào đó. Màu sắc được xác định bởi phạm vi `sessionRate`/`weeklyRate` riêng của thẻ Color thresholds, độc lập với các dải màu dựa trên tỷ lệ phần trăm sử dụng thô mà phần còn lại của trang sử dụng. Bên dưới mỗi kết luận là một dòng thống kê nhỏ cho biết tốc độ tiêu thụ thô (phiên tính theo %/giờ, hàng tuần tính theo %/ngày — vì %/giờ sẽ là một con số quá nhỏ để đọc trên cửa sổ 7 ngày) cùng với việc tốc độ đó nhanh hay chậm hơn bao nhiêu lần so với **tốc độ bền vững** — tốc độ sẽ vừa khít chạm mốc 100% đúng vào thời điểm cửa sổ đặt lại ("23.4%/hr · 1.8× sustainable pace") — vì một con số đơn lẻ (vốn mang ý nghĩa rất khác nhau vào phút thứ 10 của cửa sổ so với 10 phút trước khi đặt lại) không đủ để tự nó nói lên "đây là bình thường hay đang quá mức". Thẻ **Rotation plan** (Kế hoạch xoay vòng) nối chuỗi cùng dữ liệu tốc độ tiêu thụ hàng tuần theo từng tài khoản đó thành một kế hoạch xoay vòng có thể hành động được, hoàn toàn tính toán phía client: nên dùng tài khoản nào ngay bây giờ, và thời điểm dự kiến chuyển sang tài khoản tiếp theo, nối tiếp qua mọi tài khoản đang bật, chiếu xa khoảng 9 ngày. Việc tính toán dùng tốc độ hàng tuần nhanh nhất quan sát được trong số các tài khoản làm tốc độ thay thế cho "nếu tài khoản này trở thành tài khoản đang hoạt động" (vì tốc độ gần như bằng 0 của một tài khoản đang rảnh không nói lên được gì về việc nó sẽ hoạt động ra sao khi thực sự được sử dụng), và đưa tài khoản về 0% tại thời điểm tài khoản đó tự đặt lại khi chiếu về tương lai. Mỗi bước trong danh sách nêu tên tài khoản, khung thời gian bắt đầu/kết thúc, và một huy hiệu cho biết bước đó kết thúc vì tài khoản được dự đoán sẽ dùng hết hạn mức (**Switch next** — Chuyển tiếp theo) hay vì nó bền vững suốt phạm vi chiếu (**Sustainable** — Bền vững) |
| **Tabby** | Chú mèo SVG dễ thương được ghim ở góc dưới bên phải trên mọi trang, lắng nghe luồng phiên WebSocket thời gian thực và phản ứng theo đó. **Linh vật biết phản ứng**: 8 tâm trạng dựa trên luồng phiên thời gian thực — idle, watching, happy, worried, stuck, thinking, sleeping và disconnected; mắt mèo dõi theo con trỏ, mỗi tâm trạng có hoạt ảnh riêng. **Lời thoại bong bóng** bật lên khi có sự kiện đáng chú ý (phiên bắt đầu/kết thúc, có lỗi, lần chạy hoàn tất), được throttle và có thể tắt tiếng. Nó cũng theo dõi cả đồng hồ, không chỉ sự kiện: một phiên đang hoạt động chạy một model đắt tiền (Opus hoặc Fable) liên tục hơn 15 phút sẽ nhận một lời nhắc nhở một-lần duy nhất ("bạn đang dùng một model đắt tiền — bạn có chắc muốn tiếp tục không?"), và chỉ tái kích hoạt nếu phiên rời khỏi model đó rồi quay lại sau. Nhấp vào chú mèo hoặc nhấn **⌘B / Ctrl+B** để mở **bảng điều khiển** (Esc để đóng): dòng trạng thái thời gian thực (N đang chạy · M bị lỗi · trạng thái kết nối), các hành động nhanh (nhảy tới Run Claude / Activity / Sessions / các phiên bị lỗi, tắt tiếng, xóa cảnh báo) và một ô **Ask**. Ô Ask trả lời cục bộ các câu hỏi trạng thái đơn giản; những câu hỏi khác được chuyển sang trang **Run Claude** sẵn có (`/run?prompt=...`) để khởi chạy một phiên Claude Code thật sự — **không cần backend mới, không cần API key**. Được xây dựng hoàn toàn trên luồng WebSocket sẵn có, thân thiện với trợ năng (bàn phím, `aria-live`, tôn trọng `prefers-reduced-motion`), bật/tắt trong Settings. Mã nguồn tại `client/src/components/Tabby/` |
...
| **Màn hình splash thương hiệu** | Màn hình splash thương hiệu hiển thị một lần mỗi phiên trình duyệt khi tải ứng dụng — lời chào theo thời điểm trong ngày (Chào buổi sáng / chiều / tối), một dòng tiêu đề đậm cùng hai dòng phụ đã bản địa hóa, và một biểu tượng thương hiệu dạng đồ thị nút có hoạt ảnh trên nền tối (radial glow, chòm sao, hạt nhiễu). Đục hoàn toàn ngay từ khung hình đầu (không lóe nội dung ứng dụng phía sau), giữ ~2.5 giây rồi mờ dần, bấm để bỏ qua, tôn trọng `prefers-reduced-motion`, bản địa hóa en/zh/vi/ko |
...
**Chia sẻ liên kết LAN.** Khi đã đặt `DASHBOARD_TOKEN`, phần đầu trang Kanban Board có biểu tượng "Sao chép liên kết chia sẻ" (cạnh nút chuyển đổi hình bánh răng **phiên nội bộ**, dùng để ẩn các phiên do chính bộ phân loại tiêu điểm nền của dashboard tạo ra từ một thư mục tạm, chứ không phải phiên do bạn chạy) để sao chép URL hiện tại kèm `?token=` vào clipboard. Mở liên kết đó trên trình duyệt khác sẽ tự động lưu token vào `localStorage` của trình duyệt đó và xóa token khỏi thanh địa chỉ, nên người nhận được xác thực ngay mà không cần thao tác thủ công nào — tuy nhiên nút này chỉ đính kèm được token nếu trình duyệt hiện tại đã có sẵn token (ví dụ đã từng mở một liên kết chia sẻ, hoặc đã chạy `localStorage.setItem('dashboard_token', '...')`).
...
3. **Máy chủ** xử lý sự kiện bên trong giao dịch SQLite:
   ...
   - Trên `Stop` không lỗi, Agent chính chuyển sang `waiting` — Claude đã hoàn thành lượt. Các Subagent nền vẫn tiếp tục chạy. Phiên vẫn giữ `active`. Cờ **Đang chờ** (`awaiting_input_since`) chỉ được đóng dấu khi **không còn subagent nào đang working** — phiên có agent còn đang chạy hiển thị là **Hoạt động** (nó đang chờ agent của mình, không phải người dùng), và việc đóng dấu được hoãn đến `SubagentStop` cuối cùng. `Stop` với `stop_reason=error` đánh dấu Agent `error` và phiên `error`
   ...
   - `SubagentStop` cố tình KHÔNG xóa cờ chờ — Subagent nền hoàn thành không nói lên gì về việc người dùng đã trả lời hay chưa. Nó làm điều ngược lại khi toàn bộ subagent hoàn tất: nếu đó là subagent **cuối cùng** còn đang working và Agent chính đã kết thúc lượt, nó đóng dấu cờ chờ mà `Stop` trước đó đã hoãn — phiên lật từ Hoạt động → Đang chờ đúng thời điểm tất cả agent xong việc
...
stateDiagram-v2
    ...
    working --> waiting: Stop, không lỗi (reason=stop, hoặc subagent nếu vẫn còn fleet đang chạy)
    working --> waiting: Notification (lý do được phân loại: notification / subagent / shell / monitor)
...
stateDiagram-v2
    ...
    active --> waiting: Stop, không lỗi, không subagent nào đang working (reason=stop)
    active --> waiting: Stop khi một subagent còn working (reason=subagent, được đóng dấu ngay)
    waiting --> waiting: SubagentStop cuối cùng hoàn tất (reason=subagent hạ xuống thành stop)
    active --> waiting: Notification (lý do được phân loại: notification / subagent / shell / monitor)
...
| `DASHBOARD_PLAN_POLL_MS` | `10000` (10 giây) | Khoảng poll (ms) cho watcher `AGENT-PLAN.md` của **Giám sát theo kế hoạch**: mọi cwd phiên/dự án đã biết đều được stat-check và tệp kế hoạch có thay đổi sẽ được nhập lại rồi phát đi (`plan_updated`). Đặt thành `0` để tắt poll (`SessionStart` vẫn nhập theo cơ hội, và `POST /api/plans/refresh` buộc chạy một lần) |
| `DASHBOARD_FOCUS_AUDIT_MS` | `300000` (5 phút) | Nhịp (ms) cho **kiểm tra trôi lệch tiêu điểm** — lượt kiểm nền xác nhận hoạt động gần đây của một phiên có tiêu điểm khớp với mục kế hoạch nó đã khai báo, đóng dấu huy hiệu hổ phách "có thể là detour chưa khai báo?". Đặt thành `0` để tắt hoàn toàn việc kiểm tra |
| `DASHBOARD_FOCUS_IDLE_GRACE_SECONDS` | `300` (5 phút) | Cửa sổ ân hạn nhàn rỗi cho **báo cáo thời gian tiêu điểm** theo từng dự án (`GET /api/projects/:id/focus-report`): một khoảng cách giữa hai sự kiện hook liên tiếp dài hơn giá trị này chỉ được tính là hoạt động trong phạm vi cửa sổ ân hạn — phần còn lại bị loại trừ vì được coi là nhàn rỗi, nên một phiên nằm ở trạng thái Đang chờ suốt nhiều giờ sẽ không làm phồng lên con số "thời gian trên mục" của nó. Các khoảng cách nhỏ hơn cửa sổ (nhịp suy nghĩ-và-trả lời bình thường) luôn được tính đầy đủ. Đặt thành `0` để tắt hoàn toàn việc khấu trừ (100% thời gian thực tính là hoạt động) |
| `DASHBOARD_FOCUS_AUDIT_MODE` | `llm` | Bộ phán xử trôi lệch: `llm` (`claude -p` headless trên một model nhỏ dùng xác thực CLI sẵn có của bạn, tự chuyển sang heuristic khi CLI không khả dụng), `heuristic` (chỉ so trùng từ khóa), hoặc `off` |
| `DASHBOARD_FOCUS_AUDIT_MODEL` | `haiku` | Model được truyền vào lệnh `claude -p --model` của kiểm tra trôi lệch |
| `DASHBOARD_FOCUS_AUDIT_TIMEOUT_MS` | `30000` (30 giây) | Đồng hồ kill (ms) cho một lần chạy `claude -p` của kiểm tra trôi lệch (SIGTERM, rồi SIGKILL) |
| `DASHBOARD_FOCUS_INFER_MS` | `600000` (10 phút) | Nhịp (ms) cho **suy luận tiêu điểm (focus inference)** — bộ phân loại nền quy các phiên chưa bao giờ khai báo tiêu điểm cho một mục kế hoạch hoặc một đoạn rẽ hướng được suy luận, để báo cáo thời gian tiêu điểm không có lỗ hổng câm lặng. Một lượt backfill chạy ngay sau khi khởi động, sau đó theo nhịp này. Đặt thành `0` để tắt |
| `DASHBOARD_FOCUS_INFER_MODE` | `llm` | Bộ phân loại suy luận tiêu điểm: `llm` (`claude -p` headless dùng xác thực CLI sẵn có của bạn, sau khi một heuristic từ khóa thận trọng được xem xét trước), `heuristic` (chỉ so trùng từ khóa — không bao giờ kết luận detour, chỉ nhận các trùng khớp mục rõ ràng), hoặc `off` |
| `DASHBOARD_FOCUS_INFER_MODEL` | `haiku` | Model được truyền vào lệnh `claude -p --model` của suy luận tiêu điểm |
| `DASHBOARD_FOCUS_SUMMARY_MODEL` | dự phòng về `DASHBOARD_FOCUS_INFER_MODEL`, rồi `haiku` | Model cho phần tóm tắt "What happened in this window" của trang Focus (`GET /api/focus-report/summary`) — một biến ghi đè chuyên dụng để các gạch đầu dòng dành cho các bên liên quan có thể dùng một model mạnh hơn (ví dụ `sonnet`), trong khi bộ phân loại theo từng phiên chạy thường xuyên hơn nhiều vẫn giữ mặc định rẻ |
| `DASHBOARD_VALUE_SUMMARY_MODEL` | dự phòng về `DASHBOARD_FOCUS_SUMMARY_MODEL`, rồi `DASHBOARD_FOCUS_INFER_MODEL`, rồi `haiku` | Model cho phần tổng hợp "cấp độ" của Value Pool trên trang Project Detail (`POST /api/project-plans/altitudes`) — các cấp độ Project/Stakeholder bằng ngôn ngữ đơn giản hiển thị cạnh sự kiện gốc của mỗi đơn vị trong pool, cùng cơ chế phân cấp như tóm tắt cửa sổ Focus ở trên, chỉ ở một tầm cao hơn |
| `DASHBOARD_FOCUS_INFER_TIMEOUT_MS` | `30000` (30 giây) | Đồng hồ kill (ms) cho một lần chạy `claude -p` của suy luận tiêu điểm (SIGTERM, rồi SIGKILL) |
| `DASHBOARD_REMOTE_SYNC_MS` | `60000` | Khoảng thời gian (ms) để kéo dữ liệu từ các nguồn từ xa qua `rsync`. Đặt `0` để tắt việc poll nguồn từ xa |
| `DASHBOARD_REMOTE_ACTIVE_WINDOW_MS` | `600000` (10 phút) | Cửa sổ độ mới cho trạng thái live của một phiên **Nguồn dữ liệu từ xa**. Ở mỗi lần đồng bộ, một phiên từ xa có transcript đã sao chép (mirror) được sửa trong vòng cửa sổ này sẽ được giữ ở `active`; khi bản sao ngừng cập nhật lâu hơn ngưỡng này, phiên được đối chiếu về `completed`. Các phiên từ xa không nhận hook trực tiếp, nên tùy chọn này thay thế các lượt quét liveness/stale cục bộ (vốn bỏ qua chúng). Tăng giá trị này cho các kết nối chậm hoặc các lượt nghỉ (idle) dài |
| `DASHBOARD_REMOTE_SYNC_TIMEOUT_MS` | `600000` | Thời gian chờ tối đa cho lần `rsync` của mỗi nguồn từ xa |
...
# Kế hoạch & tiêu điểm (AGENT-PLAN.md)
ccam focus [--session id]         # kế hoạch + tiêu điểm + trôi lệch hiện tại của một phiên (bí danh: focus status)
ccam focus set <n> [note]         # khai báo phiên này đang phục vụ mục kế hoạch nào
ccam focus push <description>     # khai báo một detour (việc ngoài kế hoạch nhưng cần thiết)
ccam focus bug "<title>" "<summary>" [--detail "<text>"]     # push một detour gắn nhãn sửa lỗi (bug fix)
ccam focus feature "<title>" "<summary>" [--detail "<text>"] # push một detour gắn nhãn tính năng nhỏ (small feature)
ccam focus pop                    # kết thúc detour hiện tại (thường, bug, hoặc feature)
ccam focus done <n>               # khai báo một mục kế hoạch hoàn thành (checkbox trong tệp vẫn do con người quản)
                                  # (bên trong một phiên Claude Code, luồng hook ghi nhận các thao tác ghi;
                                  #  bên ngoài, CLI xác định phiên theo cwd hoặc --session)
...
| `npm run desktop:dmg`   | **macOS:** Tạo **cả hai** DMG theo kiến trúc (arm64 + x64) — đúng cho bản phát hành. **Chậm.** |
...
Ngoài các sự kiện, Tabby còn theo dõi cả đồng hồ: nếu một phiên đang hoạt động chạy một model đắt tiền (Opus hoặc Fable) liên tục hơn 15 phút, nó sẽ hiển thị một lời nhắc nhở một-lần duy nhất — *"bạn đang dùng một model đắt tiền — bạn có chắc muốn tiếp tục không?"* — để một phiên chạy model cao cấp kéo dài không âm thầm tích lũy chi phí mà không ai để ý. Lời nhắc chỉ hiện một lần cho mỗi streak; chuyển khỏi model đó rồi quay lại sẽ tái kích hoạt nó. Các thẻ phiên trên toàn dashboard cũng hiển thị tên model bằng màu đỏ tươi bất cứ khi nào phiên đang dùng Opus hoặc Fable, để tín hiệu chi phí hiện rõ trước khi Tabby kịp nói gì.
...
| `POST`   | `/api/remote-sources/:id/sync`   | Kích hoạt `rsync` + nhập ngay cho nguồn đó                  |
...
**Các loại tin nhắn:** `session_created`, `session_updated`, `agent_created`, `agent_updated`, `new_event`, `remote_source.status`
...
| `Stop`            | Claude trả lời xong            | Không lỗi: Agent chính → `waiting` — Claude xong lượt. Cờ **Đang chờ** chỉ được đóng dấu khi không còn subagent nào đang working; nếu vẫn còn agent đang chạy, phiên vẫn giữ **Hoạt động** (đang chờ agent của mình) và việc đóng dấu được hoãn đến `SubagentStop` cuối cùng. `stop_reason=error`: đánh dấu Agent và phiên `error`. Subagent nền vẫn tiếp tục chạy |
| `SubagentStop`    | Subagent nền đã hoàn tất       | Khớp và hoàn thành Subagent theo mô tả, loại hoặc nhiệm vụ. Cố tình KHÔNG xóa cờ chờ — Subagent xong không nói lên gì về người dùng. Nếu đó là subagent **cuối cùng** còn đang working và lượt chính đã kết thúc, đóng dấu cờ chờ mà `Stop` trước đó đã hoãn (phiên lật từ Hoạt động → Đang chờ). **Kích hoạt quét JSONL fire-and-forget** (`scanAndImportSubagents`) phát các sự kiện `PreToolUse` + `PostToolUse` cho từng tool dưới `agent_id` của chính subagent, để Timeline hiển thị đầy đủ tool subagent đã chạy chứ không chỉ marker spawn |
...
| `Focus`        | Chạy `ccam focus …` trong tool Bash của phiên | Được parse từ `tool_input.command` của hook `PostToolUse` (kênh duy nhất mang sẵn session id). `set`/`push`/`pop`/`done`/`bug`/`feature` cập nhật hàng `session_focus` của phiên (con trỏ mục kế hoạch + ngăn xếp detour) và ghi một sự kiện `Focus` kèm ảnh chụp nội dung mục cho dòng thời gian; phát `session_focus` (và `plan_updated` sau `done`). `bug`/`feature` còn mang thêm `kind`/`title`/`detail` trên khung detour được push, được gắn huy hiệu trong màn hình Plan. `status` là chỉ-đọc và không ghi nhận gì |
