...
| **Hàng chờ WIP**                  | Trang trực tiếp (`/wip`, ngay sau Trang tổng quan trong thanh bên) hiển thị một hàng chờ duy nhất, ưu tiên theo thứ hạng, gồm mọi phiên `active` — phiên đang chờ bạn nhập liệu luôn xếp trên; hòa (và thứ tự giữa các phiên không chờ) được phân giải bằng **priority** kéo-thả của dự án sở hữu phiên đó, rồi đến hoạt động gần nhất. Thẻ tái sử dụng cùng một thẻ phiên, có thêm tiêu đề tên dự án nổi bật. Bố cục 1/2/3 cột responsive lấp đầy từ trên xuống, dựa trên chiều rộng đo được của chính hàng chờ (không phải viewport trình duyệt), nên việc mở/thu thanh bên vẫn đổi đúng số cột. Thanh bên **ưu tiên** bên phải có thể thu gọn liệt kê các dự án (mặc định chỉ những dự án đang có mặt trong hàng chờ, có thể bật "hiện tất cả dự án") và cho kéo-thả để sắp xếp lại — commit qua `PUT /api/projects/reorder` và phát trực tiếp tới mọi tab/máy khác đang mở qua `project_updated`, giống cách bố cục màn hình của Kanban Board đồng bộ |
| **Dự án**                         | Gom các phiên theo (các) thư mục mà chúng chạy từ đó thành một dự án do người dùng đặt tên, thay vì các khung nhìn Agent/Phiên dạng phẳng. Một dự án sở hữu một hoặc nhiều thư mục làm việc (một thư mục chỉ thuộc về nhiều nhất một dự án); tư cách thành viên được suy ra bằng cách đối chiếu `cwd` của mỗi phiên với các thư mục đã ánh xạ của dự án — không có thay đổi schema nào trên chính `sessions`. Mỗi dự án hiển thị dưới dạng một hàng có thể cuộn ngang gồm các phiên của nó (các `SessionCard`), kèm huy hiệu tổng số phiên / số đang hoạt động / hoạt động gần nhất được tính toán ở phía máy chủ. Tạo một dự án (có thể gắn ngay một thư mục hiện có), đổi tên nó inline, thêm/xóa ánh xạ thư mục về sau, hoặc xóa dự án (các ánh xạ thư mục biến mất theo kiểu cascade; các phiên bên dưới không bị ảnh hưởng và chỉ đơn giản quay về bucket **Chưa phân loại**, nơi cũng cung cấp thao tác một-chạm "Tạo dự án mới từ thư mục này"). Hậu thuẫn bởi `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id`, và `POST/DELETE /api/projects/:id/paths[/:pathId]` |
