...
| **코치**                          | 독립 페이지(`/coach`, 사이드바 라벨 **코치**, Usage 바로 다음에 위치 — 둘 다 사이드바 자체의 시각적으로 구분된 "우리가 추가한" 영역에 함께 있으며, 이 영역은 Projects/Calendar/Focus/Usage/코치를 하나의 구역으로 읽히게 하는 약간 더 밝은 배경 패널로 감싸져 있습니다)로, 계정이 실제로 Claude Code를 사용하는 방식 — 속도, 우회, 재시도, 유휴 시간 — 을 관찰하는 패턴 감지 및 추천 레이어를 위해 예약되어 있습니다. 현재는 초기 스캐폴드 단계로, 아직 분석 엔진이 연결되어 있지 않아 실제 데이터 대신 정직한 빈 상태(`EmptyState`)와 앞으로 제공될 기능(패턴 감지, 추천, 놓치기 쉬운 부분)을 보여주는 세 개의 미리보기 타일만 렌더링합니다 |
