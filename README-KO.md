...
사이드바에서 대시보드, 진행 중 대기열, 달력, 포커스, Kanban 보드, 세션 목록, 활동 피드, 분석, 워크플로, 설정에 빠르게 접근할 수 있습니다. 각 페이지는 실시간 업데이트와 풍부한 시각화로 Claude Code 에이전트 활동에 대한 깊이 있는 인사이트를 제공하도록 설계되었습니다.
...
| **진행 중 대기열**                  | 사이드바에서 대시보드 바로 다음에 있는 실시간 페이지(`/wip`)로, 모든 `active` 세션을 우선순위 순으로 하나의 대기열에 정렬합니다 — 입력을 기다리는 세션이 항상 맨 위에 오며, 동점(및 대기 중이 아닌 세션들 사이의 순서)은 해당 세션이 속한 프로젝트의 드래그 설정 **우선순위**, 그다음 최근 활동 순으로 결정됩니다. 카드는 동일한 세션 카드에 프로젝트 이름을 눈에 띄게 얹은 형태입니다. 반응형 1/2/3열 레이아웃은 브라우저 뷰포트가 아니라 대기열 자체의 측정된 너비로 위에서 아래로 채워지므로, 사이드바를 펼치거나 접어도 열 수가 올바르게 바뀝니다. 접을 수 있는 오른쪽 **우선순위 사이드카**는 프로젝트를 나열하고(기본은 대기열에 있는 프로젝트만, "모든 프로젝트 표시" 토글 가능) 드래그로 순서를 바꿀 수 있으며 — `PUT /api/projects/reorder`로 커밋되고, Kanban Board의 모니터 레이아웃과 같은 방식으로 `project_updated`를 통해 열려 있는 다른 모든 탭/컴퓨터에 실시간으로 반영됩니다 |
| **프로젝트**                        | 평평한 에이전트/세션 뷰 대신, 세션이 실행되는 폴더를 기준으로 사용자가 이름을 지정하는 프로젝트로 그룹화합니다. 프로젝트는 하나 이상의 작업 디렉터리를 소유하며(폴더는 최대 하나의 프로젝트에만 속할 수 있음), 소속 여부는 각 세션의 `cwd`를 프로젝트에 매핑된 폴더와 조인하여 도출됩니다 — `sessions` 자체에는 스키마 변경이 없습니다. 각 프로젝트는 서버 측에서 계산된 집계 세션 수 / 활성 수 / 마지막 활동 배지와 함께, 소속 세션(`SessionCard`)들을 가로로 스크롤 가능한 행으로 렌더링합니다. 프로젝트를 생성하고(기존 폴더를 즉시 연결하는 옵션 포함), 인라인으로 이름을 바꾸고, 이후에 폴더 매핑을 추가/제거하거나, 삭제할 수 있습니다(폴더 매핑은 함께 사라지지만 기반이 되는 세션은 그대로 유지되어 **미할당** 버킷으로 돌아갑니다 — 이 버킷은 원클릭 "이 폴더로 새 프로젝트 만들기" 작업도 제공합니다). `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id`, `POST/DELETE /api/projects/:id/paths[/:pathId]`가 이를 뒷받침합니다 |
