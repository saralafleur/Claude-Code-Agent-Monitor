...
| **코치**                          | 두 개의 페이지(`/coach`와 `/coach/playbook`, 사이드바 라벨 **코치**, Usage 바로 다음에 위치 — 둘 다 사이드바 자체의 시각적으로 구분된 "우리가 추가한" 영역에 함께 있으며, 이 영역은 Projects/Calendar/Focus/Usage/코치를 하나의 구역으로 읽히게 하는 약간 더 밝은 배경 패널로 감싸져 있습니다)가 상단의 공유 **Feed**/**Playbook** 탭 바(`CoachTabs`)로 전환됩니다. `/coach`는 **Feed** — 실시간으로 갱신되는 **Observation** 목록입니다. `/coach/playbook`은 **Playbook** 편집기로, **Practice**(점검 항목)의 카탈로그입니다. 각 Practice는 사용자가 편집할 수 있는 **config**(임계값)를 가지며, 서버 측에 저장되어 연결된 모든 컴퓨터에서 실시간으로 공유됩니다 — Usage 페이지의 Color Thresholds 카드와 동일한 구조입니다(`GET`/`PUT`, 변경 시 WebSocket 업데이트 브로드캐스트). 어떤 Practice의 조건이 특정 범위(세션/프로젝트/전역)에서 충족되면 Coach 엔진이 Observation을 기록합니다 — 이것이 Feed에 표시되는 단위입니다. 사용자는 카드에서 Observation을 범위와 상관없이 **Dismiss**(닫기)할 수 있습니다(acknowledge/resolve도 상태로 존재하지만 v1 UI에는 아직 노출되지 않습니다). 현재 두 개의 Practice가 제공됩니다. `session-token-ceiling`(범위: 세션)은 세션의 총 토큰 사용량(`token_usage`의 모든 차원을 합산)을 관찰하다가 설정 가능한 임계값(기본값 100,000,000 토큰)을 넘으면 `/compact` 또는 `/clear`를 권장하는 Warning 종류의 Observation을 생성하며, 카드에는 두 가지 제안 작업 — 해당 세션으로 딥링크되는 "Open session"과 클립보드로 복사하는 "Copy summary" — 이 붙습니다. `account-weekly-balance`(범위: 전역)는 활성화된 각 Claude 계정의 최신 주간 한도 사용률(Usage 페이지의 Accounts 패널)을 관찰하다가, 여유가 남은(사용률 100% 미만) 계정이 둘 이상이고 그중 가장 낮은 계정과 가장 높은 계정의 사용률 차이가 설정 가능한 퍼센트포인트 임계값(기본값 25)을 넘으면, 사용률이 더 낮은 계정으로 작업을 전환하라고 권장하는 Reminder 종류의 Observation을 생성합니다 — 이렇게 하면 세션 전환이 필요할 때 언제나 여유 있는 계정으로 넘어갈 수 있고, 한 계정만 홀로 소진되는 일도 막을 수 있습니다. 백그라운드 스케줄러(`server/lib/playbook/engine.js`, reconciliation 스케줄러의 틱 구조를 그대로 따름)가 5분 간격 틱(`DASHBOARD_PLAYBOOK_MS`, 기본값 `300000`; 킬 스위치 `DASHBOARD_PLAYBOOK_MODE=off`)마다 활성화된 모든 세션 범위 Practice를 모든 활성 세션에 대해 평가하고, 활성화된 모든 전역 범위 Practice는 한 번씩 추가로 평가하며, 동일한 Practice+범위 조합에 대해 아직 열려 있는(`open`/`acknowledged`/`dismissed`/`resolved`) Observation이 있는 동안에는 중복 발생시키지 않습니다. Playbook 페이지는 카탈로그의 Practice마다 카드를 하나씩 렌더링하여 각각 켜고 끄거나 임계값을 편집할 수 있으며, 실제 생성될 Observation 카드를 실시간으로 미리 볼 수 있습니다 — 실제 Feed와 동일한 `ObservationCard` 컴포넌트로 렌더링되며 유사품이 아닙니다. `GET`/`PUT /api/playbook/practices[/:id/config]`와 `GET /api/coach/observations`, `POST /api/coach/observations/:id/respond`가 이를 지원합니다. 이 프레임워크(스키마, 엔진, API, UI)는 완전히 범용적이어서 — 새 Practice는 새로운 배관이 아니라 카탈로그에 새 항목을 추가하는 것으로 그칩니다 |
