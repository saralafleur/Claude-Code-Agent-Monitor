...
> [!TIP]
> 참고: 영어 원문은 [README.md](./README.md), 중국어 버전은 [README-CN.md](./README-CN.md) (中文版本), 베트남어 버전은 [README-VN.md](./README-VN.md) (Phiên bản tiếng Việt)를 확인하세요. 이 문서는 한국어 버전입니다.
...
사이드바에서 대시보드, 달력, 포커스, Kanban 보드, 세션 목록, 활동 피드, 분석, 워크플로, 설정에 빠르게 접근할 수 있습니다. 각 페이지는 실시간 업데이트와 풍부한 시각화로 Claude Code 에이전트 활동에 대한 깊이 있는 인사이트를 제공하도록 설계되었습니다.
...
| **프로젝트**                        | 평평한 에이전트/세션 뷰 대신, 세션이 실행되는 폴더를 기준으로 사용자가 이름을 지정하는 프로젝트로 그룹화합니다. 프로젝트는 프로젝트마다 하나씩 전체 카드를 쌓는 대신, 이름 / 세션 수 / 활성 수 / 마지막 활동 / 고정·계획·리포트·삭제 빠른 작업을 담은 한 행씩으로 이루어진 컴팩트한 표로 나열되며(행 자체를 드래그로 재정렬 가능), 프로젝트가 많은 머신에서 페이지가 세로로 끝없이 늘어지지 않게 합니다. 프로젝트는 하나 이상의 작업 디렉터리를 소유하며(폴더는 최대 하나의 프로젝트에만 속할 수 있음), 소속 여부는 각 세션의 `cwd`를 프로젝트에 매핑된 폴더와 조인하여 도출됩니다 — `sessions` 자체에는 스키마 변경이 없습니다. 각 행은 셰브런으로 펼칠 수 있으며(기본값은 접힌 상태) 펼치면 폴더 매핑, AGENT-PLAN.md 체크리스트, 그리고 가로로 스크롤 가능한 세션(`SessionCard`) 행이 나타납니다(**미할당** 버킷 안의 각 폴더도 동일하게 기본 접힌 세션 카드 행을 사용합니다). 프로젝트를 생성하고(기존 폴더를 즉시 연결하는 옵션 포함), 인라인으로 이름을 바꾸고, 행의 고정 아이콘으로 **고정**하여(수동 드래그 순서와 무관하게 고정되지 않은 모든 프로젝트 위로 띄웁니다 — 고정된 행과 고정되지 않은 행은 각자 그룹 내 상대적 순서를 유지하지만 고정된 그룹이 항상 먼저 표시되며, 고정 상태는 수동 드래그 순서와 달리 브라우저별이 아니라 서버에서 공유됩니다), 이후에 폴더 매핑을 추가/제거하거나, 삭제할 수 있습니다(폴더 매핑은 함께 사라지지만 기반이 되는 세션은 그대로 유지되어 **미할당** 버킷으로 돌아갑니다 — 이 버킷은 원클릭 "이 폴더로 새 프로젝트 만들기" 작업도 제공합니다). `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id`(이름 변경 및/또는 고정), `POST/DELETE /api/projects/:id/paths[/:pathId]`가 이를 뒷받침합니다 |
| **달력**(프로젝트 간 보드) | `/focus-calendar`, 사이드바 레이블 **달력**, 페이지 제목도 그냥 "달력"이며, 프로젝트 바로 다음에 위치하는 독립 페이지로, 프로젝트별 포커스 시간 리포트의 달력 탭과 동일한 스윔레인 데이-뷰 캘린더를 렌더링하되, 모니터링되는 **모든** 프로젝트에 걸쳐 한 번에, 페이지의 전체 가용 너비를 차지합니다(자체 좌우 패딩은 25px뿐이며, 앱 셸의 일반적인 패딩을 벗어남) — 가운데 정렬된, 폭이 제한된 컬럼 대신입니다. 세 가지 독립적인 필터가 의존 체인이 아니라 교집합 방식으로 조합됩니다: **프로젝트** 필터(칩 — 현재 창에서 활동이 있는 프로젝트마다 하나씩, 그리고 "모든 프로젝트"와 고정되어 항상 보이는 호박색 **"미할당"** 칩 — 폴더가 어떤 프로젝트에도 매핑되지 않은 세션으로 범위를 지정하며, "미할당"과 실제 프로젝트 선택은 상호 배타적이고 클라이언트 측과 서버 측 모두에서 강제됩니다), **세션** 필터(모든 프로젝트에 걸친 모든 세션을 나열하는 단일 전역 드롭다운 — 현재 선택된 프로젝트로 범위가 제한되지 않으므로, 선택된 프로젝트에 속하지 않는 폴더를 가진 세션을 고르는 것도 정당한 조합이며 단순히 빈 결과를 반환합니다), 그리고 **기간** 필터(하루 단위의 이전/오늘/다음 탐색, 기본값은 "오늘", 클릭 가능한 요일/날짜 레이블을 통해 네이티브 날짜 선택기를 열어 화살표 탐색 없이 원하는 날짜로 곧바로 이동할 수 있으며, 그리고 사용자 지정 날짜 범위 모드) — 이 중 하나를 변경해도 다른 필터는 절대 초기화되지 않습니다. 새로운 집계 엔드포인트 `GET /api/focus-report`가 이를 지원하며, 이 엔드포인트는 항상 명시적인 `from`/`to` 구간을 요구하고(숨겨진 서버 측 기본값 없음) `server/lib/focus-report.js`의 리포트 구성 함수를 완전히 그대로 재사용합니다. 기존의 프로젝트별 리포트 모달(프로젝트 카드에서 열림)은 영향을 받지 않습니다 — 그 통계 타일/List-Calendar 토글/목록 본문 렌더링은 중복 구현되는 대신 `FocusReportBody.tsx`를 통해 이 페이지와 공유됩니다. 이제 하나의 리포트가 여러 프로젝트에 걸칠 수 있으므로, 여기서는 Concurrency 통계 타일이 **"동시 에이전트 세션"**으로 다시 레이블링됩니다(동일한 `concurrency_ratio` 수치이며, 하나의 프로젝트 자체의 멀티태스킹이 아니라 프로젝트 간 중첩으로 올바르게 해석됩니다) |
| **포커스**(활동 리포트) | `/focus`, 사이드바 레이블 **포커스**, 달력 바로 다음에 위치하는 독립 페이지로, 스윔레인 그리드 대신 평범하고 이해관계자가 읽기 쉬운 목록으로 "우리가 실제로 무엇을 했는가"에 답합니다. 달력 보드와 동일한 세 가지 독립적인 필터 — 프로젝트 칩("모든 프로젝트"와 호박색 **"미할당"** 칩 포함), 전역 세션 드롭다운, 그리고 하루/사용자 지정 범위 기간 선택기 — 를 그대로 공유하며, 정확히 동일한 `ProjectScopeFilters` 컴포넌트와 `GET /api/focus-report` 엔드포인트를 재사용하므로 백엔드 작업이 전혀 필요하지 않았습니다. 달력 보드와 동일한 하루 내 **시간대 확대(hour-window zoom)** 기능도 제공합니다 — 기간 프리셋(`4h`/`8h`/`12h`/`24h`), 시작 시각 스테퍼/직접 입력, "실시간"(Live) 토글, 빠른 시작 프리셋(`HourWindowZoomBar`/`useHourWindowZoom` — `FocusCalendarView`에서 분리되어 두 페이지가 동일한 컨트롤을 공유) — 이 페이지에서는 달력 보드 자체의 4시간 기본값 대신 전체(확대하지 않은) 기간을 기본값으로 삼는데, 이 페이지는 원래 항상 선택한 구간 전체를 보여주었고 확대 기능은 추가로 좁혀보는 선택 사항일 뿐이기 때문입니다. 동일한 통계 타일(총 에이전트 시간, Concurrency, 선언된 항목 비율 %, 계획 외 비율 %, 유휴 제외) 아래에는 **활동 카드**가 있습니다: 선택한 구간(확대가 활성화되면 확대된 구간으로 추가 축소되어, 통계 타일과 목록이 절대 서로 어긋나지 않습니다)에서 일어난 서로 다른 일마다 한 행씩 — 플랜 항목, 우회 작업/버그/기능(그것에 걸린 모든 세션에 걸쳐 제목별로 그룹화됨), 또는 미분류 시간 — 각 행은 종류 칩(앱에 이미 있는 `FOCUS_KIND_CONFIG`/`FOCUS_KIND_ICONS` 어휘를 재사용), 실제 경과/활성 시간 수치, 그리고 사람이 읽기 쉬운 **시작/종료 시각** 범위(예: "8:49 AM – 9:12 AM", `client/src/lib/format.ts`의 `formatTimeRange()` 헬퍼 사용 — 병합된 행의 경우 가장 이른 기여 세그먼트의 시작부터 가장 늦은 세그먼트의 종료까지이며 반드시 연속적이지는 않습니다), 이어서 줄표(em dash)와 두 시각 사이의 경과 시간을 일/시간/분 단위로 표시한 값(예: "8:49 AM – 9:12 AM — 23m", `formatDurationLong()` 헬퍼 사용)을 표시하며, 분류기가 **추론한** 항목의 경우 한 문장짜리 `inferred_reason`도 함께 표시합니다(`client/src/lib/focusActivity.ts`의 `groupFocusActivity()`가 cwd별로 키를 부여하므로 서로 다른 두 프로젝트의 동일한 항목 번호가 절대 합쳐지지 않습니다; 추론된 이유가 **있는** 미분류 세그먼트는 세션마다 하나의 서술 행으로 유지됩니다 — 실제로 서로 다른 계획 외 작업을 한 두 세션은 더 큰 세션의 이야기만 담은 한 행이 아니라 두 행으로 표시됩니다 — 반면 이유가 없는 미분류 세그먼트만 공유 꼬리 버킷으로 합쳐집니다; 하나의 키에 두 개 이상의 세션이 겹치면 표시되는 레이블/이유는 시간 비중이 가장 큰 세션에서 가져오며, "세션 N개 더" 메모는 **토글**로서 펼치면 기여한 각 세션의 이름, 시각 범위, 실제 경과/활성 시간 구분, 이유를 보여줍니다). 활동 카드 위에는 AI가 종합한 **요약** 블록이 있습니다 — 선택한 구간에 대한 쉬운 언어로 된 "우리가 실제로 무엇을 했는가" 불릿 — "모든 프로젝트" 범위에서는 **프로젝트별로 그룹화**되어 활동이 있는 프로젝트마다 하나의 불릿 묶음이 실제 경과 시간 비중이 큰 순서로 표시되고, 매핑되지 않은 폴더를 위한 "미할당" 그룹이 추가되며, 단일 프로젝트 범위에서는 그 하나의 그룹이 헤더 없이 표시됩니다 — (`GET /api/focus-report/summary`, 포커스 추론 서비스의 헤드리스 `claude -p` 계약과 설정 값을 재사용; 서버 측에서 데이터 다이제스트로 게이트되는 캐시를 사용하므로 끝난 하루는 한 번만 생성되고, 아직 진행 중인 하루는 데이터가 바뀔 때만 다시 생성됩니다; 작은 푸터 메모가 불릿을 작성한 모델의 이름을 표시합니다). 여러 날에 걸친 구간은 **계층적으로** 요약됩니다 — 먼저 하루하루가 각각 요약되고(그날의 데이터가 더 이상 바뀌지 않으면 영구적으로 캐시됩니다), 그다음 단 한 번의 롤업이 일별 불릿으로부터 전체 구간을 종합하며, 불릿 예산은 4개(하루 이틀)에서 8개(여러 주)까지 구간에 따라 늘어납니다 — 그래서 3주로 축소해 보아도 수십 개의 세션을 모호한 주제로 평균 내는 대신 일별 요약이 지목한 구체적인 산출물이 유지됩니다. 사용할 수 없을 때(LLM 경로 꺼짐, 빈 구간, 또는 생성 실패)에는 완전히 숨겨지며 절대 오류 상태로 표시되지 않습니다. "모든 프로젝트" 범위에서는 각 행 앞에 해당 프로젝트 이름이 붙고, 단일 프로젝트 범위에서는 이 접두어가 생략됩니다. 5행을 넘는 항목은 "더 보기"/"간략히 보기" 토글 뒤로 접힙니다. 실시간 `ccam focus push/bug/feature` 선언은 오늘 기준으로 자신의 레이블과 별개인 이유를 갖지 않습니다 — 분류기가 추론한 항목만 이유를 갖습니다 — 아직 해결되지 않은 알려진 공백입니다 |
| **프로젝트 매니저**(포트폴리오 롤업) | `/project-manager`, 사이드바 레이블 **프로젝트 매니저**, 포커스 바로 다음에 위치하는 독립 페이지로, 추적 중인 모든 프로젝트를 하나씩 확인하는 대신 "내가 추적하는 모든 프로젝트에서 지금 무슨 일이 일어나고 있는가"에 한눈에 답합니다. 프로젝트마다 한 행씩(목표/마일스톤 진행률 바, 페이스 배지, 미해결 결정 건수, 마지막 활동)으로 이루어진 롤업 테이블이 **결정 큐(Decision Queue)** 위에 놓입니다 — 결정 큐는 대기 중인 레이어 6 조정(reconciliation) 항목(`pace_alert`/`detour_volume`/`detour_disposition`/기록 실패)을 실행 가능한 카드로 렌더링하며, 각 카드는 그 자리에서 바로 해결할 수 있습니다(우회 작업의 경우 `fold_in`/`new_item`/`deliberate`/`discard` — 조정 틱 자체가 제안한 문구로 미리 채워짐; 그 외 모든 항목은 resolve/dismiss/retry). 여기에 **페이스 워치(Pace Watch)** 레일(모든 프로젝트에 걸쳐 지연된 모든 항목을, 가장 많이 지연된 순으로)과 **최근 해결됨(Recently Resolved)** 로그가 더해집니다. 프로젝트 범위 칩 행이 테이블·큐·레일을 하나의 프로젝트로 필터링합니다. 기존/신규 엔드포인트 세 개(`GET /api/projects`, `GET /api/portfolio/summary`, `GET /api/decision-queue`)를 그 로직을 중복 구현하지 않고 조합하며, 그동안 서버 전용으로만 출시되어 있던 레이어 4–6 포트폴리오 관리 작업(우회 작업 처리, 페이스 추적, 조정)을 기반으로 만들어진 첫 클라이언트 페이지입니다 |
| **Kanban 보드**                   | 헤더 토글(`localStorage`에 저장)로 전환하는 세 가지 보기: **에이전트** — 4개 컬럼(Working / Waiting / Completed / Error); **세션** — 5개 컬럼(Active / Waiting / Completed / Error / Abandoned); 그리고 **프로젝트** — 프로젝트별로 하나씩 컬럼이 생성되며(세션은 해당 프로젝트에 매핑된 폴더 기준으로 그룹화되고, 상태 컬럼과 구분되도록 컬럼 색상이 순환됩니다), 아직 어떤 프로젝트에도 속하지 않은 폴더의 세션을 위한 **미할당** 컬럼이 마지막에 추가됩니다. 적어도 하나의 **모니터** 그룹을 만들면(물리적 디스플레이를 본뜬 것으로, 헤더의 **+ 모니터 추가** 버튼으로 생성) 거기에 배정된 프로젝트 컬럼은 그 모니터 전용의 테두리 있는 드래그 가능한 박스 안에 실제로 담겨 렌더링됩니다 — 단순히 태그만 붙는 게 아니라 진짜로 그 안에 포함됩니다 — 이 박스들은 같은 가로 스크롤 한 줄 안에서 서로 나란히 놓이고, 마지막에는 아직 배정되지 않은 프로젝트 컬럼을 실제로 담는 **미분류** 박스가 추가됩니다(모니터 박스와 같은 컨테이너지만 고정되어 있어 드래그·이름 변경·삭제가 불가능합니다). 프로젝트 컬럼을 어떤 모니터의 박스 위로(또는 이미 그 박스 안에 있는 다른 컬럼 위로) 드래그하면 그 박스 안으로 옮겨지고, 미분류 박스 위로(또는 행 끝을 지나서) 드래그하면 배정이 해제되어 그 컬럼이 미분류 박스 안에 들어가며, 한 모니터의 박스를(그 헤더를 통해) 다른 모니터의 박스 위로 드래그하면 박스 전체가 좌우로 위치를 바꿉니다 — 실제 책상 위에서 모니터가 놓인 위치를 그대로 반영합니다. 박스 이름은 그 자리에서 바로 수정할 수 있고, 헤더의 셰브론을 통해 접거나 펼칠 수 있으며 — 접으면 박스 전체가 메인 행에서 완전히 빠져나와 그 위의, 접힌 모든 모니터가 공유하는 얇은 띠로 옮겨가므로 그저 좁아진 채로 자리만 차지하는 대신 실제로 가로 공간을 확보하고, 펼치면 원래 순서대로 메인 행으로 다시 내려옵니다(그동안 컬럼은 계속 마운트된 채로 시각적으로만 숨겨지므로 카드 자체의 "더 보기" 상태가 토글 후에도 유지됩니다) — 삭제하면 그 안의 프로젝트는 미분류로 돌아갑니다. 셰브론 옆의 두 번째 아이콘 버튼은 **레이아웃 메뉴**를 엽니다 — orientation과 wrap의 조합 하나하나를 나타내는 시각적 타일들(열 1–4 또는 자동, 행 1–4 또는 자동)로 이루어진 팝오버로, 각 타일은 해당 배치를 그대로 축소한 미리보기입니다 — 그래서 레이아웃을 고르는 데는 메뉴를 여는 클릭 한 번과 원하는 타일을 누르는 클릭 한 번이면 충분하며, 그 즉시 두 값이 함께 적용되고 팝오버가 닫힙니다. 모니터마다 독립적으로 설정되며, 같은 `MonitorGroup`의 `orientation` 필드(**가로**, 기본값, 프로젝트 컬럼이 나란히 배치됨, 또는 **세로**, 위에서 아래로 쌓임)와 `wrap` 필드(`*` 기본값, 지금까지의 무제한 한 줄/한 칸, 또는 `1`–`4`, 한 줄/칸에 몇 개의 프로젝트 컬럼이 들어갈지 정한 뒤 새 줄/칸으로 넘어감)에 저장됩니다. 모든 프로젝트 컬럼(그리고 독립적인 미할당 컬럼)에도 동일한 셰브론이 있어, 그 컬럼이 어느 박스 안에 있든 상관없이 해당 컬럼 하나만 제자리에서 얇은 제목 표시줄만 남도록 접을 수 있습니다. 이 두 가지는 이제 단일한 전역 레이아웃으로, 서버 측(`dashboard_layout` 테이블, `GET`/`PUT /api/monitors`를 통해)에 저장되며 이 대시보드에 연결된 모든 컴퓨터가 공유합니다(사용자 계정이 없으므로 모두가 동일한 하나의 레이아웃을 사용합니다) — 더 이상 `localStorage`에 저장되는 개인 설정이 아니며, 한 컴퓨터에서 변경하면 `monitors_updated` WebSocket 메시지를 통해 새로고침 없이 연결된 다른 모든 컴퓨터에 즉시 반영됩니다. **Waiting** 컬럼은 에이전트에 저장된 `waiting` 상태에 직접 매핑됩니다 — Claude Code가 프롬프트에서 대기 중일 때(새 세션, 턴 사이, 또는 권한 Notification으로 차단된 상태) 설정되며, 사용자가 재개하는 순간(UserPromptSubmit / PreToolUse) `working`으로 전환됩니다. 각 컬럼 헤더에는 라이프사이클 전환을 설명하는 `?` 툴팁이 표시됩니다. Agents/Sessions 상태 컬럼에도 자체 레이아웃 메뉴가 있습니다 — 개수 배지 옆의 동일한 결합형 그리드 아이콘으로, Projects 뷰에서 모니터 박스의 메뉴가 있는 바로 그 위치입니다 — 동일한 시각적 타일 팝오버를 열어 해당 컬럼의 카드를 기본값인 세로 스택, 고정 너비 카드가 가로로 스크롤되는 행, 그리고 임의의 고정 줄바꿈 개수 사이에서 전환합니다. 위의 모니터 메뉴와 달리 이 메뉴는 서버 공유가 아니라 순수하게 브라우저별로(`localStorage`, 보기 + 상태로 키가 지정됨, 예: `agents-working`, `kanban-status-column-orientation`과 `kanban-status-column-wrap` 두 키에 걸쳐) 저장되며, 각 상태 컬럼의 설정은 다른 모든 컬럼과 독립적입니다. 카드는 저장된 상태별로 서버에서 가져오고(상태당 사실상 무제한), 이후 클라이언트 측에서 컬럼당 10개 카드로 페이지네이션되며 "더 보기" 버튼이 제공됩니다. WS 구독은 활성 보기에 범위가 지정되므로(`agent_*` vs `session_*` 프레임) 보이지 않는 보기의 업데이트가 다시 가져오기를 유발하지 않습니다. Waiting 배지는 행의 `awaiting_reason`을 호버 툴팁으로 표시합니다 — **입력 필요** (`notification`), **턴 완료** (`stop`), **프롬프트 대기** (`session_start`), **중단됨** (`interrupted`) — 컴팩트한 카드에서는 제목 공간을 유지하기 위해 툴팁으로만 표시되며, 더 넓은 화면(세션 테이블, 세션 상세 헤더)에서는 추가로 이유를 중첩된 칩 형태로 인라인 표시하고, 긴급한 이유(권한 프롬프트, 중단)는 더 진한 앰버 색으로 렌더링됩니다. 세 가지 이유가 더 있는데 — **SubAgents** (`subagent`), **Shell** (`shell`), **Monitor** (`monitor`) — 이는 Claude가 사람에게 막혀 있는 것이 아니라 자식(자신이 생성한 서브에이전트 플릿, 동기적인 Bash 호출, 또는 Monitor 도구 호출)을 통해 여전히 활발히 작업 중임을 의미하므로, "Waiting" 안에 중첩되는 대신 컴팩트한 카드에서도 배지의 단어 자체를 완전히 대체합니다. 각 카드의 모델 칩은 세션이 프리미엄/고비용 모델(Opus 또는 Fable 계열)을 실행 중일 때 밝은 빨간색으로 렌더링됩니다. 세션이 토큰을 하나라도 사용한 뒤에는 세션 카드에 **입력 / 출력 / 캐시 읽기 / 캐시 쓰기** 토큰을 보여주는 컴팩트한 막대가 오른쪽 정렬된 **Effective(실효)** 합계와 함께 추가로 표시됩니다(세션이 토큰을 사용한 후) — 캐시 읽기(매 턴마다 대화 전체 프리픽스를 다시 읽으며 입력 요금의 약 10%로 과금)와 캐시 쓰기(프리미엄 요금이 붙는 새 컨텍스트)는 별도의 수치로 분리되고, Effective는 각 모델의 실제 요금으로 가중해 입력 환산한 합계를 서버에서 계산한 값이므로, 원시 토큰 합계처럼 캐시 비중이 큰 세션을 과장하지 않고 카드의 $ 수치와 함께 움직입니다. 헤더의 뷰 전환 버튼 왼쪽에는 컴팩트한 **동시 에이전트 세션** 통계 카드가 있어, 오늘의 프로젝트 전체 동시성 비율(effort time ÷ active 또는 open-session wall-clock time — Focus 리포트의 Concurrency 타일과 같은 값)을 어느 비율을 기본으로 보여줄지 전환하는 스왑 버튼과 함께 표시합니다; 첫 요청이 완료되기 전까지는 숨겨져 있고 요청이 실패하면 다시 사라지며, 폴링 타이머 대신 보드 자체 카드가 반응하는 동일한 세션/에이전트 WebSocket 이벤트에 맞춰 새로고침됩니다 |
| **세션**                          | 기록된 모든 세션의 검색·필터링 가능한 **서버 페이지네이션** 테이블. 각 페이지 클릭은 `/api/sessions?status=&q=&limit=10&offset=…`를 호출하므로, 비용 계산은 데이터베이스에 세션이 얼마나 많이 존재하든 상관없이 보이는 페이지에 대해서만 실행됩니다. 검색 상자(`q=`)는 300 ms 디바운스와 함께 서버에서 `id` / `name` / `cwd`에 대해 대소문자를 구분하지 않는 매칭을 수행하며, 응답에는 페이지네이터 UI를 위한 `total` 카운트가 포함됩니다. 상태 필터, 검색, 페이지네이션은 함께 조합됩니다. 각 세션의 사람이 읽을 수 있는 **이름**은 트랜스크립트에서 읽어 실시간으로 동기화됩니다 — `/rename`, `claude -n`, 또는 피커의 `Ctrl+R`(JSONL `custom-title` 줄)로 지정한 명시적 제목이 항상 우선하고, 그렇지 않으면 자동 생성된 `ai-title`이 채워지며, 그것도 없으면 세션의 **첫 사용자 프롬프트**(잘라내되 tool-result / 슬래시 명령 노이즈는 건너뜀)가 플레이스홀더 이름과 메인 에이전트의 플레이스홀더 이름/작업을 채웁니다 — 그래서 제목이 지정되지 않은 세션(가져온 세션 포함)도 무엇을 하고 있는지 표시됩니다. 대시보드는 카드, 대시보드, 활동 피드, Run 재개 피커에서 그 이름을(짧은 ID로 폴백하며) 표시합니다. |
...
| **활동 피드**                     | 일시정지/재개, 다차원 필터(세션 상세와 동일한 툴바에 세션 필터 추가), 서버 주도 "더 불러오기" 페이지네이션, 로드된 페이지 크기를 유지하는 디바운스된 필터 인식 실시간 새로고침, 그룹화 토글, 프로젝트 › 세션 › 서브에이전트를 보여주는 출처 접두사, 행별 "세션 →" 버튼을 갖춘 실시간 스트리밍 이벤트 로그                                         |
| **분석**                          | 토큰 사용량, 도구 빈도, 활동 히트맵(중앙 정렬, 일요일부터 시작하는 요일 정렬, 요일 이름 툴팁), 세션 추세, 라이브/오프라인 연결 표시기. 분석 페이로드가 로드되는 동안 (통계 타일뿐 아니라) 차트 영역에 차트 레이아웃을 그대로 반영한 **맥동하는 스켈레톤 플레이스홀더**가 표시되므로 페이지가 빈/제로 차트를 잠깐이라도 표시하는 일이 없습니다 |
| **실시간 업데이트**               | WebSocket 푸시 -- 폴링 없음, 즉각적인 UI 업데이트                                                                                                                                                                                                                             |
| **자동 검색**                     | 세션과 에이전트가 Hook 이벤트로부터 자동으로 생성됩니다                                                                                                                                                                                                                               |
...
| **비용 추적**                     | 구성 가능한 가격 책정 규칙과 세션별 분석이 있는 모델별 비용 추정. **기간 한정 프로모션 요금**(가격 책정 규칙의 `intro_*` + `intro_until`)을 지원합니다: 마감일 이전(당일 포함)의 사용량은 프로모션 요금으로, 이후 사용량은 표준 요금으로 책정되므로, Claude Sonnet 5의 출시 할인(2026-08-31까지) 같은 프로모션이 과거 **및** 미래 사용량 모두에 대해 정확하게 유지됩니다 — 비용 엔드포인트는 각 날짜의 사용량을 그 날짜에 유효한 요금으로 책정합니다. **프로모션 요금은 설정에서 완전히 편집할 수 있습니다** — 모델 가격 책정 편집기가 프로모션 마감일과 카테고리별 프로모션 가격(input / output / cache-read / cache-write 5m & 1h)을 노출하므로, 향후 모델 출시 프로모션에는 코드 변경 없이 편집만으로 대응할 수 있습니다. **서브에이전트 카드는 각 서브에이전트 자신의 비용을 표시합니다**(해당 서브에이전트의 트랜스크립트 토큰 사용량에서 도출되어 현재 요금으로 책정) — 세션 합계가 아닙니다. 메인 에이전트 카드는 전체 세션을 대표하여 세션 합계를 표시하고, 서브에이전트 카드는 해당 서브에이전트가 지출한 금액만 표시하므로, 서브에이전트 카드가 마치 전체 세션 비용이 든 것처럼 오해를 일으키는 일이 더 이상 없습니다. 컴팩션 인식 토큰 계산은 컨텍스트 압축 전반에 걸쳐 총계를 보존합니다. 트랜스크립트 읽기는 효율적인 토큰 추출을 위해 증분 바이트 오프셋 업데이트로 캐시됩니다        |
| **트랜스크립트 캐시**             | JSONL 트랜스크립트로부터의 실시간 추출: 토큰, 컴팩션, API 오류(`isApiErrorMessage` 항목은 `APIError` 이벤트로 저장), 턴 소요 시간(`TurnDuration` 이벤트로 저장), thinking 블록 수, usage 추가 정보(service_tier, speed, inference_geo). 항목별 증가형 배열은 `TRANSCRIPT_CACHE_MAX_ARRAY_LEN`(기본값 `1000`, 구성 가능)으로 꼬리가 제한됩니다 — 파싱 중과 마무리 시 모두 — 그래서 며칠 동안 실행되는 세션이라도 단일 캐시 항목을 무한정 키울 수 없습니다. 각 항목은 `{mtimeMs, size, bytesRead, result}`만 저장하므로, 동일한 데이터가 최상위와 `result` 내부 양쪽에 그림자 복사본으로 존재하지 않습니다. 세션 메타데이터는 이 필드들로 실시간으로 보강됩니다 |
...
| **플랜 인지 모니터링**            | "이 세션은 무엇을 하고 있고, 프로젝트 플랜의 어느 부분을 위한 것인가?"를 한눈에 답합니다. 모니터링되는 각 저장소는 루트에 사람이 승인한 `AGENT-PLAN.md`(번호가 매겨진 체크박스 항목)를 둘 수 있습니다; 백그라운드 폴링(`DASHBOARD_PLAN_POLL_MS`, 그리고 `SessionStart` 시의 기회적 수집)이 이를 읽기 전용으로 대시보드에 미러링하며, cwd를 키로 삼아 프로젝트 단위로 집계합니다. 세션은 자신의 Bash 도구에서 `ccam focus set/push/pop/done`을 실행해 자기 위치를 선언합니다 — **Hook 스트림**이 쓰기 채널이므로(`PostToolUse` 이벤트가 이미 세션 id를 담고 있음) Hook이 연결된 모든 프로젝트에서 프로젝트별 설정 없이 동작합니다. 프로젝트 페이지는 각 플랜을 진행률 바와 항목별 세션 칩이 있는 접을 수 있는 체크리스트로 보여줍니다; 모든 세션 카드에는 현재 상태를 나타내는 공통 아이콘(알려진 항목에는 크로스헤어, 그 외에는 우회/기능/버그 아이콘)이 있는 한 줄짜리 포커스 브레드크럼이 표시됩니다 — 예: `Item 4: Migrate auth ▸ npm conflict (23m)` — 기준 항목이 선언되지 않은 채로 우회 작업 중인 세션에도 표시됩니다; 세션 상세에는 포커스 배너와 **Plan** 탭(체크리스트, `Focus` 이벤트에서 해석한 포커스 타임라인, 그리고 `GET /api/sessions/:id/todos`를 통한 해당 세션의 최신 TodoWrite 마이크로 플랜)이 추가됩니다. **드리프트 감사**(`DASHBOARD_FOCUS_AUDIT_*`)는 포커스가 선언된 세션의 최근 활동이 그 선언과 일치하는지 주기적으로 확인하고 — 작은 모델에서의 headless `claude -p`에 키워드 중첩 폴백을 더한 방식 — 호박색 "선언되지 않은 우회 작업일 가능성?" 배지를 찍습니다; 감사는 절대 포커스를 다시 쓰지 않으며, 선언이 배지를 지우지도 않습니다. `ccam focus bug`/`feature "<title>" "<summary>" [--detail "<text>"]`는 종류가 태그된 우회 작업을 push하며, Plan 뷰의 항목별 포커스 줄은 각 세션의 실제 현재 상태(알려진 항목, 단순 우회 작업, 기능, 버그)를 일치하는 아이콘과 함께 반영하고 선언 당시의 항목 아래에 버킷으로 묶입니다(항목이 설정되지 않았다면 "Unknown item" 행에 표시); 버그/기능 선언은 클릭하면 세부 내용이 펼쳐집니다. 각 프로젝트의 카드/헤더에 있는 리포트 아이콘(프로젝트 페이지 및 Kanban의 프로젝트 뷰)을 클릭하면 해당 프로젝트로 범위가 지정된 **포커스 시간 리포트**가 열립니다 — `GET /api/projects/:id/focus-report`는 각 세션의 `Focus` 이벤트 기록을 시간이 찍힌 세그먼트로 재구성하고(`server/lib/focus-report.js`), 설정 가능한 유예 구간(`DASHBOARD_FOCUS_IDLE_GRACE_SECONDS`)을 넘어서는 Hook 활동이 없는 구간을 할인 처리하여 유휴 대기 시간이 "항목에 소요한 시간"을 부풀리지 않도록 합니다; 팝업은 활성/유휴 통계 타일, 세션별 세그먼트 타임라인, 항목별 롤업(우회 작업은 그것이 가로챈 항목 아래에 버킷으로 묶임), 그리고 종류별 프로젝트 전체 분할을 보여줍니다. 포커스를 한 번도 선언하지 않은 세션도 리포트의 조용한 구멍이 되지 않습니다: 백그라운드 **포커스 추론(focus inference)** 분류기(`DASHBOARD_FOCUS_INFER_*`)가 그런 침묵한 각 세션의 활동(프롬프트, 건드린 파일, 실행한 명령)을 소화해 그 시간을 플랜 항목 또는 추론된 우회 작업에 귀속시킵니다 — 추측된 귀속이 선언된 기록으로 읽히지 않도록 `≈ inferred` 칩과 함께 표시됩니다. **`AGENT-PLAN.md`가 전혀 없는** 프로젝트도 여전히 분류되지만, 방식이 다릅니다: 대조할 항목 목록이 없으므로 분류기는 곧바로 세션의 활동이 무엇을 완수했는지에 대한 평범한 한 문장짜리 요약을 요청하는 방식으로 넘어가며, 이 역시 동일한 방식으로 저장됩니다(`kind: "unclassified"`와 실제 `reason`) — 그리고 플랜이 있는 프로젝트에서도 신뢰도가 낮은 `"unclassified"` 판정은 분류기가 작성한 이유를 그대로 유지하며, 그 텍스트가 조용히 폐기되지 않습니다. 선언도 없고 아직 확신할 만한 판정도 없는 세션(가장 흔하게는 아직 실행 중이며 분류기가 처리할 만큼 충분히 조용해지지 않은 세션)도 — item/detour/feature/bug와 나란히 자체 범례 항목을 가진 중립적인 회색 **"No focus"** 블록으로 — 분류되거나 종료될 때까지 보이지 않는 대신 표시됩니다. 리포트는 또한 **노력 시간(effort time)**(모든 세션에 걸쳐 합산 — 세션이 동시에 실행되면 부풀려짐)과 **실제 경과 시간(wall-clock time)**(각 세션 자신의 구간의 합집합)을 분리하며, **Concurrency** 통계 타일이 그 차이를 서로 맞지 않아 보이는 두 개의 소요 시간 대신 하나의 알아보기 쉬운 숫자("1.08x")로 만들어 줍니다. 리포트의 List/Calendar 토글은 **스윔레인 데이-뷰 캘린더(swimlane day-view calendar)**로 전환됩니다 — 세션이 실제 24시간 축 위에 배치되고, 겹치는 세션은 나란한 레인으로 나뉘어 동시성이 통계 수치가 아니라 기하학적 형태로 읽히게 됩니다 |
...
| **원격 데이터 소스**             | SSH를 통해 다른 머신의 Claude Code 사용량을 수집합니다 — 대시보드가 각 원격의 `~/.claude/projects` 디렉터리를 `rsync`로 가져와 임포트하며, 세션에 소스별로 태그를 붙입니다. 전역 데이터 범위 선택기(로컬만 / 전체 / 특정 소스)로 앱 전체를 선택한 소스로 좁힙니다. 백그라운드 폴러를 통해 실시간에 가깝게 동작합니다 |
| **반응형 디자인**                 | 쌓이는 그리드, 스크롤 가능한 테이블, 접을 수 있는 사이드바가 있는 모바일 친화적 레이아웃                                                                                                                                                                                      |
| **UI 현지화**                     | 영어(`en`), 중국어(`zh`), 베트남어(`vi`), 한국어(`ko`)에 대해 번역된 UI 문구와 접근성 레이블을 갖춘 내장 언어 전환. 커버리지는 이제 워크플로 툴팁까지 엔드투엔드로 확장됩니다: 통계 카드 계산과 값 버킷 해석, 차트별 "무엇을 / 어떻게 읽는가 / 왜" 팝오버, 모든 그래프의 호버 툴팁(오케스트레이션, 도구 흐름, 파이프라인, 모델 위임, 동시성), 워크플로 패턴 상세 패널의 서사와 제안, 설정 → 모델 가격 책정 정보 팝오버, CLAUDE_HOME 패널, 전체 가져오기 기록 흐름                                                                                                                                                                       |
...
| **플러그인 마켓플레이스**         | 10개의 플러그인(ccam-analytics, ccam-productivity, ccam-devtools, ccam-insights, ccam-dashboard, ccam-cost-guard, ccam-sessions, ccam-workflows, ccam-quality, ccam-config)이 있는 공식 Claude Code 플러그인 마켓플레이스. 53개 스킬, 14개 에이전트, 30개 슬래시 명령, 3개 CLI 도구, 3개 Hook 구성. 모두 실제 데이터 모델에 근거합니다 — 토큰 기준선, 가격 책정 엔진, 워크플로 인텔리전스(11개 데이터셋), 세션 메타데이터. `claude plugin marketplace add`로 설치 |
...
| **Usage**                          | 현재 Claude 계정의 요금 제한 / 플랜 사용량 스냅샷을 `/usage`에서 보여줍니다. **Capture now** 버튼은 분리된(detached) tmux 세션에서 `claude`를 실행하고, 그 자체의 `/status`와 `/usage` TUI 패널을 조작하여 렌더링된 텍스트를 캡처한 뒤, 이를 최선을 다해(best-effort) 파싱하여 계정 정보(이메일, 조직, 로그인 방식, CLI 버전, 모델), 세션 비용/소요 시간/변경된 라인 수, 토큰 수(입력, 출력, 캐시 읽기, 캐시 쓰기), 그리고 정말 중요한 두 가지 요금 제한 비율 — 롤링 **5시간 세션 윈도우**와 **주간 윈도우**(모델별 세부 내역 포함) — 로 정리합니다. 각 비율은 리셋 시각과 함께 색상 코드가 적용된(초록/노랑/주황/빨강) 막대로 표시됩니다. **Session reset times**(세션 리셋 시각) 패널은 각 계정의 다음 세션 윈도우 리셋 시각을 앞으로 24시간 동안 그래프로 보여주며, 세션이 아니라 주간 윈도우가 실제 제약 조건이 되는 순간 리셋 마커의 오른쪽 절반이 주간 윈도우 자체의 색상으로 물들고, 이를 명시적으로 알려주는 "Capped by weekly(주간 한도로 제한됨, X% 남음)" 안내가 함께 표시됩니다. **Weekly reset countdown**(주간 리셋 카운트다운) 패널은 주간 윈도우의 달력일 리셋에 대해 동일한 정보를 보여줍니다. **Color thresholds**(색상 임계값) 카드를 사용하면 각 색상 구간이 시작되는 지점을 세션과 주간 범위, 그리고 이에 대응하는 **소비 속도** 범위(아래 참고)별로 독립적으로 정확히 설정할 수 있으며 — 서버 측(단일 `color_thresholds` 테이블, `GET`/`PUT /api/color-thresholds`)에 저장되어 연결된 모든 컴퓨터에서 공유되고, 변경 시 `color_thresholds_updated`로 실시간 전파됩니다. 파싱 성공 여부와 관계없이 모든 캡처는 저장됩니다(`usage_captures` 테이블): `status`는 `ok`/`partial`/`error`이며, 캡처된 원본 패널 텍스트는 항상 폴백으로 보존되므로 CLI 버전이 바뀌어 형식이 달라져도 데이터를 잃는 대신 읽을 수 있는 원본 텍스트 블록으로 저하됩니다. 최신 캡처 아래의 기록 목록에는 과거 실행이 표시되며, 캡처는 한 번에 하나만 실행할 수 있습니다(이미 진행 중이면 409). Run Claude와 마찬가지로 동일 출처 가드가 브라우저 드라이브바이 트리거를 방지합니다. 그 위의 **Accounts** 패널 — 자체 헤더의 chevron으로 접고 펼칠 수 있으며 기본값은 접힘(저장되지 않음) — 은 다른 Claude 계정을 원하는 만큼 나란히 추적합니다 — 각 계정은 이미 `claude login`으로 로그인해 둔 `CLAUDE_CONFIG_DIR`을 가리키기만 하면 되고, 계정별 "Capture now"는 해당 로그인의 OAuth 자격 증명을 CLI 자체의 저장소(macOS Keychain, 다른 플랫폼에서는 `.credentials.json` 파일)에서 직접 읽어 Anthropic API에서 곧바로 사용량을 가져옵니다 — 브라우저 세션 쿠키, 비밀번호, 별도의 로그인 과정이 전혀 필요 없습니다. 각 계정 행은 자신의 세션 윈도우와 주간 윈도우 막대를 항상 표시하므로(리셋 시각과 마지막 캡처 이후 경과 시간 포함) 모든 계정의 현황을 한눈에 볼 수 있고, 행을 펼치면 해당 계정의 캡처 기록을 지연 로드합니다(타임스탬프와 비율만 — OAuth 경로에는 없는 비용/토큰/모델 세부 정보는 표시되지 않습니다). 계정의 로그인이 없거나 만료된 경우 해당 행의 **Needs login** 배지를 클릭할 수 있으며(macOS), `CLAUDE_CONFIG_DIR=<dir> claude`가 이미 실행 중인 새 Terminal 창이 열리므로 로그인 과정을 진행하고 창을 닫기만 하면 다시 인증됩니다. 활성화된 모든 계정은 5분마다 자동으로도 캡처되므로(서버에서 끌 수 있는 백그라운드 스케줄러), 수동으로 클릭하지 않아도 비율이 최신 상태로 유지됩니다. **Activity**(활동) 카드는 각 계정의 실제 사용 현황을 보여줍니다 — 활성 상태(최근 15분 이내 사용) 또는 마지막으로 실제 사용한 지 얼마나 지났는지를, 해당 계정 고유의 `CLAUDE_CONFIG_DIR`과 무관하게 세션/주간 비율이 캡처 사이에서 실제로 상승했는지를 근거로 추론합니다 — 실제 작업은 이 계정의 전용 config 디렉터리가 아니라 기본 `~/.claude`에 로그인된 프로필을 통해 이뤄지는 경우가 많기 때문입니다. **Consumption rate**(소비 속도) 카드는 계정별로 현재 속도가 유지될 경우 세션 및 주간 할당량이 언제 소진될지 예측합니다 — 각 계정이 현재 윈도우를 시작한 이후의 최근 캡처 데이터에 시간당 퍼센트 추세선을 맞추고, 그 추세가 실제로 상승 중일 때만 100%에 도달할 시점을 앞으로 투영합니다. 예측된 소진 시점이 윈도우 자체의 리셋보다 먼저라면 카드에 그 시점까지의 색상이 적용된 카운트다운("Runs out in 2h 15m")이 표시되고, 윈도우가 먼저 리셋되거나(또는 사용량이 정체/하락 중이면) "On track — resets before limit"이 대신 표시됩니다. 색상은 Color thresholds 카드 자체의 `sessionRate`/`weeklyRate` 범위로 결정되며, 페이지의 나머지 부분이 사용하는 원시 사용률 기반 색상 구간과는 독립적입니다. 각 판정 아래에는 작은 통계 줄이 함께 표시되어, 원시 소비 속도(세션은 %/시간, 주간은 %/일 — %/시간은 7일 윈도우에서는 읽기 어려울 만큼 작은 값이 되기 때문)와 그 속도가 **지속 가능한 속도**(윈도우가 리셋되는 바로 그 순간에 정확히 100%에 도달하게 되는 속도) 대비 몇 배 빠르거나 느린지("23.4%/hr · 1.8× sustainable pace")를 함께 보여줍니다 — 수치 하나만으로는(윈도우 시작 10분 후와 리셋 10분 전에 같은 숫자가 전혀 다른 의미를 가지므로) "이게 정상인지 과도한 건지"를 판단하기 어렵기 때문입니다. **Rotation plan**(순환 계획) 카드는 동일한 계정별 주간 소비 속도 데이터를 실행 가능한 순환 계획으로 연결합니다 — 전적으로 클라이언트 측에서 계산되며, 지금 어떤 계정을 사용해야 하는지와 다음 계정으로 넘겨야 할 예상 시점을, 활성화된 모든 계정에 걸쳐 약 9일의 기간까지 이어서 보여줍니다. 계산 시 모든 계정 중 관찰된 가장 빠른 주간 속도를 "이 계정이 활성 계정이 된다면"의 대리 속도로 사용하며(유휴 계정 자체의 거의 0에 가까운 속도는 실제 사용 시 어떻게 될지 아무것도 말해주지 않으므로), 미래로 투영할 때 각 계정은 자신의 리셋 시점에 0%로 돌아갑니다. 목록의 각 단계는 해당 계정, 시작/종료 시점, 그리고 그 단계가 해당 계정이 한도 소진이 예상되어 끝나는지(**Switch next**, 다음으로 전환) 아니면 투영 기간 내내 지속 가능해서 끝나는지(**Sustainable**, 지속 가능)를 나타내는 배지를 표시합니다 |
| **코치**                          | 두 개의 페이지(`/coach`와 `/coach/playbook`, 사이드바 라벨 **코치**, Usage 바로 다음에 위치 — 둘 다 사이드바 자체의 시각적으로 구분된 "우리가 추가한" 영역에 함께 있으며, 이 영역은 Projects/Calendar/Focus/Usage/코치를 하나의 구역으로 읽히게 하는 약간 더 밝은 배경 패널로 감싸져 있습니다)가 상단의 공유 **Feed**/**Playbook** 탭 바(`CoachTabs`)로 전환됩니다. `/coach`는 **Feed** — 실시간으로 갱신되는 **Observation** 목록으로, `practice_id` 기준으로 클라이언트에서 클러스터링됩니다(`ObservationGroup`). 그래서 동일한 Practice에서 발생한 여러 개의 열린 Observation(예: 여러 세션에서 트리거된 `session-token-ceiling`)은 최신 카드 하나와 "Show more" 토글로 접혀, Feed가 거의 동일한 카드로 뒤덮이는 것을 막습니다 — 각 항목은 펼쳐서 확인하고 개별적으로 Dismiss할 수 있습니다. `/coach/playbook`은 **Playbook** 편집기로, **Practice**(점검 항목)의 카탈로그입니다. 각 Practice는 사용자가 편집할 수 있는 **config**(임계값)를 가지며, 서버 측에 저장되어 연결된 모든 컴퓨터에서 실시간으로 공유됩니다 — Usage 페이지의 Color Thresholds 카드와 동일한 구조입니다(`GET`/`PUT`, 변경 시 WebSocket 업데이트 브로드캐스트). 어떤 Practice의 조건이 특정 범위(세션/프로젝트/전역)에서 충족되면 Coach 엔진이 Observation을 기록합니다 — 이것이 Feed에 표시되는 단위입니다. 사용자는 카드에서 Observation을 범위와 상관없이 **Dismiss**(닫기)할 수 있습니다(acknowledge는 상태로 존재하지만 v1 UI에는 아직 노출되지 않습니다; resolve는 아래의 자동 해결 스윕에서 자동으로 사용되며, 아직 수동 UI 동작은 아닙니다). 현재 두 개의 Practice가 제공됩니다. `session-token-ceiling`(범위: 세션)은 세션의 총 토큰 사용량(`token_usage`의 모든 차원을 합산)을 관찰하다가 설정 가능한 임계값(기본값 100,000,000 토큰)을 넘으면 `/compact` 또는 `/clear`를 권장하는 Warning 종류의 Observation을 생성하며, 카드에는 두 가지 제안 작업 — 해당 세션으로 딥링크되는 "Open session"과 클립보드로 복사하는 "Copy summary" — 이 붙습니다. `account-weekly-balance`(범위: 전역)는 지금 실제로 **활성 상태**인 Claude 계정이 어느 것인지 관찰합니다(Usage 페이지 자체의 `is_active` 필드와 완전히 동일한 방식으로 추론합니다 — `server/lib/account-activity.js`, CLAUDE_CONFIG_DIR과 무관하게 캡처 사이의 실제 사용량 변화로 판단). 그 활성 계정의 주간 사용률이 Usage 페이지 Rotation Plan 자체의 전환 임계값(`color_thresholds.rotation_switch_pct`, 기본값 80%)을 넘고, 그 임계값 아래로 아직 여유가 있는 다른 활성화된 계정이 최소 하나 있으면, 여유가 가장 많은 계정으로 전환하라고 권장하는 Reminder 종류의 Observation을 생성합니다. 이 Practice는 자체적인 별도 숫자를 갖는 대신 Rotation Plan 카드와 의도적으로 같은 임계값을 공유합니다(가장 낮은 계정과 가장 높은 계정의 원시 사용률 차이를 비교하던 이전 로직은 폐기되었습니다) — 그 덕분에 Coach의 알림과 Rotation Plan 자체의 추천이 서로 어긋날 수 없으며, Playbook의 Account Rotation 카드에도 더 이상 편집 가능한 필드가 없고, 임계값이 실제로 어디에 있는지 알려주는 읽기 전용 안내만 있습니다. 이 Practice는 또한 시간 창 없이도 열린 Observation이 스스로 정리되는 유일한 Practice입니다: 엔진의 `autoResolveClearedGlobalObservations` 스윕이 매 틱마다 동일한 조건을 다시 검사해서, 그 조건이 더 이상 성립하지 않는 순간(주간 초기화, 또는 사용자가 실제로 계정을 전환한 경우) 바로 Observation을 해결합니다 — 이것이 `session-token-ceiling`이 쓰는 메커니즘과 왜 다른지는 아래 자동 해결 관련 설명을 참고하세요. 백그라운드 스케줄러(`server/lib/playbook/engine.js`, reconciliation 스케줄러의 틱 구조를 그대로 따름)가 5분 간격 틱(`DASHBOARD_PLAYBOOK_MS`, 기본값 `300000`; 킬 스위치 `DASHBOARD_PLAYBOOK_MODE=off`)마다 활성화된 모든 세션 범위 Practice를 모든 활성 세션에 대해 평가하고, 활성화된 모든 전역 범위 Practice는 한 번씩 추가로 평가하며, 동일한 Practice+범위 조합에 대해 아직 열려 있는(`open`/`acknowledged`/`dismissed`/`resolved`) Observation이 있는 동안에는 중복 발생시키지 않습니다. 매 틱마다 **두 가지 독립된 자동 해결 스윕**도 함께 실행됩니다 — Coach의 "마무리" 절반이며, 세션 범위 Practice의 조건과 전역 범위 Practice의 조건이 서로 정반대 형태이기 때문에 범위별로 하나씩 나뉘어 있습니다. 세션 범위: 세션이 아직 활성 상태인 동안에는 열린 Observation이 얼마나 오래 열려 있었든 절대 자동으로 해결되지 않습니다. 세션이 실제로 종료된 뒤에야 카운트가 시작됩니다 — 세션 범위 Practice의 `autoResolveOnSessionEnd`가 참(기본값이며, `session-token-ceiling`의 Playbook 카드에서 개별적으로 끌 수 있음)이면, 해당 세션이 종료된 뒤 하나의 전역 `autoResolveAfterMs` 기간이 지나야 그 Practice의 열린 Observation이 자동으로 해결됩니다. 이렇게 하면 어떤 세션 작업이 끝난 뒤 Feed가 사람이 일일이 Dismiss하지 않으면 끝없이 쌓이는 일이 없도록 합니다. 이 기간은 의도적으로 Practice별 설정이 아니라 하나의 전역 다이얼로 되어 있습니다 — Playbook 페이지 자체의 **설정** 항목에서 직접 편집할 수 있습니다(아래 레이아웃 설명 참고; 기본값 180분/3시간; 0으로 설정하면 이 자동 해결 기능이 완전히 꺼집니다 — "세션 종료 즉시 해결"이 되는 것이 아닙니다). 전역 범위(현재는 `account-weekly-balance`): 정반대 형태입니다 — 조건이 실제로 뒤집힐 수 있으므로, 이 스윕은 매 틱마다 `detect()`를 다시 실행해서 더 이상 트리거되지 않는 Practice의 이미 열려 있는 Observation을 즉시 해결합니다. 시간 창은 관여하지 않습니다. Playbook 페이지는 사이드카/마스터-디테일 레이아웃입니다: 왼쪽 레일에 **설정**이 맨 위에 오고, 그 아래로 카탈로그의 Practice마다 한 줄씩(이름과 활성/비활성 상태를 나타내는 작은 점) 나열됩니다. 한 줄을 선택하면 본문 패널이 해당 항목의 전체 카드로 전환됩니다 — 설정 카드이거나, 켜고 끄고 필드를 편집할 수 있는 Practice 자체의 카드이며, 실제 생성될 Observation을 실시간으로 미리 볼 수 있습니다 — 실제 Feed와 동일한 `ObservationCard` 컴포넌트로 렌더링되며 유사품이 아닙니다 — 한 번에 카드 하나만 표시됩니다. `GET`/`PUT /api/playbook/practices[/:id/config]`, `GET`/`PUT /api/playbook/settings`, `GET /api/coach/observations`, `POST /api/coach/observations/:id/respond`가 이를 지원합니다. 이 프레임워크(스키마, 엔진, API, UI)는 완전히 범용적이어서 — 새 Practice는 새로운 배관이 아니라 카탈로그에 새 항목을 추가하는 것으로 그칩니다 |
...
| **Tabby**                         | 모든 페이지의 오른쪽 아래 모서리에 고정된 떠다니는 고양이 친구. 기존 WebSocket `eventBus` 위에 전적으로 구축 — **새 백엔드 없음, API 키 없음, 새 의존성 없음**. 커서를 추적하는 눈과 라이브 세션 스트림에서 도출되는 **8가지 기분**(`idle`, `watching`, `happy`, `worried`, `stuck`, `thinking`, `sleeping`, `disconnected`)을 가진 반응형 SVG 마스코트로, 각 기분마다 고유한 애니메이션(꼬리 흔들기, 귀 쫑긋, 고개 까딱, 떨림, 반짝임, zzz, 경고 "!")이 있습니다. **자동 표시 말풍선**은 주목할 만한 이벤트(세션 시작/종료, 오류, 실행 완료)에 대해 짧고, 스로틀되고, 합쳐진 한마디를 게시하며 음소거할 수 있습니다. 또한 이벤트뿐 아니라 시계도 지켜봅니다: 라이브 세션이 고비용 모델(Opus 또는 Fable)을 15분 넘게 연속으로 실행 중이면 일회성 경고 말풍선("you're on an expensive model — are you sure you want to stay there?")을 띄우며, 세션이 해당 모델에서 벗어났다가 다시 돌아올 때만 재무장됩니다. 고양이를 클릭하거나 **⌘B / Ctrl+B**를 누르면(Esc로 닫기) 라이브 상태 줄(`N live · M errored · 연결 상태`), 빠른 작업(Run Claude / 활동 / 세션 / 오류 세션으로 이동, 말풍선 음소거, 알림 지우기), **Ask** 상자가 있는 **패널**이 열립니다: 간단한 상태 질문("what's running", "any errors", "status")은 캐시된 데이터에서 로컬로 답변되고, 그 외의 질문은 **Run Claude** 페이지로 넘겨(`/run?prompt=…`로 딥링크) 실제 Claude Code 세션을 생성합니다. 접근성을 갖추었고(키보드 조작 가능, `aria-live` 말풍선, `prefers-reduced-motion` 준수), 소켓이 끊기면 차분한 `disconnected` 상태로 안전하게 저하되며, **설정**에서 토글할 수 있습니다(en/zh/vi/ko로 현지화). 구현은 `client/src/components/Tabby/`에 있습니다 |
...
| **세션 스플래시 화면**            | 앱 로드 시(브라우저 세션당 한 번) 표시되는 짧은 브랜딩 스플래시: **시간 인식 인사말**(Good morning / afternoon / evening / Working late), 굵은 태그라인, 두 개의 부제 텍스트, 어두운 분위기의 배경(방사형 광채 + 떠다니는 별자리 + 그레인) 위의 애니메이션 노드 그래프 브랜드 마크. 완전히 현지화되어 있습니다(en/zh/vi/ko). 오버레이는 첫 페인트부터 **불투명**하여 앱이 절대 비쳐 보이지 않으며, 약 2.5초 유지 후 페이드아웃됩니다; 아무 곳이나 클릭하면 건너뛸 수 있고 `prefers-reduced-motion`을 준수합니다. CSS 전용 애니메이션, 추가 의존성 없음 |
...
3. **서버**는 SQLite 트랜잭션 내에서 이벤트를 처리합니다:
   ...
   - `Stop`(Claude가 응답을 마침) 시 메인 에이전트는 "waiting"으로 전환됩니다 — Claude가 자신의 턴을 마친 것입니다. 백그라운드 서브에이전트는 계속 실행됩니다. 세션은 `active`로 유지됩니다. **Waiting** 플래그(`awaiting_input_since`)는 **아직 working 상태인 서브에이전트가 하나도 없을 때만** 기록됩니다 — 에이전트가 아직 실행 중인 세션은 **Active**로 표시되며(사용자가 아니라 자신의 에이전트를 기다리고 있는 것이므로), 플래그 기록은 마지막 `SubagentStop`으로 미뤄집니다. `stop_reason=error`가 있는 Stop은 에이전트를 `error`로, 세션을 `error`로 표시합니다
   ...
   - `SubagentStop`은 의도적으로 대기 플래그를 지우지 않습니다(백그라운드로 실행되던 서브에이전트가 끝났다는 사실은 사람에 대해 아무것도 알려주지 않습니다). 플릿이 모두 비워질 때는 그 반대의 일을 합니다: 그것이 **마지막**으로 working 상태였던 서브에이전트이고 메인 에이전트가 이미 턴을 마쳤다면, 앞선 `Stop`이 미뤄 둔 대기 플래그를 이때 기록합니다 — 모든 에이전트가 끝나는 바로 그 시점에 세션이 Active → Waiting으로 전환됩니다
...
| `DASHBOARD_PLAN_POLL_MS` | `10000` (10초) | **플랜 인지 모니터링**의 `AGENT-PLAN.md` 워처 폴링 간격(ms): 알려진 모든 세션/프로젝트 cwd를 stat으로 확인하고, 변경된 플랜 파일은 다시 수집되어 브로드캐스트됩니다(`plan_updated`). 폴링을 비활성화하려면 `0`으로 설정하십시오(`SessionStart` 시의 기회적 수집은 계속되며, `POST /api/plans/refresh`로 강제 실행할 수 있음) |
| `DASHBOARD_FOCUS_AUDIT_MS` | `300000` (5분) | **포커스 드리프트 감사**의 틱 간격(ms) — 포커스가 선언된 세션의 최근 활동이 선언한 플랜 항목과 일치하는지 확인하고 호박색 "선언되지 않은 우회 작업일 가능성?" 배지를 찍는 백그라운드 검사. 감사를 완전히 비활성화하려면 `0`으로 설정하십시오 |
| `DASHBOARD_FOCUS_IDLE_GRACE_SECONDS` | `300` (5분) | 프로젝트별 **포커스 시간 리포트**(`GET /api/projects/:id/focus-report`)를 위한 유휴 유예 구간: 연속된 두 Hook 이벤트 사이의 간격이 이 값보다 길면 유예 구간만큼만 활성으로 계산되고 — 나머지는 유휴로 제외되어, 몇 시간 동안 Waiting 상태로 머무는 세션이 "항목에 소요한 시간" 수치를 부풀리지 않도록 합니다. 유예 구간보다 짧은 간격(정상적인 생각-응답 리듬)은 항상 전부 계산됩니다. 할인 처리를 완전히 비활성화하려면 `0`으로 설정하십시오(사실상 경과 시간의 100%가 활성으로 계산됨) |
| `DASHBOARD_FOCUS_AUDIT_MODE` | `llm` | 드리프트 감사의 판정 방식: `llm`(기존 CLI 인증을 사용해 작은 모델에서 headless `claude -p` 실행, CLI를 사용할 수 없으면 휴리스틱으로 폴백), `heuristic`(키워드 중첩만) 또는 `off` |
| `DASHBOARD_FOCUS_AUDIT_MODEL` | `haiku` | 드리프트 감사의 `claude -p --model` 실행에 전달되는 모델 |
| `DASHBOARD_FOCUS_AUDIT_TIMEOUT_MS` | `30000` (30초) | 드리프트 감사의 단일 `claude -p` 실행에 대한 강제 종료 타이머(ms)(SIGTERM 후 SIGKILL) |
| `DASHBOARD_FOCUS_INFER_MS` | `600000` (10분) | **포커스 추론(focus inference)**의 틱 간격(ms) — 포커스를 한 번도 선언하지 않은 세션을 플랜 항목 또는 추론된 우회 작업에 귀속시켜 포커스 시간 리포트에 조용한 구멍이 없도록 하는 백그라운드 분류기. 부팅 직후 백필 틱이 한 번 실행되고, 이후 이 간격으로 실행됩니다. 비활성화하려면 `0`으로 설정하십시오 |
...
# 플랜 & 포커스 (AGENT-PLAN.md)
ccam focus [--session id]         # 세션의 현재 플랜 + 포커스 + 드리프트 (별칭: focus status)
ccam focus set <n> [note]         # 이 세션이 어느 플랜 항목을 수행 중인지 선언
ccam focus push <description>     # 우회 작업 선언(계획에 없지만 필요한 작업)
ccam focus bug "<title>" "<summary>" [--detail "<text>"]     # 버그 수정으로 태그된 우회 작업 push
ccam focus feature "<title>" "<summary>" [--detail "<text>"] # 작은 기능으로 태그된 우회 작업 push
ccam focus pop                    # 현재 우회 작업을 종료(일반/버그/기능 모두 해당)
ccam focus done <n>               # 플랜 항목 완료를 선언(파일의 체크박스는 사람이 관리)
                                  # (Claude Code 세션 내부에서는 Hook 스트림이 쓰기를 기록하고,
                                  #  외부에서는 CLI가 cwd 또는 --session으로 세션을 결정)
...
| 명령어                 | 설명                                                |
| ----------------------- | ---------------------------------------------------------- |
| `npm run setup`         | 서버 및 클라이언트 의존성을 설치합니다                     |
...
Usage 페이지의 `/status` + `/usage` 계정 현황 캡처를 위한 HTTP 표면입니다. `/api/run`과 동일한 동일 출처(same-origin) 가드를 공유합니다(`server/lib/origin-guard.js`, 두 라우터가 함께 사용) — 브라우저 요청은 localhost 출처에서 와야 하며, Origin이 없는(CLI/curl) 요청은 통과합니다.

| 메서드   | 경로                  | 설명 |
| -------- | --------------------- | ----------- |
| `GET`    | `/api/usage`          | 캡처 기록을 최신순으로 조회합니다(`?limit=`, 기본값 50, 최대 500). 이미 캡처가 진행 중일 때 UI가 버튼을 비활성화할 수 있도록 `capturing: boolean`도 함께 반환합니다 |
| `GET`    | `/api/usage/:id`      | 캡처된 원본 `/status`/`/usage` 패널 텍스트를 포함한, 캡처 한 건의 전체 행 |
| `POST`   | `/api/usage/capture`  | 분리된(detached) tmux 세션에서 `claude`를 실행하고(`server/lib/usage-capture.js`), `/status`에 이어 `/usage`를 조작해 두 패널을 모두 캡처한 뒤, 최선을 다해 파싱한 결과를 저장합니다(`usage_captures` 테이블). 선택적 본문 `{ cwd? }`. 폴링 가능한 핸들을 반환하는 대신 tmux 왕복(~10-15초)이 끝날 때까지 블로킹합니다 — 단일 사용자용 로컬 대시보드에서는 여기서 비동기 폴링을 도입할 이점이 없습니다. 이미 캡처가 진행 중이면 `409`를 반환합니다 |

파싱 성공 여부와 관계없이 모든 캡처는 저장됩니다 — `status`는 `"ok"`(핵심 필드 파싱 성공), `"partial"`(캡처는 되었으나 일부/전체 구조화된 필드가 일치하지 않음, 예: CLI 버전이 패널 형식을 바꾼 경우), 또는 `"error"`(캡처 자체가 실패 — `tmux`/`claude`가 `PATH`에 없음, 스폰 오류, 타임아웃)입니다. `raw_status_text`/`raw_usage_text`는 항상 저장되므로 파싱 공백이 있어도 데이터가 손실되지 않고 UI에서 원본 텍스트 폴백으로만 저하됩니다. WebSocket 브로드캐스트는 없습니다 — 클라이언트는 그저 `POST` 응답을 기다립니다.

### Accounts (`/api/accounts`)

다중 계정 사용량 추적을 위한 이름 붙은 Claude 계정 — 위 tmux/TUI 방식과 나란히 존재하는 두 번째 캡처 경로입니다. 각 계정은 그저 라벨과, 사용자가 이미 `claude login`을 실행해 둔 `CLAUDE_CONFIG_DIR`의 조합일 뿐입니다 — 이 대시보드는 비밀번호, 브라우저 세션 쿠키, 또는 계정 자체의 액세스/리프레시 토큰을 결코 보거나 저장하지 않습니다. `POST /:id/capture`가 살아있는 OAuth 토큰으로 실제 외부 네트워크 호출을 수행하므로, `/api/usage` 및 `/api/run`과 동일한 동일 출처 가드(`server/lib/origin-guard.js`)를 사용합니다.

| 메서드   | 경로                        | 설명 |
| -------- | --------------------------- | ----------- |
| `GET`    | `/api/accounts`             | 구성된 모든 계정과 각 계정의 최신 세션/주간 요금 제한 비율을 조회합니다 |
| `POST`   | `/api/accounts`             | 계정을 추가합니다. 본문: `{ label, configDir }`. `configDir`은 디스크에 이미 존재해야 합니다 — 없으면 `400`, 이미 등록된 디렉터리면 `409` |
| `DELETE` | `/api/accounts/:id`         | 계정을 삭제합니다. 과거 캡처 기록은 삭제되지 않고 (이제 참조가 끊긴) `account_id`를 그대로 유지합니다 |
| `POST`   | `/api/accounts/:id/capture` | 해당 설정 디렉터리의 OAuth 자격 증명을 실시간으로 읽고(`server/lib/claude-cli-credentials.js` — macOS Keychain, 다른 플랫폼에서는 `.credentials.json` 파일), 사용 가능하면 Anthropic 자체 API에 대한 최소한의 요청(`server/lib/usage-fetch-oauth.js`)으로 사용량을 가져와 이 계정에 귀속된 `usage_captures` 행으로 저장합니다. 자격 증명을 아직 사용할 수 없는 경우(로그인 없음, 만료, 또는 읽을 수 없음) `500` 대신 `200`과 함께 `{ account, status: "not_found"|"expired"|"invalid", message }`를 반환합니다 — 이는 서버 오류가 아니라 예상되고 조치 가능한 상태입니다 |
| `POST`   | `/api/accounts/:id/login-terminal` | `CLAUDE_CONFIG_DIR=<이 계정의 설정 디렉터리> claude`가 이미 실행 중인 새 Terminal.app 창을 엽니다(macOS 전용, 세션/프로젝트 open-terminal 동작과 동일한 AppleScript 방식). Usage 페이지에서 **Needs login** 배지를 클릭하면 호출되는 엔드포인트입니다. macOS가 아니면 `501`, AppleScript 자동화 실패 시 `500` |

이 앱은 만료된 토큰을 스스로 갱신하려고 시도하지 않습니다: 그렇게 하면 CLI 자체의 리프레시 토큰을 소비하여 사용자의 실제 `claude` 로그인을 망가뜨릴 수 있기 때문입니다. 만료되었거나 존재하지 않는 로그인은 대신 계정 상태 `needs_login`으로 보고됩니다 — 해당 행의 **Needs login** 배지를 클릭하면 (`POST /:id/login-terminal`을 통해) `CLAUDE_CONFIG_DIR=<dir> claude`가 이미 실행 중인 터미널이 열리므로, 클릭하고 로그인 과정을 진행한 뒤 창을 닫기만 하면 해당 프로필이 다시 인증됩니다.
...
| `POST`   | `/api/remote-sources/:id/sync`   | 해당 소스에 대해 `rsync` + 가져오기를 즉시 트리거합니다 |
...
| `Stop`              | Claude가 응답을 마침           | 오류가 아닌 경우: 메인 에이전트 → `waiting` — Claude가 자기 턴을 마친 것입니다. **Waiting** 플래그는 아직 working 상태인 서브에이전트가 하나도 없을 때만 기록됩니다. 에이전트가 아직 실행 중이면 세션은 **Active** 상태를 유지하고(자신의 에이전트를 기다리는 중) 플래그 기록은 마지막 `SubagentStop`으로 미뤄집니다. `stop_reason=error`인 경우: 에이전트와 세션을 `error`로 표시합니다. 백그라운드 서브에이전트는 계속 실행됩니다 |
| `SubagentStop`      | 백그라운드 에이전트 완료       | 설명, 유형 또는 작업으로 서브에이전트를 매칭하여 완료 처리합니다. 대기 플래그는 의도적으로 지우지 않습니다 — 서브에이전트가 끝났다는 사실은 사용자에 대해 아무것도 알려주지 않기 때문입니다. 그것이 **마지막**으로 working 상태였던 서브에이전트이고 메인 턴이 이미 끝난 상태라면, 앞선 `Stop`이 미뤄 둔 대기 플래그를 이때 기록합니다(세션이 Active → Waiting으로 전환됨). **실행 후 잊는(fire-and-forget) JSONL 스캔을 트리거합니다**(`scanAndImportSubagents`). 이 스캔은 서브에이전트 자신의 `agent_id` 아래에 도구별 `PreToolUse` + `PostToolUse` 이벤트를 발생시켜, 타임라인이 생성 마커만이 아니라 서브에이전트가 실행한 모든 도구를 표시하게 합니다 |
...
| `Focus`        | 세션의 Bash 도구에서 `ccam focus …` 실행 | `PostToolUse` Hook의 `tool_input.command`에서 파싱됩니다(세션 id를 기본으로 담고 있는 유일한 채널). `set`/`push`/`pop`/`done`/`bug`/`feature`는 세션의 `session_focus` 행(플랜 항목 포인터 + 우회 작업 스택)을 갱신하고 타임라인용 항목 텍스트 스냅숏이 담긴 `Focus` 이벤트를 기록합니다; `session_focus`를 브로드캐스트하고 `done` 이후에는 `plan_updated`도 브로드캐스트합니다. `bug`/`feature`는 push된 우회 작업 프레임에 `kind`/`title`/`detail`을 추가로 담으며, Plan 뷰에 배지로 표시됩니다. `status`는 읽기 전용이며 아무것도 기록하지 않습니다 |
...
이벤트 외에도 Tabby는 시계를 지켜봅니다: 라이브 세션이 고비용 모델(Opus 또는 Fable)을 15분 넘게 연속으로 실행 중이면, 오래 실행되는 프리미엄 모델 세션이 눈에 띄지 않게 비용을 쌓지 않도록 일회성 경고를 표시합니다 — "you're on an expensive model — are you sure you want to stay there?". 이 경고는 연속 실행 구간(streak)마다 한 번만 발동하며, 해당 모델에서 벗어났다가 다시 돌아오면 재무장됩니다. 대시보드 전체의 세션 카드 역시 세션이 Opus나 Fable을 사용 중일 때는 모델 이름을 밝은 빨간색으로 표시하므로, Tabby가 말을 꺼내기 전에도 비용 신호를 확인할 수 있습니다.
...
Tabby는 키보드로 조작할 수 있고, 말풍선에 `aria-live`를 사용하며, `prefers-reduced-motion`을 존중합니다. WebSocket이 끊기면 오류를 내는 대신 차분한 `disconnected` 상태로 안전하게 전환됩니다. **설정**에서 Tabby를 켜거나 끌 수 있으며(영어, 중국어, 베트남어, 한국어로 현지화됨), 구현은 `client/src/components/Tabby/`에 있습니다.
...
공식 Agent Monitor 플러그인으로 Claude Code를 확장하십시오 — 분석, 생산성 도구, 개발자 유틸리티, AI 기반 인사이트, 대시보드 연결, 비용 가드레일, 세션 포렌식, 워크플로 오케스트레이션, 신뢰성/SLO, 구성 거버넌스를 제공합니다. 플러그인 10개, 스킬 53개, 에이전트 14개, 슬래시 명령어 30개, CLI 도구 3개, Hook 구성 3개, MCP 서버 1개.
...
| **ccam-dashboard** | `claude plugin install ccam-dashboard@hoangsonww-claude-code-agent-monitor` | `dashboard-status`, `quick-stats` + MCP 서버 |
...
|-- .claude-plugin/
|   +-- marketplace.json        # 플러그인 마켓플레이스 매니페스트(플러그인 10개)
|-- plugins/
