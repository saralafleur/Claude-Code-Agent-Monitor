...
| **Kanban 보드**                   | 헤더 토글(`localStorage`에 저장)로 전환하는 세 가지 보기: **에이전트** — 4개 컬럼(Working / Waiting / Completed / Error); **세션** — 5개 컬럼(Active / Waiting / Completed / Error / Abandoned); 그리고 **프로젝트** — 프로젝트별로 하나씩 컬럼이 생성되며(세션은 해당 프로젝트에 매핑된 폴더 기준으로 그룹화되고, 상태 컬럼과 구분되도록 컬럼 색상이 순환됩니다), 아직 어떤 프로젝트에도 속하지 않은 폴더의 세션을 위한 **미할당** 컬럼이 마지막에 추가됩니다. 적어도 하나의 **모니터** 그룹을 만들면(물리적 디스플레이를 본뜬 것으로, 헤더의 **+ 모니터 추가** 버튼으로 생성) 거기에 배정된 프로젝트 컬럼은 그 모니터 전용의 테두리 있는 드래그 가능한 박스 안에 실제로 담겨 렌더링됩니다 — 단순히 태그만 붙는 게 아니라 진짜로 그 안에 포함됩니다 — 이 박스들은 같은 가로 스크롤 한 줄 안에서 서로 나란히 놓이고, 마지막에는 아직 배정되지 않은 프로젝트 컬럼을 실제로 담는 **미분류** 박스가 추가됩니다(모니터 박스와 같은 컨테이너지만 고정되어 있어 드래그·이름 변경·삭제가 불가능합니다). 프로젝트 컬럼을 어떤 모니터의 박스 위로(또는 이미 그 박스 안에 있는 다른 컬럼 위로) 드래그하면 그 박스 안으로 옮겨지고, 미분류 박스 위로(또는 행 끝을 지나서) 드래그하면 배정이 해제되어 그 컬럼이 미분류 박스 안에 들어가며, 한 모니터의 박스를(그 헤더를 통해) 다른 모니터의 박스 위로 드래그하면 박스 전체가 좌우로 위치를 바꿉니다 — 실제 책상 위에서 모니터가 놓인 위치를 그대로 반영합니다. 박스 이름은 그 자리에서 바로 수정할 수 있고, 헤더의 셰브론을 통해 접거나 펼칠 수 있으며 — 접으면 박스 전체가 메인 행에서 완전히 빠져나와 그 위의, 접힌 모든 모니터가 공유하는 얇은 띠로 옮겨가므로 그저 좁아진 채로 자리만 차지하는 대신 실제로 가로 공간을 확보하고, 펼치면 원래 순서대로 메인 행으로 다시 내려옵니다(그동안 컬럼은 계속 마운트된 채로 시각적으로만 숨겨지므로 카드 자체의 "더 보기" 상태가 토글 후에도 유지됩니다) — 삭제하면 그 안의 프로젝트는 미분류로 돌아갑니다. 셰브론 옆의 두 번째 아이콘 버튼은 그 박스 자체의 배치 방향을 전환합니다 — **가로**(기본값, 프로젝트 컬럼이 나란히 배치됨) 또는 **세로**(위에서 아래로 쌓임) — 모니터마다 독립적으로 설정되며, 같은 `MonitorGroup`의 `orientation` 필드에 저장됩니다. 그 옆의 세 번째 버튼은 또 다른 독립적인 **줄바꿈 개수** 설정을 순환시킵니다 — `*`(기본값, 지금까지의 무제한 한 줄/한 칸) 또는 `1`–`4` — 가로 배치에서는 한 줄에, 세로 배치에서는 한 칸에 몇 개의 프로젝트 컬럼이 들어갈지 정한 뒤 새 줄/칸으로 넘어가도록 하며, 같은 `MonitorGroup`의 `wrap` 필드에 orientation과 함께 저장됩니다. 모든 프로젝트 컬럼(그리고 독립적인 미할당 컬럼)에도 동일한 셰브론이 있어, 그 컬럼이 어느 박스 안에 있든 상관없이 해당 컬럼 하나만 제자리에서 얇은 제목 표시줄만 남도록 접을 수 있습니다. 이 두 가지는 이제 단일한 전역 레이아웃으로, 서버 측(`dashboard_layout` 테이블, `GET`/`PUT /api/monitors`를 통해)에 저장되며 이 대시보드에 연결된 모든 컴퓨터가 공유합니다(사용자 계정이 없으므로 모두가 동일한 하나의 레이아웃을 사용합니다) — 더 이상 `localStorage`에 저장되는 개인 설정이 아니며, 한 컴퓨터에서 변경하면 `monitors_updated` WebSocket 메시지를 통해 새로고침 없이 연결된 다른 모든 컴퓨터에 즉시 반영됩니다. **Waiting** 컬럼은 에이전트에 저장된 `waiting` 상태에 직접 매핑됩니다 — Claude Code가 프롬프트에서 대기 중일 때(새 세션, 턴 사이, 또는 권한 Notification으로 차단된 상태) 설정되며, 사용자가 재개하는 순간(UserPromptSubmit / PreToolUse) `working`으로 전환됩니다. 각 컬럼 헤더에는 라이프사이클 전환을 설명하는 `?` 툴팁이 표시됩니다. Agents/Sessions 상태 컬럼에도 자체 레이아웃 토글이 있습니다 — 개수 배지 옆의 동일한 `Columns3`/`Rows3` 아이콘으로, Projects 뷰에서 모니터 박스의 토글이 있는 바로 그 위치입니다 — 해당 컬럼의 카드만 기본값인 세로 스택과 고정 너비 카드가 가로로 스크롤되는 행 사이에서 전환합니다. 위의 모니터 토글과 달리 이 토글은 서버 공유가 아니라 순수하게 브라우저별로(`localStorage`, 보기 + 상태로 키가 지정됨, 예: `agents-working`) 저장되며, 각 상태 컬럼의 설정은 다른 모든 컬럼과 독립적입니다. 바로 옆의 두 번째 버튼은 위에서 설명한 것과 동일한 **줄바꿈 개수** 설정(`*`/`1`–`4`)을 순환시켜, 줄바꿈 전에 한 줄/칸에 들어갈 카드 수를 제한합니다 — 별도의 `kanban-status-column-wrap` `localStorage` 키에 저장되며, 보기 + 상태별로 독립적입니다. 카드는 저장된 상태별로 서버에서 가져오고(상태당 사실상 무제한), 이후 클라이언트 측에서 컬럼당 10개 카드로 페이지네이션되며 "더 보기" 버튼이 제공됩니다. WS 구독은 활성 보기에 범위가 지정되므로(`agent_*` vs `session_*` 프레임) 보이지 않는 보기의 업데이트가 다시 가져오기를 유발하지 않습니다. Waiting 배지는 행의 `awaiting_reason`을 호버 툴팁으로 표시합니다 — **입력 필요** (`notification`), **턴 완료** (`stop`), **프롬프트 대기** (`session_start`), **중단됨** (`interrupted`) — 컴팩트한 카드에서는 제목 공간을 유지하기 위해 툴팁으로만 표시되며, 더 넓은 화면(세션 테이블, 세션 상세 헤더)에서는 추가로 이유를 중첩된 칩 형태로 인라인 표시하고, 긴급한 이유(권한 프롬프트, 중단)는 더 진한 앰버 색으로 렌더링됩니다. 세 가지 이유가 더 있는데 — **SubAgents** (`subagent`), **Shell** (`shell`), **Monitor** (`monitor`) — 이는 Claude가 사람에게 막혀 있는 것이 아니라 자식(자신이 생성한 서브에이전트 플릿, 동기적인 Bash 호출, 또는 Monitor 도구 호출)을 통해 여전히 활발히 작업 중임을 의미하므로, "Waiting" 안에 중첩되는 대신 컴팩트한 카드에서도 배지의 단어 자체를 완전히 대체합니다. 각 카드의 모델 칩은 세션이 프리미엄/고비용 모델(Opus 또는 Fable 계열)을 실행 중일 때 밝은 빨간색으로 렌더링됩니다. 세션이 토큰을 하나라도 사용한 뒤에는 세션 카드에 **입력 / 출력 / 캐시** 토큰을 보여주는 컴팩트한 막대가 오른쪽 정렬된 합계와 함께 추가로 표시됩니다 |
