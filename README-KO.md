...
이 앱은 만료된 토큰을 스스로 갱신하려고 시도하지 않습니다: 그렇게 하면 CLI 자체의 리프레시 토큰을 소비하여 사용자의 실제 `claude` 로그인을 망가뜨릴 수 있기 때문입니다. 만료되었거나 존재하지 않는 로그인은 대신 계정 상태 `needs_login`으로 보고되며, 해결 방법은 `CLAUDE_CONFIG_DIR=<dir> claude`를 한 번 실행하여 해당 프로필을 (다시) 인증하는 것입니다.
