...
| **Coach**                          | Standalone page (`/coach`, sidebar label **Coach**, positioned right after Usage — both live in the sidebar's own visually offset "our additions" section, a slightly lighter background panel around Projects/Calendar/Focus/Usage/Coach so that group reads as one distinct section) reserved for the pattern-detection and recommendation layer over how the account actually uses Claude Code — pace, detours, retries, idle stretches — and the concrete suggestions that come out of it. Currently an initial scaffold only: no analysis engine is wired up yet, so the page renders an honest empty state (`EmptyState`) plus a three-tile preview of what's coming (pattern detection, recommendations, blind spots) rather than any live data |
