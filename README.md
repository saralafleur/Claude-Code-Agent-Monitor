...
| Method   | Path                  | Description |
| -------- | --------------------- | ----------- |
| `GET`    | `/api/usage`          | Capture history, newest first (`?limit=`, default 50, max 500). Also returns `capturing: boolean` so the UI can disable the button while one is already running |
