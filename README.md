...
| **Coach**                          | Two pages (`/coach` and `/coach/playbook`, sidebar label **Coach**, positioned right after Usage — both live in the sidebar's own visually offset "our additions" section, a slightly lighter background panel around Projects/Calendar/Focus/Usage/Coach so that group reads as one distinct section) sharing a **Feed**/**Playbook** tab bar (`CoachTabs`) at the top. `/coach` is the **Feed** — a real-time list of **Observations**. `/coach/playbook` is the **Playbook** editor: the catalog of **Practices** (checks), each with user-editable **config** (thresholds) that's server-persisted and shared live across every connected computer — same architecture as the Usage page's Color Thresholds card (`GET`/`PUT`, broadcasting a WebSocket update on change). When a practice's condition is met for a scope (session/project/global), the Coach engine records an Observation, the unit that shows up in the Feed; the user can **Dismiss** an observation from its card, whatever its scope (acknowledge/resolve exist as states but aren't yet exposed in the v1 UI). Ships two practices. `session-token-ceiling` (scope: session) watches a session's total token usage (summed across `token_usage`, all dimensions) and, once it crosses a configurable threshold (default 100,000,000 tokens), produces a Warning-kind Observation recommending `/compact` or `/clear`, with two suggested actions on the card — "Open session" (deep-links to that session) and "Copy summary" (clipboard). `account-weekly-balance` (scope: global) watches every enabled Claude account's latest weekly-quota-used percentage (Usage page's Accounts panel) and, once at least two accounts still have headroom (< 100% used) and the gap between the lowest- and highest-used of them crosses a configurable percentage-point threshold (default 25), produces a Reminder-kind Observation recommending you switch active work to the lower-used account — so a session-window swap always has a fallback with weekly quota left, and neither account gets drained solo. A background scheduler (`server/lib/playbook/engine.js`, mirroring the reconciliation scheduler's tick shape) evaluates every enabled session-scoped practice against every active session, plus every enabled global-scoped practice once, on a 5-minute tick (`DASHBOARD_PLAYBOOK_MS`, default `300000`; kill switch `DASHBOARD_PLAYBOOK_MODE=off`), and won't re-fire a duplicate Observation for the same practice+scope while one is still open (`open`/`acknowledged`/`dismissed`/`resolved`). The Playbook page renders one card per catalog practice, letting you toggle each on/off and edit its threshold, with a live preview of the exact Observation card it would produce — rendered via the same `ObservationCard` component the real Feed uses, not a look-alike. Backed by `GET`/`PUT /api/playbook/practices[/:id/config]` plus `GET /api/coach/observations` and `POST /api/coach/observations/:id/respond`. The framework (schema, engine, API, UI) is fully generic — a further practice is a new catalog entry, not new plumbing |
