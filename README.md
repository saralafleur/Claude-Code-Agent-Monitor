...
| **Project Detail**                 | Dedicated full page for one project (`/projects/:id`, reached via an "open detail" icon on its row in the Projects list) for sustained work against a single project — its **Plan** (the same AGENT-PLAN.md checklist as the Projects page, no new plan UI), which of its mapped folders are actual **git repos** with their live **worktrees** (branch, HEAD, dirty/clean/unknown status from `git worktree list`/`git status`), **sibling repos** detected via a mapped repo's own `PROJECT-CONTEXT.md` that aren't part of the project yet (shown as suggestions with an explicit "add to project" action — never added automatically), and **team-intake initiatives** — `intake/<slug>/` folders produced by the delivery-team pipeline skills, with a stage (`requested → planned → qa → built → released`) inferred purely from which known artifact files exist. Backed by two new live-computed (nothing persisted) endpoints, `GET /api/projects/:id/repos` (`server/lib/repo-topology.js`) and `GET /api/projects/:id/intake` (`server/lib/intake-scan.js`) |
