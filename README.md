...
| **Projects**                       | Groups sessions by the folder(s) they run from into a user-named project, instead of the flat Agents/Sessions views. Projects list as a compact table — one row per project (name, session count, active count, last-activity, and quick plan/report/delete actions), drag-reorderable on the row — rather than a full card each, so a machine with many projects doesn't turn into a long vertical scroll. A project claims one or more working directories (a folder belongs to at most one project); membership is derived by joining each session's `cwd` against the project's mapped folders — no schema change on `sessions` itself. Each row expands via a chevron (collapsed by default) to reveal its folder mappings, any AGENT-PLAN.md checklist, and its sessions rendered as a horizontally scrollable row of `SessionCard`s (the same collapsed-by-default row-of-cards treatment applies to each folder inside the **Unassigned** bucket below). Create a project (optionally attaching an existing folder immediately), rename it inline, add/remove folder mappings after the fact, or delete it (folder mappings cascade away; the underlying sessions are untouched and simply fall back to **Unassigned**, which also offers a one-click "New project from this folder" action). Backed by `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id`, and `POST/DELETE /api/projects/:id/paths[/:pathId]` |
