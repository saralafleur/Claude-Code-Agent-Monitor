...
> [!TIP]
> See also: [README-CN.md](./README-CN.md) (中文版本), [README-VN.md](./README-VN.md) (Phiên bản tiếng Việt), and [README-KO.md](./README-KO.md) (한국어 버전) for localized documentation with region-specific tips and best practices.
...
<p align="center">
  ...
  <br>
  <em>🛰️ <strong>Settings · Remote Data Sources</strong> — pull Claude Code activity from other machines over SSH: add sources by destination, test connectivity, sync manually or on a background poller, and switch the global data scope between local, all sources, or a specific machine, with per-session source badges</em>
</p>

The sidebar provides quick access to the Dashboard, Projects, Calendar, Focus, Kanban Board, Sessions list, Activity Feed, Analytics, Workflows, and Settings. Each page is designed to give you deep insights into your Claude Code agent activity with real-time updates and rich visualizations.
...
| **Projects**                       | Groups sessions by the folder(s) they run from into a user-named project, instead of the flat Agents/Sessions views. Projects list as a compact table — one row per project (name, session count, active count, last-activity, and quick pin/plan/report/delete actions), drag-reorderable on the row — rather than a full card each, so a machine with many projects doesn't turn into a long vertical scroll. A project claims one or more working directories (a folder belongs to at most one project); membership is derived by joining each session's `cwd` against the project's mapped folders — no schema change on `sessions` itself. Each row expands via a chevron (collapsed by default) to reveal its folder mappings, any AGENT-PLAN.md checklist, and its sessions rendered as a horizontally scrollable row of `SessionCard`s (the same collapsed-by-default row-of-cards treatment applies to each folder inside the **Unassigned** bucket below). Create a project (optionally attaching an existing folder immediately), rename it inline, **pin it** (via the row's pin icon) to float it above every unpinned project regardless of the manual drag order — pinned and unpinned rows each keep their own relative order, but the pinned group always renders first, and the pinned state is server-shared (not per-browser like the manual drag order) — add/remove folder mappings after the fact, or delete it (folder mappings cascade away; the underlying sessions are untouched and simply fall back to **Unassigned**, which also offers a one-click "New project from this folder" action). Backed by `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id` (rename and/or pin), and `POST/DELETE /api/projects/:id/paths[/:pathId]` |
| **Calendar** (cross-project board) | Standalone page (`/focus-calendar`, sidebar label **Calendar**, page heading also just "Calendar," positioned right after Projects) rendering the same swimlane day-view calendar as the per-project focus-time report's Calendar tab, but across **every** monitored project at once, taking the full available page width (only 25px of its own left/right padding, breaking out of the app shell's usual padding) rather than a centered, capped column. Three independent filters compose by intersection rather than a dependency chain: a **project** filter (chips — one per project with activity in the current window, plus "All projects" and a fixed, always-visible, amber-tinted **"Unassigned"** chip scoping to sessions whose folder isn't mapped to any project — "Unassigned" and a real project selection are mutually exclusive, enforced client-side and by the server), a **session** filter (a single global dropdown listing every session across every project — not scoped to whichever project is currently selected, so picking a session whose folder isn't part of the selected project is a legitimate combination that simply yields an empty result), and a **time-period** filter (day-by-day Prev/Today/Next navigation, defaulting to "today," a clickable weekday/date label that opens a native date picker to jump straight to any date without arrowing, plus a custom date-range mode) — changing any one never resets another. Powered by a new aggregate endpoint, `GET /api/focus-report`, which always requires an explicit `from`/`to` window (no hidden server-side default) and reuses `server/lib/focus-report.js`'s report-building functions completely unchanged. The existing per-project report modal (opened from a project's card) is unaffected — its stat-tile/List-Calendar-toggle/list-body rendering is shared with this page via `FocusReportBody.tsx` rather than duplicated. Because a report can now span multiple projects, the Concurrency stat tile is relabeled **"Concurrent agent sessions"** here (the same `concurrency_ratio` figure, read correctly as cross-project overlap instead of one project's own multitasking) |
| **Focus** (activity report)        | Standalone page (`/focus`, sidebar label **Focus**, positioned right after Calendar) answering "what did we actually do" as a plain, stakeholder-readable list instead of a swimlane grid. Same three independent filters as the Calendar board — project chips (incl. "All projects" and the amber **"Unassigned"** chip), a global session dropdown, and a day/custom-range time-period picker — sharing the exact same `ProjectScopeFilters` component and `GET /api/focus-report` endpoint, so no backend work was needed. Also offers the Calendar board's own intraday **hour-window zoom** — duration presets (`4h`/`8h`/`12h`/`24h`), a start-time stepper/typed input, a "Live" toggle, and quick-start presets (`HourWindowZoomBar`/`useHourWindowZoom`, extracted out of `FocusCalendarView` so both pages share the identical control) — defaulting to the full, unzoomed period here rather than the Calendar's own 4h default, since this page previously always showed the whole selected window and the zoom is a purely additive, opt-in narrowing. Below the same stat tiles (Total agent time, Concurrency, On declared item %, Off-plan %, Idle excluded) sits the **activity card**: one row per distinct thing that happened in the selected window (further narrowed to the zoom window when active, so the tiles and list never disagree) — a plan item, a detour/bug/feature (grouped by title across every session that landed on it), or unclassified time — each showing a kind chip (reusing the app's existing `FOCUS_KIND_CONFIG`/`FOCUS_KIND_ICONS` vocabulary), a wall-clock/active time figure, a human-friendly clock-time **start/stop range** (e.g. "8:49 AM – 9:12 AM", via the `formatTimeRange()` helper in `client/src/lib/format.ts` — the earliest contributing segment's start through the latest one's end for a merged row, not necessarily contiguous) followed by an em dash and the elapsed **duration** between those same two timestamps as a day/hour/minute breakdown (e.g. "8:49 AM – 9:12 AM — 23m", via `formatDurationLong()`), and, for a classifier-**inferred** entry, its one-sentence `inferred_reason` (`client/src/lib/focusActivity.ts`'s `groupFocusActivity()`, keyed per-cwd so the same item number in two different projects never merges; unclassified segments **with** an inferred reason stay one narrative row per session — two sessions that did genuinely different un-planned work read as two rows, not one row wearing only the bigger session's story — while only reason-less unclassified segments collapse into a shared tail bucket; when more than one session lands on the same key, the shown label/reason come from whichever contributed the largest share of the time, and the "+N more sessions" note is a **toggle** expanding into each contributing session's own name, clock range, wall/active split, and reason). Above the activity card sits an AI-synthesized **Summary** block — plain-language "what we actually did" bullets for the selected window, **grouped by project** in all-projects scope (one bullet set per project with activity, largest wall-clock share first, plus an "Unassigned" group for unmapped folders; a single-project scope shows its one group headerless) (`GET /api/focus-report/summary`, reusing the focus-inference service's headless `claude -p` contract and knobs; cached server-side gated by a data digest, so a finished day generates once and a still-running day regenerates only when its data changes; a small footer note names the model that wrote the bullets). Multi-day windows summarize **hierarchically** — each day is summarized first (and cached permanently once it stops changing), then one rollup synthesizes the window from the per-day bullets, with the bullet budget scaling from 4 (a day or two) to 8 (multi-week) — so zooming out to three weeks keeps the concrete deliverables the daily summaries name instead of averaging dozens of sessions into vague themes. Hidden entirely when unavailable (LLM path off, empty window, or generation failure), never an error state. In "all projects" scope each row is prefixed with its project name; a single-project scope omits it. Collapses past 5 rows behind a "show more"/"show fewer" toggle. A live `ccam focus push/bug/feature` declaration has no separate reason distinct from its own label today — only classifier-inferred entries carry one — a known, not-yet-closed gap |
| **Project Manager** (portfolio rollup) | Standalone page (`/project-manager`, sidebar label **Project Manager**, positioned right after Focus) answering "what's going on across every project I'm tracking" at a glance instead of checking each one individually. A rollup table (one row per project: objective/milestone progress bar, pace badge, open-decision count, last activity) sits above the **Decision Queue** — pending layer-6 reconciliation items (`pace_alert`/`detour_volume`/`detour_disposition`/write-back failures) rendered as actionable cards, each resolved in place (`fold_in`/`new_item`/`deliberate`/`discard` for a detour, pre-filled with the reconciliation tick's own proposed text; resolve/dismiss/retry for everything else) — plus a **Pace Watch** rail (every behind item across every project, worst-overdue first) and a **Recently Resolved** log. A project-scope chip row filters the table, queue, and rail to one project. Composes three existing/new endpoints (`GET /api/projects`, `GET /api/portfolio/summary`, `GET /api/decision-queue`) rather than duplicating their logic, and is the first client page built on top of the layers 4–6 portfolio-management work (detour disposition, pace tracking, reconciliation), which had shipped server-only |
| **Project Detail**                 | Dedicated full page for one project (`/projects/:id`, reached via an "open detail" icon on its row in the Projects list, or by clicking a row in the Project Manager rollup table — its **Back** link returns to whichever of those two pages linked in, carried via router navigation state rather than the URL, defaulting to Projects) for sustained work against a single project — a page header with a **Refresh** button next to the project name (re-scans both the Repos card and the team-intake initiatives together, in one round trip, and pops up a summary of what it found), its **Plan** (the same AGENT-PLAN.md checklist as the Projects page, no new plan UI), a **Repos card** — collapsible via a chevron on its own header (always starts expanded; not persisted) and switchable between a **Full** view (every action available) and a **Compact** view (all the same information, but the Remove/Add/Ignore/Unignore action buttons hidden — a preference persisted in `localStorage`) — showing which of its mapped folders are actual **git repos** with their live **worktrees** (branch, HEAD, dirty/clean/unknown status from `git worktree list`/`git status`, full path always shown — never hidden behind a hover-only tooltip, since a worktree can legitimately live at a "physically different" location than its own repo) — every mapped repo/folder has its own **Remove** action (two-click confirm) that unmaps it, and every worktree row has its own **Continue** button (macOS only, always visible regardless of Full/Compact — it's a launcher, not a project-mutating action) that opens a Terminal.app window in that exact worktree with a FRESH `claude` instance — deliberately never `-c`/`--continue`, since silently resuming whatever prior conversation happened in that directory could drag in stale context or an unreviewed permission/tool state — seeded with a server-built prompt naming its branch/dirty state and telling it to check `git status`/`git log`/`git diff` itself before picking work back up (`POST /api/projects/:id/continue-worktree`, re-validated live against the project's own worktree paths) — plus **related repos** not yet part of the project, detected three ways and each tagged with how it was found — named in a mapped repo's own `PROJECT-CONTEXT.md`, found sitting next to one on disk (the sibling scan is off by default — a checkbox next to the section header opts a project in, since scanning the parent folder surfaces every unrelated repo sitting there in a flat multi-project workspace), or found nested inside one's own subfolders (submodules, vendored checkouts, or unrelated checkouts inside a plain non-repo "workspace" folder — the disk-nested scan is the one source that also runs against a mapped folder that isn't a repo itself, and is unaffected by the sibling-scan toggle) — shown as suggestions with explicit "Add to project" and "Ignore" actions (an ignored suggestion moves into a separate, dismissible **Ignored repos** list with its own "Unignore" action, so a suggestion the project genuinely doesn't want stops reappearing on every rescan), never added or removed automatically; and **team-intake initiatives** — `intake/<slug>/` folders produced by the delivery-team pipeline skills, with a stage (`requested → planned → qa → built → released`) inferred purely from which known artifact files exist. Backed by two live-computed (nothing persisted, aside from the ignore list) endpoints, `GET /api/projects/:id/repos` + `POST`/`DELETE /api/projects/:id/ignored-repos[/:ignoredId]` (`server/lib/repo-topology.js`) and `GET /api/projects/:id/intake` (`server/lib/intake-scan.js`) |
| **Kanban Board**                   | Three views with a header toggle (persisted in `localStorage`): **Agents** — 4 columns (Working / Waiting / Completed / Error) — **Sessions** — 5 columns (Active / Waiting / Completed / Error / Abandoned) — and **Projects** — one column per project (session cards grouped by the project's mapped folders, color-cycled to stay visually distinct from the status columns) plus a trailing **Unassigned** column for sessions whose folder isn't part of a project yet. Once at least one **monitor** group exists (mirroring a physical display — create one via the header's **+ Add Monitor** button), its assigned project columns render inside a bordered, draggable box for that monitor — visually containing them, not just tagging them — with each box sitting side by side with the others in that same single scrollable row, plus a trailing **Ungrouped** box that visually contains everything not yet assigned (like a monitor's box, but fixed in place — it can't be dragged, renamed, or deleted). Drag a project column onto a monitor's box (or onto a column already inside it) to move it there, drag onto the Ungrouped box (or past the end of the row) to clear its assignment — the column then lands inside that box, and drag a monitor's box by its header onto another monitor's box to reposition the whole box left/right — mirroring how the monitors physically sit on the desk. Rename a box inline, collapse/expand it via the chevron in its header — collapsing moves the whole box out of the main row entirely into its own thin strip above it, shared by every other collapsed monitor, so it frees the horizontal space it would otherwise still occupy instead of just sitting there narrow; expanding it drops it back into the main row at its ordered position (its columns stay mounted the whole time, just visually hidden, so a card's own "show more" state survives the toggle) — or delete it (its projects fall back to Ungrouped). A second icon button next to the chevron opens a **layout menu** — a popover of visual tiles, one per orientation+wrap combination (Columns 1–4 or Auto, Rows 1–4 or Auto), each rendered as a miniature preview of that exact arrangement — so picking a layout is one click to open the menu and one click on the tile you want, which applies both values at once and closes the popover immediately. Independently per monitor, stored in the same `MonitorGroup`'s `orientation` field (**horizontal**, the default, project columns side by side, or **vertical**, stacked top to bottom) and `wrap` field (`*` default, today's unbounded single row/column, or `1`–`4` capping how many project columns land per row/column before wrapping to a new one). Every project column (and the standalone Unassigned column) carries its own chevron too, shrinking that single column down to a slim header-only bar in place, independently of whichever monitor (or Ungrouped) box it sits in. Both are a single global layout — persisted server-side (the `dashboard_layout` table, via `GET`/`PUT /api/monitors`) and shared across every computer connected to this dashboard (there are no user accounts, so it's one layout for everyone), not a per-browser `localStorage` arrangement — a change made from one connected computer is pushed live to every other one over a `monitors_updated` WebSocket message, without a reload. The **Waiting** column maps directly to the persisted `waiting` status on agents — set when Claude Code is sitting at a prompt (fresh session, between turns, or blocked on a permission Notification) and transitions to `working` the moment the user resumes (UserPromptSubmit / PreToolUse). Each column header shows a `?` tooltip explaining lifecycle transitions (project columns show their mapped folder path(s) instead). Every Agents/Sessions status column carries its own layout menu too — the same combined grid icon next to its count badge, right where the monitor box's menu sits in the Projects view — opening the same popover of visual tiles to switch that column's cards between the default vertical stack, a horizontally-scrolling row of fixed-width cards, and any fixed wrap count. Unlike the monitor menu above, this one is purely per-browser (`localStorage`, keyed by view + status, e.g. `agents-working`, across sibling `kanban-status-column-orientation` and `kanban-status-column-wrap` keys) rather than server-shared, and each status column's setting is independent of every other column's. Cards fetch by persisted status from the server (effectively unlimited per status) for the Agents/Sessions views, or the full session set grouped client-side by cwd for the Projects view, then paginate client-side at 10 cards per column with a "Show more" affordance. WS subscription scopes to the active view (`agent_*` vs `session_*` frames, `session_*` for Projects too) so off-view updates don't trigger refetches. Waiting badges expose the row's `awaiting_reason` — **Needs input** (`notification`), **Turn done** (`stop`), **At prompt** (`session_start`), **Interrupted** (`interrupted`) — as a hover tooltip, kept tooltip-only on the compact cards so titles keep their space; wider surfaces (Sessions table, session-detail header) additionally show these four inline as a nested chip, with urgent reasons (permission prompts, interruptions) in a hotter amber. Three more reasons — **SubAgents** (`subagent`), **Shell** (`shell`), **Monitor** (`monitor`) — mean Claude is still actively working via a child (a subagent fleet it spawned, a synchronous Bash call, or a Monitor tool call) rather than blocked on the human, so they replace the badge's word entirely, even on compact cards, instead of nesting inside "Waiting". Each card's model chip renders in bright red when the session is running a premium/expensive model (Opus or Fable family). Session cards additionally show a compact **input / output / cache-read / cache-write** token strip with a right-aligned **Effective** total, once the session has used any tokens — cache reads (the conversation prefix re-read every turn, billed at ~10% of the input rate) and cache writes (premium-priced new context) are split into separate figures, and Effective is the cost-weighted input-equivalent sum, computed server-side from each model's actual pricing rules, so it tracks the card's $ figure instead of overstating cache-heavy sessions the way a raw token total would. A compact **Concurrent agent sessions** stat card sits in the header to the left of the view toggle, showing today's cross-project concurrency ratio (effort time ÷ active or open-session wall-clock time — the same figure as the Focus report's Concurrency tile) with a swap button to flip which ratio is primary; it stays hidden until the first fetch resolves and disappears again on a failed fetch, and refreshes on the same session/agent WebSocket events the board's own cards react to rather than a poll timer |
...
| **Settings**                       | System info, hook status, model pricing management, notification preferences, data export **and restore** (the Import History panel's **Restore backup** mode re-imports a full export `.json` — idempotent and non-destructive, so you can consolidate several machines' history into one dashboard), session cleanup. The Model Pricing section exposes an info popover (the `i` icon next to the title) explaining how rule lookup works (first matching pattern wins), the SQL-style `%` wildcard syntax with concrete examples (`claude-opus-4-7%`, `claude-%-haiku`, exact ids), and that prices must be updated manually when Anthropic publishes new rates — already-stored sessions keep the price applied at ingest time. Each rule's editor also carries a collapsible **Introductory rates** block (a `YYYY-MM-DD` promo cutoff + per-category intro prices); leaving the date empty means no promo, and an empty date clears any stored intro rates. The CLAUDE_HOME box and Import History panel are fully i18n-driven across en/vi/zh |
...
| **Plan-Aware Monitoring**          | Answers "what is this session doing, and which part of the project plan is it for?" at a glance. Each monitored repo may keep a human-approved `AGENT-PLAN.md` (numbered checkbox items) at its root; a background poll (`DASHBOARD_PLAN_POLL_MS`, plus opportunistic ingest on `SessionStart`) mirrors it read-only into the dashboard, keyed by cwd and rolled up to Projects. Sessions declare their position by running `ccam focus set/push/pop/done` in their Bash tool — the **hook stream** is the write channel (the `PostToolUse` event already carries the session id), so it works in every hooked project with zero per-project setup. The Projects page shows each plan as a collapsible checklist with a progress bar and per-item session chips; every session card gets a one-line focus breadcrumb with a shared icon for its current state (crosshair on a known item, a detour/feature/bug icon otherwise) — e.g. `Item 4: Migrate auth ▸ npm conflict (23m)` — rendered even for a session mid-detour with no base item declared; Session Detail gains a focus banner plus a **Plan** tab (checklist, interpreted focus timeline from `Focus` events, and the session's latest TodoWrite micro-plan via `GET /api/sessions/:id/todos`). A **drift audit** (`DASHBOARD_FOCUS_AUDIT_*`) periodically checks that a focused session's recent activity matches its declaration — headless `claude -p` on a small model with a keyword-overlap fallback — and stamps an amber "possible undeclared detour?" badge; it never rewrites focus, and declarations never clear the badge. `ccam focus bug`/`feature "<title>" "<summary>" [--detail "<text>"]` push a kind-tagged detour; the Plan view's per-item focus line reflects each session's actual current state (known item, plain detour, feature, or bug) with a matching icon, bucketed under the item it was declared on (or an "Unknown item" row when none was set), with click-to-expand detail for bug/feature declarations. A report icon on each project's card/header (Projects page and Kanban's Projects view) opens a **focus-time report** scoped to that project — `GET /api/projects/:id/focus-report` replays each session's `Focus` event history into timestamped segments (`server/lib/focus-report.js`) and discounts stretches with no hook activity past a configurable grace window (`DASHBOARD_FOCUS_IDLE_GRACE_SECONDS`) so idle waiting time doesn't inflate "time on item"; the popup shows active/idle stat tiles, a per-session segmented timeline, a per-item rollup (detours bucketed under the item they interrupted), and a project-wide split by kind. Sessions that never declared a focus aren't a silent hole in the report: a background **focus inference** classifier (`DASHBOARD_FOCUS_INFER_*`) digests each silent session's activity (prompts, files touched, commands run) and attributes its time to a plan item or an inferred detour — surfaced with an `≈ inferred` chip so guessed attribution never reads as declared history. A project with **no `AGENT-PLAN.md` at all** is still classified, just differently: with no item list to match against, the classifier skips straight to asking for a plain one-sentence summary of what the session's activity accomplished instead, stored the same way (`kind: "unclassified"` with a real `reason`) — and even a low-confidence `"unclassified"` verdict IN a planned project keeps whatever reason the classifier wrote, rather than that text being silently dropped. A session with neither a declaration nor a confident verdict yet (most commonly one that's still running and hasn't gone quiet long enough for the classifier to pick it up, or one where even a summary attempt came back with nothing to say) still shows up — as a neutral gray **"No focus"** block, its own legend entry alongside item/detour/feature/bug — rather than being invisible until it's classified or ends. The report also splits **effort time** (summed across every session — inflates when sessions run concurrently) from **wall-clock time** (the union of each session's own span) with a **Concurrency** stat tile making the gap legible as one number ("1.08x") instead of two durations that look like they don't add up. A List/Calendar toggle in the report switches to a **swimlane day-view calendar** — sessions positioned on a real 24-hour axis, with overlapping sessions splitting into side-by-side lanes so concurrency reads as geometry instead of a stat |
...
| **Remote Data Sources**            | Live remote / multi-machine data collection over SSH. The dashboard `rsync`s each configured machine's `~/.claude/projects` into a sandboxed per-source staging dir, feeds it through the **same importer** used for local history, and tags the imported sessions with their origin (`sessions.source`). A background poller (`DASHBOARD_REMOTE_SYNC_MS`, default 60 s; `0` disables) keeps it near-real-time, and a global **data-scope** selector (Local only / All / specific sources) narrows the whole app. Sources are managed in **Settings → Remote Data Sources** (add / test / sync / remove) and via `ccam remote-sources`, status streams over the `remote_source.status` WebSocket message, and Sessions rows show a source badge for remote-origin sessions. Remote sessions get **no live local hooks**, so they are excluded from every local liveness/stale heuristic (the `ps`/`lsof` reap, the watchdog transcript scan, and the startup + periodic abandon sweeps — all gated on `source = 'local'`); instead their **live status is reconciled from the mirrored transcript on every sync** — a transcript touched within `DASHBOARD_REMOTE_ACTIVE_WINDOW_MS` (default 10 min) means the remote CLI is still running (→ `active`), and once it stops advancing the session flips to `completed`. Authentication defers entirely to the host's own SSH stack (`~/.ssh/config`, ssh-agent, keys, `known_hosts`) — no passwords or secrets are stored, external commands run via arg arrays (no shell), and `StrictHostKeyChecking` stays at the SSH default so an unknown host key fails rather than being trusted |
| **Responsive Design**              | Mobile-friendly layouts with stacking grids, scrollable tables, and collapsible sidebar                                                                                                                                                                                      |
| **UI Localization**                | Built-in language switching with translated UI copy and accessibility labels for English (`en`), Chinese (`zh`), Vietnamese (`vi`), and Korean (`ko`). Coverage now extends end-to-end through the Workflows tooltips: stat-card calculations and value-bucket interpretations, per-chart "What / How to read / Why" popovers, every graph's hover tooltip (orchestration, tool flow, pipeline, model delegation, concurrency), the Workflow Patterns detail-panel narratives and suggestions, the Settings → Model Pricing info popover, the CLAUDE_HOME panel, and the entire Import History flow                                                                                                                                                                       |
...
| **Usage**                          | Rate-limit / plan-usage snapshot for the current Claude account at `/usage`. A **Capture now** button launches `claude` in a detached tmux session, drives its own `/status` and `/usage` TUI panels, captures the rendered text, and best-effort parses it into account info (email, org, login method, CLI version, model), session cost/duration/lines-changed, token counts (input, output, cache read, cache write), and the two rate-limit percentages that matter — the rolling **5-hour session window** and the **weekly window** (with its per-model breakdown) — each shown as a color-coded bar (green/yellow/orange/red) with its reset time. A **Session reset times** panel plots every account's next session-window reset across the next 24h, its reset marker tinting the right half toward the weekly window's own color once the weekly window (not the session) is the real constraint — paired with a "Capped by weekly (X% left)" callout naming it explicitly; a **Weekly reset countdown** panel does the same for the weekly window's calendar-day reset. A **Color thresholds** card lets you configure exactly where each color band starts, independently for the session and weekly scopes plus their **Consumption rate** counterparts (see below) — persisted server-side (the singleton `color_thresholds` table, via `GET`/`PUT /api/color-thresholds`) and shared across every connected computer, broadcasting `color_thresholds_updated` live on change. Every capture is persisted (`usage_captures` table) regardless of parse success: `status` is `ok`/`partial`/`error` and the raw captured pane text is always kept as a fallback, so a CLI-version format change degrades to a readable raw block instead of losing data. History list below the latest capture shows past runs; only one capture can run at a time (409 if one's already in flight). Same-origin guard prevents browser drive-by triggering, same as Run Claude. An **Accounts** panel above it — collapsible via a chevron on its own header, collapsed by default (not persisted) — tracks any number of *other* Claude accounts side by side: each account just points at a `CLAUDE_CONFIG_DIR` the user has already logged into via `claude login`, and per-account "Capture now" reads that login's own OAuth credential from the CLI's storage (macOS Keychain, or a `.credentials.json` file elsewhere) and fetches usage directly from Anthropic's API — no browser session cookie, password, or extra login flow needed. Each account's row always shows its own session-window and weekly-window bars (with reset time and how long ago it was last captured) so every account's standing is visible at a glance; expanding a row lazily loads that account's own capture history (timestamps + percentages — no cost/token/model detail, since the OAuth path doesn't have that). When an account's login is missing or expired, its **Needs login** badge is clickable (macOS): it opens a new Terminal window already running `CLAUDE_CONFIG_DIR=<dir> claude`, so re-authenticating is just walking through the login and closing the window. Every enabled account also captures automatically every 5 minutes (a background scheduler, disable-able server-side), so its percentages stay fresh without a manual click. An **Activity** card lists every account's real usage gauge — active (used in the last 15 minutes) or how long since it was last actually used — inferred from movement in that account's own session/weekly percentage between captures rather than from anything tied to its `CLAUDE_CONFIG_DIR`, since real work is often done through whichever profile is logged into your *default* `~/.claude` instead of this account's own named config dir. A **Consumption rate** card predicts, per account, when its session and weekly quotas will run out at their current pace: a %/hour trend is fit over each account's own recent captures since the current window started, and — only when that trend is actually rising — projected forward to the instant it would cross 100%. If that predicted exhaustion lands before the window's own reset, the card shows a colored countdown to it ("Runs out in 2h 15m"); if the window will reset first (or usage is flat/falling), it reads "On track — resets before limit" instead. Colored via the Color thresholds card's own `sessionRate`/`weeklyRate` scopes, independent of the raw %-used bands the rest of the page uses. Underneath each verdict, a small stats line gives the raw burn rate (session in %/hour, weekly in %/day, since %/hour reads as an unreadably tiny number over a 7-day window) plus how many times faster or slower that is than the *sustainable* pace — the rate that would land exactly at 100% right when the window resets ("23.4%/hr · 1.8× sustainable pace") — so a number alone (which reads very differently 10 minutes into a window versus 10 minutes before its reset) isn't the only signal for "is this normal or way too much". A **Rotation plan** card chains that same per-account weekly burn-rate data into an actionable rotation entirely client-side: which account to be on right now, and the projected instant to hand off to the next one, chained across every enabled account out to a ~9-day horizon — using whichever account shows the fastest observed weekly pace as a stand-in for "if this became the active one" (an idle account's own near-zero rate says nothing about what it would do under real use), snapping an account back to 0% at its own reset when projecting forward. Each step names its account, its start/end window, and a badge for whether it ends because that account is projected to exhaust (**Switch next**) or because it's sustainable through the projection horizon (**Sustainable**) |
| **Coach**                          | Two pages (`/coach` and `/coach/playbook`, sidebar label **Coach**, positioned right after Usage — both live in the sidebar's own visually offset "our additions" section, a slightly lighter background panel around Projects/Calendar/Focus/Usage/Coach so that group reads as one distinct section) sharing a **Feed**/**Playbook** tab bar (`CoachTabs`) at the top. `/coach` is the **Feed** — a real-time list of **Observations**, clustered client-side by `practice_id` (`ObservationGroup`) so several open Observations from the same practice (e.g. `session-token-ceiling` tripping across many sessions) collapse to their single latest card plus a "Show more" toggle instead of flooding the Feed with near-duplicates — every instance stays reachable, expandable, and independently dismissible. `/coach/playbook` is the **Playbook** editor: the catalog of **Practices** (checks), each with user-editable **config** (thresholds) that's server-persisted and shared live across every connected computer — same architecture as the Usage page's Color Thresholds card (`GET`/`PUT`, broadcasting a WebSocket update on change). When a practice's condition is met for a scope (session/project/global), the Coach engine records an Observation, the unit that shows up in the Feed; the user can **Dismiss** an observation from its card, whatever its scope (acknowledge exists as a state but isn't yet exposed in the v1 UI; resolve is used automatically by the auto-resolve sweep below, not yet as a manual UI action). Ships two practices. `session-token-ceiling` (scope: session) watches a session's total token usage (summed across `token_usage`, all dimensions) and, once it crosses a configurable threshold (default 100,000,000 tokens), produces a Warning-kind Observation recommending `/compact` or `/clear`, with two suggested actions on the card — "Open session" (deep-links to that session) and "Copy summary" (clipboard). `account-weekly-balance` (scope: global) watches which Claude account is actually the ACTIVE one right now (inferred the same way the Usage page's own `is_active` field is — `server/lib/account-activity.js` — real quota movement between captures, not anything CLAUDE_CONFIG_DIR-local) and, once that active account's weekly usage crosses the Usage page's own Rotation Plan switch threshold (`color_thresholds.rotation_switch_pct`, default 80%) and at least one other enabled account still has headroom under that same threshold, produces a Reminder-kind Observation recommending you switch to whichever eligible account has the most headroom. Deliberately shares that one threshold with the Rotation Plan card instead of carrying its own independent number (the practice's earlier "raw gap between the highest- and lowest-used account" logic is retired) — the Coach's nudge and the Rotation Plan's own recommendation can never disagree about when it's time to switch, and the Playbook's Account Rotation card has no editable field of its own, just a read-only note pointing at where the threshold actually lives. This is also the one practice whose open Observations self-clear without any time window: the engine's `autoResolveClearedGlobalObservations` sweep just re-runs the same condition each tick and resolves the Observation the moment it stops holding (a weekly reset, or the user actually rotating) — see the auto-resolve paragraph below for why that's a different mechanism than `session-token-ceiling`'s. A background scheduler (`server/lib/playbook/engine.js`, mirroring the reconciliation scheduler's tick shape) evaluates every enabled session-scoped practice against every active session, plus every enabled global-scoped practice once, on a 5-minute tick (`DASHBOARD_PLAYBOOK_MS`, default `300000`; kill switch `DASHBOARD_PLAYBOOK_MODE=off`), and won't re-fire a duplicate Observation for the same practice+scope while one is still open (`open`/`acknowledged`/`dismissed`/`resolved`). Every tick also runs TWO independent auto-resolve sweeps — the Coach's closing half — one per scope, since a session-scoped practice's condition and a global-scoped one's are opposite shapes. For session scope: a still-active session's open Observation is NEVER auto-resolved, however long it's sat open; only once a session has actually ended does the clock start — for a session-scoped practice with `autoResolveOnSessionEnd` true (the default, `session-token-ceiling`'s own toggle on its Playbook card), its open Observations auto-resolve once that session has been ended for at least one global `autoResolveAfterMs` window, so the Feed can never accumulate indefinitely waiting on a human Dismiss once you're done with a session. That window is deliberately a single dial, not per-practice config — editable from the Playbook page's own **Auto-resolve** entry (see layout below; default 180 minutes/3h, `0` disables the sweep entirely — it does not mean "resolve instantly on session end"). For global scope (`account-weekly-balance` today): the opposite shape — its condition genuinely reverses, so this sweep just re-runs `detect()` each tick and resolves any already-open Observation whose practice no longer fires, no time window involved. The Playbook page is a sidecar/master-detail layout: a left rail lists **Settings** first, then one row per catalog practice (name plus an enabled/disabled dot); selecting a row swaps the body panel to that item's full card — the Settings card, or a practice's own toggle-on/off-and-edit-its-fields card with a live preview of the exact Observation it would produce, rendered via the same `ObservationCard` component the real Feed uses, not a look-alike — never more than one card at a time. Backed by `GET`/`PUT /api/playbook/practices[/:id/config]`, `GET`/`PUT /api/playbook/settings`, plus `GET /api/coach/observations` and `POST /api/coach/observations/:id/respond`. The framework (schema, engine, API, UI) is fully generic — a further practice is a new catalog entry, not new plumbing |
...
3. **Server** processes the event inside a SQLite transaction:
   ...
   - On `Stop` (Claude finishes responding), main agent goes to "waiting" — Claude finished its turn. Background subagents continue running. Session stays `active`. The **Waiting** flag (`awaiting_input_since`) is only stamped when **no subagent is still working** — a session whose agents are still running reads as **Active** (it's waiting on its agents, not the user), and the stamp is deferred to the last `SubagentStop`. Stop with `stop_reason=error` marks the agent `error` and the session `error`
   ...
   - `SubagentStop` deliberately does NOT clear the waiting flag (a backgrounded subagent finishing tells us nothing about the human). It does the opposite when the fleet drains: if that was the **last** working subagent and the main agent already ended its turn, it stamps the waiting flag the earlier `Stop` deferred — the session flips Active → Waiting exactly when all agents are done
...
| `DASHBOARD_PLAN_POLL_MS` | `10000` (10 s) | Poll interval (ms) for **Plan-Aware Monitoring**'s `AGENT-PLAN.md` watcher: every known session/project cwd is stat-checked and a changed plan file is re-ingested and broadcast (`plan_updated`). Set to `0` to disable the poll (SessionStart still ingests opportunistically, and `POST /api/plans/refresh` forces one) |
| `DASHBOARD_FOCUS_AUDIT_MS` | `300000` (5 min) | Tick interval (ms) for the **focus drift audit** — the background check that a focused session's recent activity matches its declared plan item, stamping the amber "possible undeclared detour?" badge. Set to `0` to disable auditing entirely |
| `DASHBOARD_FOCUS_IDLE_GRACE_SECONDS` | `300` (5 min) | Idle-grace window for the per-project **focus-time report** (`GET /api/projects/:id/focus-report`): a gap between two consecutive hook events longer than this only counts the grace window's worth as active — the rest is excluded as idle, so a session sitting in Waiting for hours doesn't inflate its "time on item" figure. Gaps under the window (the normal think-and-reply rhythm) always count in full. Set to `0` to disable discounting entirely (100% of wall-clock counts as active) |
| `DASHBOARD_FOCUS_AUDIT_MODE` | `llm` | Drift-audit judge: `llm` (headless `claude -p` on a small model using your existing CLI auth, falling back to the heuristic when the CLI is unavailable), `heuristic` (keyword-overlap only), or `off` |
| `DASHBOARD_FOCUS_AUDIT_MODEL` | `haiku` | Model passed to the drift audit's `claude -p --model` spawn |
| `DASHBOARD_FOCUS_AUDIT_TIMEOUT_MS` | `30000` (30 s) | Kill timer (ms) for a single drift-audit `claude -p` spawn (SIGTERM, then SIGKILL) |
| `DASHBOARD_FOCUS_INFER_MS` | `600000` (10 min) | Tick interval (ms) for **focus inference** — the background classifier that attributes sessions which never declared a focus to a plan item or an inferred detour, so the focus-time report has no silent holes. One backfill tick runs shortly after boot, then this interval. Set to `0` to disable |
| `DASHBOARD_FOCUS_INFER_MODE` | `llm` | Focus-inference classifier: `llm` (headless `claude -p` using your existing CLI auth, after a conservative keyword heuristic gets first look), `heuristic` (keyword-overlap only — never claims a detour, only clear item matches), or `off` |
| `DASHBOARD_FOCUS_INFER_MODEL` | `haiku` | Model passed to the focus-inference `claude -p --model` spawn |
| `DASHBOARD_FOCUS_SUMMARY_MODEL` | falls back to `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku` | Model for the Focus page's "What happened in this window" summary (`GET /api/focus-report/summary`) — a dedicated override so the stakeholder-facing bullets can use a stronger model (e.g. `sonnet`) while the far-more-frequent per-session classifier stays on the cheap default |
| `DASHBOARD_VALUE_SUMMARY_MODEL` | falls back to `DASHBOARD_FOCUS_SUMMARY_MODEL`, then `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku` | Model for the Project Detail page's Value Pool **altitude synthesis** (`POST /api/project-plans/altitudes`) — the Project/Stakeholder plain-language levels shown alongside each pool unit's raw ground fact, same leveling mechanism as the Focus window summary above, applied one altitude higher |
| `DASHBOARD_FOCUS_INFER_TIMEOUT_MS` | `30000` (30 s) | Kill timer (ms) for a single focus-inference `claude -p` spawn (SIGTERM, then SIGKILL) |
| `DASHBOARD_REMOTE_SYNC_MS` | `60000` (60 s) | Poll interval (ms) for the **Remote Data Sources** background sync that pulls each enabled remote's `~/.claude/projects` over SSH and re-imports it through the local importer. Set to `0` to disable the poller (manual / on-demand syncs still work) |
| `DASHBOARD_REMOTE_ACTIVE_WINDOW_MS` | `600000` (10 min) | Freshness window for a **Remote Data Source** session's live status. On each sync, a remote session whose mirrored transcript was modified within this window is treated as still running (`active`); once the mirror stops advancing for longer than this, the session is reconciled to `completed`. Remote sessions receive no live hooks, so this replaces the local liveness/stale sweeps (which skip them). Raise it for slow links or very long idle turns |
| `DASHBOARD_REMOTE_SYNC_TIMEOUT_MS` | `600000` (10 min) | Per-source timeout (ms) for a single remote sync (`rsync` pull + import) before it is aborted |
...
# Plan & focus (AGENT-PLAN.md)
ccam focus [--session id]         # current plan + focus + drift for a session (alias: focus status)
ccam focus set <n> [note]         # declare which plan item this session is serving
ccam focus push <description>     # declare a detour (unplanned-but-necessary work)
ccam focus bug "<title>" "<summary>" [--detail "<text>"]     # push a detour tagged as a bug fix
ccam focus feature "<title>" "<summary>" [--detail "<text>"] # push a detour tagged as a small feature
ccam focus pop                    # resolve the current detour (plain, bug, or feature)
ccam focus done <n>               # declare a plan item complete (file checkbox stays human-owned)
...
| `POST`   | `/api/remote-sources`           | --           | Create a source. Body: `{ label, host, ssh_port?, identity_file?, remote_home?, enabled? }` |
...
HTTP surface for the Usage page's `/status` + `/usage` account-standing captures. Same same-origin guard as `/api/run` (`server/lib/origin-guard.js`, shared by both routers) — browser requests must come from a localhost origin; missing-Origin (CLI/curl) requests pass.

| Method   | Path                  | Description |
| -------- | --------------------- | ----------- |
| `GET`    | `/api/usage`          | Capture history, newest first (`?limit=`, default 50, max 500, `?accountId=` to scope to one named account — see Accounts below). Also returns `capturing: boolean` so the UI can disable the button while one is already running |
| `GET`    | `/api/usage/:id`      | One capture's full row, including its raw captured `/status`/`/usage` pane text |
| `POST`   | `/api/usage/capture`  | Launch `claude` in a detached tmux session (`server/lib/usage-capture.js`), drive `/status` then `/usage`, capture both panes, and persist the best-effort parsed result (`usage_captures` table). Optional body `{ cwd? }`. Blocks for the duration of the tmux round-trip (~10-15s) rather than returning a pollable handle — a single-user local dashboard has no benefit from async polling here. Returns `409` if a capture is already in flight |

Every capture is persisted regardless of parse success — `status` is `"ok"` (key fields parsed), `"partial"` (captured but some/all structured fields didn't match, e.g. a CLI version reformatted the panel), or `"error"` (the capture itself failed — missing `tmux`/`claude` on `PATH`, spawn error, timeout). `raw_status_text`/`raw_usage_text` are always stored so a parse gap never loses data, only degrades to a raw-text fallback in the UI. No WebSocket broadcast — the client just awaits the `POST` response.

### Accounts (`/api/accounts`)

Named Claude accounts for multi-account usage tracking, a second capture path alongside the tmux/TUI one above. Each account is just a label plus a `CLAUDE_CONFIG_DIR` the user already ran `claude login` into — this dashboard never sees or stores a password, browser session cookie, or the account's own access/refresh token. Same same-origin guard as `/api/usage` and `/api/run` (`server/lib/origin-guard.js`) since `POST /:id/capture` makes a real outbound network call using a live OAuth token.

| Method   | Path                        | Description |
| -------- | --------------------------- | ----------- |
| `GET`    | `/api/accounts`             | List every configured account plus its latest known session/weekly rate-limit percentages |
| `POST`   | `/api/accounts`             | Add an account. Body: `{ label, configDir }`. `configDir` must already exist on disk; `400` if it doesn't, `409` if that dir is already registered |
| `DELETE` | `/api/accounts/:id`         | Remove an account. Its past captures keep their (now orphaned) `account_id` rather than being deleted |
| `POST`   | `/api/accounts/:id/capture` | Read that config dir's OAuth credential live (`server/lib/claude-cli-credentials.js` — macOS Keychain, or a `.credentials.json` file elsewhere) and, if usable, fetch usage via a minimal request to Anthropic's own API (`server/lib/usage-fetch-oauth.js`) and persist a `usage_captures` row scoped to this account. If the credential isn't usable yet (no login, expired, or unreadable), responds `200` with `{ account, status: "not_found"\|"expired"\|"invalid", message }` instead of a `500` — that's an expected, actionable state, not a server failure |
| `POST`   | `/api/accounts/:id/login-terminal` | Open a new Terminal.app window already running `CLAUDE_CONFIG_DIR=<this account's config dir> claude` (macOS only, same AppleScript machinery as the session/project open-terminal actions), dropping the user into that profile's interactive login flow. This is what clicking a row's **Needs login** badge calls. `501` off-macOS, `500` on an AppleScript automation failure |

This app never attempts to refresh an expired token itself: doing so could consume the CLI's own refresh token and break the user's real `claude` login. An expired/missing login is instead reported as account status `needs_login` — the row's **Needs login** badge is clickable and opens a terminal already running `CLAUDE_CONFIG_DIR=<dir> claude` (via `POST /:id/login-terminal`), so re-authenticating that profile is a click, a login walk-through, and closing the window.
...
| `Stop`              | Claude finishes responding     | Non-error: main agent → `waiting` — Claude finished its turn. The **Waiting** flag is stamped only when no subagent is still working; with agents still running the session stays **Active** (waiting on its agents) and the stamp is deferred to the last `SubagentStop`. `stop_reason=error`: marks the agent and session `error`. Background subagents keep running |
| `SubagentStop`      | Background agent finished      | Matches and completes the subagent by description, type, or task. Deliberately does NOT clear the waiting flag — a subagent finishing tells us nothing about the human. If it was the **last** working subagent and the main turn already ended, stamps the waiting flag the earlier `Stop` deferred (session flips Active → Waiting). **Triggers a fire-and-forget JSONL scan** (`scanAndImportSubagents`) that emits per-tool `PreToolUse` + `PostToolUse` events under the subagent's own `agent_id` so the Timeline shows every tool the subagent ran, not just the spawn marker |
...
| `Focus`        | `ccam focus …` run in the session's Bash tool | Parsed out of the `PostToolUse` hook's `tool_input.command` (the only channel that natively carries the session id). `set`/`push`/`pop`/`done`/`bug`/`feature` update the session's `session_focus` row (plan-item pointer + detour stack) and write a `Focus` event with an item-text snapshot for the timeline; broadcasts `session_focus` (and `plan_updated` after `done`). `bug`/`feature` additionally carry `kind`/`title`/`detail` on the pushed detour frame, badged in the Plan view. `status` is read-only and records nothing |
...
Beyond events, Tabby also watches the clock: if a live session has been running an **expensive model** (Opus or Fable) continuously for more than **15 minutes**, it surfaces a one-time nag — *"you're on an expensive model — are you sure you want to stay there?"* — so a long-running premium-model session doesn't rack up cost unnoticed. The nag fires once per streak; switching off that model and back onto it re-arms it. Session cards throughout the dashboard also render the model name in bright red whenever a session is on Opus or Fable, so the cost signal is visible before Tabby ever says anything.
