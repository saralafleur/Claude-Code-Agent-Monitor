...
The Usage page's global green/yellow/orange/red color thresholds (`routes/color-thresholds.js`, `/api/color-thresholds`) — a single **global** config, not per-user: this app has no accounts, so every computer connected to the dashboard reads and writes the same thresholds, persisted server-side in the singleton `color_thresholds` row. Four independent scopes: `session` and `weekly` (raw %-used — the session (5h) window and the weekly window are separate quotas) plus `sessionRate` and `weeklyRate` (the Consumption Rate card's runway-risk percentage — see Accounts below — a different quantity from raw %-used, so it gets its own pair of bands).

| Method | Path                     | Description |
| ------ | ------------------------ | ----------- |
| `GET`  | `/api/color-thresholds`  | Current `{ session, weekly, sessionRate, weeklyRate }`, each `{yellowAt,orangeAt,redAt}` |
| `PUT`  | `/api/color-thresholds`  | Patch any subset of the four scopes, and within a scope any subset of its three fields; `400 INVALID_THRESHOLDS` on an out-of-range value or a non-increasing ordering |
...
The Coach's Playbook (`routes/playbook.js` + `routes/coach.js`, `/api/playbook/*` + `/api/coach/*`) — a rule-based system that watches usage patterns and surfaces recommendations. A **practice** is a named, built-in check (`lib/playbook/practices.js`; ships two: `session-token-ceiling`, scope session, and `account-weekly-balance`, scope global — the latter fires when the Claude account currently ACTIVE (inferred via `lib/account-activity.js`'s `isAccountActive`/`computeLastUsedAt`, same as `routes/accounts.js`'s own `is_active`) has crossed the Usage page's Rotation Plan switch threshold (`color_thresholds.rotation_switch_pct`) and another enabled account still has headroom under it, recommending a switch to whichever eligible account has the most headroom — it deliberately shares that one threshold with the Rotation Plan card instead of a Coach-only field); each practice's config (enabled + fields) is a single **global** setting, same no-accounts/server-shared model as Monitors and Color Thresholds, persisted in `playbook_practice_config` — `account-weekly-balance` has no `fields` of its own at all. A field's `type` is `"number"` (a threshold, validated against `min`) or `"boolean"` — `session-token-ceiling`'s `autoResolveOnSessionEnd` is the only one so far, consumed by the engine's auto-resolve sweep below rather than `detect()`. A detected occurrence of a practice firing for a scope is an **Observation** (`coach_observations`), produced by a background scheduler (`lib/playbook/engine.js`, mirrors `lib/reconciliation.js`'s tick shape — boot delay, `setInterval`, a `running` re-entrancy guard) that evaluates every enabled session-scoped practice against every active session, plus every enabled global-scoped practice once, every `DASHBOARD_PLAYBOOK_MS` (default 5 min; `DASHBOARD_PLAYBOOK_MODE=off` to disable), deduped so a practice+scope with an already-`open` Observation never re-fires. Every tick also runs TWO independent auto-resolve sweeps, one per scope. Session scope (`autoResolveStaleObservations`): a still-active session's open Observation is never auto-resolved, however long it's been open — only once a session has actually ended does the clock start, and an open session-scoped Observation for a practice with `autoResolveOnSessionEnd` true auto-resolves once its session has been ended for at least `playbook_settings.auto_resolve_after_ms` (a single global window, not per-practice — `/api/playbook/settings`, default 180min/3h; `0` disables the sweep entirely, it does not mean "resolve instantly on session end") — so the Feed doesn't accumulate open Observations forever waiting on a human Dismiss once a session is done. Global scope (`autoResolveClearedGlobalObservations`): the opposite shape — the condition genuinely reverses (e.g. `account-weekly-balance`'s active-over-threshold clears on a weekly reset or once the user actually rotates), so this sweep just re-runs `detect()` against fresh ctx every tick and resolves any already-open Observation whose practice no longer fires — no time window at all.

| Method | Path                                       | Description |
| ------ | ------------------------------------------ | ----------- |
| `GET`  | `/api/playbook/practices`                  | Every catalog practice merged with its current config (or catalog defaults if never touched) |
| `PUT`  | `/api/playbook/practices/:id/config`       | Patch `{ enabled?, config? }`; `404 UNKNOWN_PRACTICE` / `400 INVALID_CONFIG` on an unknown field, a type mismatch, or an out-of-range value |
| `GET`  | `/api/playbook/settings`                   | The Playbook's global settings: `{ autoResolveAfterMs }` |
| `PUT`  | `/api/playbook/settings`                   | Patch `{ autoResolveAfterMs? }`; `400 INVALID_SETTINGS` on a value that isn't a finite number `>= 0` |
...
`PUT /api/playbook/practices/:id/config` broadcasts `playbook_practice_config_updated`; `PUT /api/playbook/settings` broadcasts `playbook_settings_updated`; the engine's own tick broadcasts `coach_observation_created` when a practice fires; `POST .../respond` **and** either of the engine's auto-resolve sweeps broadcast `coach_observation_updated` — see [docs/API.md](../docs/API.md#playbook) for full request/response shapes.
...
| `PATCH`  | `/api/projects/:id`                | Rename a project and/or set `pinned` (floats it to the top of `GET /api/projects`, ahead of the regular alphabetical order) and/or set `siblingScanEnabled` (default `false`; gates the `"disk-sibling"` suggestion source below) |
...
| `PATCH`  | `/api/projects/:id/paths/:pathId`  | Set a mapped folder's `terminalDefault` (default `true`) — whether it's offered by `POST /api/projects/:id/open-terminal`'s folder picker; returns the recomputed repo topology |
...
| `POST`   | `/api/projects/:id/open-terminal`  | Open a new Terminal.app window running `claude` in one of the project's `terminalDefault`-eligible mapped folders (macOS only, `lib/terminal-focus.js`'s `openTerminalForCwd`) — backs the shared project/session terminal picker |
| `POST`   | `/api/projects/:id/continue-worktree` | Open a new Terminal.app window in one specific live worktree (any worktree of any of the project's mapped repos, re-validated live against the current topology) and start a FRESH `claude` instance there (never `-c`/`--continue` — silently resuming a prior conversation is considered unsafe) seeded with a server-built resume-nudge prompt naming its branch/dirty state (macOS only) — backs the Project Detail page's per-worktree **Continue** button |
...
`GET /api/projects/:id/repos` (`lib/repo-topology.js`): splits the project's mapped folders into actual git repos (has a `.git`) versus plain folders, lists each repo's live worktrees via `git worktree list --porcelain` (`git` invoked with `execFile` and an argument array — never a shell string — using the isolated env helper shared with `lib/update-check.js`, see `lib/git-env.js`), and each worktree's dirty state via `git status --porcelain` (capped at 25 dirty-checks per request; `dirty: null` means genuinely undetermined, render as "unknown" not "clean"). Also best-effort-detects related repos not yet mapped to the project from three sources, surfaced as `detectedSiblings` tagged with `source`: `"context"` (named in a mapped repo's own `PROJECT-CONTEXT.md` "Repo topology" section), `"disk-sibling"` (a live, one-level-deep scan of a mapped repo's own parent directory for other git repos, capped at 200 entries), and `"disk-nested"` (a depth-capped walk — max depth 4, max 2000 dirs scanned — of a mapped folder's own subfolders for nested git repos such as submodules, vendored checkouts, or unrelated checkouts sitting inside a plain non-repo "workspace" folder, excluding `node_modules`/`.git`/`dist`/`build`/etc. and never descending past a repo it already found). `"disk-sibling"` only runs when the project's `siblingScanEnabled` is `true` (default `false`) — off by default because it surfaces every git repo in the same parent folder regardless of relatedness, which is noisy in a flat workspace holding many unrelated repos; `"context"` and `"disk-nested"` are unaffected and always run. `"disk-nested"` is the only one of the three that also runs against a mapped folder that isn't itself a git repo — `findNestedReposOnDisk` doesn't require its target to be a repo, so a plain container folder still gets scanned for repos nested inside it, arbitrarily deep. Every mapped repo's own live worktrees (gathered in a first pass before any detection source runs) are excluded from all three sources too — a linked worktree can sit at an arbitrary, unrelated-looking path, but it's still the same repo, not a new one to suggest. `"context"` wins the dedup when the same path is found more than one way. These are suggestions only; the client adds one explicitly via the existing `POST /api/projects/:id/paths`, or dismisses it via `POST /api/projects/:id/ignored-repos` (`{path, name, source}`, idempotent) — dismissed suggestions are filtered out of `detectedSiblings` and returned separately as `ignoredRepos` (`project_ignored_repos` table) until reversed via `DELETE /api/projects/:id/ignored-repos/:ignoredId`. Nothing is added to or removed from the project automatically. The Project Detail page's header has its own **Refresh** button (next to the project name, not inside the Repos card) that re-fetches this endpoint together with `GET /:id/intake` (below) and pops up a summary of what the rescan found, plus a **Remove** action (two-click confirm) on every mapped repo/folder row that unmaps it via the existing `DELETE /api/projects/:id/paths/:pathId`. `404` for an unknown project id.

`GET /api/projects/:id/intake` (`lib/intake-scan.js`): finds every `intake/` directory reachable from each mapped folder within 3 levels of subdirectories (`findIntakeDirs`, entry-capped at 2000 dirs scanned, skipping the same dependency/build directory names as `repo-topology.js`'s disk-nested-repo scan — some projects nest their app code, and `intake/`, a few folders below the mapped folder's own root rather than directly at it), scans each found `intake/<slug>/` directory (the layout the `team-intake`/`team-qa`/`team-build`/`team-release` delivery-team pipeline skills produce), and infers a pipeline stage — `requested → planned → qa → built → released` — purely from which known artifact files exist (`request-brief.md`, `technical-plan.md`, `qa/qa-assessment.md`|`qa/test-plan.md`, `build/*/build-report.md`, `merge.json`); no markdown content is parsed. `404` for an unknown project id.
...
**Portfolio plan lifecycle & value ledger:** `/api/project-plans` (`server/routes/project-plans.js`) is a separate, `project_id`-keyed plan lifecycle sitting alongside the layers above — additive tables (`project_plans`/`project_plan_items`/`value_claims`, zero `ALTER TABLE`), never blended into `/api/plans` responses. `server/lib/plan-lifecycle.js` owns plan/item CRUD, the derived generation-ordinal walk, the single closure composer (`closePlan` — the only door to `status: 'closed'`), and the DEC-P2 import inversion (an existing `AGENT-PLAN.md` becomes generation 1, idempotent on `(project_id, content_hash)`, never `cwd`). `server/lib/value-ledger.js` is the single home for every derived number: the live value pool (mechanical tier from intake + the live `detectTrunkDrift()` feed, a judgment tier from `detour_dispositions`, a correlational tier suggesting session-bracketed trunk commits), plan health metrics, and the AC-6 whole-life delivered-value summary. `server/lib/cwd-identity.js` is the sole canonicalizer (`realpathSync`/`git rev-parse --show-toplevel`/`--git-common-dir`) this layer ever calls, folding worktree cwds to their parent repo root and surfacing case-variant duplicates as `identityWarnings` rather than double-counting them. CLI: `ccam ledger plans|pool|health|history|import|claim|close --project <id|name>` — every printed number comes straight from the API response (`server/__tests__/ledger-metrics-parity.test.js` proves API/CLI parity against one seeded DB and a real spawned process). See [docs/DATABASE.md](../docs/DATABASE.md) and [docs/API.md](../docs/API.md) for the schema and route reference, and `ARCHITECTURE.md`'s "Portfolio Plan Lifecycle & Value Ledger" section for the full design rationale. The `<PlanLedgerPanel>` UI has shipped, rendered as its own card on `ProjectDetail` (`client/src/components/PlanLedgerPanel.tsx`).

`server/lib/value-summary.js` is the altitude-synthesis layer above the pool (`POST /altitudes`): every unit's GROUND fact (`value_source`/label/attribution, exactly what `/pool` already returns) gets a PROJECT phrase and a plain-language STAKEHOLDER sentence on request — the same leveling mechanism `focus-summary.js` uses for Focus window summaries, applied one altitude higher, reusing its hermetic `runClaudePromptJson` spawn contract. The `value_unit_summaries` cache is keyed on the unit's own identity with no digest gating, but it is **mutability-aware**, not blanket generate-once-served-forever: `intake_initiative`/`detour`/`merge_commit` units (`MUTABLE_VALUE_SOURCES`, `value-ledger.js`) carry a `stage` (and sometimes a `label`) that can change after the text was cached, so every read for a mutable source compares the unit's current `value_source`/`label`/`stage` (`unitFacts()`, the single function both the prompt builder and the comparator read from — no second formula to drift) against the exact input snapshot the cached text was generated from; a mismatch is an ordinary cache miss for that ONE unit, regenerated through the same batch/cap machinery. `trunk_commit` units (and any source outside `MUTABLE_VALUE_SOURCES`) keep the original generate-once-served-forever behavior exactly — every not-yet-cached unit in one request batches into a single spawn (never one call per unit, capped at `MAX_UNITS_PER_PROMPT`/40) so sibling units can inform each other's PROJECT phrasing. Model: `DASHBOARD_VALUE_SUMMARY_MODEL`, falling back to `DASHBOARD_FOCUS_SUMMARY_MODEL`, then `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku`; gated by the same `DASHBOARD_FOCUS_INFER_MODE` availability check. The response is `{ altitudes, states, counts }` (DEC-11/DEC-14): every submitted unit lands in exactly one of `altitudes`/`states`, never both, never neither — `altitudes` for a cache hit or a fresh resolution this call, `states` for a miss beyond this call's 40-unit cap (`"queued"`, will be picked up by a later call or the sweep below) or a miss that was in scope but still produced nothing, e.g. LLM off (`"unavailable"`); `counts` is `{pool_size, cache_hits, generated, queued, unavailable, stale_regenerated}`, a single computed-once partition both this route and the background sweep log verbatim, never re-derived. A unit whose cache went stale but could not be refreshed this round (over cap, or the LLM path unavailable) is re-homed from `states` back into `altitudes` with its OLD text rather than blanked — never silently dropped. A resolved `altitudes[unitKey]` entry may additionally carry `freshness` (`"updated_unseen"` — regenerated this round or still unacknowledged from an earlier round; `"stale_refresh_queued"`/`"stale_refresh_unavailable"` — stale text served as-is because a refresh could not happen this round), `update_reason` (`"stage_changed"`/`"label_changed"`), and `regenerated_at` (`null` on first generation). The marker is a function of "has this generation been acknowledged," not "did this specific read just trigger a regeneration" — it re-surfaces on every subsequent read (cache hit or not) until explicitly cleared. `POST /altitudes/seen` (`{project_id, units:[{unit_key, regenerated_at}]}` → `{ updated }`) is that acknowledgement: a server-side compare-and-set (`regenerated_at IS ?`, never an unconditional stamp) clears the marker only if the row's current `regenerated_at` still matches what the caller last saw — a stale stamp (the tick regenerated again since the caller fetched it) is silently rejected rather than clearing a marker for a generation the user never saw; `project_id` is validated but otherwise advisory (`unit_key` already embeds the cwd). Overflow beyond the 40-unit cap is drained unattended by the `startValueSummaryTick` background sweep (below), never by this request path. `PlanLedgerPanel` fetches altitudes as a separate, non-blocking request once the pool's units are known, so a cold cache's LLM latency never delays the rest of the panel.
...
| Method   | Path                        | Description |
| -------- | --------------------------- | ----------- |
| `GET`    | `/api/accounts`             | List accounts + each one's latest known session/weekly rate-limit %, `last_used_at`/`is_active` (real-usage gauge inferred from percentage movement between captures — distinct from `last_capture_at`, which only reflects manual Refresh clicks), and a Consumption Rate prediction: `session_burn_rate_pct_per_hour`/`week_burn_rate_pct_per_hour` (a %/hour trend fit over this account's own captures since the current window started) plus `session_predicted_exhaustion_at`/`week_predicted_exhaustion_at` (the ISO instant that trend would cross 100%, `null` when the trend isn't rising or there's not enough data yet) |
...
The burn-rate/exhaustion fields (`lib/consumption-rate.js`'s `computeConsumptionRate`) power the Usage page's **Consumption Rate** card: for each of the session/weekly windows independently, fit a least-squares %/hour trend over that account's own captures since the *current* window started (a window reset drops the percentage back toward 0, so blending captures across that boundary would read as a usage crash rather than a fresh window) and, only when that trend is actually rising, project the instant it would cross 100%. `*_burn_rate_pct_per_hour` is `null` with fewer than two comparable captures in the current window; `*_predicted_exhaustion_at` is also `null` (even with a non-null rate) when the trend is flat or falling, since no exhaustion is predictable in that case. The client compares the predicted instant against the window's own `latest_session_window_reset_raw`/`latest_week_reset_raw` to decide whether the account is actually on track to run out before it resets, and colors the result via the `sessionRate`/`weeklyRate` scopes above — a rising trend that resets first still reads as "on track", however steep it looks. Same 500-capture lookback cap as `last_used_at` above, which for the 7-day weekly window means the trend narrows to roughly the most recent ~41 hours of pace once more than about a day and a half has passed since the last weekly reset — a deliberate choice, since recent pace predicts the near future better than diluting it with a slower stretch from days ago.

Underneath each verdict, the client (`client/src/pages/Usage.tsx`'s `ConsumptionRateScopeRow`) derives two more numbers purely from the wire fields already above — no additional API surface: the raw rate reformatted as `%/hr` (session) or `%/day` (weekly, since `%/hr` is unreadably small over a 7-day window), and `computePaceRatio` — how many times faster or slower that rate is than the *sustainable* pace, `(100 - currentPct) / hoursUntilReset`, the rate that would land exactly at 100% right when the window resets. A raw rate alone can't say whether it's normal or alarming (the same `%/hr` reads very differently 10 minutes into a fresh window versus 10 minutes before its reset); the ratio normalizes for that, rendering as e.g. "23.4%/hr · 1.8× sustainable pace".

The Usage page's **Rotation Plan** card (`client/src/pages/Usage.tsx`'s `computeRotationPlan`) goes one step further, entirely client-side over the same `week_burn_rate_pct_per_hour`/`latest_week_window_pct`/`latest_week_reset_raw` fields already above - no new API surface. It's a greedy simulation, not a guarantee: at each step it picks whichever not-yet-capped enabled+`ok` account has the most weekly runway if it became the active one, using `activeBurnRateProxy` (the fastest positive weekly rate observed across any account) as the assumed pace for that account - an idle account's own near-zero rate says nothing about what it would do under real use. It runs that account forward until it would either cross 100% or a ~9-day horizon ends, then advances every account's simulated state (idle accounts drift at their own observed rate; any account whose weekly reset falls inside the elapsed window snaps back to 0% with its next reset pushed out another 7 days) before picking the next segment. Starts from whichever account `is_active` reports, falling back to the best-runway pick when that account is already capped or unset. Returns no segments (card reads "not enough data") when no account is usable or none has a positive weekly rate yet to build a proxy pace from.
...
`startValueSummaryTick` (`server/lib/value-summary-tick.js`, wired into `startBackgroundServices`) is the durable fix for `POST /api/project-plans/altitudes`'s 40-unit-per-request cap: the request path stays synchronous and unchanged, and this tick drains whatever it could not reach this round, unattended, across later cycles. Per tick it sweeps up to `MAX_PROJECTS_PER_TICK` projects in least-recently-swept rotation (never-swept projects sort first, then oldest `last_swept_at`, `project_id` only as a deterministic tiebreak — never the sole ordering key), and for each one calls `value-ledger.js`'s `assembleValuePool` (the single home for pool membership — this tick never hand-rolls its own pool query) followed by exactly one `enrichPoolAltitudes` call (the same composer the request path uses, and its one lexical writer of the `value_unit_summaries` cache; this tick only ever invokes it, never writes that table itself). Fail-safe per project: one project's assembly or synthesis failure never stops the rest of the sweep, and that project's rotation still advances — its `last_swept_at` write and the audit-log insert both happen unconditionally, in a guarded block of their own, so neither a synthesis failure nor a DB write failure on one project can starve every project behind it in rotation order. A failed sweep never overwrites that project's `pending_after_sweep` with 0 (which would be indistinguishable from "fully drained"); only a successful sweep re-derives that count, from that sweep's own `queued + unavailable` totals — never decremented from a prior value, never read from a stale `pool_size`, so a pool that grows between sweeps shows that growth rather than a falsely-converging number. Every sweep attempt (success or failure) writes one row to `value_summary_generation_log` — `pool_size`, `cache_hits`, `generated`, `queued`, `unavailable` (a strict partition: the four counted columns always sum to `pool_size`), `stale_regenerated` (an OVERLAP counter — how many of `generated` were a mutability-triggered regeneration, not a genuinely new unit; never part of the four-term partition identity), `model`, `duration_ms` — the operator-readable audit trail behind AC-2 (see [docs/DATABASE.md](../docs/DATABASE.md)). `counts` is computed exactly once per `enrichPoolAltitudes` call and read verbatim by both this tick and the `/altitudes` route — neither re-derives any of its terms. One backfill tick runs 30s after boot, then the steady-state interval, mirroring `startFocusInference`'s scheduler shape. Knobs: `DASHBOARD_VALUE_SUMMARY_TICK_MODE` (`on` (default) | `off`), `DASHBOARD_VALUE_SUMMARY_TICK_MS` (default `600000` ms; `<=0` disables), `MAX_PROJECTS_PER_TICK` (default `3`, clamped to >= 1 — lower than `reconciliation.js`'s 10 because each swept project costs a git walk AND up to one LLM spawn).

**Value Pool Slice 2 (coverage-on-demand)** adds `runCoverageDrain(dbModule, projectId, opts)` to the same file, sharing this tick's module-scope overlap guard (one runner, one guard — a concurrent call to either function returns `{skipped: "overlap"}`). `POST /api/project-plans/coverage-request` stamps `value_summary_sweep_state.coverage_requested_at`, jumping that project to the head of the passive rotation, and kicks the drain fire-and-forget: bounded back-to-back ≤`MAX_UNITS_PER_PROMPT`-unit batches, hard-capped at `MAX_DRAIN_BATCHES_PER_RUN = 25` (a safety bound sized against the measured 182-unit pool — never an env knob; the drain deliberately never reads `MAX_PROJECTS_PER_TICK`). Each iteration re-derives `pending` from that iteration's own full-pool counts, never a decremented counter, so a pool that grows mid-drain extends the drain for free. A request older than `COVERAGE_REQUEST_TTL_MS` (24h, not currently an env knob) reverts to passive with a logged revert, never on a single transient sweep error. `server/lib/value-coverage.js`'s `coverageSnapshot`/`estimateEta` are the single home for `described`/`pending`/`complete`/`demand`/an ETA — `GET /api/project-plans/coverage` and the WS broadcast's `coverage` field both carry that object verbatim (see [docs/API.md](../docs/API.md)).
...
DASHBOARD_VALUE_SUMMARY_MODEL=sonnet # Model for the Value Pool's altitude synthesis (POST /api/project-plans/altitudes); falls back to DASHBOARD_FOCUS_SUMMARY_MODEL, then DASHBOARD_FOCUS_INFER_MODEL, then haiku
DASHBOARD_VALUE_SUMMARY_UNIT_MODEL=   # Per-stage override for summaryModel("unit") (Value Pool Slice 2) — takes precedence over DASHBOARD_VALUE_SUMMARY_MODEL for per-unit PROJECT/STAKEHOLDER synthesis specifically; unset by default (falls through the same chain as DASHBOARD_VALUE_SUMMARY_MODEL above)
DASHBOARD_VALUE_SUMMARY_GROUPING_MODEL= # Per-stage override for summaryModel("grouping") (Value Pool Slice 2) — reserved for a future cross-unit grouping-synthesis stage; NO consumer calls stage "grouping" yet, so this knob has no effect until that stage ships
DASHBOARD_FOCUS_INFER_TIMEOUT_MS=30000 # Kill timer (ms) for a single focus-inference spawn (SIGTERM, then SIGKILL)
DASHBOARD_VALUE_SUMMARY_TICK_MODE=on  # Value-summary background sweep (drains the altitudes request path's 40-unit cap): on | off
DASHBOARD_VALUE_SUMMARY_TICK_MS=600000 # Value-summary sweep tick interval (ms); <=0 disables (backfill tick ~30s after boot, then this)
MAX_PROJECTS_PER_TICK=3               # Projects swept per value-summary tick (clamped to >= 1); each costs a git walk plus up to one LLM spawn — governs PASSIVE rotation width only; the Value Pool Slice 2 coverage drain (runCoverageDrain) never reads this knob (its own bound is the non-tunable MAX_DRAIN_BATCHES_PER_RUN=25 safety constant, not an env var)
