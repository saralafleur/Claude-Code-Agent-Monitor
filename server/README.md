...
`server/lib/value-summary.js` is the altitude-synthesis layer above the pool (`POST /altitudes`): every unit's GROUND fact (`value_source`/label/attribution, exactly what `/pool` already returns) gets a PROJECT phrase and a plain-language STAKEHOLDER sentence on request — the same leveling mechanism `focus-summary.js` uses for Focus window summaries, applied one altitude higher, reusing its hermetic `runClaudePromptJson` spawn contract. Unlike `focus_summaries`, the `value_unit_summaries` cache is keyed on the unit's own identity with no digest gating — a value unit's ground fact is immutable once seen, so it generates once and serves forever; every not-yet-cached unit in one request batches into a single spawn (never one call per unit, capped at `MAX_UNITS_PER_PROMPT`/40) so sibling units can inform each other's PROJECT phrasing. Model: `DASHBOARD_VALUE_SUMMARY_MODEL`, falling back to `DASHBOARD_FOCUS_SUMMARY_MODEL`, then `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku`; gated by the same `DASHBOARD_FOCUS_INFER_MODE` availability check. The response is `{ altitudes, states }` (DEC-11): every submitted unit lands in exactly one of the two, never both, never neither — `altitudes` for a cache hit or a fresh resolution this call, `states` for a miss beyond this call's 40-unit cap (`"queued"`, will be picked up by a later call or the sweep below) or a miss that was in scope but still produced nothing, e.g. LLM off (`"unavailable"`). Overflow beyond the 40-unit cap is drained unattended by the `startValueSummaryTick` background sweep (below), never by this request path. `PlanLedgerPanel` fetches altitudes as a separate, non-blocking request once the pool's units are known, so a cold cache's LLM latency never delays the rest of the panel.
