...
| `server/routes/playbook.js` | HTTP surface for the Coach's **Playbook** — the practice catalog (`server/lib/playbook/practices.js`, ships two practices: `session-token-ceiling`, scope session, and `account-weekly-balance`, scope global) merged with its user-editable config: `GET /api/playbook/practices` (every practice with `{ id, category, scope, kind, defaultSeverity, fields, enabled, config }`, config merged from the `playbook_practice_config` row or the catalog's own defaults if none exists — a new practice needs no migration/seed) and `PUT /api/playbook/practices/:id/config` (patch `{ enabled?, config? }`; validates each config key against that practice's own `fields` schema (`type: "number"` against `min`, `type: "boolean"` as true/false) — unknown key or a type/range mismatch is a 400). A single global config shared by every connected computer, not per-user — broadcasts `playbook_practice_config_updated` with the full resulting practice on every write, same pattern as `color-thresholds.js`. Also `GET`/`PUT /api/playbook/settings` — the Playbook's one global (not per-practice) setting, `{ autoResolveAfterMs }`: how long after a session ends before its open Observations auto-resolve (default 180min/3h; `0` disables the sweep entirely, not "resolve instantly on session end"; a still-active session's Observation is never auto-resolved), persisted to the singleton `playbook_settings` row, broadcasting `playbook_settings_updated` |
| `server/routes/coach.js` | HTTP surface for the Coach's **Feed** — Observations the Playbook engine (`server/lib/playbook/engine.js`) has recorded: `GET /api/coach/observations?status=` (most recent first, optional status filter) and `POST /api/coach/observations/:id/respond` `{response}` (one of acknowledged/dismissed/resolved; broadcasts `coach_observation_updated`). `coach_observation_created` itself is broadcast by the engine's own tick, not this router; `coach_observation_updated` is also broadcast by the engine's own auto-resolve sweep (see `playbook.js` row above and `lib/playbook/engine.js`) — this router only ever broadcasts state a human explicitly changed via the respond endpoint |
...
| `server/lib/trunk-drift.js` | **Direct-to-trunk detection** (`GET /api/projects/:id/trunk-drift`, Phase 1a) — same recompute-per-request, never-cached posture as `repo-topology.js`: reads no SQLite, writes nothing, caches nothing. `detectTrunkDrift(repoPath, opts)` resolves the trunk branch via `git-refs.resolveDefaultBranch`, then runs one bounded `git log --first-parent --no-merges --since=<lookback> --max-count=<cap+1> --shortstat refs/heads/<branch>` walk — DEC-5's git-native false-positive guard (`intake/2026-08-02-trunk-drift-detection/decisions.md`): a commit counts as "direct-to-trunk" only if it's on the default branch's first-parent line and is not itself a merge commit (a `--no-ff` merge's own commits live on the merge commit's second-parent line, so `--first-parent` already excludes them regardless of whether the feature branch ref still exists). **Deliberately does not** additionally require "not reachable from any other local branch" — that clause was removed (`intake/2026-08-03-trunk-drift-open-branch-blindness/decisions.md`): it's topologically impossible to distinguish a fast-forward-merged-but-undeleted branch from an unrelated, freshly-created sibling branch (same merge-base, zero commits ahead either way), and the latter — an ordinary open effort worktree — was blinding the detector across a repo's *entire* history any time one merely existed, far more often than the former needed protecting against; this project's own convention is always `--no-ff` + branch deletion, so the scenario the old clause protected is not expected to occur in practice (`WATCH-1` in the trunk-drift-detection decision log, widened accordingly). Commit order is git's own DAG order, never re-sorted by commit date. Bounded by `MAX_TRUNK_DRIFT_COMMITS` (200) and `DASHBOARD_TRUNK_DRIFT_LOOKBACK_DAYS` (default 7). Every git failure resolves to `{ skipped: "git_error" }` — never a throw, never a false "clean"; other `skipped` reasons (`not_a_repo`, `no_default_branch`, `no_commits`) are likewise explicit, never a guess. Accepts a caller-supplied `seenShas` Set for idempotency filtering with no DB read of its own (DB-free by design). Phase 1a is read-only: no schema change, no `detour_dispositions` write — see DEC-1/WATCH-5 for the gated Phase 1b (persistence + reconciliation pickup) |
...
```typescript
{
  type: "session_created" | "session_updated" | "session_deleted" | "agent_created" | "agent_updated" | "new_event"
      ...
      | "playbook_practice_config_updated" | "playbook_settings_updated"
  ...
  data: Session | { id: string } | Agent | DashboardEvent | AlertEvent | WorkflowRun | PlanUpdate | SessionFocus
      ...
      | PlaybookPractice | PlaybookSettings | CoachObservation;
...
The `playbook_settings_updated` message (emitted by `PUT
/api/playbook/settings`) carries the full resulting global Playbook settings
— `{ autoResolveAfterMs }` — same broadcast-on-write pattern as
`playbook_practice_config_updated`, just for the one setting that applies
across every practice instead of one practice's own config.
...
resolves one, **or** by the same engine tick's own auto-resolve sweep once
a session-scoped practice's Observation is eligible (its session has ended
AND at least `autoResolveAfterMs` has elapsed since — a still-active
session's Observation is never auto-resolved by time alone) — see the
`server/routes/playbook.js` / `server/routes/coach.js` rows in the module
table above for the full route surface.
...
A fourth consumer, `client/src/pages/KanbanBoard.tsx` (see the `/kanban`
routes-table row below), renders the same tile as a standalone header card
via a new optional `compact` prop on both `ConcurrencyStatTile` and the
underlying `StatTile` — a smaller, self-bordered rendering (border/rounded
card, smaller value text) meant to sit on its own outside the `gap-px
bg-border` stat grid the default rendering assumes. `KanbanBoard` fetches
today's (local midnight-to-midnight, all-projects/all-sessions)
`GET /api/focus-report` once on mount via its own `loadConcurrency`
callback — independent of the board's `view` state so the card stays
visible across all three views — and re-fetches (debounced 300ms) on the
same `session_created`/`session_updated`/`session_deleted`/`agent_updated`
WebSocket events the board's own agent/session loads react to, rather than
a poll timer (this file has none). The card renders label `t("plan:report.
board.concurrentSessions")` — the same "Concurrent agent sessions" copy and
translation key DEC-6 introduced for the Calendar board (cross-namespace
lookup, the same pattern already used here for `t("common:live")`/`t("common:
offline")`) — and simply stays unmounted (`concurrencyReport` state is
`null`) until the first fetch resolves, or again after any fetch that
throws, rather than showing a loading skeleton or error state.
