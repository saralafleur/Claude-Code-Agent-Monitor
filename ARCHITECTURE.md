...
| `server/routes/projects.js` | HTTP surface for **Projects** — a user-named grouping of one or more session working directories: `GET /api/projects` (every project with its mapped folders + server-aggregated `session_count`/`active_count`/`last_activity`/`priority`, plus an `unassigned` bucket for cwds with sessions not mapped to any project), `POST /api/projects` (create, optionally attaching folders — 409 `ALREADY_MAPPED` if any is already claimed elsewhere), `PATCH /api/projects/:id` (rename), `DELETE /api/projects/:id` (folder mappings cascade; sessions are untouched and fall back to unassigned), `POST`/`DELETE /api/projects/:id/paths[/:pathId]` (add/remove a folder mapping — a folder belongs to at most one project), `GET /api/projects/:id/focus-report` (project-scoped focus-time report — see `server/lib/focus-report.js`), and `PUT /api/projects/reorder` (WIP queue page: bulk-sets dense `priority` ranks `0..N-1` from an `order: string[]` of project ids — first bulk-array-order-persistence endpoint in this codebase, modeled on `routes/monitors.js`'s pattern; broadcasts `project_updated`, the one deliberate exception to this router's otherwise-unbroadcast CRUD). No `sessions` schema change: membership is derived by joining `sessions.cwd` against `project_paths.cwd` |
...
| `/wip`          | WIP           | The WIP (work-in-progress) queue: a single, live, priority-ordered list of `status === "active"` session cards, fetched once via `GET /api/sessions?status=active&limit=500` + `GET /api/projects`, then kept live over `session_created`/`session_updated`/`session_deleted`/`project_updated` WS pushes (no polling). Sessions awaiting Sara's input always sort above ones that aren't (`lib/wipQueue.ts`'s `sortWipQueue`, reusing `isSessionAwaitingInput` and Kanban's own primary-awaiting-reason carve-out exactly, not a re-derived predicate); ties (and non-awaiting ordering) break on the owning project's `priority` (ascending — lower wins, unmapped cwd falls back to `0`) then `last_activity`/`started_at` descending. Project resolution goes through the shared `lib/projectLookup.ts` join (`buildCwdProjectIndex`/`projectForSession`) — the same extracted cwd→project lookup `KanbanBoard.tsx`'s Projects view now calls, so the two pages can't silently disagree (see `client/src/lib/__tests__/sessionSurfaceParity.test.ts`, a standing cross-consumer guard). Cards are `WipSessionCard` — `SessionCard` wrapped (not edited) with a prominent project-name header. Layout is a responsive 1/2/3-column contiguous-chunk fill (`lib/wipQueue.ts`'s `assignToColumns`) driven by the queue container's own measured `ResizeObserver` width (`<768px` → 1 col, `768–1023px` → 2, `≥1024px` → 3) — not `window`/viewport breakpoints, since the right-hand `WipPrioritySidecar` can narrow the container independent of window size. The sidecar (collapsed by default, scoped to queue-represented projects with a "show all projects" toggle) reorders each project's `priority` via the same native HTML5 drag-and-drop shape `KanbanBoard.tsx` already uses, committing to `PUT /api/projects/reorder` (broadcasts `project_updated`) |
| `/kanban`       | KanbanBoard   | View toggle persisted in `localStorage`. Agents view: `GET /api/agents?status={each}` per-status (default 10000 cap). Sessions view: `GET /api/sessions?status={each}&limit=10000` per-status. Projects view: `GET /api/projects` + sessions grouped client-side by `cwd`; once at least one **monitor** group exists (`lib/monitorGroups.ts` — a server-backed global store shared across every computer connected to the dashboard, hydrated from `GET /api/monitors` and kept live via `monitors_updated` WebSocket pushes, mirroring `lib/focusStore.ts`'s pattern; a one-time migration seeds the server from any pre-existing `localStorage` layout the first time it hydrates empty), its assigned project columns render as real DOM children inside a bordered, drag-reorderable box for that monitor (`MonitorBox`) rather than as loose columns, with each box sitting side by side with the others in the same single row plus a trailing **Ungrouped** box (`UngroupedBox` — a container just like a monitor's box, but non-draggable/non-renameable and backed by no stored group) whose unassigned project columns render inside it — dragging a box by its header repositions it left/right; dragging a project column onto a box (or a column already inside one) reassigns it, deferred to `dragend` via a ref rather than previewed live, since a column moving into a *different* box is a genuine reparent across two real DOM elements that would detach it from the tree the native HTML5 drag is tracking mid-gesture. `Column`'s own drag handlers call `stopPropagation()` so a nested column's drag doesn't bubble into its enclosing box and misfire the box's own reposition handlers. A monitor box's header chevron toggles `MonitorGroup.collapsed` (persisted alongside name/id); collapsing hides the columns row (`hidden` class, not a conditional unmount of the *columns* - a card's own "show more" state survives the toggle) and drops the box's own `self-align` to `start` so it shrinks to its header's height instead of stretching to match the row's tallest sibling. A second header button (`Columns3`/`Rows3` icon, toggling `handleToggleMonitorOrientation`) flips `MonitorGroup.orientation` between `"horizontal"` (default, omitted for pre-existing stored monitors) and `"vertical"`, which switches the columns row's own `flex-row`/`flex-col` class so that monitor's project columns lay out side by side or stacked independently of every other monitor's box. A third header button (text label, `handleCycleMonitorWrap`) cycles `MonitorGroup.wrap` through `"*"` (default, omitted for pre-existing stored monitors — today's unbounded row/column) and `"1"`-`"4"`; whenever `wrap` isn't `"*"`, the columns row switches from `flex` to CSS `grid` (`wrapGridStyle`, shared with `Column` below) with `grid-template-columns: repeat(N, max-content)` for horizontal orientation or `grid-template-rows: repeat(N, max-content)` + `grid-auto-flow: column` for vertical, so N project columns land per row/column before wrapping to a new one. The box itself is additionally split out of the main row: `collapsedMonitorClusters` vs `expandedMonitorClusters` (filtered from `monitorClusters` by `monitor.collapsed`) render in two separate sibling rows - a `kanban-collapsed-monitor-row` strip above `kanban-board-row`, shared by every collapsed monitor, and the main row for everything expanded - so a collapsed box frees the horizontal space it would otherwise still occupy rather than just sitting there narrow (this does mean the `MonitorBox` itself unmounts from one row and remounts in the other on toggle, since React can't preserve a component instance across two different parent elements; the state that matters - each column's pagination limit - lives in `KanbanBoard`'s own `expanded` map, not in `Column`, so nothing is lost). Every `Column` (project columns and the standalone Unassigned column) carries its own chevron toggling a `collapsed` prop the same way — persisted separately, per column key, in the same server-backed layout's `collapsedProjects` map (`monitorStore.saveCollapsedProjects`) — hiding that column's card list and shrinking its width (`w-72` → `w-56`) independently of any monitor box it sits inside. Every Agents/Sessions status `Column` also gets its own `orientation`/`onToggleOrientation` prop pair (Projects-view project columns never pass these) — a header icon button, reusing the same `Columns3`/`Rows3` icons as `MonitorBox`'s toggle, that flips that single column's card list between the default `"vertical"` stack and a `"horizontal"` row of fixed-width (`w-72`) cards in their own `overflow-x-auto` strip. State lives in `KanbanBoard`'s own `statusColumnOrientation` map (`toggleStatusColumnOrientation`), keyed by `${view}-${status}` (e.g. `agents-working`, `sessions-active`) and persisted client-side only, in a new `kanban-status-column-orientation` `localStorage` key — unlike `MonitorGroup.orientation`, a status column has no server-backed identity to hang a shared value off, so this toggle is intentionally per-browser and not pushed over `monitors_updated`. A paired second button (text label, `cycleStatusColumnWrap`) cycles a sibling `statusColumnWrap` map (same `${view}-${status}` key, own `kanban-status-column-wrap` `localStorage` key) through the same `"*"`/`"1"`-`"4"` values and `wrapGridStyle` grid switch as `MonitorGroup.wrap` above, but scoped to that one column's cards instead of a monitor's project columns. Each column then paginates client-side at `COLUMN_PAGE_SIZE=10`; the WS subscription scopes to the active view. |
...
```typescript
{
  type: "session_created" | "session_updated" | "session_deleted" | "agent_created" | "agent_updated" | "new_event"
      ...
      | "remote_source.status" | "monitors_updated" | "project_updated";
  data: Session | { id: string } | Agent | DashboardEvent | AlertEvent | WorkflowRun | PlanUpdate | SessionFocus
      | RemoteSourceStatus | MonitorLayoutPayload | ProjectPriorityUpdatedPayload;
...
The `project_updated` message (emitted by `PUT /api/projects/reorder`, see
[Server-Side Architecture](#server-side-architecture)) carries
`{ projects: [{ id, priority }] }` — only the ids/priorities the triggering
reorder call actually touched, not every project. This is a deliberate,
narrow exception to "project mutations are plain CRUD, not broadcast":
every other project mutation (create, rename, folder add/remove, delete)
stays unbroadcast. The WIP queue page (`/wip`) is the only consumer today —
it patches matching projects' `priority` in place so a sidecar drag in one
tab re-ranks the queue live in every other open tab.
