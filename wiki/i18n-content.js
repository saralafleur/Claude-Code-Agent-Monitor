...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "Groups sessions by the folder(s) they run from into a user-named <strong>project</strong>, replacing the flat Agents/Sessions view with one grouped by where the work actually happens. A project claims one or more working directories — a folder belongs to at most one project — and membership is derived by joining each session's <code>cwd</code> against the project's mapped folders, with no schema change on <code>sessions</code> itself. Each project renders as a horizontally scrollable row of its sessions, with an aggregated session count, active count, and last-activity badge computed server-side. Create a project (optionally from an existing folder), rename it inline, add or remove folder mappings, or delete it — sessions are never deleted, they simply fall back to the <strong>Unassigned</strong> bucket.":
      "按会话运行所在的文件夹将会话分组为一个用户命名的 <strong>项目</strong>，取代原本扁平的 Agents/Sessions 视图，按实际工作发生的位置进行分组。一个项目可以拥有一个或多个工作目录——一个文件夹最多只能属于一个项目——成员关系是通过将每个会话的 <code>cwd</code> 与项目已映射的文件夹进行匹配来推导的，<code>sessions</code> 本身无需任何模式变更。每个项目都会渲染为其会话的一个可横向滚动的行，并带有服务端计算的会话数、活跃数与最近活动徽章。可以创建项目（可选地立即从某个现有文件夹创建）、内联重命名、添加或移除文件夹映射，或将其删除——会话永远不会被删除，只会回退到 <strong>Unassigned</strong> 分组中。",
  ...
  vi: {
    ...
    "Groups sessions by the folder(s) they run from into a user-named <strong>project</strong>, replacing the flat Agents/Sessions view with one grouped by where the work actually happens. A project claims one or more working directories — a folder belongs to at most one project — and membership is derived by joining each session's <code>cwd</code> against the project's mapped folders, with no schema change on <code>sessions</code> itself. Each project renders as a horizontally scrollable row of its sessions, with an aggregated session count, active count, and last-activity badge computed server-side. Create a project (optionally from an existing folder), rename it inline, add or remove folder mappings, or delete it — sessions are never deleted, they simply fall back to the <strong>Unassigned</strong> bucket.":
      "Nhóm các phiên theo (các) thư mục mà chúng chạy từ đó thành một <strong>dự án</strong> do người dùng đặt tên, thay thế khung nhìn Agents/Sessions phẳng bằng một khung nhìn được nhóm theo nơi công việc thực sự diễn ra. Một dự án sở hữu một hoặc nhiều thư mục làm việc — một thư mục chỉ thuộc về nhiều nhất một dự án — và tư cách thành viên được suy ra bằng cách đối chiếu <code>cwd</code> của mỗi phiên với các thư mục đã ánh xạ của dự án, không cần thay đổi schema trên chính <code>sessions</code>. Mỗi dự án được hiển thị dưới dạng một hàng có thể cuộn ngang gồm các phiên của nó, cùng với huy hiệu tổng số phiên, số đang hoạt động và hoạt động gần nhất được tính toán phía máy chủ. Tạo một dự án (tùy chọn từ một thư mục có sẵn), đổi tên trực tiếp, thêm hoặc xóa ánh xạ thư mục, hoặc xóa dự án — các phiên không bao giờ bị xóa, chúng chỉ đơn giản rơi trở lại nhóm <strong>Unassigned</strong>.",
    "A standalone <strong>Focus</strong> page (route <code>/focus</code>, in the sidebar right after <strong>Calendar</strong>) turns the same focus-time data into a stakeholder-readable activity report instead of a calendar grid. It shares the Calendar board's own project chips, session dropdown, and time-period picker, then swaps the swimlane for stat tiles plus a new activity list — one row per plan item, detour, bug, feature, or unclassified time bucket, each with a kind-colored chip and a wall/active time figure, and, for entries the background classifier inferred rather than a live declaration, its one-sentence reason. It needed no backend changes at all, reusing the existing <code>GET /api/focus-report</code> endpoint exactly as the Calendar board does.":
