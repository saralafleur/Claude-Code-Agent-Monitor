...
window.__WIKI_CONTENT_I18N = {
  zh: {
    "A compact stat card in the header, to the left of the view toggle, shows today's cross-project concurrency ratio at a glance — the same figure the Focus report's Concurrency tile computes — and refreshes itself as sessions and agents change.":
      "页头有一张紧凑的统计卡片，位于视图切换开关的左侧，一眼展示今日跨项目的并发比率——与 Focus 报告的 Concurrency 磁贴计算的是同一个数值——并会随会话和 Agent 的变化自动刷新。",
    'Answers "what is this session doing, and which part of the project plan is it for?" at a glance. Each monitored repo may keep a human-approved <code>AGENT-PLAN.md</code> (numbered checkbox items) at its root; a background poll (<code>DASHBOARD_PLAN_POLL_MS</code>, plus opportunistic ingest on <code>SessionStart</code>) mirrors it read-only into the dashboard, keyed by <code>cwd</code> and rolled up to Projects. Sessions declare their position by running <code>ccam focus set/push/bug/feature/pop/done</code> in their Bash tool — the hook stream is the write channel, so it works in every hooked project with zero per-project setup. <code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code> push a kind-tagged detour; the Plan view\'s per-item focus line reflects each session\'s actual current state (known item, plain detour, feature, or bug) with a matching icon, bucketed under the item it was declared on (or an "Unknown item" row when none was set), with click-to-expand detail for bug/feature declarations.':
      '一眼回答“这个会话在做什么、它服务于项目计划的哪一部分？”。每个被监控的仓库都可以在其根目录保留一份经人工批准的 <code>AGENT-PLAN.md</code>（带编号的复选框条目）；后台轮询（<code>DASHBOARD_PLAN_POLL_MS</code>，外加 <code>SessionStart</code> 时的即时摄取）会将其只读镜像到仪表盘，按 <code>cwd</code> 关联并汇总到 Projects。会话通过在其 Bash 工具中运行 <code>ccam focus set/push/bug/feature/pop/done</code> 来声明自己的位置——hook 流就是写入通道，因此它在每个安装了 hook 的项目中开箱即用，无需任何按项目的设置。<code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code> 会推送一个带类型标记的绕行；Plan 视图中逐条目的焦点行会反映每个会话的实际当前状态（已知条目、普通绕行、功能或缺陷），并配有相应图标，归入其被声明所属的条目下（未设置条目时归入“Unknown item”行），缺陷/功能声明可点击展开详情。',
    "The Projects page shows each plan as a collapsible checklist with a progress bar and per-item session chips; every session card gets a one-line focus breadcrumb with a shared icon for its current state (crosshair on a known item, a detour/feature/bug icon otherwise), rendered even for a session mid-detour with no base item declared; Session Detail gains a focus banner plus a <strong>Plan</strong> tab (checklist, focus timeline from <code>Focus</code> events, and the session's latest TodoWrite micro-plan). A periodic drift audit (<code>DASHBOARD_FOCUS_AUDIT_*</code>) — headless <code>claude -p</code> on a small model, with a keyword-overlap fallback — stamps an amber \"possible undeclared detour?\" badge when a focused session's recent activity stops matching its declaration; it never rewrites focus, and declarations never clear the badge.":
      "Projects 页面将每个计划显示为可折叠的清单，带进度条和逐条目的会话小标签；每张会话卡片都会获得一行式的焦点面包屑，并带有反映其当前状态的统一图标（已知条目上为十字准星，其余情况下为绕行/功能/缺陷图标），即使会话正处于绕行中且未声明基础条目，也会渲染该面包屑；会话详情新增焦点横幅和一个 <strong>Plan</strong> 标签页（清单、来自 <code>Focus</code> 事件的焦点时间线，以及该会话最新的 TodoWrite 微计划）。周期性漂移审计（<code>DASHBOARD_FOCUS_AUDIT_*</code>）——在小模型上无头运行 <code>claude -p</code>，并有关键词重叠的回退方案——当聚焦会话最近的活动不再与其声明相符时，会盖上琥珀色的“possible undeclared detour?”徽章；它从不改写焦点，声明也从不清除该徽章。",
    'A report icon next to each project\'s "view plan" icon (Projects page and Kanban\'s Projects view) opens a <strong>focus-time report</strong> scoped to that project: how long its sessions actually spent on a declared item versus a detour, a feature aside, or a bug fix — reconstructed from existing <code>Focus</code> event history, not a new capture mechanism. A configurable idle-grace window (<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>, default 5 min) discounts stretches with no hook activity past the window, so a session sitting in Waiting for hours doesn\'t inflate its "time on item" figure. The popup shows active/idle stat tiles, a per-session segmented timeline, a per-item rollup (detours bucketed under the item they interrupted), and a project-wide split by kind.':
      "每个项目卡片/页头上“查看计划”图标旁新增的报告图标（项目页面与看板项目视图）会打开一份聚焦于该项目的<strong>焦点时间报告</strong>：展示其会话实际把时间花在已声明条目上，还是花在绕行、功能插曲或缺陷修复上——这些数据全部从既有的 <code>Focus</code> 事件历史重建而来，而非新增的采集机制。一个可配置的空闲宽限窗口（<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>，默认 5 分钟）会折算掉超出该窗口、且没有 hook 活动的时间段，避免会话在 Waiting 状态停留数小时导致其“条目用时”虚增。弹窗展示活跃/空闲统计磁贴、逐会话的分段时间线、逐条目汇总（绕行归入其打断的条目下），以及按类型划分的项目级占比。",
    "Poll interval for the <code>AGENT-PLAN.md</code> watcher — a changed plan file is re-ingested and broadcast as <code>plan_updated</code>; <code>0</code> disables the poll (<code>SessionStart</code> still ingests opportunistically, and <code>POST /api/plans/refresh</code> forces one)":
      "<code>AGENT-PLAN.md</code> 监视器的轮询间隔——计划文件发生变化时会被重新摄取并以 <code>plan_updated</code> 广播；<code>0</code> 禁用轮询（<code>SessionStart</code> 仍会即时摄取，<code>POST /api/plans/refresh</code> 可强制执行一次）",
    "Tick interval for the focus drift audit — the background check that a focused session's recent activity matches its declared plan item; <code>0</code> disables auditing entirely":
      "焦点漂移审计的滴答间隔——该后台检查验证聚焦会话最近的活动与其声明的计划条目相符；<code>0</code> 完全禁用审计",
    "Idle-grace window for the per-project focus-time report — a gap between two consecutive hook events longer than this only counts the grace window's worth as active, the rest is excluded as idle; <code>0</code> disables discounting (100% of wall-clock counts as active)":
      "逐项目焦点时间报告的空闲宽限窗口——两次连续 hook 事件之间的间隔如果超过该窗口，只有相当于宽限窗口那部分时长会被计为活跃，其余部分记为空闲；<code>0</code> 可禁用折算（100% 的实际耗时都计为活跃）",
    "Drift-audit judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap only), or <code>off</code>":
      "漂移审计的评判方式：<code>llm</code>（在小模型上无头运行 <code>claude -p</code>，CLI 不可用时回退到启发式）、<code>heuristic</code>（仅关键词重叠）或 <code>off</code>",
    "Model passed to the drift audit's <code>claude -p --model</code> spawn":
      "传给漂移审计 <code>claude -p --model</code> 进程的模型",
    "Kill timer for a single drift-audit <code>claude -p</code> spawn (SIGTERM, then SIGKILL)":
      "单次漂移审计 <code>claude -p</code> 进程的终止计时器（先 SIGTERM，后 SIGKILL）",
    "Sessions that never declared a <code>ccam focus</code> don't punch silent holes in that report: a background <strong>focus inference</strong> job (<code>DASHBOARD_FOCUS_INFER_*</code>) digests each untracked session's activity and attributes it to the best-matching plan item or an inferred detour, storing one verdict per session in the <code>focus_inferences</code> table. Segments it produces carry <code>inferred: true</code> and render with an \"≈ inferred\" chip, so guessed time is never dressed up as declared time. Declared focus history always wins — inference only fills sessions with no declarations at all — and a session with neither a declaration nor a confident verdict yet (most often one that's still running and hasn't gone quiet long enough for a tick to reach it) still shows up as a neutral gray \"No focus\" block instead of a silent unclassified hole. A project with no <code>AGENT-PLAN.md</code> at all used to be invisible to this classifier entirely; it's now still classified, asking for a plain one-sentence summary of what the session's activity accomplished instead of trying to match a plan item, and storing it the same way. A low-confidence <code>unclassified</code> verdict's reason — which the classifier already generated — is no longer silently dropped by the report layer; it now surfaces there too.":
      "从未声明过 <code>ccam focus</code> 的会话不会在该报告中留下无声的空洞：一个后台<strong>焦点推断</strong>作业（<code>DASHBOARD_FOCUS_INFER_*</code>）会消化每个未跟踪会话的活动，将其归属到最匹配的计划条目或一个推断出的绕行，并在 <code>focus_inferences</code> 表中为每个会话存储一条判定。它生成的时间段带有 <code>inferred: true</code>，并以“≈ inferred”小标签渲染，因此推测出的时间绝不会被包装成已声明的时间。已声明的焦点历史始终优先——推断只填补完全没有任何声明的会话——而一个既没有声明、也还没有可靠判定的会话（最常见的情况是仍在运行、且尚未安静足够长时间以便某次轮询触及它）依然会显示为一个中性灰色的“No focus”色块，而不是一个安静的未分类空洞。完全没有 <code>AGENT-PLAN.md</code> 的项目此前对该分类器完全不可见；现在它仍会被分类，只是会请求一句关于该会话活动完成了什么的简明摘要，而不是尝试匹配某个计划条目，并以相同方式存储。低置信度的 <code>unclassified</code> 判定的原因——分类器此前就已生成——不再被报告层悄悄丢弃；现在它也会显示在报告中。",
    "A header toggle in the focus-time report switches between the List view described above and a <strong>Calendar</strong> view — a day-view swimlane calendar that positions every session's segments on a real 24-hour axis, with sessions that overlap in time split into side-by-side lanes via a greedy lane-assignment algorithm, the same way a Teams or Outlook calendar day view spreads concurrent meetings instead of stacking them. Dashed-border blocks mark inferred/undeclared attribution and solid borders mark declared focus, matching the List view's own convention; a pulsing, open-ended block marks a session that's still actively running, using the API's <code>ended_at: null</code> field to tell a genuinely live session from one that only just ended. Prev/Today/Next controls step through days — this v1 ships Day view and that simple navigation only; Week/Month zoom and an aggregate time-range selector (3d/7d/30d/All) are future work. The report's stat tiles also gained a <strong>Concurrency</strong> tile (e.g. \"1.08x\", effort time ÷ wall-clock time), and the \"Total agent time\" tile's wall-clock sub-label now reflects the real union-based wall-clock span instead of a misleading per-segment sum, so effort time (which inflates with overlapping sessions) and true wall-clock time read as two distinct, honest numbers. The Concurrency tile now also carries a \"while active\" sub-figure (<code>active_concurrency_ratio</code>) that divides effort by only the time at least one session was actually active — so sessions left open but idle (overnight, say) no longer dilute the parallelism reading — and a small swap button that inverts which of the two ratios is the tile's primary figure, a per-browser choice that persists across reloads.":
      "焦点时间报告头部的切换开关可在上文所述的 List 视图与新的 <strong>Calendar</strong> 视图之间切换——一个日视图泳道日历，把每个会话的时间段定位在真实的 24 小时轴上，时间上相互重叠的会话会通过贪心车道分配算法拆分到并排的车道中，就像 Teams 或 Outlook 日历的日视图把并发会议并排展示，而不是堆叠起来。虚线边框的方块标记推断/未声明的归属，实线边框标记已声明的焦点，与 List 视图自身的约定保持一致；带脉冲动画的开放式方块标记一个仍在实际运行中的会话，判断依据是接口的 <code>ended_at: null</code> 字段，用来区分真正存活的会话与刚刚结束的会话。Prev/Today/Next 控件按天步进——这个 v1 版本只提供 Day 视图和这种简单导航；Week/Month 缩放与聚合时间范围选择器（3d/7d/30d/All）留待未来实现。报告的统计磁贴还新增了一个 <strong>Concurrency</strong>（并发度）磁贴（例如 “1.08x”，即投入时间 ÷ 实际耗时），且 “总代理时间” 磁贴的实际耗时子标签现在反映的是真实的并集实际耗时，而不是容易误导的逐段求和；这样投入时间（会因会话重叠而虚增）与真实的实际耗时就呈现为两个截然不同、诚实的数字。Concurrency 磁贴现在还带有一个“活跃时”子数值（<code>active_concurrency_ratio</code>），它只用至少有一个会话真正活跃的时间去除投入时间——因此开着却闲置的会话（例如挂机过夜）不再稀释并行度读数——磁贴上还有一个小小的切换按钮，可以反转两个比率中哪一个作为主数值显示，这一按浏览器保存的选择在重新加载后依然保留。",
    "Tick interval for background focus inference — the classifier that attributes sessions which never declared a <code>ccam focus</code> to a plan item or an inferred detour (first backfill tick ~30 s after boot); <code>0</code> disables inference entirely":
      "后台焦点推断的滴答间隔——该分类器将从未声明过 <code>ccam focus</code> 的会话归属到某个计划条目或推断出的绕行（启动后约 30 秒进行首次回填）；<code>0</code> 完全禁用推断",
    "Focus-inference judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap that only ever claims an item match), or <code>off</code>":
      "焦点推断的评判方式：<code>llm</code>（在小模型上无头运行 <code>claude -p</code>，CLI 不可用时回退到启发式）、<code>heuristic</code>（关键词重叠，只会认定条目匹配）或 <code>off</code>",
    "Model passed to the focus-inference <code>claude -p --model</code> spawn":
      "传给焦点推断 <code>claude -p --model</code> 进程的模型",
    "falls back to <code>DASHBOARD_FOCUS_INFER_MODEL</code>, then <code>haiku</code>":
    ...
    "<code>ccam focus …</code> run in the session's Bash tool":
      "在会话的 Bash 工具中运行 <code>ccam focus …</code>",
    "Parsed out of the <code>PostToolUse</code> hook's <code>tool_input.command</code> (the only channel that natively carries the session id). <code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code> update the session's <code>session_focus</code> row (plan-item pointer plus detour stack) and write a <code>Focus</code> event with an item-text snapshot for the timeline; broadcasts <code>session_focus</code> (and <code>plan_updated</code> after <code>done</code>). <code>bug</code>/<code>feature</code> additionally carry <code>kind</code>/<code>title</code>/<code>detail</code> on the pushed detour frame, badged in the Plan view. <code>status</code> is read-only and records nothing.":
      "从 <code>PostToolUse</code> hook 的 <code>tool_input.command</code> 中解析（这是唯一原生携带会话 id 的通道）。<code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code> 会更新该会话的 <code>session_focus</code> 记录（计划条目指针加绕行栈），并写入一条带条目文本快照的 <code>Focus</code> 事件供时间线使用；广播 <code>session_focus</code>（<code>done</code> 之后还广播 <code>plan_updated</code>）。<code>bug</code>/<code>feature</code> 还会在被推送的绕行帧上携带 <code>kind</code>/<code>title</code>/<code>detail</code>，在 Plan 视图中以徽章显示。<code>status</code> 是只读的，不记录任何内容。",
    ...
    '<span class="caption-icon">📈</span> <span><strong>Prometheus · Graph</strong> — ad-hoc PromQL against scraped CCAM series; starter expressions ship in the CCAM console and <code>monitoring/README.md</code>, with derived rates in <code>monitoring/prometheus/ccam-rules.yml</code></span>':
      '<span class="caption-icon">📈</span> <span><strong>Prometheus · Graph</strong> — 对已抓取 CCAM 序列运行即席 PromQL；入门表达式见 CCAM 控制台与 <code>monitoring/README.md</code>，派生速率见 <code>monitoring/prometheus/ccam-rules.yml</code></span>',
    "CCAM exposes a read-only <strong>Prometheus text-exposition</strong> endpoint at <code>GET /api/metrics</code> — sessions and agents by status, cumulative events and tokens, connected WebSocket clients, configured remote sources, process uptime/RSS, and build version — so the same dashboard you use for Claude Code monitoring can be scraped into your own observability stack. A turnkey stack lives in <code>monitoring/</code>: npm-managed binaries (no Homebrew/apt) or optional Docker Compose, with Grafana datasource and four dashboards auto-provisioned on first start.":
      "CCAM 在 <code>GET /api/metrics</code> 提供只读的 <strong>Prometheus 文本暴露</strong> 端点 — 按状态的 sessions/agents、累计事件与 token、已连接 WebSocket 客户端、已配置远程源、进程运行时长/RSS 与构建版本 — 因此用于 Claude Code 监控的同一仪表盘可被抓取到你自己的可观测性栈。<code>monitoring/</code> 中有开箱即用组合：npm 管理二进制（无需 Homebrew/apt）或可选 Docker Compose，首次启动自动配置 Grafana 数据源与四个仪表盘。",
    ...
    "<code>monitoring/prometheus/ccam-rules.yml</code> derives convenient rates and aggregates from raw <code>ccam_*</code> series (e.g. <code>rate(ccam_tokens_total[5m])</code>) so Grafana panels and ad-hoc Graph queries stay fast without hand-rolling every expression.":
      "<code>monitoring/prometheus/ccam-rules.yml</code> 从原始 <code>ccam_*</code> 序列派生便捷速率与聚合（如 <code>rate(ccam_tokens_total[5m])</code>），使 Grafana 面板与即席 Graph 查询无需手写每条表达式。",
    "Scrape &amp; auth guard": "抓取与认证防护",
    "Loopback scrapes (<code>127.0.0.1:4820</code>) work out of the box. When Prometheus runs in Docker and reaches the host via <code>host.docker.internal</code>, add that host to <code>DASHBOARD_ALLOWED_HOSTS</code> or the scrape returns <code>403 EBADHOST</code>. If <code>DASHBOARD_TOKEN</code> is set, scrapers must send the same bearer token.":
      "回环抓取（<code>127.0.0.1:4820</code>）开箱即用。Prometheus 在 Docker 中通过 <code>host.docker.internal</code> 访问主机时，须将该主机加入 <code>DASHBOARD_ALLOWED_HOSTS</code>，否则返回 <code>403 EBADHOST</code>。若设置了 <code>DASHBOARD_TOKEN</code>，抓取器须发送相同 bearer token。",
    "Full reference": "完整参考",
    'Commands, Docker vs npm paths, dashboard UIDs, starter PromQL, and troubleshooting (Grafana login reset, target DOWN) are documented in <a href="../monitoring/README.md"><code>monitoring/README.md</code></a> and the <a href="#api-reference">API reference</a> entry for <code>GET /api/metrics</code>.':
      '命令、Docker 与 npm 路径、仪表盘 UID、入门 PromQL 与故障排查（Grafana 登录重置、目标 DOWN）见 <a href="../monitoring/README.md"><code>monitoring/README.md</code></a> 与 <a href="#api-reference">API 参考</a> 中的 <code>GET /api/metrics</code> 条目。',
    ...
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it down to just its title (columns stay mounted, only hidden, so an in-flight drag or a card's own \"show more\" state survives the toggle) — handy for tucking away a monitor you're not looking at right now.":
      "在 <strong>Agents</strong>（Working / Waiting / Completed / Error）和 <strong>Sessions</strong>（Active / Waiting / Completed / Error / Abandoned）泳道之间切换。黄色的 <strong>Waiting</strong> 列标记出等待用户的条目——刚收到的提示、回合之间，或权限关卡。将鼠标悬停在任意列标题上可查看解释每次状态转换的生命周期提示。卡片会展示模型名称、累计成本以及当前正在调用的工具。对于高级/高成本模型——Opus 或 Fable——模型徽章会以亮红色显示，让人一眼看出成本高昂的会话。计数通过 WebSocket 实时更新，因此看板始终与实时事件存储保持同步。将鼠标悬停在 Waiting 徽章上即可查看原因——需要输入、回合结束、等待提示或已中断。第三个 <strong>Projects</strong> 视图按项目而非状态对会话分组；一旦创建了至少一个命名的 <strong>monitor</strong>（显示器）分组，其被分配的项目列会渲染在该显示器专属的一个带边框、可拖拽排序的方框内部，这些方框在同一行内彼此并排——通过标题栏把一个方框拖到另一个上即可重新定位，便于映射多显示器桌面布局，仅保存在 <code>localStorage</code> 中。每个方框标题栏中的箭头图标可将其折叠为仅显示标题（其内的列仍保持挂载、只是被视觉隐藏，因此进行中的拖拽或某张卡片自身的“显示更多”状态在切换后依然保留）——便于把当前不需要查看的显示器收起来。",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it down to just its title bar — the whole box shrinks instead of sitting there empty — while its columns stay mounted, only hidden, so an in-flight drag or a card's own \"show more\" state survives the toggle. Every project column (and the standalone Unassigned column) has the same chevron, collapsing that one column down to a slim header-only bar on its own, independent of whichever box it's sitting in.":
      "在 <strong>Agents</strong>（Working / Waiting / Completed / Error）和 <strong>Sessions</strong>（Active / Waiting / Completed / Error / Abandoned）泳道之间切换。黄色的 <strong>Waiting</strong> 列标记出等待用户的条目——刚收到的提示、回合之间，或权限关卡。将鼠标悬停在任意列标题上可查看解释每次状态转换的生命周期提示。卡片会展示模型名称、累计成本以及当前正在调用的工具。对于高级/高成本模型——Opus 或 Fable——模型徽章会以亮红色显示，让人一眼看出成本高昂的会话。计数通过 WebSocket 实时更新，因此看板始终与实时事件存储保持同步。将鼠标悬停在 Waiting 徽章上即可查看原因——需要输入、回合结束、等待提示或已中断。第三个 <strong>Projects</strong> 视图按项目而非状态对会话分组；一旦创建了至少一个命名的 <strong>monitor</strong>（显示器）分组，其被分配的项目列会渲染在该显示器专属的一个带边框、可拖拽排序的方框内部，这些方框在同一行内彼此并排——通过标题栏把一个方框拖到另一个上即可重新定位，便于映射多显示器桌面布局，仅保存在 <code>localStorage</code> 中。每个方框标题栏中的箭头图标可将其折叠为仅显示标题栏——整个方框会随之收缩，而不是留着一个空壳——其内的列仍保持挂载、只是被视觉隐藏，因此进行中的拖拽或某张卡片自身的“显示更多”状态在切换后依然保留。每个项目列（以及独立的“未分配”列）都带有同样的箭头图标，可将该列单独折叠为一条纤细的、仅显示标题的横条，与它所在的方框无关。",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      "在 <strong>Agents</strong>（Working / Waiting / Completed / Error）和 <strong>Sessions</strong>（Active / Waiting / Completed / Error / Abandoned）泳道之间切换。黄色的 <strong>Waiting</strong> 列标记出等待用户的条目——刚收到的提示、回合之间，或权限关卡。将鼠标悬停在任意列标题上可查看解释每次状态转换的生命周期提示。卡片会展示模型名称、累计成本以及当前正在调用的工具。对于高级/高成本模型——Opus 或 Fable——模型徽章会以亮红色显示，让人一眼看出成本高昂的会话。计数通过 WebSocket 实时更新，因此看板始终与实时事件存储保持同步。将鼠标悬停在 Waiting 徽章上即可查看原因——需要输入、回合结束、等待提示或已中断。第三个 <strong>Projects</strong> 视图按项目而非状态对会话分组；一旦创建了至少一个命名的 <strong>monitor</strong>（显示器）分组，其被分配的项目列会渲染在该显示器专属的一个带边框、可拖拽排序的方框内部，这些方框在同一行内彼此并排——通过标题栏把一个方框拖到另一个上即可重新定位，便于映射多显示器桌面布局，仅保存在 <code>localStorage</code> 中。每个方框标题栏中的箭头图标可将其折叠为仅显示标题栏——整个方框会移出该行，进入其上方一条由所有其他已折叠显示器共享的纤细条带，从而真正释放它原本仍会占用的横向空间；展开后会把方框按其排序位置放回该行（其内的列全程保持挂载、只是被视觉隐藏，因此某张卡片自身的“显示更多”状态在切换后依然保留）。每个项目列（以及独立的“未分配”列）也都带有自己的箭头图标，可就地将该列单独折叠为一条纤细的、仅显示标题的横条，与它所在的方框无关。",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). A second icon button next to the chevron toggles that box's own layout direction — <strong>horizontal</strong> (the default, columns side by side) or <strong>vertical</strong> (columns stacked top to bottom) — independently per monitor. Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      "在 <strong>Agents</strong>（Working / Waiting / Completed / Error）和 <strong>Sessions</strong>（Active / Waiting / Completed / Error / Abandoned）泳道之间切换。黄色的 <strong>Waiting</strong> 列标记出等待用户的条目——刚收到的提示、回合之间，或权限关卡。将鼠标悬停在任意列标题上可查看解释每次状态转换的生命周期提示。卡片会展示模型名称、累计成本以及当前正在调用的工具。对于高级/高成本模型——Opus 或 Fable——模型徽章会以亮红色显示，让人一眼看出成本高昂的会话。计数通过 WebSocket 实时更新，因此看板始终与实时事件存储保持同步。将鼠标悬停在 Waiting 徽章上即可查看原因——需要输入、回合结束、等待提示或已中断。第三个 <strong>Projects</strong> 视图按项目而非状态对会话分组；一旦创建了至少一个命名的 <strong>monitor</strong>（显示器）分组，其被分配的项目列会渲染在该显示器专属的一个带边框、可拖拽排序的方框内部，这些方框在同一行内彼此并排——通过标题栏把一个方框拖到另一个上即可重新定位，便于映射多显示器桌面布局，仅保存在 <code>localStorage</code> 中。每个方框标题栏中的箭头图标可将其折叠为仅显示标题栏——整个方框会移出该行，进入其上方一条由所有其他已折叠显示器共享的纤细条带，从而真正释放它原本仍会占用的横向空间；展开后会把方框按其排序位置放回该行（其内的列全程保持挂载、只是被视觉隐藏，因此某张卡片自身的“显示更多”状态在切换后依然保留）。箭头图标旁边的第二个图标按钮可切换该方框自身的排列方向——<strong>水平</strong>（默认，列并排排列）或<strong>垂直</strong>（列自上而下堆叠）——按每个显示器独立设置。每个项目列（以及独立的“未分配”列）也都带有自己的箭头图标，可就地将该列单独折叠为一条纤细的、仅显示标题的横条，与它所在的方框无关。",
    ...
    "Per-session deep dive with a collapsible agent hierarchy tree and a full chronological event timeline showing every tool call name and summary. An overview panel at the top surfaces tile counters for events, tool calls, subagents, compactions, errors, and duration. Top-tool usage bars and a subagent type breakdown give quick distribution reads. The conversation viewer renders markdown with syntax highlighting, per-tool styled blocks, slash-command pills with their captured TUI output, and inline session-rename markers. Messages typed mid-turn (queued while Claude was still working) appear where Claude actually received them, and harness notifications are attributed to System. Export the entire session as JSON or share the permalink for async review. When the session is waiting on you, a yellow banner under the header names the reason and how long it has been waiting.":
      "针对单个会话的深入剖析，配有可折叠的 agent 层级树和一条完整的按时间排序的事件时间线，显示每一次工具调用的名称和摘要。顶部的概览面板展示事件、工具调用、subagent、压缩、错误和时长的磁贴计数器。顶部工具使用条形图和 subagent 类型细分让你快速读取分布情况。对话查看器渲染带语法高亮的 markdown、按工具分类的样式化代码块、带其捕获的 TUI 输出的斜杠命令气泡，以及内联的会话重命名标记。在回合进行中输入的消息（Claude 仍在工作时排队）会出现在 Claude 实际接收它们的位置，来自框架的通知则归属为 System。可将整个会话导出为 JSON，或分享永久链接以供异步审阅。当会话在等待你时，标题下方的黄色横幅会注明原因以及已等待的时长。",
    ...
    "Captures the CLI's own <code>/status</code> and <code>/usage</code> TUI panels on demand at <code>/usage</code>, via a detached tmux session that drives a real <code>claude</code> process and scrapes its rendered pane text — no separate accounting API, so it shows exactly what the CLI itself reports. Each capture surfaces account info (email, organization, login method, CLI version, active model), the current session's cost, API and wall-clock duration, and lines added/removed, plus input, output, and cache-read/cache-write token counts. The two headline numbers are the <strong>5-hour session window</strong> and <strong>weekly window</strong> rate-limit percentages — the weekly figure broken down per model — each paired with its own reset time. Every capture is persisted with a <code>status</code> of <code>ok</code>, <code>partial</code>, or <code>error</code>, and always keeps the full raw pane text alongside the parsed fields, so a future CLI output-format change degrades to raw text instead of losing data. Only one capture runs at a time — a second request while one is in flight is rejected rather than racing the tmux session. Same-origin guard included, shared with Run Claude. An <strong>Accounts</strong> panel above it — collapsible via a chevron on its own header, collapsed by default — tracks any number of other Claude accounts side by side, each pointing at a <code>CLAUDE_CONFIG_DIR</code> already logged in via <code>claude login</code> — per-account capture reads that login's own OAuth credential straight from the CLI's storage (Keychain, or a file on non-macOS) and calls Anthropic's API directly, so tracking a second or third account needs no browser session cookie, password, or extra login flow. A missing or expired login shows a clickable <strong>Needs login</strong> badge (macOS) that opens a new Terminal window already running <code>CLAUDE_CONFIG_DIR=&lt;dir&gt; claude</code> — re-authenticating is just a login walk-through and closing the window.":
      "通过一个独立的 tmux 会话按需捕获 CLI 自身的 <code>/status</code> 与 <code>/usage</code> TUI 面板（在 <code>/usage</code> 页面），该会话驱动一个真实的 <code>claude</code> 进程并抓取其渲染出的面板文本——没有单独的计费 API，因此展示的正是 CLI 自身报告的内容。每次捕获都会展示账户信息（邮箱、组织、登录方式、CLI 版本、当前模型）、当前会话的成本、API 与实际耗时、以及增删的代码行数，还有输入、输出以及缓存读/写的 token 计数。两个核心数字是 <strong>5 小时会话窗口</strong> 与 <strong>每周窗口</strong> 的速率限制百分比——每周数字按模型细分——各自都配有自己的重置时间。每次捕获都会以 <code>status</code> 为 <code>ok</code>、<code>partial</code> 或 <code>error</code> 的状态持久化保存，并始终在解析字段之外保留完整的原始面板文本，因此未来 CLI 输出格式发生变化时会退化为原始文本而不会丢失数据。同一时间只能运行一次捕获——当已有一次捕获正在进行时，新的请求会被拒绝，而不会与 tmux 会话产生竞争。同源保护同样适用，与 Run Claude 共享。上方的 <strong>Accounts</strong> 面板——可通过其自身表头上的 chevron 折叠，默认折叠——可并排追踪任意数量的其他 Claude 账户，每个账户只需指向一个已通过 <code>claude login</code> 登录的 <code>CLAUDE_CONFIG_DIR</code>——按账户捕获会直接从 CLI 自身的存储（Keychain，非 macOS 上则是文件）读取该登录的 OAuth 凭据，并直接调用 Anthropic 的 API，因此追踪第二或第三个账户无需浏览器会话 cookie、密码或额外的登录流程。登录缺失或过期时会显示可点击的 <strong>Needs login</strong> 徽章（macOS），点击即打开一个已在运行 <code>CLAUDE_CONFIG_DIR=&lt;dir&gt; claude</code> 的新 Terminal 窗口——重新认证只需走完登录流程并关闭窗口。",
    "A <strong>Session reset times</strong> panel plots every account's next session-window reset across the next 24h, tinting the reset marker's right half toward the weekly window's own color once the weekly window — not the session — is the real constraint, paired with a <strong>Capped by weekly</strong> callout naming it explicitly. A <strong>Weekly reset countdown</strong> panel does the same for the weekly window's calendar-day reset. A <strong>Color thresholds</strong> card lets you configure exactly where each green/yellow/orange/red band starts, independently for the session and weekly scopes plus their <strong>Consumption rate</strong> counterparts — shared live across every connected computer. Every enabled account also captures automatically every 15 minutes (a disable-able background scheduler), so percentages stay fresh without a manual click. An <strong>Activity</strong> card shows whether you're actively using each account (used within the last 15 minutes) or how long ago it was last used — inferred from whether that account's own session/weekly percentage actually rose between captures, not from anything tied to its <code>CLAUDE_CONFIG_DIR</code>, since real work is often done through whichever profile is logged into your default <code>~/.claude</code> instead of this account's own named config directory. A <strong>Consumption rate</strong> card predicts, per account, when its session and weekly quotas will run out at their current pace — a %/hour trend fit over recent captures since the current window started, projected forward to the instant it would cross 100% only when actually rising — and colors the result by whether that predicted exhaustion lands before or after the window's own reset. Underneath each verdict, a small stats line gives the raw rate (%/hour for session, %/day for weekly) plus how many times faster or slower that is than the <strong>sustainable pace</strong> — the rate that would land exactly at 100% right when the window resets — since a bare rate alone can't say whether it's normal or alarming. A <strong>Rotation plan</strong> card chains that same weekly data into an actionable rotation — which account to be on now, and when to hand off to the next one — entirely client-side, out to a ~9-day horizon.":
      "<strong>会话重置时刻</strong> 面板会绘制每个账户接下来 24 小时内下一次会话窗口重置的时间，一旦周窗口（而非会话窗口）才是真正的限制因素，重置标记的右半段就会随之染上周窗口自身的颜色，并配有一个 <strong>受周限额限制</strong> 的提示明确指出这一点。<strong>每周重置倒计时</strong> 面板对周窗口的日历日重置做相同的展示。<strong>颜色阈值</strong> 卡片可让你精确配置每个绿/黄/橙/红色段的起始百分比，会话与每周两个作用域、以及它们各自对应的 <strong>消耗速率</strong> 作用域可分别独立设置——并在每台已连接的电脑之间实时共享。每个已启用的账户还会每 15 分钟自动捕获一次（可关闭的后台调度器），因此百分比无需手动点击即可保持最新。<strong>活动状态</strong> 卡片显示你当前是否正在使用每个账户（最近 15 分钟内使用过）或距离上次使用已经过去多久——其依据是该账户自身的会话/周百分比在两次捕获之间是否真的上升了，而非任何与其 <code>CLAUDE_CONFIG_DIR</code> 相关的信号，因为实际工作往往是通过登录在默认 <code>~/.claude</code> 中的账号完成的，而不是这个账户自己专属的 config 目录。<strong>消耗速率</strong> 卡片会针对每个账户预测：按当前速度，会话与每周额度何时会耗尽——基于自当前窗口开始以来的最近捕获数据拟合一条百分比/小时的趋势线，且仅当该趋势确实在上升时才向前推算到达 100% 的时刻——并根据预计耗尽时刻是在窗口自身重置之前还是之后为结果着色。每条判定下方还有一行小字统计：原始速率（会话用 %/小时，每周用 %/天），以及这一速率相对于 <strong>可持续速率</strong>（即恰好在窗口重置那一刻用满 100% 所需的速率）快或慢多少倍——因为单看一个原始速率并不能说明这是正常水平还是令人担忧。<strong>轮换计划</strong>（Rotation plan）卡片将这份每周数据串联成一份可执行的轮换方案——现在应该使用哪个账户，以及何时移交给下一个账户——完全在客户端计算，投影至约 9 天的时间范围。",
    ...
    "Creates session and main agent. Stamps <code>awaiting_input_since</code> (with <code>awaiting_reason</code> = <code>session_start</code>) so the row lands in <strong>Waiting</strong> from the start (the CLI is at a prompt) — except a <code>compact</code>-source SessionStart (mid-turn auto-compaction), which leaves the flag untouched so a working session stays <strong>Active</strong>. Reactivates resumed sessions. Abandons orphaned sessions with no activity for <code>DASHBOARD_STALE_MINUTES</code> (default 180).":
      "创建会话和主 agent。打上 <code>awaiting_input_since</code>（附带 <code>awaiting_reason</code> = <code>session_start</code>）时间戳，使该行从一开始就落入 <strong>Waiting</strong>（CLI 处于提示符状态）——但 <code>compact</code> 来源的 SessionStart（回合中自动压缩）不改动标志，使正在工作的会话保持 <strong>Active</strong>。重新激活已恢复的会话。将在 <code>DASHBOARD_STALE_MINUTES</code>（默认 180）期间无活动的孤立会话标记为放弃。",
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span > until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >. Background subagents keep running.':
      '非错误情况：主 agent → <code>waiting</code> — UI 显示 <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span >，直到下一次用户输入。会话的等待中标志（<code>awaiting_input_since</code>）仅在没有子 agent 仍在工作时才会被打上——若仍有子 agent 在运行，会话保持 <strong>Active</strong>（它在等待自己的 agent，而非用户），打标被推迟到最后一个 <code>SubagentStop</code>。<code>stop_reason=error</code>：将该 agent 和会话标记为 <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >。后台子 agent 继续运行。',
    "Background agent finished": "后台 agent 已完成",
    "Matched subagent → <span class=\"status-chip chip-completed\" ><span class=\"chip-dot\"></span>Completed</span >. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      '匹配到的子 agent → <span class="status-chip chip-completed" ><span class="chip-dot"></span>Completed</span >。刻意 <strong>不</strong> 清除等待标志——后台子 agent 的完成并不能说明人类用户的任何情况。不过它可以<em>设置</em>该标志：当最后一个仍在工作的子 agent 在一次被推迟的非错误 <code>Stop</code> 之后完成时，排空检查会打上 <code>stop</code> 等待标志，会话恰好在整个 agent 编队全部完成时由 <strong>Active</strong> 翻转为 <strong>Waiting</strong>（等待中）。同时启动一次发后即忘的 JSONL 扫描（<code>scanAndImportSubagents</code>），它会遍历会话的 <code>subagents/agent-*.jsonl</code> 文件，按 <code>tool_use_id</code> 将 <code>tool_use</code> ↔ <code>tool_result</code> 块配对，并在每个子 agent 自身的 <code>agent_id</code> 下为每个工具发出 <code>PreToolUse</code> + <code>PostToolUse</code> 事件——从而呈现子 agent 内部进行的、从不触发任何 hook 的工具调用。同一次扫描还会重建嵌套子 agent 的层级——它把每个子 agent 的 <code>parent_agent_id</code> 重新指向从 transcript 的 Task 工具结果(<code>toolUseResult.agentId</code>)中恢复出的真正 spawn 方,因此自己再 spawn 子 agent 的子 agent 会正确嵌套,而不会平铺到 main 之下。',
    "Agent sends notification": "Agent 发送通知",
    'Event logged to activity feed. Permission/input-prompt patterns (e.g. "needs your permission", "waiting for your input") set the agent to <code>waiting</code> and stamp <code>awaiting_input_since</code> — the reason is re-derived rather than always assumed to be <code>notification</code>: a permission/approval-worded message always stamps <code>notification</code>, otherwise a still-working subagent stamps <code>subagent</code>, a synchronous Bash call in flight stamps <code>shell</code>, a Monitor tool call in flight stamps <code>monitor</code>, else the genuine <code>notification</code>. Compaction-related notifications tagged as <code>Compaction</code> events. Triggers a browser notification if enabled.':
      '事件记录到活动信息流。权限/输入提示模式（例如 "needs your permission"、"waiting for your input"）将 agent 置为 <code>waiting</code> 并打上 <code>awaiting_input_since</code> 时间戳 — 原因会被重新推导，而不是一律假定为 <code>notification</code>：带权限/批准措辞的消息始终打上 <code>notification</code>；否则，若仍有子 agent 在工作则打上 <code>subagent</code>，若有同步 Bash 调用正在进行则打上 <code>shell</code>，若有 Monitor 工具调用正在进行则打上 <code>monitor</code>，都不是的话才是真正的 <code>notification</code>。与压缩相关的通知会被标记为 <code>Compaction</code> 事件。如果已启用，则触发浏览器通知。',
    ...
    "Build <strong>both</strong> per-arch DMGs (arm64 + x64). Correct for release — slower (packages each arch)":
      "构建<strong>两个</strong>各架构 DMG（arm64 + x64）。适用于发布——较慢（会分别打包每个架构）",
    ...
    'Session status lifecycle. <code>waiting</code> is a UI overlay — persisted as <code>active</code> with <code>awaiting_input_since</code> set and <code>awaiting_reason</code> recording why (<code>session_start</code>, <code>stop</code>, <code>notification</code>, <code>interrupted</code>, <code>subagent</code>, <code>shell</code>, or <code>monitor</code> — the last three mean Claude is still actively working via a child, not blocked on you). <code>SessionEnd</code> preserves error state. Error recovery requires <code>UserPromptSubmit</code> or <code>PreToolUse</code>. The watchdog also recovers <code>Esc</code>-cancelled turns (marker or idle-timeout path) and moves the session to Waiting. The dashboard surfaces the reason on every Waiting badge (hover tooltip, plus an inline chip on the Sessions table and session detail header) and in a waiting-for-input banner on the session detail page; the three "still working" reasons replace the badge\'s whole word instead of nesting a chip.':
      "会话状态生命周期。<code>waiting</code> 是一个 UI 叠加层 — 实际持久化为 <code>active</code> 并设置了 <code>awaiting_input_since</code>，同时用 <code>awaiting_reason</code> 记录原因（<code>session_start</code>、<code>stop</code>、<code>notification</code>、<code>interrupted</code>、<code>subagent</code>、<code>shell</code> 或 <code>monitor</code> — 后三者表示 Claude 仍在通过某个子任务积极工作，而不是在等待你）。<code>SessionEnd</code> 会保留错误状态。错误恢复需要 <code>UserPromptSubmit</code> 或 <code>PreToolUse</code>。看门狗还会恢复被 <code>Esc</code> 取消的轮次（标记路径或空闲超时路径）并将会话置为 Waiting。仪表盘会在每个 Waiting 徽章上（悬停提示，会话列表与会话详情页头部还会显示内联小徽章）以及会话详情页的等待输入横幅中展示该原因；这三种“仍在工作”的原因会替换整个徽章文字，而不是嵌套一个小徽章。",
    ...
    "<code>desktop:dmg</code> is slower because it builds the full app tree <em>twice</em> (once per architecture) and ad-hoc-signs each — emitting two separate per-arch DMGs (<code>…-arm64.dmg</code> + <code>…-x64.dmg</code>). There is no <code>@electron/universal</code> merge; the release ships the two per-arch DMGs. For running on a single Mac, use <code>desktop:dmg:arm64</code> (Apple Silicon) or <code>desktop:dmg:x64</code> (Intel) — one architecture, finishing in roughly a minute. CI runs <code>desktop:dmg</code> and uploads both DMGs as <code>ClaudeCodeMonitor-dmg</code>, so you rarely need to build them yourself.":
      "<code>desktop:dmg</code> 较慢，因为它会<em>两次</em>构建完整的应用树（每个架构一次）并分别进行临时（ad-hoc）签名——生成两个独立的各架构 DMG（<code>…-arm64.dmg</code> + <code>…-x64.dmg</code>）。没有 <code>@electron/universal</code> 合并；发布版本会附带这两个各架构 DMG。若只在一台 Mac 上运行，请使用 <code>desktop:dmg:arm64</code>（Apple Silicon）或 <code>desktop:dmg:x64</code>（Intel）——单一架构，大约一分钟即可完成。CI 会运行 <code>desktop:dmg</code> 并将两个 DMG 都作为 <code>ClaudeCodeMonitor-dmg</code> 上传，因此你很少需要自己构建它们。",
    ...
    "Tabby also watches the clock, not just events: if a live session has been running an <strong>expensive model</strong> — Opus or Fable — continuously for more than <strong>15 minutes</strong>, it surfaces a one-time nag bubble asking if you're sure you want to stay there. The nag fires once per streak and only re-arms if the session drops off that model and comes back onto it later.":
      "Tabby 不仅关注事件，还会留意时钟：如果一个活跃会话持续使用<strong>高成本模型</strong>——Opus 或 Fable——超过<strong>15 分钟</strong>，它就会弹出一个一次性的提醒气泡，询问你是否确定要继续留在该模型上。该提醒每次连续使用只触发一次，只有当会话切离该模型、之后又切回来时才会重新启用。",
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting"><span class="chip-dot"></span>Waiting</span> until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error"><span class="chip-dot"></span>Error</span>. Background subagents keep running.':
      '非错误情况：主智能体 → <code>waiting</code>——UI 显示 <span class="status-chip chip-waiting"><span class="chip-dot"></span>等待中</span>，直到下一次用户输入。会话的等待中标志（<code>awaiting_input_since</code>）仅在没有子智能体仍在工作时才会被打上——若仍有子智能体在运行，会话保持 <strong>Active</strong>（它在等待自己的智能体，而非用户），打标被推迟到最后一个 <code>SubagentStop</code>。<code>stop_reason=error</code>：将该智能体和会话标记为 <span class="status-chip chip-error"><span class="chip-dot"></span>错误</span>。后台子智能体继续运行。',
    "Matched subagent → <span class=\"status-chip chip-completed\"><span class=\"chip-dot\"></span>Completed</span>. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      '匹配到的子智能体 → <span class="status-chip chip-completed"><span class="chip-dot"></span>已完成</span>。它<strong>故意</strong>不清除等待标志——一个后台子智能体完成并不能告诉我们任何关于人类的信息。不过它可以<em>设置</em>该标志：当最后一个仍在工作的子智能体在一次被推迟的非错误 <code>Stop</code> 之后完成时，排空检查会打上 <code>stop</code> 等待标志，会话恰好在整个智能体编队全部完成时由 <strong>Active</strong> 翻转为 <strong>Waiting</strong>（等待中）。它还会启动一次即发即弃的 JSONL 扫描（<code>scanAndImportSubagents</code>），遍历会话的 <code>subagents/agent-*.jsonl</code> 文件，按 <code>tool_use_id</code> 将 <code>tool_use</code> ↔ <code>tool_result</code> 块配对，并在每个子智能体自己的 <code>agent_id</code> 下为每个工具发出 <code>PreToolUse</code> + <code>PostToolUse</code> 事件——从而呈现出子智能体在内部进行、却从不触发任何钩子的工具调用。同一次扫描还会重建嵌套子智能体的层级——它把每个子智能体的 <code>parent_agent_id</code> 重新指向从转录的 Task 工具结果（<code>toolUseResult.agentId</code>）中恢复出的真正生成者，因此自己再生成子智能体的子智能体会正确嵌套，而不是平铺在 main 之下。',
    'Creates a compaction subagent → <span class="status-chip chip-completed"><span class="chip-dot"></span>Completed</span>. Detected via <code>isCompactSummary</code> entries in the transcript. Token baselines preserve pre-compaction totals. Periodic scanner (cadence ~¼ of <code>DASHBOARD_STALE_MINUTES</code>) catches compactions when no hooks fire.':
    ...
    "Groups sessions by the folder(s) they run from into a user-named <strong>project</strong>, replacing the flat Agents/Sessions view with one grouped by where the work actually happens. Projects list as a compact table — one row per project with its name, session count, active count, last-activity, and quick pin/plan/report/delete actions, drag-reorderable on the row — instead of a full card each, so a machine with many projects doesn't turn into a long vertical scroll. A project claims one or more working directories — a folder belongs to at most one project — and membership is derived by joining each session's <code>cwd</code> against the project's mapped folders, with no schema change on <code>sessions</code> itself. Each row expands via a chevron (collapsed by default) to reveal its folder mappings, any AGENT-PLAN.md checklist, and its sessions rendered as a horizontally scrollable row of session cards — the same collapsed-by-default treatment applies to each folder inside Unassigned. Create a project (optionally from an existing folder), rename it inline, pin it to float above every unpinned project regardless of the manual drag order (pinned state is server-shared, not per-browser), add or remove folder mappings, or delete it — sessions are never deleted, they simply fall back to the <strong>Unassigned</strong> bucket.":
      "按会话运行所在的文件夹将会话分组为一个用户命名的 <strong>项目</strong>，取代原本扁平的 Agents/Sessions 视图，按实际工作发生的位置进行分组。项目以一个紧凑的表格列出——每个项目一行，包含名称、会话数、活跃数、最近活动，以及置顶/计划/报告/删除的快捷操作，行本身可拖拽排序——而不再是每个项目一整张卡片，避免项目一多整页就变成一长串纵向滚动。一个项目可以拥有一个或多个工作目录——一个文件夹最多只能属于一个项目——成员关系是通过将每个会话的 <code>cwd</code> 与项目已映射的文件夹进行匹配来推导的，<code>sessions</code> 本身无需任何模式变更。每一行都可通过箭头展开（默认折叠），展开后显示其文件夹映射、任何 AGENT-PLAN.md 检查清单，以及以横向可滚动行渲染的会话卡片——Unassigned 内每个文件夹同样采用默认折叠的处理方式。可以创建项目（可选地立即从某个现有文件夹创建）、内联重命名、置顶（使其浮动到所有未置顶项目之上，无视手动拖拽顺序；置顶状态由服务端共享，而非仅存于当前浏览器）、添加或移除文件夹映射，或将其删除——会话永远不会被删除，只会回退到 <strong>Unassigned</strong> 分组中。",
    "A standalone <strong>Focus</strong> page (route <code>/focus</code>, in the sidebar right after <strong>Calendar</strong>) turns the same focus-time data into a stakeholder-readable activity report instead of a calendar grid. It shares the Calendar board's own project chips, session dropdown, and time-period picker, then swaps the swimlane for stat tiles plus a new activity list — one row per plan item, detour, bug, feature, or unclassified time bucket, each with a kind-colored chip and a wall/active time figure, and, for entries the background classifier inferred rather than a live declaration, its one-sentence reason. It needed no backend changes at all, reusing the existing <code>GET /api/focus-report</code> endpoint exactly as the Calendar board does.":
      "一个独立的 <strong>Focus</strong> 页面（路由 <code>/focus</code>，位于侧边栏中紧跟在 <strong>Calendar</strong> 之后）把同样的焦点时间数据转化为一份面向利益相关者可读的活动报告，而不是日历网格。它复用 Calendar 看板自身的项目筛选标签、会话下拉菜单和时间段选择器，然后用统计磁贴加一个新的活动列表取代泳道视图——每个计划条目、绕行、缺陷、功能或未分类时间段各占一行，每行都带有按类型着色的标签和一个实际耗时/活跃耗时数字；对于由后台分类器推断而非实时声明得出的条目，还会附上其一句话理由。它完全不需要任何后端改动，与 Calendar 看板一样直接复用既有的 <code>GET /api/focus-report</code> 接口。",
    "The Focus page also offers the Calendar board's own intraday hour-window zoom — duration presets (<code>4h</code>/<code>8h</code>/<code>12h</code>/<code>24h</code>), a start-time stepper/typed input, a \"Live\" toggle, and quick-start presets — but defaults to the full, unzoomed period rather than the Calendar board's own 4-hour default, since this page previously always showed the whole selected window and the zoom is a purely additive, opt-in narrowing. When zoomed, both the stat tiles and the activity list below scope together to the selected window, so the two never disagree.":
      "Focus 页面还提供 Calendar 看板自身的日内“小时窗口缩放”功能——时长预设（<code>4h</code>/<code>8h</code>/<code>12h</code>/<code>24h</code>）、起始时间步进器/输入框、“实时”（Live）切换，以及快速起始预设——但此处默认展示完整、未缩放的时间段，而非 Calendar 看板自身的 4 小时默认值，因为该页面此前始终显示所选的完整窗口，缩放只是一项可选的额外收窄功能。缩放激活时，上方的统计磁贴与下方的活动列表会一同收窄到所选窗口，两者绝不会不一致。",
    'The activity list\'s unclassified bucket is narrative-aware: a "No focus" segment that carries a classifier-inferred one-sentence reason keeps its own row per session instead of collapsing into one shared bucket — only reason-less unclassified segments still collapse into a single shared tail row. And any row backed by multiple sessions renders its "+N more sessions" note as a toggle that expands into each contributing session\'s own name, clock-time range, wall/active time split, and reason.':
      '活动列表中的未分类分组现在能区分不同叙述：携带分类器推断出的一句话理由的 "No focus" 时间段会按会话各保留一行，而不是全部塌缩进一个共享分组——只有没有理由的未分类时间段才仍会合并为一个共享的末尾行。此外，任何由多个会话汇总而成的行都会把它的 "+N more sessions" 注记渲染为一个开关，展开后逐一列出每个参与会话自己的名称、时钟时间范围、实际/活跃耗时拆分和理由。',
    "Above the activity list, an AI-synthesized <strong>Summary</strong> block distills the selected window into plain-language \"what we actually did\" bullets, served by a new <code>GET /api/focus-report/summary</code> endpoint (<code>server/lib/focus-summary.js</code>). In all-projects scope the summary is grouped by project — one bullet set per project with activity (largest wall-clock share first, plus an Unassigned group for sessions in unmapped folders), each group cached identically to a directly project-scoped request so nothing generates twice. The bullets are no longer fixed at 2–4 — the budget scales with the window: 4 for a day or two, 6 up to a week, 8 beyond. Windows wider than two days summarize <strong>hierarchically</strong>: per-day summaries are generated and cached first — permanent once a day's data stops changing — then one rollup call synthesizes the window from the daily bullets; an unchanged multi-day window is served from cache with zero LLM calls, and a day whose own synthesis fails contributes its raw activity facts instead of being dropped. Generation runs on <code>DASHBOARD_FOCUS_SUMMARY_MODEL</code> when set — a dedicated override so the stakeholder-facing bullets can use a stronger model (e.g. <code>sonnet</code>) while the far-more-frequent per-session classifier stays on the cheap default — falling back to <code>DASHBOARD_FOCUS_INFER_MODEL</code>, then <code>haiku</code>; the spawn still reuses the focus-inference service's hermetic <code>claude -p</code> contract and its <code>DASHBOARD_FOCUS_INFER_TIMEOUT_MS</code> kill timer. Results are cached in the <code>focus_summaries</code> table keyed by scope and window, gated by a digest of the input data, so a finished day generates once and a still-running day regenerates only when its data changes. When no summary can be produced (LLM mode off, <code>claude</code> CLI unavailable, an empty window, or a generation failure) the endpoint returns <code>{ summary: null }</code> with a 200 and the page hides the block entirely.":
      "活动列表上方新增一个由 AI 综合生成的 <strong>Summary</strong> 区块，把所选窗口提炼成通俗语言的“我们实际做了什么”要点，由新的 <code>GET /api/focus-report/summary</code> 接口提供（<code>server/lib/focus-summary.js</code>）。在“所有项目”范围下，摘要会按项目分组——每个有活动的项目一组要点（按实际用时占比从大到小排列，另有一个 Unassigned 分组收纳位于未映射文件夹中的会话），每个分组的缓存方式与直接以该项目为范围的请求完全相同，因此不会有任何内容生成两次。要点数量不再固定为 2–4 条——预算随窗口伸缩：一两天为 4 条，一周以内为 6 条，超过一周为 8 条。跨度超过两天的窗口会<strong>分层</strong>汇总：先逐天生成并缓存每日摘要——一旦某天的数据不再变化就永久缓存——再由一次汇总调用基于各天要点合成整个窗口；未发生变化的多天窗口完全由缓存提供、零次 LLM 调用，而某一天自身合成失败时会改为贡献其原始活动事实，而不是被丢弃。生成时优先使用已设置的 <code>DASHBOARD_FOCUS_SUMMARY_MODEL</code>——这是一个专用覆盖项，让面向干系人的要点可以使用更强的模型（如 <code>sonnet</code>），而调用频率高得多的逐会话分类器仍停留在廉价的默认模型上——未设置时依次回退到 <code>DASHBOARD_FOCUS_INFER_MODEL</code>，再到 <code>haiku</code>；派生过程仍复用焦点推断服务的封闭式 <code>claude -p</code> 约定及其 <code>DASHBOARD_FOCUS_INFER_TIMEOUT_MS</code> 终止计时器。结果缓存在以范围加窗口为键的 <code>focus_summaries</code> 表中，并以输入数据的摘要作为门控，因此已结束的一天只生成一次，仍在进行的一天只有在其数据发生变化时才会重新生成。当无法生成摘要时（LLM 模式关闭、<code>claude</code> CLI 不可用、窗口为空，或生成失败），接口会以 200 返回 <code>{ summary: null }</code>，页面则完全隐藏该区块。",
    "Every Agents/Sessions status column gets its own layout toggle too — the same icon next to its count badge — switching just that column's cards between the default vertical stack and a horizontally-scrolling row. Unlike the monitor toggle above, this one is saved per-browser only, and each status column's setting is independent of the others.":
      "每个 Agent/会话 状态列也都有自己的布局切换开关——就在其计数徽标旁边的同一个图标——仅切换该列自身卡片在默认的垂直堆叠与水平滚动行之间。与上面的显示器切换不同，这个开关只按浏览器保存，并且每个状态列的设置相互独立。",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Session cards also show a compact <strong>input / output / cache-read / cache-write</strong> token strip with a cost-weighted <strong>Effective</strong> total, once the session has used any tokens — cache reads and writes are split into separate figures (reads bill at ~10% of the input rate, writes at a premium), and Effective weighs each category by the model's actual pricing rates so it tracks the $ cost instead of overstating cache-heavy sessions. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). A second icon button next to the chevron toggles that box's own layout direction — <strong>horizontal</strong> (the default, columns side by side) or <strong>vertical</strong> (columns stacked top to bottom) — independently per monitor. Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      "在 <strong>Agents</strong>（Working / Waiting / Completed / Error）和 <strong>Sessions</strong>（Active / Waiting / Completed / Error / Abandoned）泳道之间切换。黄色的 <strong>Waiting</strong> 列标记出等待用户的条目——刚收到的提示、回合之间，或权限关卡。将鼠标悬停在任意列标题上可查看解释每次状态转换的生命周期提示。卡片会展示模型名称、累计成本以及当前正在调用的工具。对于高级/高成本模型——Opus 或 Fable——模型徽章会以亮红色显示，让人一眼看出成本高昂的会话。一旦该会话产生过 token 用量，会话卡片还会额外显示一条紧凑的 <strong>输入 / 输出 / 缓存读 / 缓存写</strong> token 条，并附带按成本加权的 <strong>Effective（有效）</strong>总计——缓存读取与写入分列为两个独立数字（读取按输入费率的约 10% 计费，写入按溢价计费），Effective 按该模型的实际费率为各类别加权，因此它跟随 $ 成本变动，而不会夸大缓存密集型会话。计数通过 WebSocket 实时更新，因此看板始终与实时事件存储保持同步。将鼠标悬停在 Waiting 徽章上即可查看原因——需要输入、回合结束、等待提示或已中断。第三个 <strong>Projects</strong> 视图按项目而非状态对会话分组；一旦创建了至少一个命名的 <strong>monitor</strong>（显示器）分组，其被分配的项目列会渲染在该显示器专属的一个带边框、可拖拽排序的方框内部，这些方框在同一行内彼此并排——通过标题栏把一个方框拖到另一个上即可重新定位，便于映射多显示器桌面布局，仅保存在 <code>localStorage</code> 中。每个方框标题栏中的箭头图标可将其折叠为仅显示标题栏——整个方框会移出该行，进入其上方一条由所有其他已折叠显示器共享的纤细条带，从而真正释放它原本仍会占用的横向空间；展开后会把方框按其排序位置放回该行（其内的列全程保持挂载、只是被视觉隐藏，因此某张卡片自身的“显示更多”状态在切换后依然保留）。箭头图标旁边的第二个图标按钮可切换该方框自身的排列方向——<strong>水平</strong>（默认，列并排排列）或<strong>垂直</strong>（列自上而下堆叠）——按每个显示器独立设置。每个项目列（以及独立的“未分配”列）也都带有自己的箭头图标，可就地将该列单独折叠为一条纤细的、仅显示标题的横条，与它所在的方框无关。",
    '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board (sessions)</strong> — sessions swim-laned across 5 columns: Active, Waiting, Completed, Error, Abandoned. Each card shows agent count, duration, model, cumulative cost, and a compact token strip with the cache read/write split and a cost-weighted Effective total</span>':
      '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board（session）</strong> — session 横跨 5 列分泳道排列：Active、Waiting、Completed、Error、Abandoned。每张卡片显示 agent 数量、持续时间、模型、累计成本，以及一条带缓存读/写拆分和按成本加权 Effective 总计的紧凑 token 条</span>',
    "A standalone page (<code>/project-manager</code>, sidebar label <strong>Project Manager</strong>, positioned right after <strong>Focus</strong>) answering \"what's going on across every project I'm tracking\" at a glance instead of checking each one individually. A portfolio rollup table — one row per project, with an objective/milestone progress bar, a pace badge, an open-decision count, and last activity — sits above the <strong>Decision Queue</strong>: pending reconciliation items (<code>pace_alert</code>, <code>detour_volume</code>, <code>detour_disposition</code>, and write-back failures) rendered as actionable cards, each resolved in place — <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code> for a detour, or resolve/dismiss/retry for everything else. A <strong>Pace Watch</strong> rail lists every behind item across every project, worst-overdue first, and a <strong>Recently Resolved</strong> log records what was just cleared. A project-scope chip row filters the table, queue, and rail down to a single project. It composes the dashboard's existing and new portfolio-management endpoints — <code>GET /api/projects</code>, <code>GET /api/portfolio/summary</code>, and <code>GET /api/decision-queue</code> — rather than duplicating their logic, and is the first client page built on top of the pace-tracking, detour-disposition, and reconciliation work that had previously shipped server-only.":
      "一个独立页面（<code>/project-manager</code>，侧栏标签为 <strong>Project Manager</strong>，位置紧跟在 <strong>Focus</strong> 之后），一眼回答“我正在跟踪的每个项目现在情况如何”，而不必逐个查看。一张项目组合汇总表——每个项目一行，包含目标/里程碑进度条、节奏徽章、待处理决策数量与最近活动——位于 <strong>Decision Queue</strong>（决策队列）之上：待处理的协调项（<code>pace_alert</code>、<code>detour_volume</code>、<code>detour_disposition</code>以及回写失败）以可操作卡片形式渲染，每项均可就地解决——绕行项使用 <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code>，其余项则用 resolve/dismiss/retry。<strong>Pace Watch</strong>（节奏监控）侧栏列出所有项目中每一个落后的条目，按超期程度从重到轻排列，<strong>Recently Resolved</strong>（近期已解决）日志记录刚被清除的项。项目范围筛选条可将表格、队列与侧栏都缩小到单个项目。它组合了仪表盘现有及新增的项目组合管理端点——<code>GET /api/projects</code>、<code>GET /api/portfolio/summary</code> 与 <code>GET /api/decision-queue</code>——而非重复其逻辑，是首个建立在此前仅限服务端上线的节奏跟踪、绕行处置与协调工作之上的客户端页面。",
  },
  vi: {
    "A compact stat card in the header, to the left of the view toggle, shows today's cross-project concurrency ratio at a glance — the same figure the Focus report's Concurrency tile computes — and refreshes itself as sessions and agents change.":
      "Một thẻ thống kê gọn nhẹ ở đầu trang, bên trái nút chuyển chế độ xem, hiển thị ngay tỷ lệ đồng thời trên toàn bộ dự án của hôm nay — cùng con số mà ô Concurrency trong báo cáo Focus tính ra — và tự làm mới khi phiên và agent thay đổi.",
    'Answers "what is this session doing, and which part of the project plan is it for?" at a glance. Each monitored repo may keep a human-approved <code>AGENT-PLAN.md</code> (numbered checkbox items) at its root; a background poll (<code>DASHBOARD_PLAN_POLL_MS</code>, plus opportunistic ingest on <code>SessionStart</code>) mirrors it read-only into the dashboard, keyed by <code>cwd</code> and rolled up to Projects. Sessions declare their position by running <code>ccam focus set/push/bug/feature/pop/done</code> in their Bash tool — the hook stream is the write channel, so it works in every hooked project with zero per-project setup. <code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code> push a kind-tagged detour; the Plan view\'s per-item focus line reflects each session\'s actual current state (known item, plain detour, feature, or bug) with a matching icon, bucketed under the item it was declared on (or an "Unknown item" row when none was set), with click-to-expand detail for bug/feature declarations.':
      'Trả lời ngay câu hỏi "phiên này đang làm gì, và nó phục vụ phần nào của kế hoạch dự án?". Mỗi kho được giám sát có thể giữ một tệp <code>AGENT-PLAN.md</code> đã được con người phê duyệt (các mục checkbox đánh số) ở thư mục gốc; một vòng thăm dò nền (<code>DASHBOARD_PLAN_POLL_MS</code>, cộng với việc thu nạp tùy cơ khi <code>SessionStart</code>) phản chiếu nó ở chế độ chỉ đọc vào bảng điều khiển, gắn theo <code>cwd</code> và tổng hợp lên Projects. Các phiên khai báo vị trí của mình bằng cách chạy <code>ccam focus set/push/bug/feature/pop/done</code> trong công cụ Bash — luồng hook chính là kênh ghi, nên nó hoạt động trong mọi dự án đã cài hook mà không cần thiết lập riêng cho từng dự án. <code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code> đẩy một lượt rẽ ngang được gắn nhãn loại; dòng tiêu điểm theo từng mục của khung nhìn Plan phản ánh trạng thái hiện tại thực sự của mỗi phiên (mục đã biết, rẽ ngang thường, feature, hoặc bug) với biểu tượng tương ứng, được xếp vào nhóm theo mục mà nó được khai báo dưới (hoặc dòng "Unknown item" khi chưa đặt mục nào), có thể bấm để mở rộng chi tiết đối với các khai báo bug/feature.',
    "The Projects page shows each plan as a collapsible checklist with a progress bar and per-item session chips; every session card gets a one-line focus breadcrumb with a shared icon for its current state (crosshair on a known item, a detour/feature/bug icon otherwise), rendered even for a session mid-detour with no base item declared; Session Detail gains a focus banner plus a <strong>Plan</strong> tab (checklist, focus timeline from <code>Focus</code> events, and the session's latest TodoWrite micro-plan). A periodic drift audit (<code>DASHBOARD_FOCUS_AUDIT_*</code>) — headless <code>claude -p</code> on a small model, with a keyword-overlap fallback — stamps an amber \"possible undeclared detour?\" badge when a focused session's recent activity stops matching its declaration; it never rewrites focus, and declarations never clear the badge.":
      'Trang Projects hiển thị mỗi kế hoạch dưới dạng danh sách kiểm có thể thu gọn với thanh tiến độ và chip phiên theo từng mục; mỗi thẻ phiên có một dòng breadcrumb tiêu điểm với một biểu tượng dùng chung thể hiện trạng thái hiện tại của nó (biểu tượng tâm ngắm cho mục đã biết, biểu tượng rẽ ngang/feature/bug trong các trường hợp khác), được hiển thị ngay cả với một phiên đang ở giữa một đoạn rẽ ngang mà chưa khai báo mục gốc nào; Chi tiết phiên có thêm banner tiêu điểm cùng tab <strong>Plan</strong> (danh sách kiểm, dòng thời gian tiêu điểm từ các sự kiện <code>Focus</code>, và vi-kế hoạch TodoWrite mới nhất của phiên). Một kiểm toán trôi dạt định kỳ (<code>DASHBOARD_FOCUS_AUDIT_*</code>) — chạy <code>claude -p</code> không giao diện trên một mô hình nhỏ, với phương án dự phòng so trùng từ khóa — đóng huy hiệu hổ phách "possible undeclared detour?" khi hoạt động gần đây của một phiên đang tập trung không còn khớp với khai báo của nó; nó không bao giờ ghi đè tiêu điểm, và các khai báo cũng không bao giờ xóa huy hiệu này.',
    "Poll interval for the <code>AGENT-PLAN.md</code> watcher — a changed plan file is re-ingested and broadcast as <code>plan_updated</code>; <code>0</code> disables the poll (<code>SessionStart</code> still ingests opportunistically, and <code>POST /api/plans/refresh</code> forces one)":
      "Khoảng thời gian thăm dò cho trình theo dõi <code>AGENT-PLAN.md</code> — tệp kế hoạch thay đổi sẽ được thu nạp lại và phát đi dưới dạng <code>plan_updated</code>; <code>0</code> tắt thăm dò (<code>SessionStart</code> vẫn thu nạp tùy cơ, và <code>POST /api/plans/refresh</code> buộc chạy một lần)",
    'A report icon next to each project\'s "view plan" icon (Projects page and Kanban\'s Projects view) opens a <strong>focus-time report</strong> scoped to that project: how long its sessions actually spent on a declared item versus a detour, a feature aside, or a bug fix — reconstructed from existing <code>Focus</code> event history, not a new capture mechanism. A configurable idle-grace window (<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>, default 5 min) discounts stretches with no hook activity past the window, so a session sitting in Waiting for hours doesn\'t inflate its "time on item" figure. The popup shows active/idle stat tiles, a per-session segmented timeline, a per-item rollup (detours bucketed under the item they interrupted), and a project-wide split by kind.':
      'Một biểu tượng báo cáo cạnh biểu tượng "xem kế hoạch" của mỗi dự án (trang Projects và khung nhìn Projects của Kanban) mở một <strong>báo cáo thời gian tiêu điểm</strong> giới hạn trong dự án đó: các phiên của dự án đó đã thực sự dành bao nhiêu thời gian cho một mục đã khai báo so với một lượt rẽ ngang, một nhánh feature, hay một lần sửa bug — được tái dựng từ lịch sử sự kiện <code>Focus</code> sẵn có, không phải một cơ chế ghi nhận mới. Một cửa sổ ân hạn nhàn rỗi có thể cấu hình (<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>, mặc định 5 phút) khấu trừ các khoảng không có hoạt động hook vượt quá cửa sổ đó, nên một phiên nằm ở trạng thái Waiting suốt nhiều giờ sẽ không làm phồng lên con số "thời gian trên mục" của nó. Popup hiển thị các ô thống kê hoạt động/nhàn rỗi, một dòng thời gian phân đoạn theo từng phiên, một bảng tổng hợp theo từng mục (các lượt rẽ ngang được gộp vào mục mà nó làm gián đoạn), và một biểu đồ chia theo loại trên toàn dự án.',
    "Tick interval for the focus drift audit — the background check that a focused session's recent activity matches its declared plan item; <code>0</code> disables auditing entirely":
      "Khoảng thời gian giữa các lần chạy của kiểm toán trôi dạt tiêu điểm — kiểm tra nền xác nhận hoạt động gần đây của phiên đang tập trung khớp với mục kế hoạch đã khai báo; <code>0</code> tắt hoàn toàn việc kiểm toán",
    "Idle-grace window for the per-project focus-time report — a gap between two consecutive hook events longer than this only counts the grace window's worth as active, the rest is excluded as idle; <code>0</code> disables discounting (100% of wall-clock counts as active)":
      "Cửa sổ ân hạn nhàn rỗi cho báo cáo thời gian tiêu điểm theo từng dự án — một khoảng cách giữa hai sự kiện hook liên tiếp dài hơn giá trị này chỉ được tính là hoạt động trong phạm vi cửa sổ ân hạn, phần còn lại bị loại trừ vì được coi là nhàn rỗi; <code>0</code> tắt hoàn toàn việc khấu trừ (100% thời gian thực tính là hoạt động)",
    "Drift-audit judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap only), or <code>off</code>":
      "Bộ phán xét của kiểm toán trôi dạt: <code>llm</code> (chạy <code>claude -p</code> không giao diện trên mô hình nhỏ, tự chuyển sang heuristic khi CLI không khả dụng), <code>heuristic</code> (chỉ so trùng từ khóa), hoặc <code>off</code>",
    "Model passed to the drift audit's <code>claude -p --model</code> spawn":
      "Mô hình được truyền cho tiến trình <code>claude -p --model</code> của kiểm toán trôi dạt",
    "Kill timer for a single drift-audit <code>claude -p</code> spawn (SIGTERM, then SIGKILL)":
      "Bộ đếm giờ kết thúc cho một lần chạy <code>claude -p</code> của kiểm toán trôi dạt (SIGTERM, rồi SIGKILL)",
    "Sessions that never declared a <code>ccam focus</code> don't punch silent holes in that report: a background <strong>focus inference</strong> job (<code>DASHBOARD_FOCUS_INFER_*</code>) digests each untracked session's activity and attributes it to the best-matching plan item or an inferred detour, storing one verdict per session in the <code>focus_inferences</code> table. Segments it produces carry <code>inferred: true</code> and render with an \"≈ inferred\" chip, so guessed time is never dressed up as declared time. Declared focus history always wins — inference only fills sessions with no declarations at all — and a session with neither a declaration nor a confident verdict yet (most often one that's still running and hasn't gone quiet long enough for a tick to reach it) still shows up as a neutral gray \"No focus\" block instead of a silent unclassified hole. A project with no <code>AGENT-PLAN.md</code> at all used to be invisible to this classifier entirely; it's now still classified, asking for a plain one-sentence summary of what the session's activity accomplished instead of trying to match a plan item, and storing it the same way. A low-confidence <code>unclassified</code> verdict's reason — which the classifier already generated — is no longer silently dropped by the report layer; it now surfaces there too.":
      'Các phiên chưa từng khai báo <code>ccam focus</code> sẽ không để lại lỗ hổng âm thầm trong báo cáo đó: một tác vụ nền <strong>suy luận focus</strong> (<code>DASHBOARD_FOCUS_INFER_*</code>) tổng hợp hoạt động của từng phiên chưa được theo dõi và quy nó về mục kế hoạch khớp nhất hoặc một lần đi vòng được suy luận, lưu một phán quyết cho mỗi phiên trong bảng <code>focus_inferences</code>. Các phân đoạn nó tạo ra mang <code>inferred: true</code> và hiển thị kèm nhãn "≈ inferred", nên thời gian ước đoán không bao giờ bị khoác áo thời gian đã khai báo. Lịch sử focus đã khai báo luôn thắng — suy luận chỉ lấp các phiên hoàn toàn không có khai báo nào — và một phiên chưa có khai báo lẫn phán quyết đáng tin cậy (thường gặp nhất là phiên đang chạy và chưa đủ yên tĩnh để một lượt tick chạm tới) vẫn sẽ hiển thị dưới dạng một khối màu xám trung tính "No focus" thay vì một lỗ hổng chưa phân loại âm thầm. Một dự án hoàn toàn không có <code>AGENT-PLAN.md</code> trước đây hoàn toàn vô hình với bộ phân loại này; giờ đây nó vẫn được phân loại, chỉ là sẽ yêu cầu một câu tóm tắt đơn giản về những gì hoạt động của phiên đã hoàn thành thay vì cố khớp với một mục kế hoạch, và lưu lại theo cùng cách. Lý do của một phán quyết <code>unclassified</code> có độ tin cậy thấp — thứ mà bộ phân loại đã tạo ra từ trước — không còn bị lớp báo cáo âm thầm loại bỏ nữa; giờ đây nó cũng hiển thị trên báo cáo.',
    "A header toggle in the focus-time report switches between the List view described above and a <strong>Calendar</strong> view — a day-view swimlane calendar that positions every session's segments on a real 24-hour axis, with sessions that overlap in time split into side-by-side lanes via a greedy lane-assignment algorithm, the same way a Teams or Outlook calendar day view spreads concurrent meetings instead of stacking them. Dashed-border blocks mark inferred/undeclared attribution and solid borders mark declared focus, matching the List view's own convention; a pulsing, open-ended block marks a session that's still actively running, using the API's <code>ended_at: null</code> field to tell a genuinely live session from one that only just ended. Prev/Today/Next controls step through days — this v1 ships Day view and that simple navigation only; Week/Month zoom and an aggregate time-range selector (3d/7d/30d/All) are future work. The report's stat tiles also gained a <strong>Concurrency</strong> tile (e.g. \"1.08x\", effort time ÷ wall-clock time), and the \"Total agent time\" tile's wall-clock sub-label now reflects the real union-based wall-clock span instead of a misleading per-segment sum, so effort time (which inflates with overlapping sessions) and true wall-clock time read as two distinct, honest numbers. The Concurrency tile now also carries a \"while active\" sub-figure (<code>active_concurrency_ratio</code>) that divides effort by only the time at least one session was actually active — so sessions left open but idle (overnight, say) no longer dilute the parallelism reading — and a small swap button that inverts which of the two ratios is the tile's primary figure, a per-browser choice that persists across reloads.":
      'Một nút chuyển đổi ở đầu báo cáo thời gian tiêu điểm cho phép chuyển giữa chế độ xem List đã mô tả ở trên và chế độ xem <strong>Calendar</strong> mới — một lịch dạng xem theo ngày với các làn bơi (swimlane), định vị các đoạn thời gian của mỗi phiên trên một trục 24 giờ thực; các phiên chồng lấn thời gian được tách thành các làn cạnh nhau bằng thuật toán gán làn tham lam (greedy), giống cách lịch ngày của Teams hay Outlook trải các cuộc họp diễn ra đồng thời cạnh nhau thay vì xếp chồng lên nhau. Khối viền nét đứt đánh dấu sự quy kết được suy luận/chưa khai báo, viền nét liền đánh dấu tiêu điểm đã khai báo, khớp với quy ước của chính chế độ xem List; một khối hở đầu có hiệu ứng nhấp nháy đánh dấu một phiên vẫn đang thực sự chạy, dựa trên trường <code>ended_at: null</code> của API để phân biệt một phiên thực sự còn sống với một phiên vừa mới kết thúc. Các điều khiển Prev/Today/Next di chuyển từng ngày một — phiên bản v1 này chỉ cung cấp chế độ xem Day và kiểu điều hướng đơn giản đó; thu phóng Week/Month và bộ chọn khoảng thời gian tổng hợp (3d/7d/30d/All) là công việc trong tương lai. Các ô thống kê của báo cáo cũng có thêm một ô <strong>Concurrency</strong> (mức đồng thời) (ví dụ "1.08x", tức thời gian nỗ lực ÷ thời gian thực), và nhãn phụ thời gian thực của ô "Tổng thời gian tác nhân" giờ phản ánh đúng khoảng thời gian thực dựa trên phép hợp (union) thay vì một tổng theo từng đoạn dễ gây hiểu nhầm; nhờ vậy thời gian nỗ lực (tăng ảo khi các phiên chồng lấn) và thời gian thực thực sự hiện ra như hai con số riêng biệt, trung thực. Ô Concurrency giờ còn kèm một số phụ "khi đang hoạt động" (<code>active_concurrency_ratio</code>), chỉ chia thời gian nỗ lực cho khoảng thời gian có ít nhất một phiên thực sự hoạt động — nhờ đó các phiên để mở nhưng không làm gì (chẳng hạn qua đêm) không còn làm loãng chỉ số song song — và một nút hoán đổi nhỏ trên ô cho phép đảo tỷ lệ nào trong hai tỷ lệ là con số chính, một lựa chọn lưu theo trình duyệt và được giữ nguyên sau khi tải lại trang.',
    "Tick interval for background focus inference — the classifier that attributes sessions which never declared a <code>ccam focus</code> to a plan item or an inferred detour (first backfill tick ~30 s after boot); <code>0</code> disables inference entirely":
      "Chu kỳ tick của suy luận focus chạy nền — bộ phân loại quy các phiên chưa từng khai báo <code>ccam focus</code> về một mục kế hoạch hoặc một lần đi vòng được suy luận (lần backfill đầu tiên chạy ~30 giây sau khi khởi động); <code>0</code> tắt hoàn toàn suy luận",
    "Focus-inference judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap that only ever claims an item match), or <code>off</code>":
      "Bộ phán định của suy luận focus: <code>llm</code> (chạy <code>claude -p</code> headless trên một model nhỏ, lui về heuristic khi CLI không khả dụng), <code>heuristic</code> (trùng lặp từ khóa, chỉ bao giờ nhận khớp mục kế hoạch), hoặc <code>off</code>",
    "Model passed to the focus-inference <code>claude -p --model</code> spawn":
      "Model được truyền cho tiến trình <code>claude -p --model</code> của suy luận focus",
    "falls back to <code>DASHBOARD_FOCUS_INFER_MODEL</code>, then <code>haiku</code>":
    ...
    "<code>ccam focus …</code> run in the session's Bash tool":
      "<code>ccam focus …</code> được chạy trong công cụ Bash của phiên",
    "Parsed out of the <code>PostToolUse</code> hook's <code>tool_input.command</code> (the only channel that natively carries the session id). <code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code> update the session's <code>session_focus</code> row (plan-item pointer plus detour stack) and write a <code>Focus</code> event with an item-text snapshot for the timeline; broadcasts <code>session_focus</code> (and <code>plan_updated</code> after <code>done</code>). <code>bug</code>/<code>feature</code> additionally carry <code>kind</code>/<code>title</code>/<code>detail</code> on the pushed detour frame, badged in the Plan view. <code>status</code> is read-only and records nothing.":
      "Được phân tích từ <code>tool_input.command</code> của hook <code>PostToolUse</code> (kênh duy nhất mang sẵn id phiên). <code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code> cập nhật bản ghi <code>session_focus</code> của phiên (con trỏ mục kế hoạch cùng ngăn xếp rẽ ngang) và ghi một sự kiện <code>Focus</code> kèm ảnh chụp văn bản mục cho dòng thời gian; phát <code>session_focus</code> (và <code>plan_updated</code> sau <code>done</code>). <code>bug</code>/<code>feature</code> còn mang thêm <code>kind</code>/<code>title</code>/<code>detail</code> trên khung rẽ ngang được đẩy vào, hiển thị dưới dạng huy hiệu trong khung nhìn Plan. <code>status</code> chỉ đọc và không ghi lại gì.",
    "CCAM exposes a read-only <strong>Prometheus text-exposition</strong> endpoint at <code>GET /api/metrics</code> — sessions and agents by status, cumulative events and tokens, connected WebSocket clients, configured remote sources, process uptime/RSS, and build version — so the same dashboard you use for Claude Code monitoring can be scraped into your own observability stack. A turnkey stack lives in <code>monitoring/</code>: npm-managed binaries (no Homebrew/apt) or optional Docker Compose, with Grafana datasource and four dashboards auto-provisioned on first start.":
      "CCAM cung cấp endpoint <strong>Prometheus text-exposition</strong> chỉ-đọc tại <code>GET /api/metrics</code> — sessions/agents theo trạng thái, sự kiện và token tích lũy, client WebSocket đang kết nối, nguồn từ xa đã cấu hình, uptime/RSS tiến trình và phiên bản build — để dashboard bạn dùng giám sát Claude Code có thể được scrape vào observability stack riêng. Stack sẵn dùng trong <code>monitoring/</code>: binary quản lý bằng npm (không cần Homebrew/apt) hoặc Docker Compose tùy chọn, datasource Grafana và bốn dashboard tự cấp hình khi khởi động lần đầu.",
    "<strong>Quick start (npm):</strong> run the dashboard on loopback, then <code>npm run monitoring:install</code> once and <code>npm run monitoring:up</code>. Open Grafana at <code>http://localhost:3000</code> and Prometheus at <code>http://localhost:9090</code>. The pre-built CCAM console is at <code>http://localhost:9090/consoles/index.html</code> (also reachable from the Prometheus UI under <strong>Consoles</strong>). For an all-Docker path use <code>npm run monitoring:docker:up</code> or <code>npm run docker:full:up</code> for dashboard + Prometheus + Grafana together. Verify with <code>npm run monitoring:verify</code> — the <code>ccam</code> scrape target should read <strong>UP</strong>.":
      "<strong>Khởi động nhanh (npm):</strong> chạy dashboard trên loopback, rồi <code>npm run monitoring:install</code> một lần và <code>npm run monitoring:up</code>. Grafana: <code>http://localhost:3000</code>, Prometheus: <code>http://localhost:9090</code>. Console CCAM dựng sẵn: <code>http://localhost:9090/consoles/index.html</code> (cũng có trong Prometheus UI mục <strong>Consoles</strong>). Đường Docker đầy đủ: <code>npm run monitoring:docker:up</code> hoặc <code>npm run docker:full:up</code>. Xác minh bằng <code>npm run monitoring:verify</code> — target scrape <code>ccam</code> phải <strong>UP</strong>.",
    "Four Grafana boards": "Bốn bảng Grafana",
    "Prometheus console": "Console Prometheus",
    "Prometheus 3.x dropped the legacy console template libraries, so CCAM ships a <strong>static HTML console</strong> at <code>monitoring/prometheus/consoles/index.html</code> that fetches <code>/api/v1/query</code> directly — live cards, session tables, and Graph links without Go-template errors on <code>/consoles/index.html</code>.":
      "Prometheus 3.x bỏ thư viện template console cũ, nên CCAM cung cấp <strong>console HTML tĩnh</strong> tại <code>monitoring/prometheus/consoles/index.html</code> gọi <code>/api/v1/query</code> trực tiếp — thẻ trực tiếp, bảng phiên và liên kết Graph, không lỗi Go-template trên <code>/consoles/index.html</code>.",
    "Scrape &amp; auth guard": "Bảo vệ scrape &amp; xác thực",
    "Loopback scrapes (<code>127.0.0.1:4820</code>) work out of the box. When Prometheus runs in Docker and reaches the host via <code>host.docker.internal</code>, add that host to <code>DASHBOARD_ALLOWED_HOSTS</code> or the scrape returns <code>403 EBADHOST</code>. If <code>DASHBOARD_TOKEN</code> is set, scrapers must send the same bearer token.":
      "Scrape loopback (<code>127.0.0.1:4820</code>) hoạt động ngay. Khi Prometheus chạy trong Docker và truy cập host qua <code>host.docker.internal</code>, thêm host đó vào <code>DASHBOARD_ALLOWED_HOSTS</code> nếu không scrape trả <code>403 EBADHOST</code>. Nếu đặt <code>DASHBOARD_TOKEN</code>, scraper phải gửi cùng bearer token.",
    "Full reference": "Tham chiếu đầy đủ",
    'Commands, Docker vs npm paths, dashboard UIDs, starter PromQL, and troubleshooting (Grafana login reset, target DOWN) are documented in <a href="../monitoring/README.md"><code>monitoring/README.md</code></a> and the <a href="#api-reference">API reference</a> entry for <code>GET /api/metrics</code>.':
      'Lệnh, đường Docker vs npm, UID dashboard, PromQL khởi đầu và xử lý sự cố (đặt lại đăng nhập Grafana, target DOWN) có trong <a href="../monitoring/README.md"><code>monitoring/README.md</code></a> và mục <a href="#api-reference">API reference</a> cho <code>GET /api/metrics</code>.',
    "Pull Claude Code history from other machines over SSH. Each remote's <code>~/.claude/projects</code> tree is rsynced into a sandboxed staging directory, fed through the same importer as local history, and every session is tagged with its source. A background poller keeps remote data near-real-time, a global data-scope selector (Local only / All / a specific source) narrows the whole app, and sessions from a remote show a source badge. Auth defers entirely to the host's SSH stack — no passwords or secrets are stored. Managed in <b>Settings → Remote Data Sources</b>.":
    ...
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it down to just its title (columns stay mounted, only hidden, so an in-flight drag or a card's own \"show more\" state survives the toggle) — handy for tucking away a monitor you're not looking at right now.":
      'Chuyển đổi giữa các làn <strong>Agents</strong> (Working / Waiting / Completed / Error) và <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned). Một cột <strong>Waiting</strong> màu vàng đánh dấu các mục đang chờ người dùng — lời nhắc mới, giữa các lượt, hoặc cổng cấp quyền. Di chuột qua bất kỳ tiêu đề cột nào để xem các chú giải về vòng đời giải thích từng lần chuyển trạng thái. Các thẻ hiển thị tên mô hình, chi phí tích lũy, và công cụ đang được gọi hiện tại. Đối với các mô hình cao cấp/tốn kém — Opus hoặc Fable — huy hiệu mô hình sẽ hiển thị màu đỏ tươi, giúp nhận ra ngay các phiên tốn nhiều chi phí. Số đếm cập nhật theo thời gian thực qua WebSocket nên bảng luôn đồng bộ với kho sự kiện trực tiếp. Di chuột lên huy hiệu Waiting để xem lý do — cần phản hồi, xong lượt, chờ prompt hoặc bị ngắt. Chế độ xem <strong>Projects</strong> thứ ba nhóm phiên theo dự án thay vì theo trạng thái; khi đã tạo ít nhất một nhóm <strong>monitor</strong> (màn hình) có tên, các cột dự án được gán cho nó sẽ hiển thị bên trong một khung có viền, có thể kéo sắp xếp dành riêng cho màn hình đó, các khung này nằm cạnh nhau trong cùng hàng đó — kéo một khung (qua phần tiêu đề) vào khung khác để đổi vị trí, tiện để mô phỏng cách bố trí nhiều màn hình vật lý, chỉ lưu trong <code>localStorage</code>. Một biểu tượng chevron trên tiêu đề mỗi khung thu gọn nó xuống chỉ còn tiêu đề (các cột vẫn được mount, chỉ bị ẩn đi, nên một thao tác kéo đang diễn ra hoặc trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó) — tiện để cất gọn một màn hình mà bạn hiện không cần xem tới.',
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it down to just its title bar — the whole box shrinks instead of sitting there empty — while its columns stay mounted, only hidden, so an in-flight drag or a card's own \"show more\" state survives the toggle. Every project column (and the standalone Unassigned column) has the same chevron, collapsing that one column down to a slim header-only bar on its own, independent of whichever box it's sitting in.":
      'Chuyển đổi giữa các làn <strong>Agents</strong> (Working / Waiting / Completed / Error) và <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned). Một cột <strong>Waiting</strong> màu vàng đánh dấu các mục đang chờ người dùng — lời nhắc mới, giữa các lượt, hoặc cổng cấp quyền. Di chuột qua bất kỳ tiêu đề cột nào để xem các chú giải về vòng đời giải thích từng lần chuyển trạng thái. Các thẻ hiển thị tên mô hình, chi phí tích lũy, và công cụ đang được gọi hiện tại. Đối với các mô hình cao cấp/tốn kém — Opus hoặc Fable — huy hiệu mô hình sẽ hiển thị màu đỏ tươi, giúp nhận ra ngay các phiên tốn nhiều chi phí. Số đếm cập nhật theo thời gian thực qua WebSocket nên bảng luôn đồng bộ với kho sự kiện trực tiếp. Di chuột lên huy hiệu Waiting để xem lý do — cần phản hồi, xong lượt, chờ prompt hoặc bị ngắt. Chế độ xem <strong>Projects</strong> thứ ba nhóm phiên theo dự án thay vì theo trạng thái; khi đã tạo ít nhất một nhóm <strong>monitor</strong> (màn hình) có tên, các cột dự án được gán cho nó sẽ hiển thị bên trong một khung có viền, có thể kéo sắp xếp dành riêng cho màn hình đó, các khung này nằm cạnh nhau trong cùng hàng đó — kéo một khung (qua phần tiêu đề) vào khung khác để đổi vị trí, tiện để mô phỏng cách bố trí nhiều màn hình vật lý, chỉ lưu trong <code>localStorage</code>. Một biểu tượng chevron trên tiêu đề mỗi khung thu gọn nó xuống chỉ còn tiêu đề — cả khung sẽ co lại thay vì cứ để trống — trong khi các cột của nó vẫn được mount, chỉ bị ẩn đi, nên một thao tác kéo đang diễn ra hoặc trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó. Mỗi cột dự án (và cột Chưa phân loại độc lập) đều có cùng biểu tượng chevron đó, cho phép thu gọn riêng cột đó xuống thành một thanh tiêu đề mảnh, độc lập với khung mà nó đang nằm trong.',
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      'Chuyển đổi giữa các làn <strong>Agents</strong> (Working / Waiting / Completed / Error) và <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned). Một cột <strong>Waiting</strong> màu vàng đánh dấu các mục đang chờ người dùng — lời nhắc mới, giữa các lượt, hoặc cổng cấp quyền. Di chuột qua bất kỳ tiêu đề cột nào để xem các chú giải về vòng đời giải thích từng lần chuyển trạng thái. Các thẻ hiển thị tên mô hình, chi phí tích lũy, và công cụ đang được gọi hiện tại. Đối với các mô hình cao cấp/tốn kém — Opus hoặc Fable — huy hiệu mô hình sẽ hiển thị màu đỏ tươi, giúp nhận ra ngay các phiên tốn nhiều chi phí. Số đếm cập nhật theo thời gian thực qua WebSocket nên bảng luôn đồng bộ với kho sự kiện trực tiếp. Di chuột lên huy hiệu Waiting để xem lý do — cần phản hồi, xong lượt, chờ prompt hoặc bị ngắt. Chế độ xem <strong>Projects</strong> thứ ba nhóm phiên theo dự án thay vì theo trạng thái; khi đã tạo ít nhất một nhóm <strong>monitor</strong> (màn hình) có tên, các cột dự án được gán cho nó sẽ hiển thị bên trong một khung có viền, có thể kéo sắp xếp dành riêng cho màn hình đó, các khung này nằm cạnh nhau trong cùng hàng đó — kéo một khung (qua phần tiêu đề) vào khung khác để đổi vị trí, tiện để mô phỏng cách bố trí nhiều màn hình vật lý, chỉ lưu trong <code>localStorage</code>. Một biểu tượng chevron trên tiêu đề mỗi khung thu gọn nó xuống chỉ còn tiêu đề — cả khung sẽ rời hẳn khỏi hàng đó, chuyển sang một dải mỏng riêng ở phía trên, dùng chung cho mọi màn hình đã thu gọn khác, nhờ đó giải phóng thật sự khoảng không gian ngang mà nó vẫn sẽ chiếm; mở rộng lại sẽ đưa khung quay về đúng vị trí theo thứ tự trong hàng đó (trong suốt quá trình các cột của nó vẫn được mount, chỉ bị ẩn đi, nên trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó). Mỗi cột dự án (và cột Chưa phân loại độc lập) cũng đều có biểu tượng chevron riêng, cho phép thu gọn riêng cột đó tại chỗ xuống thành một thanh tiêu đề mảnh, độc lập với khung mà nó đang nằm trong.',
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). A second icon button next to the chevron toggles that box's own layout direction — <strong>horizontal</strong> (the default, columns side by side) or <strong>vertical</strong> (columns stacked top to bottom) — independently per monitor. Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      'Chuyển đổi giữa các làn <strong>Agents</strong> (Working / Waiting / Completed / Error) và <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned). Một cột <strong>Waiting</strong> màu vàng đánh dấu các mục đang chờ người dùng — lời nhắc mới, giữa các lượt, hoặc cổng cấp quyền. Di chuột qua bất kỳ tiêu đề cột nào để xem các chú giải về vòng đời giải thích từng lần chuyển trạng thái. Các thẻ hiển thị tên mô hình, chi phí tích lũy, và công cụ đang được gọi hiện tại. Đối với các mô hình cao cấp/tốn kém — Opus hoặc Fable — huy hiệu mô hình sẽ hiển thị màu đỏ tươi, giúp nhận ra ngay các phiên tốn nhiều chi phí. Số đếm cập nhật theo thời gian thực qua WebSocket nên bảng luôn đồng bộ với kho sự kiện trực tiếp. Di chuột lên huy hiệu Waiting để xem lý do — cần phản hồi, xong lượt, chờ prompt hoặc bị ngắt. Chế độ xem <strong>Projects</strong> thứ ba nhóm phiên theo dự án thay vì theo trạng thái; khi đã tạo ít nhất một nhóm <strong>monitor</strong> (màn hình) có tên, các cột dự án được gán cho nó sẽ hiển thị bên trong một khung có viền, có thể kéo sắp xếp dành riêng cho màn hình đó, các khung này nằm cạnh nhau trong cùng hàng đó — kéo một khung (qua phần tiêu đề) vào khung khác để đổi vị trí, tiện để mô phỏng cách bố trí nhiều màn hình vật lý, chỉ lưu trong <code>localStorage</code>. Một biểu tượng chevron trên tiêu đề mỗi khung thu gọn nó xuống chỉ còn tiêu đề — cả khung sẽ rời hẳn khỏi hàng đó, chuyển sang một dải mỏng riêng ở phía trên, dùng chung cho mọi màn hình đã thu gọn khác, nhờ đó giải phóng thật sự khoảng không gian ngang mà nó vẫn sẽ chiếm; mở rộng lại sẽ đưa khung quay về đúng vị trí theo thứ tự trong hàng đó (trong suốt quá trình các cột của nó vẫn được mount, chỉ bị ẩn đi, nên trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó). Một biểu tượng nút thứ hai cạnh chevron chuyển hướng bố cục của riêng khung đó — <strong>ngang</strong> (mặc định, các cột xếp cạnh nhau) hoặc <strong>dọc</strong> (các cột xếp chồng từ trên xuống) — độc lập theo từng màn hình. Mỗi cột dự án (và cột Chưa phân loại độc lập) cũng đều có biểu tượng chevron riêng, cho phép thu gọn riêng cột đó tại chỗ xuống thành một thanh tiêu đề mảnh, độc lập với khung mà nó đang nằm trong.',
    "<strong>Server-paginated</strong> table of every recorded session — each page fetches only its slice so cost computation stays bounded no matter how many sessions exist. Case-insensitive search across <code>id</code>, <code>name</code>, and <code>cwd</code> runs server-side with a 300 ms debounce; the status filter composes with search for precise narrowing. Each row shows the session's real name (synced live from the transcript — a <code>/rename</code> or <code>claude -n</code> title, else the auto title, else the first user prompt, with a short-ID fallback), status badge, agent count, duration, model, and estimated cost. Click any row to drill into the full session detail view with conversation transcript and agent hierarchy.":
    ...
    "Captures the CLI's own <code>/status</code> and <code>/usage</code> TUI panels on demand at <code>/usage</code>, via a detached tmux session that drives a real <code>claude</code> process and scrapes its rendered pane text — no separate accounting API, so it shows exactly what the CLI itself reports. Each capture surfaces account info (email, organization, login method, CLI version, active model), the current session's cost, API and wall-clock duration, and lines added/removed, plus input, output, and cache-read/cache-write token counts. The two headline numbers are the <strong>5-hour session window</strong> and <strong>weekly window</strong> rate-limit percentages — the weekly figure broken down per model — each paired with its own reset time. Every capture is persisted with a <code>status</code> of <code>ok</code>, <code>partial</code>, or <code>error</code>, and always keeps the full raw pane text alongside the parsed fields, so a future CLI output-format change degrades to raw text instead of losing data. Only one capture runs at a time — a second request while one is in flight is rejected rather than racing the tmux session. Same-origin guard included, shared with Run Claude. An <strong>Accounts</strong> panel above it — collapsible via a chevron on its own header, collapsed by default — tracks any number of other Claude accounts side by side, each pointing at a <code>CLAUDE_CONFIG_DIR</code> already logged in via <code>claude login</code> — per-account capture reads that login's own OAuth credential straight from the CLI's storage (Keychain, or a file on non-macOS) and calls Anthropic's API directly, so tracking a second or third account needs no browser session cookie, password, or extra login flow. A missing or expired login shows a clickable <strong>Needs login</strong> badge (macOS) that opens a new Terminal window already running <code>CLAUDE_CONFIG_DIR=&lt;dir&gt; claude</code> — re-authenticating is just a login walk-through and closing the window.":
      "Chụp lại các bảng TUI <code>/status</code> và <code>/usage</code> của chính CLI theo yêu cầu tại <code>/usage</code>, thông qua một phiên tmux tách rời điều khiển một tiến trình <code>claude</code> thật và lấy văn bản pane đã render của nó — không có API kế toán riêng, nên nó hiển thị đúng những gì CLI tự báo cáo. Mỗi lần chụp hiển thị thông tin tài khoản (email, tổ chức, phương thức đăng nhập, phiên bản CLI, model đang dùng), chi phí của phiên hiện tại, thời lượng API và thời gian thực, cùng số dòng được thêm/xóa, cộng với số lượng token đầu vào, đầu ra và cache-read/cache-write. Hai con số nổi bật là tỷ lệ phần trăm giới hạn tốc độ của <strong>cửa sổ phiên 5 giờ</strong> và <strong>cửa sổ hàng tuần</strong> — con số hàng tuần được chia nhỏ theo từng model — mỗi con số đi kèm thời điểm reset riêng. Mỗi lần chụp được lưu lại với <code>status</code> là <code>ok</code>, <code>partial</code>, hoặc <code>error</code>, và luôn giữ lại toàn bộ văn bản pane thô bên cạnh các trường đã phân tích, để khi định dạng đầu ra của CLI thay đổi trong tương lai, nó chỉ giảm cấp về văn bản thô thay vì mất dữ liệu. Chỉ một lượt chụp chạy tại một thời điểm — một yêu cầu thứ hai trong khi một lượt đang chạy sẽ bị từ chối thay vì tranh chấp với phiên tmux. Có cùng cơ chế bảo vệ same-origin, dùng chung với Run Claude. Bảng <strong>Accounts</strong> phía trên — có thể thu gọn qua biểu tượng chevron trên tiêu đề riêng của nó, mặc định thu gọn — theo dõi song song bất kỳ số lượng tài khoản Claude nào khác, mỗi tài khoản chỉ cần trỏ tới một <code>CLAUDE_CONFIG_DIR</code> đã đăng nhập sẵn qua <code>claude login</code> — lượt chụp theo từng tài khoản đọc trực tiếp thông tin xác thực OAuth của lượt đăng nhập đó từ bộ lưu trữ của CLI (Keychain, hoặc một tệp trên các hệ điều hành không phải macOS) và gọi thẳng API của Anthropic, nên việc theo dõi tài khoản thứ hai hoặc thứ ba không cần cookie phiên trình duyệt, mật khẩu, hay quy trình đăng nhập bổ sung. Khi đăng nhập bị thiếu hoặc hết hạn, huy hiệu <strong>Needs login</strong> có thể bấm được (macOS) sẽ mở một cửa sổ Terminal mới đã chạy sẵn <code>CLAUDE_CONFIG_DIR=&lt;dir&gt; claude</code> — việc xác thực lại chỉ là hoàn tất quy trình đăng nhập rồi đóng cửa sổ.",
    "A <strong>Session reset times</strong> panel plots every account's next session-window reset across the next 24h, tinting the reset marker's right half toward the weekly window's own color once the weekly window — not the session — is the real constraint, paired with a <strong>Capped by weekly</strong> callout naming it explicitly. A <strong>Weekly reset countdown</strong> panel does the same for the weekly window's calendar-day reset. A <strong>Color thresholds</strong> card lets you configure exactly where each green/yellow/orange/red band starts, independently for the session and weekly scopes plus their <strong>Consumption rate</strong> counterparts — shared live across every connected computer. Every enabled account also captures automatically every 15 minutes (a disable-able background scheduler), so percentages stay fresh without a manual click. An <strong>Activity</strong> card shows whether you're actively using each account (used within the last 15 minutes) or how long ago it was last used — inferred from whether that account's own session/weekly percentage actually rose between captures, not from anything tied to its <code>CLAUDE_CONFIG_DIR</code>, since real work is often done through whichever profile is logged into your default <code>~/.claude</code> instead of this account's own named config directory. A <strong>Consumption rate</strong> card predicts, per account, when its session and weekly quotas will run out at their current pace — a %/hour trend fit over recent captures since the current window started, projected forward to the instant it would cross 100% only when actually rising — and colors the result by whether that predicted exhaustion lands before or after the window's own reset. Underneath each verdict, a small stats line gives the raw rate (%/hour for session, %/day for weekly) plus how many times faster or slower that is than the <strong>sustainable pace</strong> — the rate that would land exactly at 100% right when the window resets — since a bare rate alone can't say whether it's normal or alarming. A <strong>Rotation plan</strong> card chains that same weekly data into an actionable rotation — which account to be on now, and when to hand off to the next one — entirely client-side, out to a ~9-day horizon.":
      "Bảng <strong>Thời điểm đặt lại phiên</strong> vẽ biểu đồ thời điểm đặt lại cửa sổ phiên tiếp theo của mỗi tài khoản trong 24 giờ tới, tô nửa bên phải của vạch đánh dấu đặt lại theo màu riêng của cửa sổ hàng tuần một khi cửa sổ hàng tuần — chứ không phải phiên — mới là giới hạn thực sự, kèm theo nhãn <strong>Bị giới hạn bởi hạn mức tuần</strong> nêu rõ điều đó. Bảng <strong>Đếm ngược đặt lại hàng tuần</strong> làm điều tương tự cho việc đặt lại theo ngày lịch của cửa sổ hàng tuần. Thẻ <strong>Ngưỡng màu sắc</strong> cho phép bạn cấu hình chính xác điểm bắt đầu của từng dải màu xanh lá/vàng/cam/đỏ, độc lập cho phạm vi phiên và hàng tuần cùng phạm vi <strong>Tốc độ tiêu thụ</strong> tương ứng của chúng — được chia sẻ trực tiếp trên mọi máy tính đã kết nối. Mọi tài khoản đang bật cũng tự động được ghi lại mỗi 15 phút (một scheduler chạy nền, có thể tắt), nên tỷ lệ phần trăm luôn cập nhật mà không cần bấm thủ công. Thẻ <strong>Activity</strong> (Hoạt động) cho biết bạn có đang thực sự sử dụng từng tài khoản hay không (đã dùng trong 15 phút gần nhất) hoặc đã bao lâu kể từ lần sử dụng gần nhất — được suy ra từ việc tỷ lệ phần trăm phiên/hàng tuần của chính tài khoản đó có thực sự tăng lên giữa các lần ghi lại hay không, chứ không dựa vào bất cứ điều gì gắn với <code>CLAUDE_CONFIG_DIR</code> của nó, vì công việc thực tế thường được thực hiện qua hồ sơ đã đăng nhập ở <code>~/.claude</code> mặc định chứ không phải qua thư mục config riêng của tài khoản này. Thẻ <strong>Consumption rate</strong> (Tốc độ tiêu thụ) dự đoán, theo từng tài khoản, thời điểm hạn mức phiên và hàng tuần sẽ dùng hết nếu tiếp tục với tốc độ hiện tại — một đường xu hướng %/giờ được khớp trên các lần ghi lại gần đây kể từ khi cửa sổ hiện tại bắt đầu, được ngoại suy đến thời điểm sẽ chạm mốc 100% chỉ khi xu hướng đó thực sự đang tăng — và tô màu kết quả tùy theo thời điểm dự đoán hết hạn mức đến trước hay sau khi cửa sổ tự đặt lại. Bên dưới mỗi kết luận là một dòng thống kê nhỏ cho biết tốc độ thô (%/giờ đối với phiên, %/ngày đối với hàng tuần) cùng với việc tốc độ đó nhanh hay chậm hơn bao nhiêu lần so với <strong>tốc độ bền vững</strong> — tốc độ sẽ vừa khít chạm mốc 100% đúng vào thời điểm cửa sổ đặt lại — vì chỉ riêng một con số tốc độ thô không thể nói lên đây là bình thường hay đáng lo ngại. Thẻ <strong>Rotation plan</strong> (Kế hoạch xoay vòng) nối chuỗi cùng dữ liệu hàng tuần đó thành một kế hoạch xoay vòng có thể hành động được — nên dùng tài khoản nào ngay bây giờ, và khi nào nên chuyển sang tài khoản tiếp theo — hoàn toàn tính toán phía client, chiếu xa khoảng 9 ngày.",
    "A 12-tab inspector at <code>/cc-config</code> for everything Claude Code knows about: skills, subagents, slash commands, output styles, plugins, marketplaces, MCP servers, hooks, settings, memory, keybindings, and statusline scripts. The Settings tab leads with a Current configuration summary of the options <code>/config</code> controls — model, verbose, theme, output style, auto-compact, notifications — resolved across scopes. The Memory tab surfaces both the user and project <code>CLAUDE.md</code> files and the per-project file-based memory store — every auto-memory <code>*.md</code> under <code>~/.claude/projects/&lt;slug&gt;/memory/</code> (a <code>MEMORY.md</code> index plus one file per remembered fact, often 100+), grouped by project, searchable, and editable. Create, edit, and delete the low-risk text-file surfaces with mandatory timestamped backups before every write. Plugins, MCP, hooks, and live settings stay read-only with explainer banners and copy-able CLI commands. Per-plugin contribution breakdowns show author and license.":
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span > until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >. Background subagents keep running.':
      'Không lỗi: agent chính → <code>waiting</code> — UI hiển thị <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span > cho đến lần nhập liệu tiếp theo của người dùng. Cờ Đang chờ của phiên (<code>awaiting_input_since</code>) chỉ được đóng dấu khi không còn subagent nào đang làm việc — nếu vẫn có subagent đang chạy, phiên giữ nguyên <strong>Active</strong> (nó đang chờ các agent của mình, không phải người dùng) và việc đóng dấu được hoãn đến <code>SubagentStop</code> cuối cùng. <code>stop_reason=error</code>: đánh dấu agent và phiên là <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >. Các subagent chạy nền vẫn tiếp tục chạy.',
    "Background agent finished": "Agent chạy nền đã hoàn thành",
    "Matched subagent → <span class=\"status-chip chip-completed\" ><span class=\"chip-dot\"></span>Completed</span >. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      'Subagent khớp → <span class="status-chip chip-completed" ><span class="chip-dot"></span>Completed</span >. Cố ý <strong>không</strong> xóa cờ chờ — việc một subagent chạy nền hoàn thành không cho ta biết gì về con người. Tuy nhiên, nó có thể <em>đặt</em> cờ này: khi subagent cuối cùng còn đang làm việc hoàn thành sau một <code>Stop</code> không lỗi bị hoãn, bước kiểm tra rút cạn sẽ đóng dấu cờ chờ <code>stop</code> và phiên chuyển từ <strong>Active</strong> → <strong>Waiting</strong> (Đang chờ) đúng vào lúc toàn bộ đội agent đã hoàn tất. Đồng thời khởi động một lượt quét JSONL kiểu phát-rồi-quên (<code>scanAndImportSubagents</code>) duyệt qua các tệp <code>subagents/agent-*.jsonl</code> của phiên, ghép cặp các khối <code>tool_use</code> ↔ <code>tool_result</code> theo <code>tool_use_id</code>, và phát ra các sự kiện <code>PreToolUse</code> + <code>PostToolUse</code> theo từng công cụ dưới <code>agent_id</code> riêng của mỗi subagent — làm lộ ra các lời gọi công cụ mà subagent thực hiện nội bộ và vốn không bao giờ kích hoạt bất kỳ hook nào. Cùng lượt quét đó còn dựng lại phân cấp subagent lồng nhau — nó trỏ lại <code>parent_agent_id</code> của mỗi subagent về đúng agent đã spawn, khôi phục từ kết quả tool Task trong transcript (<code>toolUseResult.agentId</code>), nên subagent tự spawn subagent con sẽ lồng đúng chỗ thay vì dồn phẳng dưới main.',
    "Agent sends notification": "Agent gửi thông báo",
    'Event logged to activity feed. Permission/input-prompt patterns (e.g. "needs your permission", "waiting for your input") set the agent to <code>waiting</code> and stamp <code>awaiting_input_since</code> — the reason is re-derived rather than always assumed to be <code>notification</code>: a permission/approval-worded message always stamps <code>notification</code>, otherwise a still-working subagent stamps <code>subagent</code>, a synchronous Bash call in flight stamps <code>shell</code>, a Monitor tool call in flight stamps <code>monitor</code>, else the genuine <code>notification</code>. Compaction-related notifications tagged as <code>Compaction</code> events. Triggers a browser notification if enabled.':
      'Sự kiện được ghi vào nguồn cấp hoạt động. Các mẫu nhắc quyền/nhắc nhập liệu (ví dụ "needs your permission", "waiting for your input") đặt agent thành <code>waiting</code> và đóng dấu thời gian <code>awaiting_input_since</code> — lý do được suy luận lại thay vì luôn mặc định là <code>notification</code>: một thông điệp mang ngôn từ quyền/phê duyệt luôn đóng dấu <code>notification</code>; nếu không, một subagent vẫn đang hoạt động sẽ đóng dấu <code>subagent</code>, một lệnh gọi Bash đồng bộ đang chạy sẽ đóng dấu <code>shell</code>, một lệnh gọi công cụ Monitor đang chạy sẽ đóng dấu <code>monitor</code>, còn lại mới là <code>notification</code> thực sự. Các thông báo liên quan đến nén được gắn thẻ là sự kiện <code>Compaction</code>. Kích hoạt thông báo trình duyệt nếu được bật.',
    ...
    "Build <strong>both</strong> per-arch DMGs (arm64 + x64). Correct for release — slower (packages each arch)":
      "Dựng <strong>cả hai</strong> tệp DMG cho từng kiến trúc (arm64 + x64). Đúng cho việc phát hành — chậm hơn (đóng gói riêng từng kiến trúc)",
    "Build an Apple-Silicon-only DMG — fast (~1 min), recommended for a single machine":
    ...
    'Session status lifecycle. <code>waiting</code> is a UI overlay — persisted as <code>active</code> with <code>awaiting_input_since</code> set and <code>awaiting_reason</code> recording why (<code>session_start</code>, <code>stop</code>, <code>notification</code>, <code>interrupted</code>, <code>subagent</code>, <code>shell</code>, or <code>monitor</code> — the last three mean Claude is still actively working via a child, not blocked on you). <code>SessionEnd</code> preserves error state. Error recovery requires <code>UserPromptSubmit</code> or <code>PreToolUse</code>. The watchdog also recovers <code>Esc</code>-cancelled turns (marker or idle-timeout path) and moves the session to Waiting. The dashboard surfaces the reason on every Waiting badge (hover tooltip, plus an inline chip on the Sessions table and session detail header) and in a waiting-for-input banner on the session detail page; the three "still working" reasons replace the badge\'s whole word instead of nesting a chip.':
      'Vòng đời trạng thái của phiên. <code>waiting</code> là một lớp phủ giao diện — được lưu trữ dưới dạng <code>active</code> với <code>awaiting_input_since</code> đã được đặt và <code>awaiting_reason</code> ghi lại lý do (<code>session_start</code>, <code>stop</code>, <code>notification</code>, <code>interrupted</code>, <code>subagent</code>, <code>shell</code>, hoặc <code>monitor</code> — ba lý do cuối nghĩa là Claude vẫn đang tích cực làm việc thông qua một tác vụ con, không phải đang chờ bạn). <code>SessionEnd</code> giữ nguyên trạng thái lỗi. Việc khôi phục lỗi đòi hỏi <code>UserPromptSubmit</code> hoặc <code>PreToolUse</code>. Watchdog cũng khôi phục các lượt bị hủy bằng <code>Esc</code> (đường dấu hiệu hoặc đường chờ rảnh-rỗi) và chuyển phiên sang Waiting. Bảng điều khiển hiển thị lý do này trên mọi huy hiệu Waiting (tooltip khi di chuột, kèm chip nội dòng trên bảng Sessions và tiêu đề trang chi tiết phiên) và trong banner chờ phản hồi trên trang chi tiết phiên; ba lý do "vẫn đang làm việc" sẽ thay thế toàn bộ chữ trên huy hiệu thay vì lồng thêm một chip.',
    "Complete event ingestion from hook fire to browser re-render":
    ...
    "<code>desktop:dmg</code> is slower because it builds the full app tree <em>twice</em> (once per architecture) and ad-hoc-signs each — emitting two separate per-arch DMGs (<code>…-arm64.dmg</code> + <code>…-x64.dmg</code>). There is no <code>@electron/universal</code> merge; the release ships the two per-arch DMGs. For running on a single Mac, use <code>desktop:dmg:arm64</code> (Apple Silicon) or <code>desktop:dmg:x64</code> (Intel) — one architecture, finishing in roughly a minute. CI runs <code>desktop:dmg</code> and uploads both DMGs as <code>ClaudeCodeMonitor-dmg</code>, so you rarely need to build them yourself.":
      "<code>desktop:dmg</code> chậm hơn vì nó dựng toàn bộ cây ứng dụng <em>hai lần</em> (mỗi kiến trúc một lần) và ký tạm thời (ad-hoc) cho từng bản — tạo ra hai tệp DMG riêng cho từng kiến trúc (<code>…-arm64.dmg</code> + <code>…-x64.dmg</code>). Không có bước hợp nhất <code>@electron/universal</code>; bản phát hành đi kèm hai tệp DMG cho từng kiến trúc này. Để chạy trên một máy Mac duy nhất, hãy dùng <code>desktop:dmg:arm64</code> (Apple Silicon) hoặc <code>desktop:dmg:x64</code> (Intel) — một kiến trúc, hoàn tất trong khoảng một phút. CI chạy <code>desktop:dmg</code> và tải lên cả hai tệp DMG dưới dạng <code>ClaudeCodeMonitor-dmg</code>, nên bạn hiếm khi cần tự dựng chúng.",
    "Double-click the downloaded <code>.dmg</code> to mount it":
    ...
    "Tabby also watches the clock, not just events: if a live session has been running an <strong>expensive model</strong> — Opus or Fable — continuously for more than <strong>15 minutes</strong>, it surfaces a one-time nag bubble asking if you're sure you want to stay there. The nag fires once per streak and only re-arms if the session drops off that model and comes back onto it later.":
      "Tabby cũng theo dõi cả đồng hồ, không chỉ sự kiện: nếu một phiên đang chạy liên tục trên một <strong>mô hình tốn kém</strong> — Opus hoặc Fable — quá <strong>15 phút</strong>, nó sẽ hiện lên một bong bóng nhắc nhở một lần, hỏi bạn có chắc muốn tiếp tục dùng mô hình đó không. Lời nhắc chỉ xuất hiện một lần cho mỗi đợt liên tục, và chỉ được kích hoạt lại nếu phiên rời khỏi mô hình đó rồi sau đó quay lại.",
    "Click the cat — or press <code>⌘B</code> / <code>Ctrl+B</code> — to open Tabby's panel (<code>Esc</code> closes it). The panel groups a live status line, quick actions, and an Ask box.":
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting"><span class="chip-dot"></span>Waiting</span> until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error"><span class="chip-dot"></span>Error</span>. Background subagents keep running.':
      'Không phải lỗi: agent chính → <code>waiting</code> — UI hiển thị <span class="status-chip chip-waiting"><span class="chip-dot"></span>Đang chờ</span> cho đến lần nhập tiếp theo của người dùng. Cờ Đang chờ của phiên (<code>awaiting_input_since</code>) chỉ được đóng dấu khi không còn subagent nào đang làm việc — nếu vẫn có subagent đang chạy, phiên giữ nguyên <strong>Active</strong> (nó đang chờ các agent của mình, không phải người dùng) và việc đóng dấu được hoãn đến <code>SubagentStop</code> cuối cùng. <code>stop_reason=error</code>: đánh dấu agent và phiên là <span class="status-chip chip-error"><span class="chip-dot"></span>Lỗi</span>. Các subagent chạy nền vẫn tiếp tục chạy.',
    "Matched subagent → <span class=\"status-chip chip-completed\"><span class=\"chip-dot\"></span>Completed</span>. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      'Subagent được khớp → <span class="status-chip chip-completed"><span class="chip-dot"></span>Hoàn tất</span>. Nó cố ý <strong>không</strong> xóa cờ chờ — việc một subagent chạy nền hoàn tất không cho ta biết gì về con người. Tuy nhiên, nó có thể <em>đặt</em> cờ này: khi subagent cuối cùng còn đang làm việc hoàn tất sau một <code>Stop</code> không lỗi bị hoãn, bước kiểm tra rút cạn sẽ đóng dấu cờ chờ <code>stop</code> và phiên chuyển từ <strong>Active</strong> → <strong>Waiting</strong> (Đang chờ) đúng vào lúc toàn bộ đội agent đã hoàn tất. Nó cũng khởi động một lượt quét JSONL kiểu fire-and-forget (<code>scanAndImportSubagents</code>) duyệt qua các tệp <code>subagents/agent-*.jsonl</code> của phiên, ghép các khối <code>tool_use</code> ↔ <code>tool_result</code> theo <code>tool_use_id</code>, và phát ra các sự kiện <code>PreToolUse</code> + <code>PostToolUse</code> cho từng công cụ dưới <code>agent_id</code> riêng của mỗi subagent — qua đó làm lộ ra các lệnh gọi công cụ mà subagent thực hiện nội bộ và vốn không bao giờ kích hoạt bất kỳ hook nào. Cùng lượt quét đó còn dựng lại phân cấp subagent lồng nhau — nó trỏ lại <code>parent_agent_id</code> của mỗi subagent về đúng agent đã spawn, khôi phục từ kết quả tool Task trong transcript (<code>toolUseResult.agentId</code>), nên subagent tự spawn subagent con sẽ lồng đúng chỗ thay vì dồn phẳng dưới main.',
    'Creates a compaction subagent → <span class="status-chip chip-completed"><span class="chip-dot"></span>Completed</span>. Detected via <code>isCompactSummary</code> entries in the transcript. Token baselines preserve pre-compaction totals. Periodic scanner (cadence ~¼ of <code>DASHBOARD_STALE_MINUTES</code>) catches compactions when no hooks fire.':
    ...
    "Groups sessions by the folder(s) they run from into a user-named <strong>project</strong>, replacing the flat Agents/Sessions view with one grouped by where the work actually happens. Projects list as a compact table — one row per project with its name, session count, active count, last-activity, and quick pin/plan/report/delete actions, drag-reorderable on the row — instead of a full card each, so a machine with many projects doesn't turn into a long vertical scroll. A project claims one or more working directories — a folder belongs to at most one project — and membership is derived by joining each session's <code>cwd</code> against the project's mapped folders, with no schema change on <code>sessions</code> itself. Each row expands via a chevron (collapsed by default) to reveal its folder mappings, any AGENT-PLAN.md checklist, and its sessions rendered as a horizontally scrollable row of session cards — the same collapsed-by-default treatment applies to each folder inside Unassigned. Create a project (optionally from an existing folder), rename it inline, pin it to float above every unpinned project regardless of the manual drag order (pinned state is server-shared, not per-browser), add or remove folder mappings, or delete it — sessions are never deleted, they simply fall back to the <strong>Unassigned</strong> bucket.":
      "Nhóm các phiên theo (các) thư mục mà chúng chạy từ đó thành một <strong>dự án</strong> do người dùng đặt tên, thay thế khung nhìn Agents/Sessions phẳng bằng một khung nhìn được nhóm theo nơi công việc thực sự diễn ra. Các dự án được liệt kê dưới dạng một bảng gọn — mỗi dự án một hàng gồm tên, số phiên, số đang hoạt động, hoạt động gần nhất, cùng các thao tác nhanh ghim/xem kế hoạch/báo cáo/xóa, hàng có thể kéo để sắp xếp lại — thay vì mỗi dự án một thẻ đầy đủ, nhờ đó một máy có nhiều dự án không biến trang thành một danh sách cuộn dọc dài dằng dặc. Một dự án sở hữu một hoặc nhiều thư mục làm việc — một thư mục chỉ thuộc về nhiều nhất một dự án — và tư cách thành viên được suy ra bằng cách đối chiếu <code>cwd</code> của mỗi phiên với các thư mục đã ánh xạ của dự án, không cần thay đổi schema trên chính <code>sessions</code>. Mỗi hàng có thể mở rộng qua biểu tượng mũi tên (mặc định thu gọn) để hiện các ánh xạ thư mục, checklist AGENT-PLAN.md nếu có, và các phiên của nó hiển thị dưới dạng một hàng có thể cuộn ngang gồm các thẻ phiên — mỗi thư mục bên trong Unassigned cũng dùng cùng kiểu mặc định thu gọn này. Tạo một dự án (tùy chọn từ một thư mục có sẵn), đổi tên trực tiếp, ghim để nổi lên trên mọi dự án chưa ghim bất kể thứ tự kéo thủ công (trạng thái ghim được chia sẻ phía server, không theo từng trình duyệt), thêm hoặc xóa ánh xạ thư mục, hoặc xóa dự án — các phiên không bao giờ bị xóa, chúng chỉ đơn giản rơi trở lại nhóm <strong>Unassigned</strong>.",
    "A standalone <strong>Focus</strong> page (route <code>/focus</code>, in the sidebar right after <strong>Calendar</strong>) turns the same focus-time data into a stakeholder-readable activity report instead of a calendar grid. It shares the Calendar board's own project chips, session dropdown, and time-period picker, then swaps the swimlane for stat tiles plus a new activity list — one row per plan item, detour, bug, feature, or unclassified time bucket, each with a kind-colored chip and a wall/active time figure, and, for entries the background classifier inferred rather than a live declaration, its one-sentence reason. It needed no backend changes at all, reusing the existing <code>GET /api/focus-report</code> endpoint exactly as the Calendar board does.":
      "Một trang <strong>Focus</strong> độc lập (route <code>/focus</code>, nằm trong thanh bên ngay sau <strong>Calendar</strong>) biến cùng dữ liệu thời gian tiêu điểm đó thành một báo cáo hoạt động dễ đọc cho các bên liên quan, thay vì một lưới lịch. Nó dùng chung bộ chip lọc dự án, ô chọn phiên dạng dropdown, và bộ chọn khoảng thời gian của chính Calendar board, rồi thay làn bơi bằng các ô thống kê cộng với một danh sách hoạt động mới — mỗi mục kế hoạch, lượt rẽ ngang, bug, feature, hoặc khoảng thời gian chưa phân loại chiếm một dòng, mỗi dòng có một chip được tô màu theo loại và một con số thời gian thực/hoạt động, và với các mục mà bộ phân loại nền đã suy luận ra thay vì được khai báo trực tiếp, kèm theo lý do một câu của nó. Nó hoàn toàn không cần thay đổi gì ở backend, tái sử dụng nguyên trạng endpoint <code>GET /api/focus-report</code> giống hệt như Calendar board.",
    "The Focus page also offers the Calendar board's own intraday hour-window zoom — duration presets (<code>4h</code>/<code>8h</code>/<code>12h</code>/<code>24h</code>), a start-time stepper/typed input, a \"Live\" toggle, and quick-start presets — but defaults to the full, unzoomed period rather than the Calendar board's own 4-hour default, since this page previously always showed the whole selected window and the zoom is a purely additive, opt-in narrowing. When zoomed, both the stat tiles and the activity list below scope together to the selected window, so the two never disagree.":
      'Trang Focus cũng cung cấp tính năng "thu phóng cửa sổ giờ" (hour-window zoom) trong ngày của chính bảng Calendar — các mức thời lượng đặt sẵn (<code>4h</code>/<code>8h</code>/<code>12h</code>/<code>24h</code>), bộ điều khiển bước nhảy/nhập giờ bắt đầu, nút chuyển "Trực tiếp" (Live), và các mốc bắt đầu nhanh — nhưng mặc định hiển thị toàn bộ khoảng thời gian không thu phóng thay vì mặc định 4 giờ của bảng Calendar, vì trang này trước nay luôn hiển thị trọn khoảng đã chọn và việc thu phóng chỉ là một tùy chọn thu hẹp thêm. Khi thu phóng, cả các ô thống kê phía trên lẫn danh sách hoạt động phía dưới đều thu hẹp cùng nhau theo cửa sổ đã chọn, nên hai bên không bao giờ lệch nhau.',
    'The activity list\'s unclassified bucket is narrative-aware: a "No focus" segment that carries a classifier-inferred one-sentence reason keeps its own row per session instead of collapsing into one shared bucket — only reason-less unclassified segments still collapse into a single shared tail row. And any row backed by multiple sessions renders its "+N more sessions" note as a toggle that expands into each contributing session\'s own name, clock-time range, wall/active time split, and reason.':
      'Nhóm chưa phân loại trong danh sách hoạt động giờ phân biệt theo từng diễn giải: một đoạn "No focus" mang theo lý do một câu do bộ phân loại suy luận sẽ giữ riêng một hàng cho mỗi phiên thay vì tất cả gộp vào một nhóm chung — chỉ những đoạn chưa phân loại không có lý do mới vẫn gộp vào một hàng đuôi dùng chung. Ngoài ra, bất kỳ hàng nào được hợp thành từ nhiều phiên sẽ hiển thị ghi chú "+N more sessions" của nó dưới dạng nút bật/tắt, mở rộng ra tên riêng, khoảng thời gian theo đồng hồ, phần chia thời gian thực/hoạt động và lý do của từng phiên tham gia.',
    "Above the activity list, an AI-synthesized <strong>Summary</strong> block distills the selected window into plain-language \"what we actually did\" bullets, served by a new <code>GET /api/focus-report/summary</code> endpoint (<code>server/lib/focus-summary.js</code>). In all-projects scope the summary is grouped by project — one bullet set per project with activity (largest wall-clock share first, plus an Unassigned group for sessions in unmapped folders), each group cached identically to a directly project-scoped request so nothing generates twice. The bullets are no longer fixed at 2–4 — the budget scales with the window: 4 for a day or two, 6 up to a week, 8 beyond. Windows wider than two days summarize <strong>hierarchically</strong>: per-day summaries are generated and cached first — permanent once a day's data stops changing — then one rollup call synthesizes the window from the daily bullets; an unchanged multi-day window is served from cache with zero LLM calls, and a day whose own synthesis fails contributes its raw activity facts instead of being dropped. Generation runs on <code>DASHBOARD_FOCUS_SUMMARY_MODEL</code> when set — a dedicated override so the stakeholder-facing bullets can use a stronger model (e.g. <code>sonnet</code>) while the far-more-frequent per-session classifier stays on the cheap default — falling back to <code>DASHBOARD_FOCUS_INFER_MODEL</code>, then <code>haiku</code>; the spawn still reuses the focus-inference service's hermetic <code>claude -p</code> contract and its <code>DASHBOARD_FOCUS_INFER_TIMEOUT_MS</code> kill timer. Results are cached in the <code>focus_summaries</code> table keyed by scope and window, gated by a digest of the input data, so a finished day generates once and a still-running day regenerates only when its data changes. When no summary can be produced (LLM mode off, <code>claude</code> CLI unavailable, an empty window, or a generation failure) the endpoint returns <code>{ summary: null }</code> with a 200 and the page hides the block entirely.":
      'Phía trên danh sách hoạt động, một khối <strong>Summary</strong> do AI tổng hợp chưng cất cửa sổ đã chọn thành các gạch đầu dòng bằng ngôn ngữ đời thường về "những gì chúng ta thực sự đã làm", được phục vụ bởi endpoint mới <code>GET /api/focus-report/summary</code> (<code>server/lib/focus-summary.js</code>). Ở phạm vi tất cả dự án, bản tóm tắt được nhóm theo dự án — mỗi dự án có hoạt động là một bộ gạch đầu dòng riêng (dự án chiếm tỷ trọng thời gian thực lớn nhất đứng trước, cộng thêm một nhóm Unassigned cho các phiên nằm trong thư mục chưa được ánh xạ), mỗi nhóm được lưu đệm giống hệt một yêu cầu có phạm vi trực tiếp là dự án đó, nên không có gì bị sinh hai lần. Số gạch đầu dòng không còn cố định ở mức 2–4 — ngân sách co giãn theo cửa sổ: 4 cho một hoặc hai ngày, 6 cho tới một tuần, 8 khi rộng hơn. Cửa sổ rộng hơn hai ngày được tóm tắt <strong>theo phân cấp</strong>: các bản tóm tắt theo từng ngày được sinh và lưu đệm trước — vĩnh viễn một khi dữ liệu của ngày đó ngừng thay đổi — rồi một lần gọi tổng hợp (rollup) tổng hợp cả cửa sổ từ các gạch đầu dòng theo ngày; một cửa sổ nhiều ngày không thay đổi được phục vụ từ bộ đệm mà không cần lần gọi LLM nào, và một ngày mà quá trình tổng hợp của chính nó thất bại sẽ đóng góp các dữ kiện hoạt động thô của mình thay vì bị loại bỏ. Quá trình sinh chạy trên <code>DASHBOARD_FOCUS_SUMMARY_MODEL</code> khi biến này được đặt — một tùy chỉnh ghi đè chuyên dụng để các gạch đầu dòng hướng tới bên liên quan có thể dùng model mạnh hơn (ví dụ <code>sonnet</code>) trong khi bộ phân loại theo phiên chạy thường xuyên hơn nhiều vẫn giữ model mặc định rẻ — nếu không sẽ lui về <code>DASHBOARD_FOCUS_INFER_MODEL</code>, rồi <code>haiku</code>; tiến trình khởi chạy vẫn tái sử dụng hợp đồng <code>claude -p</code> kín của dịch vụ suy luận tiêu điểm cùng bộ đếm giờ kết thúc <code>DASHBOARD_FOCUS_INFER_TIMEOUT_MS</code> của nó. Kết quả được lưu đệm trong bảng <code>focus_summaries</code> với khóa theo phạm vi và cửa sổ, được chốt bằng một bản tóm lược dữ liệu đầu vào, nên một ngày đã kết thúc chỉ sinh một lần còn một ngày vẫn đang chạy chỉ sinh lại khi dữ liệu của nó thay đổi. Khi không thể tạo ra bản tóm tắt (chế độ LLM tắt, CLI <code>claude</code> không khả dụng, cửa sổ trống, hoặc sinh thất bại), endpoint trả về <code>{ summary: null }</code> với mã 200 và trang ẩn hoàn toàn khối này.',
    "Every Agents/Sessions status column gets its own layout toggle too — the same icon next to its count badge — switching just that column's cards between the default vertical stack and a horizontally-scrolling row. Unlike the monitor toggle above, this one is saved per-browser only, and each status column's setting is independent of the others.":
      "Mỗi cột trạng thái ở chế độ Agent/Phiên cũng có nút chuyển bố cục riêng — cùng một biểu tượng cạnh huy hiệu số lượng của nó — chỉ chuyển đổi các thẻ của riêng cột đó giữa kiểu xếp chồng dọc mặc định và một hàng cuộn ngang. Khác với nút chuyển của màn hình ở trên, nút này chỉ được lưu theo trình duyệt, và thiết lập của mỗi cột trạng thái độc lập với các cột khác.",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Session cards also show a compact <strong>input / output / cache-read / cache-write</strong> token strip with a cost-weighted <strong>Effective</strong> total, once the session has used any tokens — cache reads and writes are split into separate figures (reads bill at ~10% of the input rate, writes at a premium), and Effective weighs each category by the model's actual pricing rates so it tracks the $ cost instead of overstating cache-heavy sessions. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). A second icon button next to the chevron toggles that box's own layout direction — <strong>horizontal</strong> (the default, columns side by side) or <strong>vertical</strong> (columns stacked top to bottom) — independently per monitor. Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      'Chuyển đổi giữa các làn <strong>Agents</strong> (Working / Waiting / Completed / Error) và <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned). Một cột <strong>Waiting</strong> màu vàng đánh dấu các mục đang chờ người dùng — lời nhắc mới, giữa các lượt, hoặc cổng cấp quyền. Di chuột qua bất kỳ tiêu đề cột nào để xem các chú giải về vòng đời giải thích từng lần chuyển trạng thái. Các thẻ hiển thị tên mô hình, chi phí tích lũy, và công cụ đang được gọi hiện tại. Đối với các mô hình cao cấp/tốn kém — Opus hoặc Fable — huy hiệu mô hình sẽ hiển thị màu đỏ tươi, giúp nhận ra ngay các phiên tốn nhiều chi phí. Một khi phiên đã dùng token, thẻ phiên còn hiển thị thêm một dải token gọn gàng — <strong>đầu vào / đầu ra / cache đọc / cache ghi</strong> — kèm tổng <strong>Effective (hiệu dụng)</strong> tính theo trọng số chi phí — cache đọc và ghi được tách thành hai con số riêng (đọc tính phí ~10% giá đầu vào, ghi tính phí cao hơn), và Effective nhân trọng số từng hạng mục theo biểu giá thực của model, nên nó theo sát chi phí $ thay vì thổi phồng các phiên nặng cache. Số đếm cập nhật theo thời gian thực qua WebSocket nên bảng luôn đồng bộ với kho sự kiện trực tiếp. Di chuột lên huy hiệu Waiting để xem lý do — cần phản hồi, xong lượt, chờ prompt hoặc bị ngắt. Chế độ xem <strong>Projects</strong> thứ ba nhóm phiên theo dự án thay vì theo trạng thái; khi đã tạo ít nhất một nhóm <strong>monitor</strong> (màn hình) có tên, các cột dự án được gán cho nó sẽ hiển thị bên trong một khung có viền, có thể kéo sắp xếp dành riêng cho màn hình đó, các khung này nằm cạnh nhau trong cùng hàng đó — kéo một khung (qua phần tiêu đề) vào khung khác để đổi vị trí, tiện để mô phỏng cách bố trí nhiều màn hình vật lý, chỉ lưu trong <code>localStorage</code>. Một biểu tượng chevron trên tiêu đề mỗi khung thu gọn nó xuống chỉ còn tiêu đề — cả khung sẽ rời hẳn khỏi hàng đó, chuyển sang một dải mỏng riêng ở phía trên, dùng chung cho mọi màn hình đã thu gọn khác, nhờ đó giải phóng thật sự khoảng không gian ngang mà nó vẫn sẽ chiếm; mở rộng lại sẽ đưa khung quay về đúng vị trí theo thứ tự trong hàng đó (trong suốt quá trình các cột của nó vẫn được mount, chỉ bị ẩn đi, nên trạng thái "hiện thêm" riêng của một thẻ vẫn tồn tại qua lần chuyển đổi đó). Một biểu tượng nút thứ hai cạnh chevron chuyển hướng bố cục của riêng khung đó — <strong>ngang</strong> (mặc định, các cột xếp cạnh nhau) hoặc <strong>dọc</strong> (các cột xếp chồng từ trên xuống) — độc lập theo từng màn hình. Mỗi cột dự án (và cột Chưa phân loại độc lập) cũng đều có biểu tượng chevron riêng, cho phép thu gọn riêng cột đó tại chỗ xuống thành một thanh tiêu đề mảnh, độc lập với khung mà nó đang nằm trong.',
    '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board (sessions)</strong> — sessions swim-laned across 5 columns: Active, Waiting, Completed, Error, Abandoned. Each card shows agent count, duration, model, cumulative cost, and a compact token strip with the cache read/write split and a cost-weighted Effective total</span>':
      '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board (session)</strong> — các session được xếp theo làn trên 5 cột: Active, Waiting, Completed, Error, Abandoned. Mỗi thẻ hiển thị số lượng agent, thời lượng, mô hình, chi phí tích lũy, và một dải token gọn gàng với cache tách đọc/ghi cùng tổng Effective tính theo trọng số chi phí</span>',
    "A standalone page (<code>/project-manager</code>, sidebar label <strong>Project Manager</strong>, positioned right after <strong>Focus</strong>) answering \"what's going on across every project I'm tracking\" at a glance instead of checking each one individually. A portfolio rollup table — one row per project, with an objective/milestone progress bar, a pace badge, an open-decision count, and last activity — sits above the <strong>Decision Queue</strong>: pending reconciliation items (<code>pace_alert</code>, <code>detour_volume</code>, <code>detour_disposition</code>, and write-back failures) rendered as actionable cards, each resolved in place — <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code> for a detour, or resolve/dismiss/retry for everything else. A <strong>Pace Watch</strong> rail lists every behind item across every project, worst-overdue first, and a <strong>Recently Resolved</strong> log records what was just cleared. A project-scope chip row filters the table, queue, and rail down to a single project. It composes the dashboard's existing and new portfolio-management endpoints — <code>GET /api/projects</code>, <code>GET /api/portfolio/summary</code>, and <code>GET /api/decision-queue</code> — rather than duplicating their logic, and is the first client page built on top of the pace-tracking, detour-disposition, and reconciliation work that had previously shipped server-only.":
      'Một trang độc lập (<code>/project-manager</code>, nhãn thanh bên <strong>Project Manager</strong>, đặt ngay sau <strong>Focus</strong>) trả lời câu hỏi "tình hình từng dự án tôi đang theo dõi ra sao" chỉ trong nháy mắt, thay vì phải kiểm tra từng dự án một. Một bảng tổng hợp danh mục dự án — mỗi dự án một hàng, gồm thanh tiến độ mục tiêu/mốc, huy hiệu nhịp độ, số quyết định đang chờ và hoạt động gần nhất — nằm phía trên <strong>Decision Queue</strong>: các mục đối soát đang chờ xử lý (<code>pace_alert</code>, <code>detour_volume</code>, <code>detour_disposition</code>, và lỗi ghi ngược) được hiển thị dưới dạng thẻ có thể thao tác, mỗi thẻ được xử lý ngay tại chỗ — dùng <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code> cho một lối rẽ (detour), hoặc resolve/dismiss/retry cho phần còn lại. Dải <strong>Pace Watch</strong> liệt kê mọi mục bị chậm trên tất cả dự án, mục trễ nặng nhất lên trước, và nhật ký <strong>Recently Resolved</strong> ghi lại những gì vừa được xử lý xong. Một hàng chip phạm vi dự án lọc bảng, hàng đợi và dải này xuống còn một dự án duy nhất. Trang này kết hợp các endpoint quản lý danh mục dự án hiện có và mới của dashboard — <code>GET /api/projects</code>, <code>GET /api/portfolio/summary</code>, và <code>GET /api/decision-queue</code> — thay vì lặp lại logic của chúng, và là trang client đầu tiên được xây trên nền công việc theo dõi nhịp độ, xử lý lối rẽ và đối soát vốn trước đây chỉ có ở phía server.',
  },
  ko: {
    'Answers "what is this session doing, and which part of the project plan is it for?" at a glance. Each monitored repo may keep a human-approved <code>AGENT-PLAN.md</code> (numbered checkbox items) at its root; a background poll (<code>DASHBOARD_PLAN_POLL_MS</code>, plus opportunistic ingest on <code>SessionStart</code>) mirrors it read-only into the dashboard, keyed by <code>cwd</code> and rolled up to Projects. Sessions declare their position by running <code>ccam focus set/push/bug/feature/pop/done</code> in their Bash tool — the hook stream is the write channel, so it works in every hooked project with zero per-project setup. <code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code> push a kind-tagged detour; the Plan view\'s per-item focus line reflects each session\'s actual current state (known item, plain detour, feature, or bug) with a matching icon, bucketed under the item it was declared on (or an "Unknown item" row when none was set), with click-to-expand detail for bug/feature declarations.':
      '"이 세션은 무엇을 하고 있고, 프로젝트 계획의 어느 부분을 위한 것인가?"를 한눈에 답합니다. 모니터링되는 각 저장소는 루트에 사람이 승인한 <code>AGENT-PLAN.md</code>(번호가 매겨진 체크박스 항목)를 둘 수 있으며, 백그라운드 폴링(<code>DASHBOARD_PLAN_POLL_MS</code>, 그리고 <code>SessionStart</code> 시의 기회적 수집)이 이를 읽기 전용으로 대시보드에 미러링하여 <code>cwd</code>로 연결하고 Projects로 롤업합니다. 세션은 Bash 도구에서 <code>ccam focus set/push/bug/feature/pop/done</code>을 실행해 자신의 위치를 선언합니다 — 훅 스트림이 곧 쓰기 채널이므로, 훅이 설치된 모든 프로젝트에서 프로젝트별 추가 설정 없이 동작합니다. <code>ccam focus bug</code>/<code>feature "&lt;title&gt;" "&lt;summary&gt;" [--detail "&lt;text&gt;"]</code>는 종류가 태그된 우회 작업을 push하며, Plan 뷰의 항목별 포커스 줄은 각 세션의 실제 현재 상태(알려진 항목, 단순 우회 작업, 기능, 버그)를 일치하는 아이콘과 함께 반영하고 선언 당시의 항목 아래에 버킷으로 묶입니다(항목이 설정되지 않았다면 "Unknown item" 행에 표시됩니다). 버그/기능 선언은 클릭하면 세부 정보가 펼쳐집니다.',
    "The Projects page shows each plan as a collapsible checklist with a progress bar and per-item session chips; every session card gets a one-line focus breadcrumb with a shared icon for its current state (crosshair on a known item, a detour/feature/bug icon otherwise), rendered even for a session mid-detour with no base item declared; Session Detail gains a focus banner plus a <strong>Plan</strong> tab (checklist, focus timeline from <code>Focus</code> events, and the session's latest TodoWrite micro-plan). A periodic drift audit (<code>DASHBOARD_FOCUS_AUDIT_*</code>) — headless <code>claude -p</code> on a small model, with a keyword-overlap fallback — stamps an amber \"possible undeclared detour?\" badge when a focused session's recent activity stops matching its declaration; it never rewrites focus, and declarations never clear the badge.":
      'Projects 페이지는 각 계획을 진행률 표시줄과 항목별 세션 칩이 있는 접을 수 있는 체크리스트로 보여줍니다; 모든 세션 카드에는 현재 상태를 나타내는 공통 아이콘(알려진 항목에는 크로스헤어, 그 외에는 우회/기능/버그 아이콘)이 있는 한 줄짜리 포커스 브레드크럼이 생기며, 기준 항목이 선언되지 않은 채로 우회 작업 중인 세션에도 표시됩니다; 세션 상세에는 포커스 배너와 <strong>Plan</strong> 탭(체크리스트, <code>Focus</code> 이벤트로 만든 포커스 타임라인, 세션의 최신 TodoWrite 마이크로 플랜)이 추가됩니다. 주기적 드리프트 감사(<code>DASHBOARD_FOCUS_AUDIT_*</code>) — 소형 모델에서 헤드리스로 <code>claude -p</code>를 실행하고 키워드 중복 휴리스틱으로 폴백 — 는 포커스된 세션의 최근 활동이 선언과 더 이상 일치하지 않으면 호박색 "possible undeclared detour?" 배지를 찍습니다; 포커스를 다시 쓰는 일은 없으며, 선언이 배지를 지우지도 않습니다.',
    "Poll interval for the <code>AGENT-PLAN.md</code> watcher — a changed plan file is re-ingested and broadcast as <code>plan_updated</code>; <code>0</code> disables the poll (<code>SessionStart</code> still ingests opportunistically, and <code>POST /api/plans/refresh</code> forces one)":
      "<code>AGENT-PLAN.md</code> 감시기의 폴링 간격 — 변경된 계획 파일은 다시 수집되어 <code>plan_updated</code>로 브로드캐스트됩니다; <code>0</code>은 폴링을 비활성화합니다(<code>SessionStart</code>는 여전히 기회적으로 수집하며, <code>POST /api/plans/refresh</code>로 강제 실행할 수 있습니다)",
    'A report icon next to each project\'s "view plan" icon (Projects page and Kanban\'s Projects view) opens a <strong>focus-time report</strong> scoped to that project: how long its sessions actually spent on a declared item versus a detour, a feature aside, or a bug fix — reconstructed from existing <code>Focus</code> event history, not a new capture mechanism. A configurable idle-grace window (<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>, default 5 min) discounts stretches with no hook activity past the window, so a session sitting in Waiting for hours doesn\'t inflate its "time on item" figure. The popup shows active/idle stat tiles, a per-session segmented timeline, a per-item rollup (detours bucketed under the item they interrupted), and a project-wide split by kind.':
      '각 프로젝트의 "플랜 보기" 아이콘 옆에 있는 리포트 아이콘(Projects 페이지 및 Kanban의 Projects 뷰)을 클릭하면 해당 프로젝트로 범위가 지정된 <strong>포커스 시간 리포트</strong>가 열립니다: 그 프로젝트의 세션들이 선언된 항목에 실제로 얼마나 시간을 썼는지를 우회 작업, 기능 곁가지, 버그 수정과 비교해서 보여주며 — 새로운 수집 메커니즘이 아니라 기존 <code>Focus</code> 이벤트 기록으로부터 재구성됩니다. 설정 가능한 유휴 유예 구간(<code>DASHBOARD_FOCUS_IDLE_GRACE_SECONDS</code>, 기본값 5분)은 그 구간을 넘어서는 Hook 활동 없는 구간을 할인 처리하므로, 몇 시간 동안 Waiting 상태로 머무는 세션이 "항목에 소요한 시간" 수치를 부풀리지 않습니다. 팝업은 활성/유휴 통계 타일, 세션별 세그먼트 타임라인, 항목별 롤업(우회 작업은 그것이 가로챈 항목 아래에 버킷으로 묶임), 그리고 종류별 프로젝트 전체 분할을 보여줍니다.',
    "Tick interval for the focus drift audit — the background check that a focused session's recent activity matches its declared plan item; <code>0</code> disables auditing entirely":
      "포커스 드리프트 감사의 틱 간격 — 포커스된 세션의 최근 활동이 선언된 계획 항목과 일치하는지 확인하는 백그라운드 검사입니다; <code>0</code>은 감사를 완전히 비활성화합니다",
    "Idle-grace window for the per-project focus-time report — a gap between two consecutive hook events longer than this only counts the grace window's worth as active, the rest is excluded as idle; <code>0</code> disables discounting (100% of wall-clock counts as active)":
      "프로젝트별 포커스 시간 리포트를 위한 유휴 유예 구간 — 연속된 두 Hook 이벤트 사이의 간격이 이 값보다 길면 유예 구간만큼만 활성으로 계산되고, 나머지는 유휴로 제외됩니다; <code>0</code>은 할인 처리를 비활성화합니다(경과 시간의 100%가 활성으로 계산됨)",
    "Drift-audit judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap only), or <code>off</code>":
      "드리프트 감사의 판정 방식: <code>llm</code>(소형 모델에서 헤드리스 <code>claude -p</code>, CLI를 사용할 수 없으면 휴리스틱으로 폴백), <code>heuristic</code>(키워드 중복만), 또는 <code>off</code>",
    "Model passed to the drift audit's <code>claude -p --model</code> spawn":
      "드리프트 감사의 <code>claude -p --model</code> 스폰에 전달되는 모델",
    "Kill timer for a single drift-audit <code>claude -p</code> spawn (SIGTERM, then SIGKILL)":
      "단일 드리프트 감사 <code>claude -p</code> 스폰의 종료 타이머(SIGTERM 후 SIGKILL)",
    "Sessions that never declared a <code>ccam focus</code> don't punch silent holes in that report: a background <strong>focus inference</strong> job (<code>DASHBOARD_FOCUS_INFER_*</code>) digests each untracked session's activity and attributes it to the best-matching plan item or an inferred detour, storing one verdict per session in the <code>focus_inferences</code> table. Segments it produces carry <code>inferred: true</code> and render with an \"≈ inferred\" chip, so guessed time is never dressed up as declared time. Declared focus history always wins — inference only fills sessions with no declarations at all — and a session with neither a declaration nor a confident verdict yet (most often one that's still running and hasn't gone quiet long enough for a tick to reach it) still shows up as a neutral gray \"No focus\" block instead of a silent unclassified hole.":
      '<code>ccam focus</code>를 한 번도 선언하지 않은 세션이 그 리포트에 조용한 구멍을 남기지 않습니다: 백그라운드 <strong>포커스 추론</strong> 작업(<code>DASHBOARD_FOCUS_INFER_*</code>)이 추적되지 않은 각 세션의 활동을 요약해 가장 잘 맞는 계획 항목 또는 추론된 우회 작업으로 귀속시키고, 세션당 하나의 판정을 <code>focus_inferences</code> 테이블에 저장합니다. 이 작업이 만들어내는 세그먼트는 <code>inferred: true</code>를 지니고 "≈ inferred" 칩과 함께 렌더링되므로, 추측된 시간이 선언된 시간처럼 포장되는 일은 없습니다. 선언된 포커스 기록이 항상 우선하며 — 추론은 선언이 전혀 없는 세션만 채웁니다 — 선언도 없고 아직 확신할 만한 판정도 없는 세션(가장 흔하게는 아직 실행 중이며 틱이 도달할 만큼 충분히 조용해지지 않은 세션)은 조용한 미분류 구멍 대신 중립적인 회색 "No focus" 블록으로 표시됩니다.',
    "A header toggle in the focus-time report switches between the List view described above and a <strong>Calendar</strong> view — a day-view swimlane calendar that positions every session's segments on a real 24-hour axis, with sessions that overlap in time split into side-by-side lanes via a greedy lane-assignment algorithm, the same way a Teams or Outlook calendar day view spreads concurrent meetings instead of stacking them. Dashed-border blocks mark inferred/undeclared attribution and solid borders mark declared focus, matching the List view's own convention; a pulsing, open-ended block marks a session that's still actively running, using the API's <code>ended_at: null</code> field to tell a genuinely live session from one that only just ended. Prev/Today/Next controls step through days — this v1 ships Day view and that simple navigation only; Week/Month zoom and an aggregate time-range selector (3d/7d/30d/All) are future work. The report's stat tiles also gained a <strong>Concurrency</strong> tile (e.g. \"1.08x\", effort time ÷ wall-clock time), and the \"Total agent time\" tile's wall-clock sub-label now reflects the real union-based wall-clock span instead of a misleading per-segment sum, so effort time (which inflates with overlapping sessions) and true wall-clock time read as two distinct, honest numbers. The Concurrency tile now also carries a \"while active\" sub-figure (<code>active_concurrency_ratio</code>) that divides effort by only the time at least one session was actually active — so sessions left open but idle (overnight, say) no longer dilute the parallelism reading — and a small swap button that inverts which of the two ratios is the tile's primary figure, a per-browser choice that persists across reloads.":
      '포커스 시간 리포트 헤더의 토글은 위에서 설명한 List 보기와 새로운 <strong>Calendar</strong> 보기를 전환합니다 — 각 세션의 시간 구간을 실제 24시간 축 위에 배치하는 일 단위 스윔레인 캘린더로, 시간이 겹치는 세션은 그리디 레인 배정 알고리즘을 통해 나란히 놓인 레인으로 분리됩니다. Teams나 Outlook 캘린더의 일 보기가 겹치는 회의를 쌓지 않고 나란히 표시하는 방식과 같습니다. 점선 테두리 블록은 추론된/선언되지 않은 귀속을 나타내고 실선 테두리는 선언된 포커스를 나타내며, List 보기 자체의 규칙과 일치합니다; 깜빡이는 열린 끝 블록은 실제로 아직 실행 중인 세션을 나타내며, API의 <code>ended_at: null</code> 필드를 사용해 실제로 살아있는 세션과 방금 종료된 세션을 구분합니다. Prev/Today/Next 컨트롤은 하루씩 이동합니다 — 이 v1은 Day 보기와 이 단순한 탐색만 제공합니다; Week/Month 확대와 집계 기간 선택기(3d/7d/30d/All)는 향후 작업입니다. 리포트의 통계 타일에는 <strong>Concurrency</strong>(동시성) 타일도 추가되었습니다(예: "1.08x", 즉 노력 시간 ÷ 경과 시간). 또한 "총 에이전트 시간" 타일의 경과 시간 보조 라벨은 이제 오해의 소지가 있는 구간별 합산이 아니라 실제 합집합 기반 경과 시간을 반영합니다. 그 결과 노력 시간(세션이 겹칠 때 부풀려짐)과 진짜 경과 시간이 서로 다른, 정직한 두 숫자로 나타납니다. Concurrency 타일에는 이제 "활동 중" 보조 수치(<code>active_concurrency_ratio</code>)도 표시됩니다. 이는 하나 이상의 세션이 실제로 활동하던 시간만으로 노력 시간을 나눈 값이라, 켜 둔 채 방치된 세션(예: 밤새 열어 둔 세션)이 더 이상 병렬성 수치를 희석하지 않습니다. 타일의 작은 전환 버튼으로 두 비율 중 어느 것을 주 수치로 표시할지 뒤바꿀 수 있으며, 이 선택은 브라우저별로 저장되어 새로 고침 후에도 유지됩니다.',
    "Tick interval for background focus inference — the classifier that attributes sessions which never declared a <code>ccam focus</code> to a plan item or an inferred detour (first backfill tick ~30 s after boot); <code>0</code> disables inference entirely":
      "백그라운드 포커스 추론의 틱 간격 — <code>ccam focus</code>를 한 번도 선언하지 않은 세션을 계획 항목이나 추론된 우회 작업으로 귀속시키는 분류기입니다(첫 백필 틱은 부팅 후 약 30초); <code>0</code>은 추론을 완전히 비활성화합니다",
    "Focus-inference judge: <code>llm</code> (headless <code>claude -p</code> on a small model, falling back to the heuristic when the CLI is unavailable), <code>heuristic</code> (keyword overlap that only ever claims an item match), or <code>off</code>":
      "포커스 추론의 판정 방식: <code>llm</code>(소형 모델에서 헤드리스 <code>claude -p</code>, CLI를 사용할 수 없으면 휴리스틱으로 폴백), <code>heuristic</code>(키워드 중복, 항목 일치만 주장), 또는 <code>off</code>",
    "Model passed to the focus-inference <code>claude -p --model</code> spawn":
      "포커스 추론의 <code>claude -p --model</code> 스폰에 전달되는 모델",
    "Kill timer for a single focus-inference <code>claude -p</code> spawn (SIGTERM, then SIGKILL)":
      "단일 포커스 추론 <code>claude -p</code> 스폰의 종료 타이머(SIGTERM 후 SIGKILL)",
    "<code>ccam focus …</code> run in the session's Bash tool":
      "세션의 Bash 도구에서 <code>ccam focus …</code> 실행",
    "Parsed out of the <code>PostToolUse</code> hook's <code>tool_input.command</code> (the only channel that natively carries the session id). <code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code> update the session's <code>session_focus</code> row (plan-item pointer plus detour stack) and write a <code>Focus</code> event with an item-text snapshot for the timeline; broadcasts <code>session_focus</code> (and <code>plan_updated</code> after <code>done</code>). <code>bug</code>/<code>feature</code> additionally carry <code>kind</code>/<code>title</code>/<code>detail</code> on the pushed detour frame, badged in the Plan view. <code>status</code> is read-only and records nothing.":
      "<code>PostToolUse</code> 훅의 <code>tool_input.command</code>에서 파싱됩니다(세션 id를 기본으로 담고 있는 유일한 채널입니다). <code>set</code>/<code>push</code>/<code>pop</code>/<code>done</code>/<code>bug</code>/<code>feature</code>는 세션의 <code>session_focus</code> 행(계획 항목 포인터와 우회 스택)을 갱신하고 타임라인용 항목 텍스트 스냅샷이 담긴 <code>Focus</code> 이벤트를 기록합니다; <code>session_focus</code>를 브로드캐스트합니다(<code>done</code> 이후에는 <code>plan_updated</code>도). <code>bug</code>/<code>feature</code>는 추가로 푸시된 우회 프레임에 <code>kind</code>/<code>title</code>/<code>detail</code>을 함께 담아 Plan 화면에 배지로 표시합니다. <code>status</code>는 읽기 전용이며 아무것도 기록하지 않습니다.",
    "CCAM exposes a read-only <strong>Prometheus text-exposition</strong> endpoint at <code>GET /api/metrics</code> — sessions and agents by status, cumulative events and tokens, connected WebSocket clients, configured remote sources, process uptime/RSS, and build version — so the same dashboard you use for Claude Code monitoring can be scraped into your own observability stack. A turnkey stack lives in <code>monitoring/</code>: npm-managed binaries (no Homebrew/apt) or optional Docker Compose, with Grafana datasource and four dashboards auto-provisioned on first start.":
      "CCAM은 <code>GET /api/metrics</code>에서 읽기 전용 <strong>Prometheus 텍스트 노출</strong> 엔드포인트를 제공합니다 — 상태별 sessions/agents, 누적 이벤트·토큰, 연결된 WebSocket 클라이언트, 구성된 원격 소스, 프로세스 uptime/RSS, 빌드 버전 — Claude Code 모니터링에 쓰는 동일 대시보드를 자체 관측성 스택으로 스크레이핑할 수 있습니다. 턴키 스택은 <code>monitoring/</code>에 있으며 npm 관리 바이너리(Homebrew/apt 불필요) 또는 선택적 Docker Compose로 첫 시작 시 Grafana 데이터소스와 네 개 대시보드를 자동 프로비저닝합니다.",
    "<strong>Quick start (npm):</strong> run the dashboard on loopback, then <code>npm run monitoring:install</code> once and <code>npm run monitoring:up</code>. Open Grafana at <code>http://localhost:3000</code> and Prometheus at <code>http://localhost:9090</code>. The pre-built CCAM console is at <code>http://localhost:9090/consoles/index.html</code> (also reachable from the Prometheus UI under <strong>Consoles</strong>). For an all-Docker path use <code>npm run monitoring:docker:up</code> or <code>npm run docker:full:up</code> for dashboard + Prometheus + Grafana together. Verify with <code>npm run monitoring:verify</code> — the <code>ccam</code> scrape target should read <strong>UP</strong>.":
      "<strong>빠른 시작(npm):</strong> 루프백에서 대시보드를 실행한 뒤 <code>npm run monitoring:install</code> 한 번과 <code>npm run monitoring:up</code>. Grafana <code>http://localhost:3000</code>, Prometheus <code>http://localhost:9090</code>. 사전 구축 CCAM 콘솔: <code>http://localhost:9090/consoles/index.html</code>(Prometheus UI <strong>Consoles</strong>에서도 가능). 전체 Docker: <code>npm run monitoring:docker:up</code> 또는 <code>npm run docker:full:up</code>. <code>npm run monitoring:verify</code>로 확인 — <code>ccam</code> 스크레이프 대상이 <strong>UP</strong>이어야 합니다.",
    "Four Grafana boards": "Grafana 보드 네 개",
    "Prometheus console": "Prometheus 콘솔",
    "Prometheus 3.x dropped the legacy console template libraries, so CCAM ships a <strong>static HTML console</strong> at <code>monitoring/prometheus/consoles/index.html</code> that fetches <code>/api/v1/query</code> directly — live cards, session tables, and Graph links without Go-template errors on <code>/consoles/index.html</code>.":
      "Prometheus 3.x는 레거시 콘솔 템플릿 라이브러리를 제거했으므로 CCAM은 <code>monitoring/prometheus/consoles/index.html</code>에 <code>/api/v1/query</code>를 직접 호출하는 <strong>정적 HTML 콘솔</strong>을 제공합니다 — 실시간 카드, 세션 표, Graph 링크, <code>/consoles/index.html</code> Go 템플릿 오류 없음.",
    "Recording rules": "기록 규칙",
    "Scrape &amp; auth guard": "스크레이프 및 인증 가드",
    "Loopback scrapes (<code>127.0.0.1:4820</code>) work out of the box. When Prometheus runs in Docker and reaches the host via <code>host.docker.internal</code>, add that host to <code>DASHBOARD_ALLOWED_HOSTS</code> or the scrape returns <code>403 EBADHOST</code>. If <code>DASHBOARD_TOKEN</code> is set, scrapers must send the same bearer token.":
      "루프백 스크레이프(<code>127.0.0.1:4820</code>)는 즉시 동작합니다. Docker에서 Prometheus가 <code>host.docker.internal</code>로 호스트에 접근할 때 해당 호스트를 <code>DASHBOARD_ALLOWED_HOSTS</code>에 추가하지 않으면 <code>403 EBADHOST</code>가 반환됩니다. <code>DASHBOARD_TOKEN</code>이 설정되면 스크레이퍼가 동일한 bearer 토큰을 보내야 합니다.",
    "Full reference": "전체 참고",
    'Commands, Docker vs npm paths, dashboard UIDs, starter PromQL, and troubleshooting (Grafana login reset, target DOWN) are documented in <a href="../monitoring/README.md"><code>monitoring/README.md</code></a> and the <a href="#api-reference">API reference</a> entry for <code>GET /api/metrics</code>.':
      '명령, Docker vs npm 경로, 대시보드 UID, 시작 PromQL, 문제 해결(Grafana 로그인 재설정, target DOWN)은 <a href="../monitoring/README.md"><code>monitoring/README.md</code></a>와 <code>GET /api/metrics</code>에 대한 <a href="#api-reference">API reference</a> 항목에 문서화되어 있습니다.',
    "Pull Claude Code history from other machines over SSH. Each remote's <code>~/.claude/projects</code> tree is rsynced into a sandboxed staging directory, fed through the same importer as local history, and every session is tagged with its source. A background poller keeps remote data near-real-time, a global data-scope selector (Local only / All / a specific source) narrows the whole app, and sessions from a remote show a source badge. Auth defers entirely to the host's SSH stack — no passwords or secrets are stored. Managed in <b>Settings → Remote Data Sources</b>.":
    ...
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it down to just its title (columns stay mounted, only hidden, so an in-flight drag or a card's own \"show more\" state survives the toggle) — handy for tucking away a monitor you're not looking at right now.":
      '<strong>Agents</strong>(작업 중 / 대기 중 / 완료 / 오류)와 <strong>Sessions</strong>(활성 / 대기 중 / 완료 / 오류 / 중단됨) 스윔레인 사이를 전환할 수 있습니다. 노란색 <strong>Waiting</strong> 열은 새 프롬프트, 턴 사이, 또는 권한 게이트 등 사용자 입력을 기다리는 항목을 표시합니다. 열 제목에 마우스를 올리면 각 상태 전환을 설명하는 라이프사이클 툴팁이 표시됩니다. 카드에는 모델 이름, 누적 비용, 현재 호출 중인 도구가 표시됩니다. 프리미엄/고비용 모델 — Opus 또는 Fable — 을 사용 중일 때는 모델 칩이 밝은 빨간색으로 표시되어 비용이 많이 드는 세션을 한눈에 알아볼 수 있습니다. 개수는 WebSocket을 통해 실시간으로 업데이트되므로 보드는 항상 실시간 이벤트 저장소와 동기화된 상태를 유지합니다. Waiting 배지에 마우스를 올리면 이유를 확인할 수 있습니다 — 입력 필요, 턴 완료, 프롬프트 대기, 또는 중단됨. 세 번째 <strong>Projects</strong> 보기는 상태 대신 프로젝트별로 세션을 그룹화합니다. 이름이 있는 <strong>monitor</strong>(모니터) 그룹을 하나 이상 만들면 배정된 프로젝트 컬럼은 해당 모니터 전용의 테두리 있는, 드래그로 재배치 가능한 박스 안에 렌더링되며, 이 박스들은 같은 행 안에서 서로 나란히 놓입니다 — 박스를(헤더를 통해) 다른 박스 위로 드래그하면 위치가 바뀝니다. 다중 모니터 데스크 배치를 그대로 옮기기에 유용하며, <code>localStorage</code>에만 저장됩니다. 각 박스의 헤더에 있는 셰브론은 그것을 제목만 남도록 접습니다(컬럼은 계속 마운트된 상태로 유지되며 단지 숨겨질 뿐이므로, 진행 중인 드래그나 카드 자체의 "더 보기" 상태가 전환 후에도 그대로 유지됩니다) — 지금 보고 있지 않은 모니터를 치워두기에 유용합니다.',
    "<strong>Server-paginated</strong> table of every recorded session — each page fetches only its slice so cost computation stays bounded no matter how many sessions exist. Case-insensitive search across <code>id</code>, <code>name</code>, and <code>cwd</code> runs server-side with a 300 ms debounce; the status filter composes with search for precise narrowing. Each row shows the session's real name (synced live from the transcript — a <code>/rename</code> or <code>claude -n</code> title, else the auto title, else the first user prompt, with a short-ID fallback), status badge, agent count, duration, model, and estimated cost. Click any row to drill into the full session detail view with conversation transcript and agent hierarchy.":
    ...
    "Per-session deep dive with a collapsible agent hierarchy tree and a full chronological event timeline showing every tool call name and summary. An overview panel at the top surfaces tile counters for events, tool calls, subagents, compactions, errors, and duration. Top-tool usage bars and a subagent type breakdown give quick distribution reads. The conversation viewer renders markdown with syntax highlighting, per-tool styled blocks, slash-command pills with their captured TUI output, and inline session-rename markers. Messages typed mid-turn (queued while Claude was still working) appear where Claude actually received them, and harness notifications are attributed to System. Export the entire session as JSON or share the permalink for async review. When the session is waiting on you, a yellow banner under the header names the reason and how long it has been waiting.":
      "접을 수 있는 에이전트 계층 트리와 모든 도구 호출 이름 및 요약을 보여주는 전체 시간순 이벤트 타임라인을 통해 세션별로 깊이 있게 살펴볼 수 있습니다. 상단의 개요 패널은 이벤트, 도구 호출, 서브에이전트, 컴팩션, 오류, 지속 시간에 대한 타일 카운터를 표시합니다. 상위 도구 사용 막대그래프와 서브에이전트 유형 분석을 통해 분포를 빠르게 파악할 수 있습니다. 대화 뷰어는 구문 강조가 적용된 마크다운, 도구별 스타일이 지정된 블록, 캡처된 TUI 출력이 포함된 슬래시 명령 필, 인라인 세션 이름 변경 마커를 렌더링합니다. 전체 세션을 JSON으로 내보내거나 비동기 검토를 위해 고유 링크(permalink)를 공유할 수 있습니다. Khi phiên đang chờ bạn, một banner màu vàng dưới tiêu đề sẽ nêu rõ lý do và thời gian đã chờ.",
    "A rules-based alerting engine evaluates the live event stream server-side: <strong>event pattern</strong> (match event type / tool / summary text, optionally N matches within a time window), <strong>inactivity</strong>, <strong>stuck agent</strong>, and <strong>token threshold</strong> — each with per-(rule, session, agent) cooldown dedup. Fired alerts surface in a live feed and fan out to <strong>14 first-class webhook providers</strong> — Slack, Discord, Teams, Google Chat, Mattermost, Rocket.Chat, Telegram, PagerDuty, Opsgenie, Splunk On-Call, Zapier, Make, n8n, Pipedream — plus any generic JSON endpoint (with optional HMAC-SHA256 signing and custom headers). Delivery is detached and fail-safe with a request timeout, bounded retry/backoff, secret redaction, a one-click test probe, and a per-target delivery log. Rules and channels are managed together in <strong>Settings → Alerts</strong>.":
    ...
    "Creates session and main agent. Stamps <code>awaiting_input_since</code> (with <code>awaiting_reason</code> = <code>session_start</code>) so the row lands in <strong>Waiting</strong> from the start (the CLI is at a prompt) — except a <code>compact</code>-source SessionStart (mid-turn auto-compaction), which leaves the flag untouched so a working session stays <strong>Active</strong>. Reactivates resumed sessions. Abandons orphaned sessions with no activity for <code>DASHBOARD_STALE_MINUTES</code> (default 180).":
      "세션과 메인 에이전트를 생성합니다. CLI가 프롬프트 상태이므로 처음부터 <strong>Waiting</strong>에 위치하도록 <code>awaiting_input_since</code>를(<code>awaiting_reason</code> = <code>session_start</code>와 함께) 기록합니다. 단, <code>compact</code> 소스 SessionStart(턴 도중 자동 압축)는 플래그를 그대로 두어 작업 중인 세션이 <strong>Active</strong> 상태를 유지합니다. 재개된 세션을 다시 활성화합니다. <code>DASHBOARD_STALE_MINUTES</code>(기본값 180) 동안 활동이 없는 고아 세션은 중단 처리합니다.",
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span > until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >. Background subagents keep running.':
      '오류가 아닌 경우: 메인 에이전트 → <code>waiting</code> — 다음 사용자 입력이 있을 때까지 UI에 <span class="status-chip chip-waiting" ><span class="chip-dot"></span>Waiting</span >이 표시됩니다. 세션의 대기(Waiting) 플래그(<code>awaiting_input_since</code>)는 아직 작업 중인 서브에이전트가 없을 때에만 기록됩니다 — 서브에이전트가 실행 중이면 세션은 <strong>Active</strong> 상태를 유지하고(사용자가 아니라 자신의 에이전트를 기다리는 것이므로) 기록은 마지막 <code>SubagentStop</code>으로 미뤄집니다. <code>stop_reason=error</code>인 경우: 에이전트와 세션을 <span class="status-chip chip-error" ><span class="chip-dot"></span>Error</span >로 표시합니다. 백그라운드 서브에이전트는 계속 실행됩니다.',
    "Background agent finished": "백그라운드 에이전트 완료",
    "Matched subagent → <span class=\"status-chip chip-completed\" ><span class=\"chip-dot\"></span>Completed</span >. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      '일치하는 서브에이전트 → <span class="status-chip chip-completed" ><span class="chip-dot"></span>Completed</span >. 대기 플래그는 의도적으로 지우지 <strong>않습니다</strong> — 백그라운드 서브에이전트가 완료되었다는 사실은 사용자에 대해 아무것도 알려주지 않기 때문입니다. 다만 플래그를 <em>설정</em>할 수는 있습니다: 미뤄진 비오류 <code>Stop</code> 이후 마지막으로 작업 중이던 서브에이전트가 완료되면, 드레인 검사가 <code>stop</code> 대기 플래그를 기록하고 전체 에이전트 플릿이 끝나는 바로 그 시점에 세션이 <strong>Active</strong> → <strong>Waiting</strong>으로 전환됩니다. 또한 세션의 <code>subagents/agent-*.jsonl</code> 파일을 순회하며 <code>tool_use_id</code>로 <code>tool_use</code>와 <code>tool_result</code> 블록을 짝짓고, 각 서브에이전트 고유의 <code>agent_id</code> 아래에 도구별 <code>PreToolUse</code> + <code>PostToolUse</code> 이벤트를 발생시키는 fire-and-forget JSONL 스캔(<code>scanAndImportSubagents</code>)도 시작됩니다 — 이를 통해 서브에이전트가 내부적으로 수행하지만 어떤 훅도 발생시키지 않는 도구 호출을 드러냅니다. 같은 스캔은 중첩된 서브에이전트 계층도 재구성합니다 — 트랜스크립트의 Task 도구 결과(<code>toolUseResult.agentId</code>)에서 복구한 실제 생성자로 각 서브에이전트의 <code>parent_agent_id</code>를 다시 지정하므로, 자체적으로 서브에이전트를 생성하는 서브에이전트도 메인 아래로 평탄화되지 않고 올바르게 중첩됩니다.',
    ...
    "Build <strong>both</strong> per-arch DMGs (arm64 + x64). Correct for release — slower (packages each arch)":
      "<strong>두 개</strong>의 아키텍처별 DMG(arm64 + x64)를 모두 빌드합니다. 릴리스에 적합하며, 더 느립니다(각 아키텍처를 개별 패키징)",
    "Build an Apple-Silicon-only DMG — fast (~1 min), recommended for a single machine":
    ...
    "Tabby also watches the clock, not just events: if a live session has been running an <strong>expensive model</strong> — Opus or Fable — continuously for more than <strong>15 minutes</strong>, it surfaces a one-time nag bubble asking if you're sure you want to stay there. The nag fires once per streak and only re-arms if the session drops off that model and comes back onto it later.":
      "Tabby는 이벤트뿐 아니라 시계도 지켜봅니다. 실행 중인 세션이 <strong>고비용 모델</strong>—Opus 또는 Fable—을 <strong>15분</strong> 넘게 계속 사용하고 있으면, 정말 그 모델에 계속 머물 것인지 묻는 일회성 알림 말풍선을 띄웁니다. 이 알림은 한 번의 연속 사용 구간마다 한 번만 발생하며, 세션이 해당 모델에서 벗어났다가 나중에 다시 돌아왔을 때만 다시 활성화됩니다.",
    "Click the cat — or press <code>⌘B</code> / <code>Ctrl+B</code> — to open Tabby's panel (<code>Esc</code> closes it). The panel groups a live status line, quick actions, and an Ask box.":
    ...
    'Non-error: main agent → <code>waiting</code> — UI shows <span class="status-chip chip-waiting"><span class="chip-dot"></span>Waiting</span> until the next user input. The session\'s Waiting flag (<code>awaiting_input_since</code>) is stamped only when no subagent is still working — with a subagent running the session stays <strong>Active</strong> (it is waiting on its agents, not the user) and the stamp is deferred to the last <code>SubagentStop</code>. <code>stop_reason=error</code>: marks the agent and session <span class="status-chip chip-error"><span class="chip-dot"></span>Error</span>. Background subagents keep running.':
      '오류가 아닌 경우: 메인 에이전트가 <code>waiting</code> 상태가 되며, UI는 다음 사용자 입력이 있을 때까지 <span class="status-chip chip-waiting"><span class="chip-dot"></span>Waiting</span>을 표시합니다. 세션의 대기(Waiting) 플래그(<code>awaiting_input_since</code>)는 아직 작업 중인 서브에이전트가 없을 때에만 기록됩니다 — 서브에이전트가 실행 중이면 세션은 <strong>Active</strong> 상태를 유지하고(사용자가 아니라 자신의 에이전트를 기다리는 것이므로) 기록은 마지막 <code>SubagentStop</code>으로 미뤄집니다. <code>stop_reason=error</code>인 경우: 에이전트와 세션을 <span class="status-chip chip-error"><span class="chip-dot"></span>Error</span>로 표시합니다. 백그라운드 서브에이전트는 계속 실행됩니다.',
    "Matched subagent → <span class=\"status-chip chip-completed\"><span class=\"chip-dot\"></span>Completed</span>. Deliberately does <strong>not</strong> clear the waiting flag — a backgrounded subagent finishing tells us nothing about the human. It can, however, <em>set</em> it: when the last working subagent finishes after a deferred non-error <code>Stop</code>, the drain check stamps the <code>stop</code> waiting flag and the session flips <strong>Active</strong> → <strong>Waiting</strong> exactly when the whole fleet is done. Also kicks off a fire-and-forget JSONL scan (<code>scanAndImportSubagents</code>) that walks the session's <code>subagents/agent-*.jsonl</code> files, pairs <code>tool_use</code> ↔ <code>tool_result</code> blocks by <code>tool_use_id</code>, and emits per-tool <code>PreToolUse</code> + <code>PostToolUse</code> events under each subagent's own <code>agent_id</code> — surfaces tool calls that subagents make internally and which never fire any hooks. The same scan also rebuilds the nested-subagent hierarchy — it repoints each subagent's <code>parent_agent_id</code> to the true spawner recovered from the transcript's Task tool result (<code>toolUseResult.agentId</code>), so subagents that spawn their own subagents nest correctly instead of flattening under main.":
      '일치하는 서브에이전트가 <span class="status-chip chip-completed"><span class="chip-dot"></span>Completed</span> 상태가 됩니다. 대기 플래그는 의도적으로 <strong>해제하지 않습니다</strong> — 백그라운드 서브에이전트의 완료는 사용자(human)에 대해 아무것도 알려주지 않기 때문입니다. 다만 플래그를 <em>설정</em>할 수는 있습니다: 미뤄진 비오류 <code>Stop</code> 이후 마지막으로 작업 중이던 서브에이전트가 완료되면, 드레인 검사가 <code>stop</code> 대기 플래그를 기록하고 전체 에이전트 플릿이 끝나는 바로 그 시점에 세션이 <strong>Active</strong> → <strong>Waiting</strong>으로 전환됩니다. 또한 세션의 <code>subagents/agent-*.jsonl</code> 파일을 순회하며 <code>tool_use_id</code>로 <code>tool_use</code> ↔ <code>tool_result</code> 블록을 짝짓고, 각 서브에이전트 고유의 <code>agent_id</code> 아래에 도구별 <code>PreToolUse</code> + <code>PostToolUse</code> 이벤트를 발생시키는 fire-and-forget 방식의 JSONL 스캔(<code>scanAndImportSubagents</code>)도 시작합니다 — 서브에이전트가 내부적으로 수행하며 훅을 전혀 발생시키지 않는 도구 호출을 드러냅니다. 같은 스캔은 중첩된 서브에이전트 계층도 재구성합니다 — 트랜스크립트의 Task 도구 결과(<code>toolUseResult.agentId</code>)에서 복구한 실제 생성자로 각 서브에이전트의 <code>parent_agent_id</code>를 다시 지정하므로, 자체적으로 서브에이전트를 생성하는 서브에이전트도 메인 아래로 평탄화되지 않고 올바르게 중첩됩니다.',
    'Creates a compaction subagent → <span class="status-chip chip-completed"><span class="chip-dot"></span>Completed</span>. Detected via <code>isCompactSummary</code> entries in the transcript. Token baselines preserve pre-compaction totals. Periodic scanner (cadence ~¼ of <code>DASHBOARD_STALE_MINUTES</code>) catches compactions when no hooks fire.':
    ...
    "Every Agents/Sessions status column gets its own layout toggle too — the same icon next to its count badge — switching just that column's cards between the default vertical stack and a horizontally-scrolling row. Unlike the monitor toggle above, this one is saved per-browser only, and each status column's setting is independent of the others.":
      "Agents/Sessions 상태 컬럼에도 자체 레이아웃 토글이 있습니다 — 개수 배지 옆의 동일한 아이콘으로, 해당 컬럼의 카드만 기본값인 세로 스택과 가로로 스크롤되는 행 사이에서 전환합니다. 위의 모니터 토글과 달리 이 토글은 브라우저별로만 저장되며, 각 상태 컬럼의 설정은 다른 컬럼과 독립적입니다.",
    "Toggle between <strong>Agents</strong> (Working / Waiting / Completed / Error) and <strong>Sessions</strong> (Active / Waiting / Completed / Error / Abandoned) swim lanes. A yellow <strong>Waiting</strong> column flags items sitting on the user — fresh prompt, between turns, or permission gate. Hover any column header for lifecycle tooltips explaining each state transition. Cards surface model name, cumulative cost, and the current tool being called. The model chip renders in <strong>bright red</strong> for premium/expensive models — Opus or Fable — flagging cost-heavy sessions at a glance. Session cards also show a compact <strong>input / output / cache-read / cache-write</strong> token strip with a cost-weighted <strong>Effective</strong> total, once the session has used any tokens — cache reads and writes are split into separate figures (reads bill at ~10% of the input rate, writes at a premium), and Effective weighs each category by the model's actual pricing rates so it tracks the $ cost instead of overstating cache-heavy sessions. Counts update in real time via WebSocket so the board is always in sync with the live event store. Hover a Waiting badge to see why — needs input, turn done, at prompt, or interrupted. A third <strong>Projects</strong> view groups sessions by project instead of status; once at least one named <strong>monitor</strong> group is created, its assigned project columns render inside a bordered, drag-reorderable box for that monitor, sitting side by side with the other boxes in that same row — drag a box by its header onto another to reposition it, handy for mirroring a multi-monitor desk layout, stored only in <code>localStorage</code>. A chevron on each box's header collapses it — instead of just shrinking in place, the whole box moves out of that row into its own thin strip above it, shared by every other collapsed monitor, freeing the horizontal space it would otherwise still take up; expanding it drops the box back into the row at its ordered position (its columns stay mounted the whole time, only hidden, so a card's own \"show more\" state survives the toggle). A second icon button next to the chevron toggles that box's own layout direction — <strong>horizontal</strong> (the default, columns side by side) or <strong>vertical</strong> (columns stacked top to bottom) — independently per monitor. Every project column (and the standalone Unassigned column) has its own chevron too, shrinking that one column down to a slim header-only bar right where it sits, independent of whichever box it's in.":
      '<strong>Agents</strong>(Working / Waiting / Completed / Error)와 <strong>Sessions</strong>(Active / Waiting / Completed / Error / Abandoned) 스윔레인 사이를 전환합니다. 노란색 <strong>Waiting</strong> 컬럼은 사용자를 기다리고 있는 항목을 표시합니다 — 새 프롬프트, 턴 사이, 또는 권한 게이트. 아무 컬럼 헤더에나 마우스를 올리면 각 상태 전환을 설명하는 라이프사이클 툴팁이 나타납니다. 카드는 모델 이름, 누적 비용, 현재 호출 중인 도구를 보여줍니다. 프리미엄/고비용 모델 — Opus 또는 Fable — 을 실행 중이면 모델 칩이 <strong>밝은 빨간색</strong>으로 표시되어 비용이 큰 세션을 한눈에 알아볼 수 있습니다. 세션이 토큰을 하나라도 사용한 뒤에는 세션 카드에 <strong>입력 / 출력 / 캐시 읽기 / 캐시 쓰기</strong> 토큰을 보여주는 컴팩트한 막대가 비용 가중 <strong>Effective(실효)</strong> 합계와 함께 추가로 표시됩니다 — 캐시 읽기와 쓰기는 별도의 수치로 분리되고(읽기는 입력 요금의 약 10%, 쓰기는 프리미엄 요금), Effective는 각 항목을 해당 모델의 실제 요금으로 가중하므로 캐시 비중이 큰 세션을 과장하지 않고 $ 비용을 그대로 따라갑니다. 개수는 WebSocket을 통해 실시간으로 업데이트되므로 보드는 항상 실시간 이벤트 저장소와 동기화된 상태를 유지합니다. Waiting 배지에 마우스를 올리면 이유를 확인할 수 있습니다 — 입력 필요, 턴 완료, 프롬프트 대기, 또는 중단됨. 세 번째 <strong>Projects</strong> 뷰는 상태 대신 프로젝트별로 세션을 그룹화합니다. 이름이 지정된 <strong>monitor</strong> 그룹을 하나 이상 만들면, 거기에 배정된 프로젝트 컬럼은 그 모니터 전용의 테두리 있는 드래그 가능한 박스 안에 렌더링되어 같은 행 안에서 다른 박스들과 나란히 놓입니다 — 박스를 헤더로 드래그해 다른 박스 위에 놓으면 위치를 바꿀 수 있어 여러 모니터로 이루어진 책상 배치를 그대로 반영하기 좋으며, 이 설정은 <code>localStorage</code>에만 저장됩니다. 각 박스 헤더의 셰브론을 클릭하면 접을 수 있습니다 — 제자리에서 줄어드는 대신 박스 전체가 그 행에서 완전히 빠져나와, 접힌 다른 모든 모니터와 공유하는 위쪽의 얇은 띠로 이동하여 원래 차지하던 가로 공간을 실제로 확보합니다. 펼치면 박스가 원래 순서대로 행에 다시 놓입니다(그동안 컬럼은 계속 마운트된 채로 시각적으로만 숨겨지므로 카드 자체의 "더 보기" 상태가 토글 후에도 유지됩니다). 셰브론 옆의 두 번째 아이콘 버튼은 그 박스만의 배치 방향을 전환합니다 — <strong>가로</strong>(기본값, 컬럼이 나란히 배치됨) 또는 <strong>세로</strong>(위에서 아래로 쌓임) — 모니터마다 독립적으로 설정됩니다. 모든 프로젝트 컬럼(그리고 독립된 미할당 컬럼)에도 동일한 셰브론이 있어, 그 컬럼이 어느 박스에 있든 상관없이 해당 컬럼 하나만 제자리에서 얇은 제목 표시줄로 접을 수 있습니다.',
    '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board (sessions)</strong> — sessions swim-laned across 5 columns: Active, Waiting, Completed, Error, Abandoned. Each card shows agent count, duration, model, cumulative cost, and a compact token strip with the cache read/write split and a cost-weighted Effective total</span>':
      '<span class="caption-icon">🗂️</span> <span><strong>Kanban Board (sessions)</strong> — 5개의 열로 스윔레인 처리된 세션: 활성, 대기 중, 완료, 오류, 중단됨. 각 카드는 에이전트 수, 지속 시간, 모델, 누적 비용, 그리고 캐시 읽기/쓰기 분리와 비용 가중 Effective 합계가 담긴 컴팩트한 토큰 막대를 표시합니다</span>',
    "A standalone page (<code>/project-manager</code>, sidebar label <strong>Project Manager</strong>, positioned right after <strong>Focus</strong>) answering \"what's going on across every project I'm tracking\" at a glance instead of checking each one individually. A portfolio rollup table — one row per project, with an objective/milestone progress bar, a pace badge, an open-decision count, and last activity — sits above the <strong>Decision Queue</strong>: pending reconciliation items (<code>pace_alert</code>, <code>detour_volume</code>, <code>detour_disposition</code>, and write-back failures) rendered as actionable cards, each resolved in place — <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code> for a detour, or resolve/dismiss/retry for everything else. A <strong>Pace Watch</strong> rail lists every behind item across every project, worst-overdue first, and a <strong>Recently Resolved</strong> log records what was just cleared. A project-scope chip row filters the table, queue, and rail down to a single project. It composes the dashboard's existing and new portfolio-management endpoints — <code>GET /api/projects</code>, <code>GET /api/portfolio/summary</code>, and <code>GET /api/decision-queue</code> — rather than duplicating their logic, and is the first client page built on top of the pace-tracking, detour-disposition, and reconciliation work that had previously shipped server-only.":
      '<code>/project-manager</code> 경로의 독립 페이지(사이드바 라벨은 <strong>Project Manager</strong>, <strong>Focus</strong> 바로 다음에 위치)로, 프로젝트를 하나씩 확인하는 대신 "지금 추적 중인 모든 프로젝트가 어떻게 돌아가고 있는지"를 한눈에 보여준다. 프로젝트별 목표/마일스톤 진행률 바, 페이스 배지, 미해결 결정 건수, 최근 활동을 담은 포트폴리오 롤업 테이블이 <strong>Decision Queue</strong> 위에 놓인다 — 대기 중인 조정(reconciliation) 항목(<code>pace_alert</code>, <code>detour_volume</code>, <code>detour_disposition</code>, 라이트백 실패)이 실행 가능한 카드로 표시되며, 각각 그 자리에서 해결된다 — 디투어(detour)는 <code>fold_in</code>/<code>new_item</code>/<code>deliberate</code>/<code>discard</code>로, 나머지는 resolve/dismiss/retry로 처리한다. <strong>Pace Watch</strong> 레일은 모든 프로젝트에 걸쳐 뒤처진 항목을 가장 많이 지연된 순서로 나열하고, <strong>Recently Resolved</strong> 로그는 방금 해결된 항목을 기록한다. 프로젝트 범위 칩 행은 테이블·큐·레일을 단일 프로젝트로 필터링한다. 이 페이지는 대시보드의 기존 및 신규 포트폴리오 관리 엔드포인트 — <code>GET /api/projects</code>, <code>GET /api/portfolio/summary</code>, <code>GET /api/decision-queue</code> — 를 조합할 뿐 로직을 중복하지 않으며, 이전까지 서버 전용으로만 제공되던 페이스 추적·디투어 처리·조정 작업 위에 구축된 첫 클라이언트 페이지다.',
  },
  plain: {
    zh: {
      "Plan-Aware Monitoring": "计划感知监控",
      "Remote Data Sources": "远程数据源",
      "How It Works": "工作原理",
      Setup: "设置",
      "Data Scope": "数据范围",
    ...
    vi: {
      "Plan-Aware Monitoring": "Giám sát theo kế hoạch",
      "Remote Data Sources": "Nguồn dữ liệu từ xa",
    ...
    ko: {
      "Plan-Aware Monitoring": "플랜 인지 모니터링",
      "Remote Data Sources": "원격 데이터 소스",
      "How It Works": "작동 방식",
      Setup: "설정",
      "Data Scope": "데이터 범위",
