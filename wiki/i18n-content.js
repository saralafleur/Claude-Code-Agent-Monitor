...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "A <strong>Session reset times</strong> panel plots every account's next session-window reset across the next 24h, tinting the reset marker's right half toward the weekly window's own color once the weekly window — not the session — is the real constraint, paired with a <strong>Capped by weekly</strong> callout naming it explicitly. A <strong>Weekly reset countdown</strong> panel does the same for the weekly window's calendar-day reset. A <strong>Color thresholds</strong> card lets you configure exactly where each green/yellow/orange/red band starts, independently for the session and weekly scopes — shared live across every connected computer.":
      "<strong>会话重置时刻</strong> 面板会绘制每个账户接下来 24 小时内下一次会话窗口重置的时间，一旦周窗口（而非会话窗口）才是真正的限制因素，重置标记的右半段就会随之染上周窗口自身的颜色，并配有一个 <strong>受周限额限制</strong> 的提示明确指出这一点。<strong>每周重置倒计时</strong> 面板对周窗口的日历日重置做相同的展示。<strong>颜色阈值</strong> 卡片可让你精确配置每个绿/黄/橙/红色段的起始百分比，会话与每周两个作用域可分别独立设置——并在每台已连接的电脑之间实时共享。",
    ...
    "Groups sessions by the folder(s) they run from into a user-named <strong>project</strong>, replacing the flat Agents/Sessions view with one grouped by where the work actually happens. Projects list as a compact table — one row per project with its name, session count, active count, last-activity, and quick plan/report/delete actions, drag-reorderable on the row — instead of a full card each, so a machine with many projects doesn't turn into a long vertical scroll. A project claims one or more working directories — a folder belongs to at most one project — and membership is derived by joining each session's <code>cwd</code> against the project's mapped folders, with no schema change on <code>sessions</code> itself. Each row expands via a chevron (collapsed by default) to reveal its folder mappings, any AGENT-PLAN.md checklist, and its sessions rendered as a horizontally scrollable row of session cards — the same collapsed-by-default treatment applies to each folder inside Unassigned. Create a project (optionally from an existing folder), rename it inline, add or remove folder mappings, or delete it — sessions are never deleted, they simply fall back to the <strong>Unassigned</strong> bucket.":
      "按会话运行所在的文件夹将会话分组为一个用户命名的 <strong>项目</strong>，取代原本扁平的 Agents/Sessions 视图，按实际工作发生的位置进行分组。项目以一个紧凑的表格列出——每个项目一行，包含名称、会话数、活跃数、最近活动，以及计划/报告/删除的快捷操作，行本身可拖拽排序——而不再是每个项目一整张卡片，避免项目一多整页就变成一长串纵向滚动。一个项目可以拥有一个或多个工作目录——一个文件夹最多只能属于一个项目——成员关系是通过将每个会话的 <code>cwd</code> 与项目已映射的文件夹进行匹配来推导的，<code>sessions</code> 本身无需任何模式变更。每一行都可通过箭头展开（默认折叠），展开后显示其文件夹映射、任何 AGENT-PLAN.md 检查清单，以及以横向可滚动行渲染的会话卡片——Unassigned 内每个文件夹同样采用默认折叠的处理方式。可以创建项目（可选地立即从某个现有文件夹创建）、内联重命名、添加或移除文件夹映射，或将其删除——会话永远不会被删除，只会回退到 <strong>Unassigned</strong> 分组中。",
