...
│   │   ├── KanbanBoard.tsx    # Agents/Sessions/Projects views; Projects view columns render inside drag-reorderable "monitor" boxes (lib/monitorGroups.ts — global, server-shared layout, not localStorage) side by side in the same row once one exists; every Agents/Sessions status Column also carries its own vertical/horizontal card-orientation toggle (Columns3/Rows3 icon), independent per column and persisted client-side only in the `kanban-status-column-orientation` localStorage key; header's CopyLinkButton copies a shareable URL (with ?token= when DASHBOARD_TOKEN auth is configured)
