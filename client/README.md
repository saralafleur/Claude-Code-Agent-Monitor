...
│   │   ├── StatTile.tsx        # Single labeled stat cell — extracted out of FocusReportBody so FocusPage reuses it unchanged; optional `action` slot renders a control on the label row; optional `compact` prop renders a smaller, self-bordered standalone card instead of a flush grid tile
│   │   ├── ConcurrencyStatTile.tsx # The Concurrency tile shared by FocusReportBody + FocusPage + KanbanBoard's header (compact) — both ratios at once (primary value + secondary sub-line) with a persistent localStorage-backed swap button
...
│   │   ├── ProjectDetail.tsx  # Full-page single-project view (/projects/:id, reached from a Projects row's "open detail" icon); a page-header Refresh button next to the project name that re-fetches repo topology + team-intake status together (one round trip) and pops up a scan-results summary; plan (PlanPanel/PlanModal reuse), a Repos card (repo/worktree topology, always-visible full paths, a two-click Remove per mapped repo/folder, a per-worktree Continue button — opens a terminal in that exact worktree with a FRESH `claude` instance (never `-c`/`--continue`, since silently resuming a prior conversation for that directory is considered unsafe) seeded with a server-built resume-nudge prompt naming its branch/dirty state, POST /api/projects/:id/continue-worktree, always visible regardless of Compact/Full since it's a launcher rather than a project-mutating action — + related-repo suggestions from PROJECT-CONTEXT.md and a live sibling/nested-subfolder disk scan with per-suggestion Add/Ignore and a separate Unignore-able Ignored list, GET/POST/DELETE /api/projects/:id/repos + /ignored-repos[/:id] — the disk-sibling half of that scan is gated by a project-level checkbox toggle, `siblingScanEnabled`, off by default and set via `PATCH /api/projects/:id`, since it otherwise surfaces every unrelated repo in a flat multi-project workspace folder) — the card itself has a collapse/expand toggle (always starts expanded, in-memory only) and a Compact/Full view-mode toggle (localStorage-persisted, `project-detail-repos-view-mode`; Compact hides the Remove/Add/Ignore/Unignore action buttons but keeps all information visible) — and team-intake initiative status (GET /api/projects/:id/intake) — computed live, nothing persisted aside from the ignore list
...
│   │   ├── playbookSettingsStore.ts # Coach's Playbook global settings store (a singleton, NOT per-practice — the auto-resolve time window) - bulk hydrate GET /api/playbook/settings + live playbook_settings_updated WS merges; mirrors colorThresholds.ts's pattern
...
| `playbook_settings_updated` | `{ autoResolveAfterMs }` — the full resulting global Playbook settings | `PUT /api/playbook/settings`, from any connected computer — merged into `lib/playbookSettingsStore.ts`'s store on top of the `GET /api/playbook/settings` hydrate |
...
| `coach_observation_updated` | Same Observation shape as above | `POST /api/coach/observations/:id/respond`, **or** the Playbook engine's own auto-resolve sweep once a session-scoped Observation's session has been ended for at least the global `autoResolveAfterMs` window (a still-active session's Observation is never auto-resolved) — `CoachPage.tsx` removes it from the (open-only) Feed once its status leaves `open`, either way |
