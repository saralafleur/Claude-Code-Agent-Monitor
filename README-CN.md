...
| **教练** | 两个页面(`/coach` 与 `/coach/playbook`,侧边栏标签为**教练**,紧跟在 Usage 之后——两者都位于侧边栏自己的一个视觉上略微区分开的"我们的新增功能"分区里,一块略浅的背景面板把 Projects/Calendar/Focus/Usage/教练 圈在一起,让这一组读起来是独立的一个区块),通过顶部共享的 **Feed**/**Playbook** 标签栏(`CoachTabs`)切换。`/coach` 是 **Feed**——一份实时更新的 **Observation**(观察记录)列表;`/coach/playbook` 是 **Playbook** 编辑器:**Practice**(检查项)的目录,每个 Practice 都有用户可编辑的**配置**(阈值),持久化在服务端并在每台已连接的电脑之间实时共享——与 Usage 页面的 Color Thresholds 卡片架构相同(`GET`/`PUT`,变更时广播 WebSocket 更新)。当某个 Practice 针对某个作用域(会话 / 项目 / 全局)的条件被满足时,Coach 引擎会记录一条 Observation——这正是显示在 Feed 中的最小单位;用户可以直接在卡片上 **Dismiss**(关闭)一条 Observation,无论它属于哪个作用域(acknowledge / resolve 也是可选状态,但 v1 的界面里只暴露了 Dismiss)。目前上线了两个 Practice。`session-token-ceiling`(作用域:会话)监控一个会话的 token 总用量(汇总 `token_usage` 的所有维度),一旦超过可配置的阈值(默认 100,000,000 tokens),就会生成一条 Warning 级别的 Observation,建议执行 `/compact` 或 `/clear`,卡片上附带两个建议操作——"Open session"(深链到该会话)与 "Copy summary"(复制到剪贴板)。`account-weekly-balance`(作用域:全局)监控每个已启用的 Claude 账户最新的每周额度已用百分比(Usage 页面的 Accounts 面板),一旦至少有两个账户仍有余量(已用 < 100%)且其中最低与最高已用之间的差距超过可配置的百分点阈值(默认 25),就会生成一条 Reminder 级别的 Observation,建议把当前工作切换到已用比例更低的账户——这样一次会话窗口切换总能有额度充足的备用账户,也不会让某个账户被单独耗尽。一个后台调度器(`server/lib/playbook/engine.js`,与 reconciliation 调度器的节拍结构一致)以 5 分钟为一个节拍(`DASHBOARD_PLAYBOOK_MS`,默认 `300000`;总开关 `DASHBOARD_PLAYBOOK_MODE=off`)评估每一个已启用的会话作用域 Practice 对每一个活跃会话的匹配情况,并额外评估每一个已启用的全局作用域 Practice 一次,并且在同一个 Practice+作用域 组合仍有一条未结束(`open`/`acknowledged`/`dismissed`/`resolved`)的 Observation 时不会重复触发。Playbook 页面会为目录中的每个 Practice 各渲染一张卡片,可以分别开关、编辑阈值,并实时预览它会产生的确切 Observation 卡片——通过与真实 Feed 相同的 `ObservationCard` 组件渲染,而非另做的仿制品。由 `GET`/`PUT /api/playbook/practices[/:id/config]`,以及 `GET /api/coach/observations` 与 `POST /api/coach/observations/:id/respond` 提供支持。这套框架(schema、引擎、API、UI)本身是完全通用的——新增 Practice 只是目录里新增的条目,而不需要新的管线 |
