...
| **项目** | 将会话按其运行时所在的文件夹归入用户自定义命名的项目，取代扁平的 Agent/会话视图。项目以一个紧凑的表格形式列出——每个项目一行（名称、会话数、活跃数、最近活动，以及计划/报告/删除的快捷操作），行本身可拖拽排序——而不再是每个项目一整张卡片，避免项目一多整页就变成一长串纵向滚动。一个项目可关联一个或多个工作目录（一个文件夹至多属于一个项目）；成员关系是通过将每个会话的 `cwd` 与该项目所映射的文件夹进行匹配推导得出的——`sessions` 本身无需任何表结构变更。每一行都可通过箭头展开（默认折叠），展开后显示其文件夹映射、任何 AGENT-PLAN.md 检查清单，以及以横向可滚动行渲染的会话（`SessionCard`）（**未分配**分组下每个文件夹同样采用默认折叠的会话卡片行）。可创建项目（创建时可选择立即关联一个已有文件夹）、就地内联重命名、事后添加/移除文件夹映射，或将其删除（文件夹映射会级联移除；底层会话不受影响，会直接回退至**未分配**分组，该分组还提供一键「从此文件夹新建项目」操作）。由 `GET/POST /api/projects`、`PATCH/DELETE /api/projects/:id` 以及 `POST/DELETE /api/projects/:id/paths[/:pathId]` 提供支持 |
