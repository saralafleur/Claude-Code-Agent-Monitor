...
| **教练** | 独立页面(`/coach`,侧边栏标签为**教练**,紧跟在 Usage 之后——两者都位于侧边栏自己的一个视觉上略微区分开的"我们的新增功能"分区里,一块略浅的背景面板把 Projects/Calendar/Focus/Usage/教练 圈在一起,让这一组读起来是独立的一个区块)。这里预留给未来的模式识别与建议层——观察账户实际使用 Claude Code 的节奏、绕路、重试、空闲时段,并给出具体建议。目前只是一个初始脚手架:尚未接入任何分析引擎,因此页面渲染的是一个诚实的空状态(`EmptyState`),外加三张预览卡片(模式识别、改进建议、盲点提醒),而不是任何真实数据 |
