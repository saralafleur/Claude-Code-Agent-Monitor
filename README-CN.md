...
侧边栏提供快速访问 Dashboard、进行中队列、日历、专注、看板、会话列表、活动流、分析、工作流和设置。每个页面旨在通过实时更新和丰富的可视化，为你提供对 Claude Code Agent 活动的深度洞察。
...
| **进行中队列** | 位于侧边栏 Dashboard 之后的实时页面（`/wip`）：将所有 `active` 会话按优先级排成单一队列——等待你输入的会话始终排在最前；平局（以及非等待会话之间的排序）按各会话所属项目的可拖拽**优先级**、再按最近活动时间打破。卡片复用同一个会话卡片，并加上醒目的项目名称标题。响应式 1/2/3 列布局按队列自身实测宽度（而非浏览器视口）自上而下填充，因此侧边栏展开/收起也能正确改变列数。可折叠的右侧**优先级侧栏**列出项目（默认仅显示队列中出现的项目，可切换"显示所有项目"），拖拽即可调整优先级——通过 `PUT /api/projects/reorder` 提交，并像看板的显示器布局一样，通过 `project_updated` 实时广播给所有其他打开的标签页/电脑 |
| **项目** | 将会话按其运行时所在的文件夹归入用户自定义命名的项目，取代扁平的 Agent/会话视图。一个项目可关联一个或多个工作目录（一个文件夹至多属于一个项目）；成员关系是通过将每个会话的 `cwd` 与该项目所映射的文件夹进行匹配推导得出的——`sessions` 本身无需任何表结构变更。每个项目都以其会话（`SessionCard`）组成的横向可滚动行渲染，并带有服务端计算的聚合会话数 / 活跃数 / 最近活动时间徽标。可创建项目（创建时可选择立即关联一个已有文件夹）、就地内联重命名、事后添加/移除文件夹映射，或将其删除（文件夹映射会级联移除；底层会话不受影响，会直接回退至**未分配**分组，该分组还提供一键「从此文件夹新建项目」操作）。由 `GET/POST /api/projects`、`PATCH/DELETE /api/projects/:id` 以及 `POST/DELETE /api/projects/:id/paths[/:pathId]` 提供支持 |
