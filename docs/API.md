...
Broadcast by the background overflow sweep (`server/lib/value-summary-tick.js`, see [Portfolio Plans & Value Ledger](#portfolio-plans--value-ledger)) whenever a sweep generates at least one PROJECT/STAKEHOLDER altitude, OR (Value Pool Slice 2, DEC-6) whenever the `coverage` object's `demand`/`complete` transitioned since the last broadcast for that project — a terminal "now complete" transition with zero units generated that iteration still emits, so an open tab's progress display never silently stalls. **`PlanLedgerPanel.tsx` subscribes to this message and updates in place** (superseding the prior "v1 ships no client subscriber" note) — the panel's first-ever `eventBus` subscription. `coverage` is the same `coverageSnapshot` object `GET /api/project-plans/coverage` returns (byte-same shape, single server-side home — `server/lib/value-coverage.js`); the client merges an incoming snapshot only if its `computed_at` is newer than the one currently held, so an out-of-order HTTP/WS delivery race can never visibly regress the header.
