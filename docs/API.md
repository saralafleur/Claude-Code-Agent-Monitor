...
`drift.skipped` is non-null (`"not_a_repo" | "no_default_branch" | "no_commits" | "git_error" | "budget_exceeded"`) whenever the detector couldn't (or, for `"budget_exceeded"`, wasn't asked to) compute a real answer — the client renders that as an explicit "unknown" state, distinct from the genuinely-clean `{ skipped: null, commits: [] }` state; never conflate the two. `commits` is in git's own DAG order (the `--first-parent` walk), never re-sorted by `committedAt`. The detection predicate (DEC-5, `intake/2026-08-02-trunk-drift-detection/decisions.md`) is purely git-native — first-parent, non-merge — and makes no dashboard-side attribution judgement. It does **not** additionally exclude commits reachable from another local branch (that clause was removed by `intake/2026-08-03-trunk-drift-open-branch-blindness/decisions.md`: it's topologically impossible to tell a fast-forward-merged-but-undeleted branch apart from an unrelated, freshly-created sibling branch, and the latter — an ordinary concurrently-open effort worktree — was blinding the detector across a repo's entire history far more often than the former ever needed protecting against).
...
| `POST /api/project-plans/altitudes` `{project_id, units:[...]}` | PROJECT/STAKEHOLDER altitude synthesis for a client-held batch of `/pool` units, capped at 40 synchronous resolutions per call — `{ altitudes, states, counts }` |
| `POST /api/project-plans/altitudes/seen` `{project_id, units:[{unit_key, regenerated_at}]}` | Explicit acknowledge of a regenerated (`freshness: "updated_unseen"`) unit's altitude entry — `{ updated }` |
| `GET /api/project-plans/coverage?project_id=` | The single `coverageSnapshot` object (Value Pool Slice 2) — `{ coverage: { described, pool_size, pending, complete, demand, requested_at, eta, computed_at } }`, byte-same shape as the WS broadcast's `coverage` field |
| `POST /api/project-plans/coverage-request` `{project_id}` | "Prioritize now": stamps `coverage_requested_at`, jumps the project to the head of the sweep rotation, kicks a bounded drain fire-and-forget — `202` with a probe-built `{ coverage }` |
...
**Altitude synthesis.** `POST /altitudes` is the layer above `/pool` (`server/lib/value-summary.js`) — every pool unit already carries a GROUND fact (`value_source`/`label`/attribution tier); this route adds a PROJECT phrase (what the unit is part of) and a STAKEHOLDER sentence (plain language, no jargon) per unit, the same leveling mechanism `GET /api/focus-report/summary` already uses for Focus window summaries, applied one altitude higher. Callers pass the exact units their own `/pool` fetch already resolved — this route never re-derives or duplicates pool assembly. `value_unit_summaries` is keyed on the unit's own identity with no digest gating; every not-yet-cached unit in one request batches into a single `claude -p` spawn (capped at `MAX_UNITS_PER_PROMPT` = 40, never one call per unit), so sibling units can inform each other's PROJECT phrasing. Always 200. The response is `{ altitudes, states, counts }`: a unit lands in exactly one of `altitudes`/`states`, never both, never neither. `states[unitKey]` is `"queued"` when the unit fell beyond this call's 40-unit cap and was never attempted this round (a later pass — the background sweep below — will pick it up), or `"unavailable"` when the unit WAS in scope this round and still produced nothing (LLM off/unavailable, spawn failure, or unparsable output). `counts` is `{pool_size, cache_hits, generated, queued, unavailable, stale_regenerated}`, the exact numbers this call also writes to `value_summary_generation_log`. Model: `DASHBOARD_VALUE_SUMMARY_MODEL` when set, falling back to `DASHBOARD_FOCUS_SUMMARY_MODEL`, then `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku`; gated by the same `DASHBOARD_FOCUS_INFER_MODE` availability check as Focus inference/summary.

**Mutability-aware caching.** A unit's cache key is stable forever, but its prompt input set is not: `intake_initiative`/`detour`/`merge_commit` units (`MUTABLE_VALUE_SOURCES`) carry a `stage` (and sometimes a `label`) that can change after the text was cached. Every read for a mutable source compares the unit's current `value_source`/`label`/`stage` against the exact snapshot the cached text was generated from; a mismatch is treated as an ordinary cache miss for that ONE unit and regenerates it through the same batch/cap machinery above. What could not be refreshed this round (over cap, or the LLM path unavailable) is served with its OLD text plus a `freshness` marker — never blanked. A resolved `altitudes[unitKey]` entry may additionally carry: `freshness` (`"updated_unseen"` — this round replaced stale text with fresh text not yet acknowledged; `"stale_refresh_queued"`/`"stale_refresh_unavailable"` — stale text served as-is because a refresh could not happen this round), `update_reason` (`"stage_changed"`/`"label_changed"`), and `regenerated_at` (always present on a generated entry, `null` on first generation). `trunk_commit` units (and any source outside `MUTABLE_VALUE_SOURCES`) keep the original generate-once-served-forever behavior exactly.

**`POST /altitudes/seen`** explicitly acknowledges a regenerated unit's marker, clearing `seen_at` (server-side, never a client-only dismiss). Body: `{project_id, units:[{unit_key, regenerated_at}]}` — `regenerated_at` must be the exact value the acknowledged entry carried (`null` for a first-generation entry) and is compared with SQL `IS` (not `=`) against the row's current `regenerated_at` — a stale value (the unit regenerated again since the caller last fetched it) is silently rejected rather than clearing a marker for a generation the user never saw. `project_id` is validated but otherwise advisory — `unit_key` already embeds the cwd, so cross-project collision is not a real concern. Response: `{ updated }` — the count of units actually stamped (idempotent: re-acknowledging an already-seen row is a no-op, still 200).

**Background overflow sweep.** A large pool (the measured worst case is 182 units) can take several page visits to fully resolve via `/altitudes` alone. `server/lib/value-summary-tick.js`, registered in `startBackgroundServices()`, sweeps `MAX_PROJECTS_PER_TICK` (default 3) projects per cycle in a rotation that (Value Pool Slice 2) sorts a project with a live "prioritize now" request first, then falls back to least-recently-swept — calling the exact same `assembleValuePool` + `enrichPoolAltitudes` composers `/pool` and `/altitudes` use — never a second pool-assembly or cache-write path. Disable with `DASHBOARD_VALUE_SUMMARY_TICK_MODE=off` or `DASHBOARD_VALUE_SUMMARY_TICK_MS=0`. A sweep broadcasts `value_altitudes_updated` (`{project_id, unit_keys, pending, coverage}`) under any of three conditions: it generates at least one unit; OR (Slice 2, DEC-6) `coverage.demand`/`coverage.complete` transitioned since the last broadcast recorded for that project; OR (SF-6) it is that project's FIRST-EVER observed sweep in this process's lifetime (no prior broadcast recorded) and the pool is already `complete` — otherwise a pool that was already fully described before its very first sweep in a freshly-started process would never emit its terminal state at all (a first observation that is NOT yet complete still does not spuriously broadcast). This third condition also fires for a genuinely empty pool (`pool_size: 0`) on its first observation, since `complete` (`pending === 0`) is trivially true for zero units — a deliberate, pinned side effect (`server/__tests__/value-summary-tick.test.js`'s `S2` case), not a bug; such a frame carries `unit_keys: []`. **`PlanLedgerPanel.tsx` now subscribes to this message and updates in place** (superseding the prior "v1 ships no client subscriber" note). Per-sweep outcomes are recorded in `value_summary_generation_log` (see [docs/DATABASE.md](./DATABASE.md#value_summary_sweep_state--value_summary_generation_log)) — an operator can read real per-sweep hit/generated/queued/unavailable counts directly from that table.

**Coverage-on-demand (Value Pool Slice 2).** A **coverage request** (`POST /coverage-request`) stamps `value_summary_sweep_state.coverage_requested_at`, jumping the project to the head of the sweep rotation, and kicks `runCoverageDrain()` (in `value-summary-tick.js`) fire-and-forget — a bounded loop of back-to-back ≤`MAX_UNITS_PER_PROMPT`-unit batches (hard-capped at `MAX_DRAIN_BATCHES_PER_RUN = 25`, a safety bound, not an env knob) that re-assembles the pool and re-derives `pending` from THAT iteration's own full-pool counts every time (never a decremented counter, so a pool that grows mid-drain extends the drain for free). The drain shares its overlap guard with the passive tick — a concurrent call to either returns `{skipped: "overlap"}`. `server/lib/value-coverage.js` is the single home (never re-derived anywhere else) for the `coverageSnapshot` object both `GET /coverage` and the WS broadcast carry verbatim: `described` (= `pool_size - queued - unavailable`, "fresh-or-immutable" — a stale-but-served mutable unit counts as NOT described, so `described` can read lower than the number of units actually displaying text), `pending`, `complete` (`pending === 0`, the server-authored field a future auto-group gate reads directly), `demand` (`"passive"`/`"requested"`/`"draining"`, closed registry), and `eta` (`{state: "measured", ms_remaining, per_batch_ms, batches_remaining}` from the last 5 qualifying `value_summary_generation_log` rows, per-project first then fleet-wide fallback | `{state: "estimating"}` cold start, never a fabricated number | `{state: "none"}` already complete). A coverage request older than `COVERAGE_REQUEST_TTL_MS` (24h) reverts to passive with a logged revert, never silently. `GET /coverage` runs the composer in **probe mode** (`enrichPoolAltitudes(..., {probe: true})`) — classify only, never spawns, writes no `value_summary_generation_log` row.

**WebSocket:** additive message types `project_plan_updated`, `value_claim_updated`, and `value_altitudes_updated` (see [WebSocket API](#websocket-api) below) — `plan_updated`'s own type/payload is untouched.
...
**Rotation Plan (no new fields):** the Usage page's "Rotation Plan" card (`RotationPlanCard`/`computeRotationPlan` in `client/src/pages/Usage.tsx`) is derived entirely client-side from the same weekly Consumption Rate fields above (`week_burn_rate_pct_per_hour`, `latest_week_window_pct`, `latest_week_reset_raw`) plus `enabled`/`status`/`is_active` — it adds no wire fields or endpoints of its own. It chains a greedy simulation across every enabled `status: "ok"` account: at each step, whichever not-yet-capped account has the most weekly runway *if it became the active one* is picked, using the fastest positive `week_burn_rate_pct_per_hour` observed across any account as the assumed active pace (an idle account's own near-zero rate doesn't reflect what it would do under real use). That account runs until it's projected to cross 100% or a ~9-day horizon ends, every account's simulated percentage is advanced accordingly (idle accounts drift at their own observed rate; a reset inside the elapsed window snaps an account back to 0%), and the next segment is picked the same way. Starts from whichever account `is_active` reports, falling back to the best-runway pick when that one is already capped.
...
Broadcast by the background overflow sweep (`server/lib/value-summary-tick.js`, see [Portfolio Plans & Value Ledger](#portfolio-plans--value-ledger)) under any of three conditions: a sweep generates at least one PROJECT/STAKEHOLDER altitude; OR (Value Pool Slice 2, DEC-6) the `coverage` object's `demand`/`complete` transitioned since the last broadcast recorded for that project — a terminal "now complete" transition with zero units generated that iteration still emits, so an open tab's progress display never silently stalls; OR (SF-6) it is that project's FIRST-EVER observed sweep in this process's lifetime (no prior broadcast recorded) and `coverage.complete` is already `true` — without this third condition, a pool that was already fully described before its very first sweep in a freshly-started process would never emit its terminal state at all (a first observation that is NOT yet complete still does not spuriously broadcast). This third condition also fires for a genuinely empty pool (`pool_size: 0`, `unit_keys: []`) on its first observation, since `complete` (`pending === 0`) is trivially true for zero units — a deliberate, pinned side effect (`server/__tests__/value-summary-tick.test.js`'s `S2` case), not a bug. **`PlanLedgerPanel.tsx` subscribes to this message and updates in place** (superseding the prior "v1 ships no client subscriber" note) — the panel's first-ever `eventBus` subscription. `coverage` is the same `coverageSnapshot` object `GET /api/project-plans/coverage` returns (byte-same shape, single server-side home — `server/lib/value-coverage.js`); the client merges an incoming snapshot only if its `computed_at` is newer than the one currently held, so an out-of-order HTTP/WS delivery race can never visibly regress the header.

```json
{ "type": "value_altitudes_updated", "data": { "project_id": "proj_abc", "unit_keys": ["trunk_commit::a1b2c3d::/repo"], "pending": 5, "coverage": { "project_id": "proj_abc", "described": 177, "pool_size": 182, "pending": 5, "complete": false, "demand": "draining", "requested_at": "2026-08-05T12:00:00.000Z", "eta": { "state": "measured", "ms_remaining": 42000, "per_batch_ms": 42000, "batches_remaining": 1 }, "computed_at": "2026-08-05T12:03:00.000Z" } } }
```
