...
| `POST /api/project-plans/altitudes` `{project_id, units:[...]}` | PROJECT/STAKEHOLDER altitude synthesis for a client-held batch of `/pool` units, capped at 40 synchronous resolutions per call — `{ altitudes, states }` |
...
**Altitude synthesis.** `POST /altitudes` is the layer above `/pool` (`server/lib/value-summary.js`) — every pool unit already carries a GROUND fact (`value_source`/`label`/attribution tier); this route adds a PROJECT phrase (what the unit is part of) and a STAKEHOLDER sentence (plain language, no jargon) per unit, the same leveling mechanism `GET /api/focus-report/summary` already uses for Focus window summaries, applied one altitude higher. Callers pass the exact units their own `/pool` fetch already resolved — this route never re-derives or duplicates pool assembly. Caching differs from Focus: a unit's ground fact is immutable once seen, so `value_unit_summaries` is keyed on the unit's own identity with no digest gating — generated once, served forever; every not-yet-cached unit in one request batches into a single `claude -p` spawn (capped at `MAX_UNITS_PER_PROMPT` = 40, never one call per unit), so sibling units can inform each other's PROJECT phrasing. Always 200. The response is `{ altitudes, states }`: a unit lands in exactly one of the two, never both, never neither. `states[unitKey]` is `"queued"` when the unit fell beyond this call's 40-unit cap and was never attempted this round (a later pass — the background sweep below — will pick it up), or `"unavailable"` when the unit WAS in scope this round and still produced nothing (LLM off/unavailable, spawn failure, or unparsable output). Model: `DASHBOARD_VALUE_SUMMARY_MODEL` when set, falling back to `DASHBOARD_FOCUS_SUMMARY_MODEL`, then `DASHBOARD_FOCUS_INFER_MODEL`, then `haiku`; gated by the same `DASHBOARD_FOCUS_INFER_MODE` availability check as Focus inference/summary.
