...
**Background overflow sweep.** A large pool (the measured worst case is 182 units) can take several page visits to fully resolve via `/altitudes` alone. `server/lib/value-summary-tick.js`, registered in `startBackgroundServices()`, sweeps `MAX_PROJECTS_PER_TICK` (default 3) projects per cycle in least-recently-swept rotation, calling the exact same `assembleValuePool` + `enrichPoolAltitudes` composers `/pool` and `/altitudes` use — never a second pool-assembly or cache-write path. Disable with `DASHBOARD_VALUE_SUMMARY_TICK_MODE=off` or `DASHBOARD_VALUE_SUMMARY_TICK_MS=0`. A sweep that generates at least one unit broadcasts `value_altitudes_updated` (`{project_id, unit_keys, pending}` — v1 ships no client subscriber, so an open panel picks up new coverage on its next mount, not in place). Per-sweep outcomes are recorded in `value_summary_generation_log` (see [docs/DATABASE.md](./DATABASE.md#value_summary_sweep_state--value_summary_generation_log)) — an operator can read real per-sweep hit/generated/queued/unavailable counts directly from that table.
...
Broadcast by the background overflow sweep (`server/lib/value-summary-tick.js`, see [Portfolio Plans & Value Ledger](#portfolio-plans--value-ledger)) whenever a sweep generates at least one PROJECT/STAKEHOLDER altitude — never broadcast on an all-cached or LLM-unavailable sweep. **v1 ships no client subscriber for this message** — `PlanLedgerPanel.tsx` only fetches altitudes on mount/unit-set change, so an already-open panel picks up newly-swept coverage the next time it mounts, not in place.

```json
{ "type": "value_altitudes_updated", "data": { "project_id": "proj_abc", "unit_keys": ["trunk_commit::a1b2c3d::/repo"], "pending": 5 } }
```
